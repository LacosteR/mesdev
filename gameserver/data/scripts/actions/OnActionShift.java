...
import l2p.gameserver.data.retail.holder.npcpos.Privates;
import l2p.gameserver.handler.admincommands.impl.reworked.Utils;
...
public class OnActionShift extends Functions {
    public boolean OnActionShift_NpcInstance(Player player, GameObject object) {
        ...
        if (!ConfigCharacter.ALLOW_NPC_SHIFTCLICK && !player.isGM()) {
            if (ConfigCharacter.ALT_GAME_SHOW_DROPLIST && object.isNpc()) {
        ...
        if (object.isNpc()) {
            ...
            if (ConfigCharacter.ALT_FULL_NPC_STATS_PAGE) {
                ...
                dialog = dialog.replaceFirst("%loc%", npc.getPositionComponent().getLoc().toXYZString() + ", col_r=[" + npc.getCollisionRadius() + "]" + "col_h=[" + npc.getColHeight() + "]");
                dialog = dialog.replaceFirst("%dist%", String.valueOf((int) PositionUtils.getDistance3D(npc, player)));
                ...
                dialog = dialog.replaceFirst("%xyz%", npc.getPositionComponent().getLoc().x + " " + npc.getPositionComponent().getLoc().y + " " + npc.getPositionComponent().getLoc().z);
            ...
            if (player.isGM()) {
                ...
                stringBuilder.append(String.valueOf(npc.getAI()));
                stringBuilder.append(",<br1>active: ");
                stringBuilder.append(npc.getAI().isActive());
                stringBuilder.append(",<br1>intention: ");
                stringBuilder.append(npc.getAI().getIntention());
                if(npc.getMyMaker() != null) {
                    stringBuilder.append(",<br1>maker name: ");
                    stringBuilder.append(npc.getMyMaker().getTemplate().getName());
                    stringBuilder.append(",<br1>maker ai: ");
                    stringBuilder.append(npc.getMyMaker().getTemplate().getAi());
                    if(npc.getPrivates() != null) {
                        for(Privates pvt : npc.getPrivates()) {
                            stringBuilder.append(",<br1>Has private: ");
                            stringBuilder.append(pvt.getNpcName() + ":" + pvt.getAi() + ":" + pvt.getCount() + ":" + pvt.getRespawnTime() + "sec");
                        }
                    }
                }
                dialog = dialog.replaceFirst("%AI%", stringBuilder.toString());
            }
            else
    ...
    public void droplist(Player player, NpcInstance npc) {
        ...
        if (ConfigCharacter.ALT_GAME_SHOW_DROPLIST)
    ...
    public void resists() {
        ...
        hasResist |= addResist(dialog, "Sword", 100 - npc.calcStat(Stats.p_defense_trait_sword, null, null));
        hasResist |= addResist(dialog, "Dual Sword", 100 - npc.calcStat(Stats.p_defense_trait_dual, null, null));
        hasResist |= addResist(dialog, "Blunt", 100 - npc.calcStat(Stats.p_defense_trait_blunt, null, null));
        hasResist |= addResist(dialog, "Dagger", 100 - npc.calcStat(Stats.p_defense_trait_dagger, null, null));
        hasResist |= addResist(dialog, "Bow", 100 - npc.calcStat(Stats.p_defense_trait_bow, null, null));
        hasResist |= addResist(dialog, "Crossbow", 100 - npc.calcStat(Stats.p_defense_trait_crossbow, null, null));
        hasResist |= addResist(dialog, "Polearm", 100 - npc.calcStat(Stats.p_defense_trait_pole, null, null));
        hasResist |= addResist(dialog, "Fist", 100 - npc.calcStat(Stats.p_defense_trait_fist, null, null));
    ...
    public boolean OnActionShift_DoorInstance(Player player, GameObject object) {
        if (player == null || object == null || !player.getPlayerAccess().CanOpenDoor || !object.isDoor())
            return false;
    ...
    public boolean OnActionShift_Player(Player player, GameObject object) {
        ...
        Player target = object.getPlayer();

        if (player.getPlayerAccess().CanViewChar) {
            Utils.showCharacterList(player, target);
        } else {
            if (target.isDead())
                return false;
            ...
            if (ConfigOthers.ALLOW_SHIFT_CLICK_STATS)
                player.sendPacket(new GMViewCharacterInfo(target));

            if (ConfigOthers.ALLOW_SHIFT_CLICK_SKILLS)
                player.sendPacket(new GMViewSkillInfo(target));

            if (ConfigOthers.ALLOW_SHIFT_CLICK_INVENTORY) {
    ...
    public boolean OnActionShift_PetInstance(Player player, GameObject object) {
        ...
        if (object.isPet()) {
            ...
            dialog = dialog.replaceFirst("%xyz%", pet.getPositionComponent().getLoc().x + " " + pet.getPositionComponent().getLoc().y + " " + pet.getPositionComponent().getLoc().z);
            dialog = dialog.replaceFirst("%heading%", String.valueOf(pet.getPositionComponent().getLoc().h));
            ...
            dialog = dialog.replaceFirst("%dist%", String.valueOf((int) PositionUtils.getRealDistance(pet, player)));
