...
public class TeleToParnassus extends Functions implements ScriptFile {
    ...
    @Override
    public void onLoad() {
        if (!ConfigServices.SERVICES_PARNASSUS_ENABLED)
            return;
    ...
    public void toParnassus() {
        ...
        if (player.getAdena() < ConfigServices.SERVICES_PARNASSUS_PRICE) {
            player.sendPacket(Msg.YOU_DO_NOT_HAVE_ENOUGH_ADENA);
            return;
        }

        player.reduceAdena(ConfigServices.SERVICES_PARNASSUS_PRICE, true);
        player.setVar("backCoords", player.getPositionComponent().getLoc().toXYZString(), -1);
    ...
    private static final String en = "<br>[scripts_services.TeleToParnassus:toParnassus @811;Parnassus|\"Move to Parnassus (offshore zone) - " + ConfigServices.SERVICES_PARNASSUS_PRICE + " Adena.\"]";
    private static final String ru = "<br>[scripts_services.TeleToParnassus:toParnassus @811;Parnassus|\"Parnassus (торговая зона без налогов) - " + ConfigServices.SERVICES_PARNASSUS_PRICE + " Adena.\"]";

    public String getHtmlAppends(Integer val) {
        if (val != 0 || !ConfigServices.SERVICES_PARNASSUS_ENABLED)
            return "";
        Player player = getSelf();
        if (player == null)
            return "";
    ...
    public String getHtmlAppends2(Integer val) {
        if (val != 0 || !ConfigServices.SERVICES_PARNASSUS_ENABLED)
            return "";
        Player player = getSelf();
        if (player == null || player.getPositionComponent().getReflection() != ReflectionManager.PARNASSUS)
            return "";
    ...
    public class ZoneListener implements OnZoneEnterLeaveListener {
        ...
        public void onZoneLeave(Zone zone, Creature cha) {
            ...
            if (player != null)
                if (ConfigServices.SERVICES_PARNASSUS_ENABLED && player.getPositionComponent().getReflection() == ReflectionManager.PARNASSUS && player.isVisible()) {
                    double angle = PositionUtils.convertHeadingToDegree(cha.getHeading()); // угол в градусах
                    double radian = Math.toRadians(angle - 90); // угол в радианах
                    cha.teleToLocation((int) (cha.getPositionComponent().getX() + 50 * Math.sin(radian)), (int) (cha.getPositionComponent().getY() - 50 * Math.cos(radian)), cha.getPositionComponent().getZ());
                }
        }
    }
}