...
public class NickColor extends Functions {
    public void list() {
        ...
        if (!ConfigServices.SERVICES_CHANGE_NICK_COLOR_ENABLED) {
            player.sendMessage("Service disabled.");
            return;
        }

        StringBuilder append = new StringBuilder();
        append.append("You can change nick color for small price ").append(ConfigServices.SERVICES_CHANGE_NICK_COLOR_PRICE).append(" ").append(ItemHolder.getInstance().getTemplate(ConfigServices.SERVICES_CHANGE_NICK_COLOR_ITEM).getName()).append(".");
        append.append("<br>Possible colors:<br>");
        for (String color : ConfigServices.SERVICES_CHANGE_NICK_COLOR_LIST)
    ...
    public void change(String[] param) {
        ...
        if (player.getInventory().destroyItemByItemId(ConfigServices.SERVICES_CHANGE_NICK_COLOR_ITEM, ConfigServices.SERVICES_CHANGE_NICK_COLOR_PRICE)) {
            player.setNameColor(Integer.decode("0x" + param[0]));
            player.broadcastUserInfo(true);
        } else if (ConfigServices.SERVICES_CHANGE_NICK_COLOR_ITEM == 57)
