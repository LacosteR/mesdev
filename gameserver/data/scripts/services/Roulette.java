...
public class Roulette extends Functions {
    ...
    public void dialog() {
        ...
        show(HtmCache.getInstance().getNotNull("scripts/services/roulette.htm", player).replaceFirst("%min%", Util.formatAdena(ConfigServices.SERVICES_ROULETTE_MIN_BET)).replaceFirst("%max%", Util.formatAdena(ConfigServices.SERVICES_ROULETTE_MAX_BET)), player);
    }
    ...
    public void play(String[] param) {
        ...
        if (bet < ConfigServices.SERVICES_ROULETTE_MIN_BET) {
            show("Too small bet!<br><a action=\"bypass -h scripts_services.Roulette:dialog\">Back</a>", player);
            return;
        }
        if (bet > ConfigServices.SERVICES_ROULETTE_MAX_BET) {
    ...
    public String getHtmlAppends(Integer val) {
        Player player = getSelf();
        if (ConfigServices.SERVICES_ALLOW_ROULETTE)
