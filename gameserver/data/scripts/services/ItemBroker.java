...
import l2p.gameserver.data.retail.holder.RecipeHolder;
import l2p.gameserver.data.retail.holder.recipe.Recipe;
...
public class ItemBroker extends Functions {
    ...
    public String getHtmlAppends(Integer val) {
        if (!ConfigServices.ITEM_BROKER_ITEM_SEARCH)
            return "";
                ...
                switch (type) {
                    case Player.STORE_PRIVATE_SELL:
                        ...
                        for (TradeItem item : tradeList) {
                            ...
                            Item newItem = new Item(item.getItemId(), type, item.getOwnersPrice(), item.getCount(), item.getEnchantLevel(), temp.getName(), pl.getObjectId(), pl.getName(), pl.getPositionComponent().getLoc(), item.getObjectId(), item, false);
                    ...
                    case Player.STORE_PRIVATE_SELL_PACKAGE:
                        ...
                        for (TradeItem item : tradeList) {
                            ...
                            Item newItem = new Item(item.getItemId(), type, packagePrice, item.getCount(), item.getEnchantLevel(), temp.getName(), pl.getObjectId(), pl.getName(), pl.getPositionComponent().getLoc(), item.getObjectId(), item, true);
                            long key = newItem.price * 100;
                            while (key < newItem.price * 100 + 100 && oldItems.containsKey(key))
                                // До 100 предметов с одинаковыми ценами
                                key++;
                    ...
                    case Player.STORE_PRIVATE_BUY:
                        ...
                        for (TradeItem item : tradeList) {
                            ...
                            Item newItem = new Item(item.getItemId(), type, item.getOwnersPrice(), item.getCount(), item.getEnchantLevel(), temp.getName(), pl.getObjectId(), pl.getName(), pl.getPositionComponent().getLoc(), itemObjId++, item, false);
                            long key = newItem.price * 100;
                            while (key < newItem.price * 100 + 100 && oldItems.containsKey(key))
                                // До 100 предметов с одинаковыми ценами
                                key++;
                    ...
                    case Player.STORE_PRIVATE_MANUFACTURE:
                        ...
                        for (ManufactureItem mitem : createList) {
                            ...
                            Recipe recipe = RecipeHolder.getInstance().getRecipe(recipeId);
                            if (recipe == null)
                                continue;
                            ...
                            ItemTemplate temp = ItemHolder.getInstance().getTemplate(ItemDataHolder.getInstance().getItemData(name).itemId);
                            if (temp == null)
                                continue;
                            ...
                            Item newItem = new Item(ItemDataHolder.getInstance().getItemData(name).itemId, type, mitem.getCost(), 1, 0, temp.getName(), pl.getObjectId(), pl.getName(), pl.getPositionComponent().getLoc(), itemObjId++, null, false);
                            long key = newItem.price * 100;
                            while (key < newItem.price * 100 + 100 && oldItems.containsKey(key))
                                // До 100 предметов с одинаковыми ценами
                                key++;
