...
public class Pushkin extends Functions {
    public String DialogAppend_30300(Integer val) {
        if (val != 0 || !ConfigNPC.ALT_SIMPLE_SIGNS && !ConfigNPC.ALT_BS_CRYSTALLIZE)
            return "";

        StringBuilder append = new StringBuilder();

        if ((getSelf()).isLangRus()) {
            if (ConfigNPC.ALT_SIMPLE_SIGNS) {
            ...
            if (ConfigNPC.ALT_BS_CRYSTALLIZE)// TODO: сделать у всех кузнецов
            ...
            if (ConfigNPC.ALT_BS_CRYSTALLIZE)
