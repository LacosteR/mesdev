...
public class Birthday extends Functions {
    ...
    public void summonAlegria() {
        ...
        try {
            ...
            int x = (int) (npc.getPositionComponent().getX() + 40 * Math.cos(npc.headingToRadians(npc.getHeading() - 32768 + 8000)));
            int y = (int) (npc.getPositionComponent().getY() + 40 * Math.sin(npc.headingToRadians(npc.getHeading() - 32768 + 8000)));

            NpcInstance alegria = NpcUtils.spawnSingle(NPC_ALEGRIA, x, y, npc.getPositionComponent().getZ(), 180000);
