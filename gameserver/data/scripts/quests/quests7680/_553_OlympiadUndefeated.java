    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        switch (npcId) {
            case OLYMPIAD_MANAGER:
                ...
                if (!player.isNoble() || player.getLevel() < 75 || player.getPcClass().getLevel() < 4)
                ...
                if (st.isCreated()) {
                    ...
                    if (st.getQuestItemsCount(WINS_CONFIRMATION3) > 0) {
                        st.doGiveItems(OLYMPIAD_CHEST, 6);
                        st.doGiveItems(MEDAL_OF_GLORY, 5);
                        st.doTakeItems(WINS_CONFIRMATION1, -1);
                        st.doTakeItems(WINS_CONFIRMATION2, -1);
                        st.doTakeItems(WINS_CONFIRMATION3, -1);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("olympiad_operator_q0553_07.htm")) {
            if (st.getQuestItemsCount(WINS_CONFIRMATION3) > 0) {
                ...
                st.doGiveItems(OLYMPIAD_CHEST, 3);
                st.doGiveItems(MEDAL_OF_GLORY, 3); // от балды
                st.doTakeItems(WINS_CONFIRMATION1, -1);
                st.doTakeItems(WINS_CONFIRMATION2, -1);
                st.doTakeItems(WINS_CONFIRMATION3, -1);
                st.playSound(SOUND_FINISH);
                ...
                st.doGiveItems(OLYMPIAD_CHEST, 1);
                st.doTakeItems(WINS_CONFIRMATION1, -1);
                st.doTakeItems(WINS_CONFIRMATION2, -1);
                st.doTakeItems(WINS_CONFIRMATION3, -1);
                st.playSound(SOUND_FINISH);
