...
public class _643_RiseAndFallOfTheElrokiTribe extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("singsing_q0643_05.htm")) {
            ...
            if (count >= 300) {
                st.doTakeItems(BONES_OF_A_PLAINS_DINOSAUR, 300);
                st.doGiveItems(REWARDS[Rnd.get(REWARDS.length)], 5);
            } else
        ...
        if (st.getCond() == 0) {
            ...
            if (npcId == 32106) {
                ...
                if (count == 0)
                    ...
                    htmltext = "singsing_q0643_08.htm";
                    st.doTakeItems(BONES_OF_A_PLAINS_DINOSAUR, -1);
                    st.doRewardItems(ADENA_ID, count * 1374);
                }
            } else if (npcId == 32117)
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1)
            st.doGiveOnKillItems(BONES_OF_A_PLAINS_DINOSAUR, 1, 1, DROP_CHANCE);
        return null;
    }
}