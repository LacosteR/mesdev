...
public class _10274_CollectingInTheAir extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if (event.equalsIgnoreCase("32557-03.htm")) {
            st.setCond(1);
            st.doGiveItems(StarStoneExtractionScroll, 8);
            st.setState(STARTED);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (id == COMPLETED)
            ...
            st.doTakeItems(ExtractedCoarseBlueStarStone);
            st.doTakeItems(ExtractedCoarseRedStarStone);
            st.doTakeItems(ExtractedCoarseGreenStarStone);

            st.doGiveItems(ExpertTextStarStoneExtractionSkillLevel1, 1);
            st.addExpAndSp(RATE_QUESTS_REWARD * 25160, RATE_QUESTS_REWARD * 2525);
