...
public class _195_SevenSignsSecretRitualofthePriests extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("claudiaathebaldt_q195_2.htm")) {
            ...
            st.setCond(2);
            st.doGiveItems(GuardsoftheDawnIdentityCard, 1);
            ...
            if (ref != null && player.getTransformation() == 113 && st.getQuestItemsCount(GuardsoftheDawnIdentityCard) >= 1) {
                if (npc.getPositionComponent().getLoc().equals(new Location(-75695, 213537, -7128, 0))) {
                    ref.openDoor(door1);
                    ref.openDoor(door2);
            ...
            st.doGiveItems(EmperorShunaimansContract, 1);
            ...
            player.setTransformation(0);
            st.doTakeItems(GuardsoftheDawnIdentityCard, -1);
            st.playSound(SOUND_ITEMGET);
            st.setCond(4);
        } else if (event.equalsIgnoreCase("iasonheine_q195_2.htm"))
            if (player.getBaseClassId() == player.getActiveClassId()) {
                st.doTakeItems(EmperorShunaimansContract, -1);
