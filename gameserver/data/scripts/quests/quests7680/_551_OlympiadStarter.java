...
public class _551_OlympiadStarter extends Quest implements ScriptFile {
            ...
            case OLYMPIAD_MANAGER:
                ...
                if (st.isCreated()) {
                    ...
                    if (st.getQuestItemsCount(OLYMPIAD_CERT3) > 0) {
                        st.doGiveItems(OLYMPIAD_CHEST, 4);
                        st.doGiveItems(MEDAL_OF_GLORY, 5);
                        st.doTakeItems(OLYMPIAD_CERT1, -1);
                        st.doTakeItems(OLYMPIAD_CERT2, -1);
                        st.doTakeItems(OLYMPIAD_CERT3, -1);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if (event.equalsIgnoreCase("olympiad_operator_q0551_03.htm")) {
            ...
            if (st.getQuestItemsCount(OLYMPIAD_CERT3) > 0) {
                ...
                st.doGiveItems(OLYMPIAD_CHEST, 2);
                ...
                st.doGiveItems(MEDAL_OF_GLORY, 1);
                st.doTakeItems(OLYMPIAD_CERT1, -1);
                st.doTakeItems(OLYMPIAD_CERT2, -1);
                st.doTakeItems(OLYMPIAD_CERT3, -1);
                st.playSound(SOUND_FINISH);
    ...
    public void onOlympiadEnd(OlympiadGame og, QuestState qs) {
        if (qs.getCond() == 1) {
            ...
            qs.set("count", count);
            if (count == 3) {
                qs.doGiveItems(OLYMPIAD_CERT1, 1);
                qs.playSound(SOUND_ITEMGET);
            } else if (count == 5) {
                qs.doGiveItems(OLYMPIAD_CERT2, 1);
                qs.playSound(SOUND_ITEMGET);
            } else if (count == 10) {
                qs.doGiveItems(OLYMPIAD_CERT3, 1);
                qs.setCond(2);
