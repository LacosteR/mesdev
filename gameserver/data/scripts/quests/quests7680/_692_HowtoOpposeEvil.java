    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("take_test") && cond == 0) {
            ...
            if (st.getQuestItemsCount(FleetSteedTroupsTotem) < 5)
                ...
                st.doTakeItems(FleetSteedTroupsTotem, 5 * _charmstogive);
                st.doGiveItems(FleetSteedTroupsCharm, _charmstogive);
            ...
            if (st.getQuestItemsCount(NucleusofanIncompleteSoul) < 5)
                ...
                st.doTakeItems(NucleusofanIncompleteSoul, 5 * _soulstogive);
                st.doGiveItems(NucleusofaFreedSoul, _soulstogive);
            ...
            if (st.getQuestItemsCount(SpiritStoneDust) < 5)
                ...
                st.doTakeItems(SpiritStoneDust, 5 * _soulstogive);
                st.doGiveItems(SpiritStoneFragment, _soulstogive);
                htmltext = "kutran_q692_5.htm";
            }
            ...
            if (st.getQuestItemsCount(BreathofTiat) == 0)
                ...
                st.doRewardItems(ADENA_ID, st.getQuestItemsCount(BreathofTiat) * 2500);
                st.doTakeItems(BreathofTiat, -1);
                htmltext = "kutran_q692_5.htm";
            }
            ...
            if (st.getQuestItemsCount(PortionofaSoul) == 0)
                ...
                st.doRewardItems(ADENA_ID, st.getQuestItemsCount(PortionofaSoul) * 2500);
                st.doTakeItems(PortionofaSoul, -1);
                htmltext = "kutran_q692_5.htm";
            }
            ...
            if (st.getQuestItemsCount(ConcentratedSpiritEnergy) == 0)
                ...
                st.doRewardItems(ADENA_ID, st.getQuestItemsCount(ConcentratedSpiritEnergy) * 25000);
                st.doTakeItems(ConcentratedSpiritEnergy, -1);
                htmltext = "kutran_q692_5.htm";
            }
        }
        return htmltext;
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 3) {
            if (ArrayUtils.contains(SOD, npcId))
                st.doGiveOnKillItems(FleetSteedTroupsTotem, 1, 1, 17);
            else if (ArrayUtils.contains(SOI, npcId))
                st.doGiveOnKillItems(NucleusofanIncompleteSoul, 1, 1, 17);
            else if (ArrayUtils.contains(SOA, npcId))
                st.doGiveOnKillItems(SpiritStoneDust, 1, 1, 20);
        }
        return null;
    }
}