            ...
            case OLYMPIAD_MANAGER:
                ...
                if (st.isCreated()) {
                    ...
                    if (st.getQuestItemsCount(TEAM_CERTIFICATE, CLASS_FREE_CERTIFICATE, CLASS_CERTIFICATE) == 0)
                        ...
                        st.doTakeItems(TEAM_CERTIFICATE, -1);
                        st.doTakeItems(CLASS_FREE_CERTIFICATE, -1);
                        st.doTakeItems(CLASS_CERTIFICATE, -1);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if (event.equalsIgnoreCase("olympiad_operator_q0552_03.htm")) {
            ...
            if (st.getQuestItemsCount(CLASS_CERTIFICATE) > 0)
                count++;
            if (count > 0) {
                st.doGiveItems(OLYMPIAD_CHEST, count);
                st.doTakeItems(TEAM_CERTIFICATE, -1);
                st.doTakeItems(CLASS_FREE_CERTIFICATE, -1);
                st.doTakeItems(CLASS_CERTIFICATE, -1);
                st.playSound(SOUND_FINISH);
    ...
    public void onOlympiadEnd(OlympiadGame og, QuestState qs) {
        ...
        switch (og.getType()) {
            case TEAM:
                ...
                if (count1 == 5) {
                    qs.doGiveItems(TEAM_CERTIFICATE, 1);
            ...
            case CLASSED:
                ...
                if (count2 == 5) {
                    qs.doGiveItems(CLASS_CERTIFICATE, 1);
            ...
            case NON_CLASSED:
                ...
                if (count3 == 5) {
                    qs.doGiveItems(CLASS_FREE_CERTIFICATE, 1);
