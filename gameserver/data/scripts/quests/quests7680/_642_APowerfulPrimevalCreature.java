...
public class _642_APowerfulPrimevalCreature extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("dindin_q0642_04.htm") && _state == CREATED) {
            ...
            st.doTakeItems(Dinosaur_Tissue, -1);
            st.doGiveItems(ADENA_ID, RATE_QUESTS_REWARD * Dinosaur_Tissue_Count * 5000);
            ...
            try {
                ...
                for (int reward : Rewards)
                    if (reward == rew_id) {
                        st.doTakeItems(Dinosaur_Tissue, 150);
                        st.doTakeItems(Dinosaur_Egg, 1);
                        st.doGiveItems(reward, 1);
                        st.doRewardItems(ADENA_ID, 44000);
                        st.playSound(SOUND_MIDDLE);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npc.getNpcId() == Ancient_Egg)
            st.doGiveOnKillItems(Dinosaur_Egg, 1, 1, Dinosaur_Egg_Chance);
        else
            st.doGiveOnKillItems(Dinosaur_Tissue, 1, 1, Dinosaur_Tissue_Chance);
        return null;
    }
