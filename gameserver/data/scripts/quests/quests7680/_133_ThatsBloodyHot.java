...
public class _133_ThatsBloodyHot extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("priest_kanis_q0133_12.htm") && cond == 1) {
            st.setCond(2);
            st.doGiveItems(CRYSTAL_SAMPLE, 1);
        }

        if (event.equals("Galate_q0133_06.htm") && cond == 2) {
            st.playSound(SOUND_FINISH);
            st.doTakeItems(CRYSTAL_SAMPLE, -1);
            st.doRewardItems(ADENA_ID, 254247);
            st.addExpAndSp(RATE_QUESTS_REWARD * 331457, RATE_QUESTS_REWARD * 32524);
