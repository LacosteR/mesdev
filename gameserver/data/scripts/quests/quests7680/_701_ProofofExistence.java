...
public class _701_ProofofExistence extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("artius_q701_2.htm") && cond == 0) {
            ...
            if (st.getQuestItemsCount(DeadmansRemains) >= 1) {
                st.doRewardItems(ADENA_ID, st.getQuestItemsCount(DeadmansRemains) * 2500);
                st.doTakeItems(DeadmansRemains, -1);
            ...
            if (st.getQuestItemsCount(BansheeQueensEye) >= 1) {
                st.doGiveItems(ADENA_ID, st.getQuestItemsCount(BansheeQueensEye) * 1000000);
                st.doTakeItems(BansheeQueensEye, -1);
                htmltext = "artius_q701_4.htm";
            } else
                htmltext = "artius_q701_3a.htm";
        return htmltext;
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1 && npcId != Enira)
            st.doGiveOnKillItems(DeadmansRemains, 1, 1, 100);
        else if (cond == 1 && npcId == Enira)
            st.doGiveOnKillItems(BansheeQueensEye, 1, 1, 100);
