...
public class _10275_ContainingTheAttributePower extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30839-02.htm") || event.equalsIgnoreCase("31307-02.htm")) {
            ...
            if (item > 0) {
                st.doGiveItems(item, RATE_QUESTS_REWARD * 2);
                st.addExpAndSp(RATE_QUESTS_REWARD * 202160, RATE_QUESTS_REWARD * 20375);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (id == COMPLETED) {
            ...
            if (cond == 2)
                ...
                st.doTakeItems(YinSword, 1);
                st.doTakeItems(SoulPieceWater, -1);
            } else if (cond == 6)
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Air) {
            if (st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == YangSword && (cond == 8 || cond == 10)) {
                if (st.doGiveOnKillItems(SoulPieceAir, 1, 1, 6, 30))
                    st.setCond(cond + 1);
            }
        } else if (npcId == Water) {
            if (st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == YinSword && (cond == 3 || cond == 5)) {
                if (st.doGiveOnKillItems(SoulPieceWater, 1, 1, 6, 30))
                    st.setCond(cond + 1);
            }
        }
