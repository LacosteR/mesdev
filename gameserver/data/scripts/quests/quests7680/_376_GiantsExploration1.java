...
public class _376_GiantsExploration1 extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == HR_SOBLING) {
            ...
            if (cond == 2 & st.getQuestItemsCount(MST_BK) > 0) {
                ...
                st.doTakeItems(MST_BK, -1);
                st.doGiveItems(DICT2, 1);
                st.setCond(3);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        int cond = st.getCond();
        if (cond > 0) {
            st.doGiveOnKillItems(ANCIENT_PARCHMENT, 1, 1, DROP_RATE);

            if (cond == 1)
                st.doGiveOnKillItems(MST_BK, 1, 1, 1, DROP_RATE_BOOK);
        }
        return null;
    }
}