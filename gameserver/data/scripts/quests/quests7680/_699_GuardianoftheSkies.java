    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("lekon_q699_2.htm") && cond == 0) {
            ...
            if (st.getQuestItemsCount(VulturesGoldenFeather) >= 1) {
                st.doRewardItems(ADENA_ID, (st.getQuestItemsCount(VulturesGoldenFeather) * _featherprice));
                st.doTakeItems(VulturesGoldenFeather, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1)
            if (npcId == VultureRider1 || npcId == VultureRider2 || npcId == EliteRider) {
                st.doGiveItems(VulturesGoldenFeather, RATE_QUESTS_DROP * 1);
                st.playSound(SOUND_ITEMGET);
            } else if (npcId == Valdstone) {
                st.doGiveItems(VulturesGoldenFeather, RATE_QUESTS_DROP * 50);
                st.playSound(SOUND_ITEMGET);
            }
        return null;
    }
