...
public class _196_SevenSignsSealoftheEmperor extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        Reflection ref = player.getPositionComponent().getReflection();
        ...
        if (event.equalsIgnoreCase("iasonheine_q196_1d.htm")) {
            ...
            st.doGiveItems(SacredSwordofEinhasad, 1);
            st.doGiveItems(ElmoredenHolyWater, 1);
        } else if (event.equalsIgnoreCase("courtmagician_q196_2.htm")) {
            st.playSound(SOUND_ITEMGET);
            st.doGiveItems(CourtMagiciansMagicStaff, 1);
            ...
            st.setCond(5);
            st.playSound(SOUND_MIDDLE);
            st.doTakeItems(SealOfBinding, -1);
            st.doTakeItems(ElmoredenHolyWater, -1);
            st.doTakeItems(CourtMagiciansMagicStaff, -1);
            st.doTakeItems(SacredSwordofEinhasad, -1);
        } else if (event.equalsIgnoreCase("leon_q196_2.htm"))
            player.getPositionComponent().getReflection().collapse();
        else if (event.equalsIgnoreCase("iasonheine_q196_6.htm")) {
            st.setCond(6);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("wood_q196_2.htm"))
            if (player.getBaseClassId() == player.getActiveClassId()) {
                st.addExpAndSp(ConfigQuests.SEVEN_SIGN_QUEST_RATE * 52518015, ConfigQuests.SEVEN_SIGN_QUEST_RATE * 5817677);
