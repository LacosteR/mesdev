...
public class _901_HowLavasaurusesAreMade extends Quest implements ScriptFile {
            ...
            if (cond == 0) {
                ...
                if (st.getInt("collect") == 1)
                    ...
                    if (st.haveQuestItem(LAVASAURUS_STONE_FRAGMENT, 10) && st.haveQuestItem(LAVASAURUS_HEAD_FRAGMENT, 10) && st.haveQuestItem(LAVASAURUS_BODY_FRAGMENT, 10) && st.haveQuestItem(LAVASAURUS_HORN_FRAGMENT, 10)) {
                        ...
                        st.doTakeItems(LAVASAURUS_HEAD_FRAGMENT);
                        st.doTakeItems(LAVASAURUS_STONE_FRAGMENT);
                        st.doTakeItems(LAVASAURUS_BODY_FRAGMENT);
                        st.doTakeItems(LAVASAURUS_HORN_FRAGMENT);
                        st.set("collect", 1);
                    } else
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1) {
            ...
            st.doGiveOnKillItems(LAVASAURUS_STONE_FRAGMENT, 1, 1, 10, DROP_CHANCE);
            st.doGiveOnKillItems(LAVASAURUS_HEAD_FRAGMENT, 1, 1, 10, DROP_CHANCE);
            st.doGiveOnKillItems(LAVASAURUS_BODY_FRAGMENT, 1, 1, 10, DROP_CHANCE);
            st.doGiveOnKillItems(LAVASAURUS_HORN_FRAGMENT, 1, 1, 10, DROP_CHANCE);
