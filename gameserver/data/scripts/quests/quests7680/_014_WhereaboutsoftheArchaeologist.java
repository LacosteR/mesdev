...
public class _014_WhereaboutsoftheArchaeologist extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("trader_liesel_q0014_0104.htm")) {
            st.setCond(1);
            st.doGiveItems(LETTER_TO_ARCHAEOLOGIST, 1);
            st.setState(STARTED);
            st.playSound(SOUND_ACCEPT);
        } else if (event.equalsIgnoreCase("explorer_ghost_a_q0014_0201.htm")) {
            st.doTakeItems(LETTER_TO_ARCHAEOLOGIST, -1);
            st.addExpAndSp(RATE_QUESTS_REWARD * 325881, RATE_QUESTS_REWARD * 32524);
            st.doRewardItems(ADENA_ID, 136928);
