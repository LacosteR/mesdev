...
public class _1202_CrystalCaverns extends Quest implements ScriptFile {
    ...
    public String onSkillUse(NpcInstance npc, Skill skill, QuestState qs) {
        World world = worlds.get(qs.getPlayer().getPositionComponent().getReflectionId());
    ...
    public String onFirstTalk(NpcInstance npc, Player player) {
        World world = worlds.get(player.getPositionComponent().getReflectionId());
        int npcId = npc.getNpcId();
        ...
        if (spawn_captain && Rnd.chance(50)) {
            NpcInstance captain = addSpawnToInstance(KechisCaptain3, new Location(npc.getPositionComponent().getX() - 60, npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), 32116), 0, world.instanceId);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        int npcId = npc.getNpcId();
        World world = worlds.get(npc.getPositionComponent().getReflectionId());
        if (world == null)
            return null;
        ...
        switch (world.status) {
            case 0:
                if (npcId == GK1) {
                    st.doDropOnGroundItems(npc, BLUE_CORAL_KEY, 1);
                    runEmerald(world);
                } else if (npcId == GK2) {
                    st.doDropOnGroundItems(npc, RED_CORAL_KEY, 1);
                    runSteamRoom1(world);
                }
                break;
            case 1:
        ...
        long seedsCount = (long) (1 * ConfigRates.RATE_DROP_ITEMS);
        if (npcId == DARNEL) {
            addSpawnToInstance(ORACLE_GUIDE3, new Location(152760, 145944, -12584, 0), 0, world.instanceId);
            st.doGiveItems(Rnd.chance(50) ? WHITE_SEED_OF_EVIL_SHARD : BLACK_SEED_OF_EVIL_SHARD, seedsCount);
        } else if (npcId == KECHI) {
