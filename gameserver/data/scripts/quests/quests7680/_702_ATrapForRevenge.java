...
public class _702_ATrapForRevenge extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("take") && cond == 0) {
            ...
            st.doTakeItems(DRAKES_FLESH, -1);
            if (rand == 1)
                st.doGiveItems(LEONARD, RATE_QUESTS_REWARD * 3);
            else if (rand == 2)
                st.doGiveItems(ADAMANTINE, RATE_QUESTS_REWARD * 3);
            else if (rand == 3)
                st.doGiveItems(ORICHALCUM, RATE_QUESTS_REWARD * 3);

            st.doRewardItems(ADENA_ID, 157200);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 2 && (npcId == DRAK || npcId == MUTATED_DRAKE_WING))
            st.doGiveOnKillItems(DRAKES_FLESH, 1, 1, 100, 100);
