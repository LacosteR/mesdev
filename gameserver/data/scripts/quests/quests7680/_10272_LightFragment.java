...
public class _10272_LightFragment extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("orbyu_q10272_2.htm") && cond == 0) {
            ...
            if (st.getQuestItemsCount(DestroyedLightFragmentPowder) >= 100) {
                st.doTakeItems(DestroyedLightFragmentPowder, -1);
                st.doGiveItems(SacredLightFragment, 1);
                st.setCond(8);
                st.playSound(SOUND_MIDDLE);
            } else
                htmltext = "lekon_q10272_1a.htm";
        } else if (event.equalsIgnoreCase("artius_q10272_12.htm")) {
            st.doRewardItems(ADENA_ID, 556980);
            st.addExpAndSp(RATE_QUESTS_REWARD * 1009016, RATE_QUESTS_REWARD * 91363);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 5)
            st.doGiveOnKillItems(DestroyedDarknessFragmentPowder, 1, 1, 100, 100);
