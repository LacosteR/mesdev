    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("orbyu_q700_2.htm") && cond == 0) {
            ...
            if (st.getQuestItemsCount(SwallowedSkull) >= 1 || st.getQuestItemsCount(SwallowedSternum) >= 1 || st.getQuestItemsCount(SwallowedBones) >= 1) {
                ...
                st.doGiveItems(ADENA_ID, _adenatogive);
                if (st.getQuestItemsCount(SwallowedSkull) >= 1)
                    st.doTakeItems(SwallowedSkull, -1);
                if (st.getQuestItemsCount(SwallowedSternum) >= 1)
                    st.doTakeItems(SwallowedSternum, -1);
                if (st.getQuestItemsCount(SwallowedBones) >= 1)
                    st.doTakeItems(SwallowedBones, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1)
            if (npcId == MutantBird1 || npcId == MutantBird2 || npcId == DraHawk1 || npcId == DraHawk2) {
                st.doGiveOnKillItems(SwallowedBones, 1, 1, 100);
                st.doGiveOnKillItems(SwallowedSkull, 1, 1, 20);
                st.doGiveOnKillItems(SwallowedSternum, 1, 1, 20);
            } else if (npcId == Rok) {
                st.doGiveOnKillItems(SwallowedBones, 1, 1, 100);
                st.doGiveOnKillItems(SwallowedSkull, 1, 1, 30);
                st.doGiveOnKillItems(SwallowedSternum, 1, 1, 50);
            }
        return null;
    }
}