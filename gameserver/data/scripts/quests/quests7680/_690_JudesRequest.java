...
public class _690_JudesRequest extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            if (st.getQuestItemsCount(EVIL_WEAPON) >= 100) {
                ...
                st.doTakeItems(EVIL_WEAPON, 100);
                htmltext = "jude_q0690_07.htm";

            } else if (st.getQuestItemsCount(EVIL_WEAPON) > 0 && st.getQuestItemsCount(EVIL_WEAPON) < 100) {
                if (reward == 0)
                    st.doGiveItems(ISawswordP, 1);
                else if (reward == 1)
                    st.doGiveItems(IDisperserP, 1);
                else if (reward == 2)
                    st.doGiveItems(ISpiritP, 1);
                else if (reward == 3)
                    st.doGiveItems(IHeavyArmsP, 1);
                else if (reward == 4)
                    st.doGiveItems(ITridentP, 1);
                else if (reward == 5)
                    st.doGiveItems(IHammerP, 1);
                else if (reward == 6)
                    st.doGiveItems(IHandP, 1);
                else if (reward == 7)
                    st.doGiveItems(IHallP, 1);
                else if (reward == 8)
                    st.doGiveItems(ISpitterP, 1);

                st.playSound(SOUND_FINISH);
                st.doTakeItems(EVIL_WEAPON, 5);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        Player player = st.getRandomPartyMember(STARTED, ConfigOthers.PARTY_DISTRIBUTION_RANGE);
        ...
        if (player != null) {
            ...
            if (sts != null)
                sts.doGiveOnKillItems(EVIL_WEAPON, 1, 1, EVIL_WEAPON_CHANCE);
        }
        return null;
    }
}