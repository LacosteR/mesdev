...
public class _128_PailakaSongofIceandFire extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        int refId = player.getPositionComponent().getReflectionId();
        ...
        if (event.equalsIgnoreCase("Enter")) {
            ...
            st.doGiveItems(SpritesSword, 1);
            st.doGiveItems(TempleBookofSecrets1, 1);
        } else if (event.equalsIgnoreCase("32507-03.htm")) {
            st.setCond(4);
            st.playSound(SOUND_MIDDLE);
            st.doTakeItems(TempleBookofSecrets2, -1);
            st.doGiveItems(TempleBookofSecrets3, 1);
            if (st.getQuestItemsCount(EssenceofWater) == 0)
                htmltext = "32507-01.htm";
            else {
                st.doTakeItems(SpritesSword, -1);
                st.doTakeItems(EssenceofWater, -1);
                st.doGiveItems(EnhancedSpritesSword, 1);
            }
            addSpawnToInstance(PAPION, new Location(-53903, 181484, -4555, 30456), 0, refId);
        } else if (event.equalsIgnoreCase("32507-07.htm")) {
            st.setCond(7);
            st.playSound(SOUND_MIDDLE);
            st.doTakeItems(TempleBookofSecrets5, -1);
            st.doGiveItems(TempleBookofSecrets6, 1);
            if (st.getQuestItemsCount(EssenceofFire) == 0)
                htmltext = "32507-04.htm";
            else {
                st.doTakeItems(EnhancedSpritesSword, -1);
                st.doTakeItems(EssenceofFire, -1);
                st.doGiveItems(SwordofIceandFire, 1);
            }
            addSpawnToInstance(GARGOS, new Location(-61354, 183624, -4821, 63613), 0, refId);
        } else if (event.equalsIgnoreCase("32510-02.htm")) {
            st.doGiveItems(PailakaRing, 1);
            st.doGiveItems(PailakaEarring, 1);
            st.doGiveItems(ScrollofEscape, 1);
            st.addExpAndSp(RATE_QUESTS_REWARD * 810000, RATE_QUESTS_REWARD * 50000);
            st.playSound(SOUND_FINISH);
            st.exitCurrentQuest(false);
            player.setVitality(20000);
            player.getPositionComponent().getReflection().startCollapseTimer(60000);
        }
        return htmltext;
    }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (ArrayUtils.contains(MOBS, npcId)) {
            ...
            if (Rnd.get(100) < 50)
                st.doDropOnGroundItems(npc, HPHERBS[herbRnd], 1);
            if (Rnd.get(100) < 50)
                st.doDropOnGroundItems(npc, MPHERBS[herbRnd], 1);
        } else if (npcId == HILLAS && cond == 2) {
            st.doTakeItems(TempleBookofSecrets1, -1);
            st.doGiveItems(EssenceofWater, 1);
            st.doGiveItems(TempleBookofSecrets2, 1);
            st.setCond(3);
            st.playSound(SOUND_MIDDLE);
        } else if (npcId == PAPION && cond == 4) {
            st.doTakeItems(TempleBookofSecrets3, -1);
            st.doGiveItems(TempleBookofSecrets4, 1);
            st.setCond(5);
            st.playSound(SOUND_MIDDLE);
            addSpawnToInstance(KINSUS, new Location(-61404, 181351, -4815, 63953), 0, refId);
        } else if (npcId == KINSUS && cond == 5) {
            st.doTakeItems(TempleBookofSecrets4, -1);
            st.doGiveItems(EssenceofFire, 1);
            st.doGiveItems(TempleBookofSecrets5, 1);
            st.setCond(6);
            st.playSound(SOUND_MIDDLE);
        } else if (npcId == GARGOS && cond == 7) {
            st.doTakeItems(TempleBookofSecrets6, -1);
            st.doGiveItems(TempleBookofSecrets7, 1);
            st.setCond(8);
            st.playSound(SOUND_MIDDLE);
            addSpawnToInstance(ADIANTUM, new Location(-53297, 185027, -4617, 1512), 0, refId);
        } else if (npcId == ADIANTUM && cond == 8) {
            st.doTakeItems(TempleBookofSecrets7, -1);
            st.setCond(9);
            st.playSound(SOUND_MIDDLE);
            addSpawnToInstance(ADLER2, new Location(npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading()), 0, refId);
        } else if (npcId == Bloom) {
            if (Rnd.chance(50))
                st.doDropOnGroundItems(npc, PailakaInstantShield, Rnd.get(1, 7));
            if (Rnd.chance(30))
                st.doDropOnGroundItems(npc, QuickHealingPotion, Rnd.get(1, 7));
        } else if (npcId == CrystalWaterBottle) {
            if (Rnd.chance(50))
                st.doDropOnGroundItems(npc, PailakaInstantShield, Rnd.get(1, 10));
            if (Rnd.chance(30))
                st.doDropOnGroundItems(npc, QuickHealingPotion, Rnd.get(1, 10));
            if (Rnd.chance(10))
                st.doDropOnGroundItems(npc, WaterAttributeEnhancer, Rnd.get(1, 5));
            ...
            if (Rnd.chance(30))
                st.doDropOnGroundItems(npc, QuickHealingPotion, Rnd.get(1, 10));
            if (Rnd.chance(10))
                st.doDropOnGroundItems(npc, FireAttributeEnhancer, Rnd.get(1, 5));
        }
        return null;
    }
