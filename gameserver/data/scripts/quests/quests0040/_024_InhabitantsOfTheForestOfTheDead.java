...
public class _024_InhabitantsOfTheForestOfTheDead extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState qs) {
        ...
        if (npcId == DORIAN) {
            if (cond == 0) {
                ...
                qs.doTakeItems(BROKEN_SILVER_CROSS_OF_EINHASAD, -1);
            } else if (cond == 7 && qs.getQuestItemsCount(LIDIA_HAIR_PIN) == 0) {
                htmltext = "31389-21.htm";
                qs.doGiveItems(LIDIA_HAIR_PIN, 1);
                qs.setCond(8);
            }
            ...
            qs.doTakeItems(LIDIA_HAIR_PIN, -1);
    ...
    public String onEvent(String event, QuestState qs, NpcInstance npc) {
        if (event.startsWith("seePlayer")) {
            if (qs.getQuestItemsCount(SILVER_CROSS_OF_EINHASAD) > 0) {
                qs.doTakeItems(SILVER_CROSS_OF_EINHASAD, -1);
                qs.doGiveItems(BROKEN_SILVER_CROSS_OF_EINHASAD, 1);
                qs.playSound(SOUND_HORROR2);
                qs.setCond(4);
            }
            event = null;
        } else if (event.equalsIgnoreCase("31389-03.htm")) {
            qs.doGiveItems(FLOWER_BOUQUET, 1);
            qs.setCond(1);
            qs.setState(STARTED);
            qs.playSound(SOUND_ACCEPT);
        } else if (event.equalsIgnoreCase("31531-02.htm")) {
            qs.doTakeItems(FLOWER_BOUQUET, -1);
            qs.setCond(2);
        } else if (event.equalsIgnoreCase("31389-13.htm")) {
            qs.doGiveItems(SILVER_CROSS_OF_EINHASAD, 1);
            qs.setCond(3);
            ...
            qs.doTakeItems(SUSPICIOUS_TOTEM_DOLL, -1);
            ...
            qs.doGiveItems(SUSPICIOUS_TOTEM_DOLL1, 1);
            ...
            qs.addExpAndSp(RATE_QUESTS_REWARD * 242105, RATE_QUESTS_REWARD * 22529);
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        if (ArrayUtils.contains(MOBS, npcId))
            if (cond == 9 && qs.doGiveOnKillItems(SUSPICIOUS_TOTEM_DOLL, 1, 1, 1, 70)) {
                qs.setCond(10);
            }
    ...
    public void onAbort(QuestState st) {
        if (st.getQuestItemsCount(LIDIA_HAIR_PIN) == 0)
            st.doGiveItems(LIDIA_HAIR_PIN, 1);
    }
}
