        ...
        if (event.equalsIgnoreCase("1")) {
            if (event.equalsIgnoreCase("1"))
                if (st.getPlayer().getPcClass().class_id != 0x19) {
                    if (st.getPlayer().getPcClass().class_id == 0x1d)
                    ...
                    st.doGiveItems(CRYSTAL_MEDALLION_ID, 1);
        ...
        if (npcId == MANUEL) {
            if (cond < 1)
                ...
                if (st.getQuestItemsCount(MONEY_OF_SWINDLER_ID) < 1 && st.getQuestItemsCount(DAIRY_OF_ALLANA_ID) < 1 && st.getQuestItemsCount(LIZARD_CAPTAIN_ORDER_ID) < 1 && st.getQuestItemsCount(HALF_OF_DAIRY_ID) < 1)
                    ...
                    st.doTakeItems(MONEY_OF_SWINDLER_ID, 1);
                    st.doTakeItems(DAIRY_OF_ALLANA_ID, -1);
                    st.doTakeItems(LIZARD_CAPTAIN_ORDER_ID, -1);
                    st.doTakeItems(CRYSTAL_MEDALLION_ID, -1);
                    if (st.getPlayer().getPcClass().getLevel() == 1) {
                        st.doGiveItems(LEAF_OF_ORACLE_ID, 1);
            ...
            if (st.getQuestItemsCount(CRYSTAL_MEDALLION_ID) > 0)
                if (st.getQuestItemsCount(MONEY_OF_SWINDLER_ID) < 1 && st.getQuestItemsCount(DAIRY_OF_ALLANA_ID) < 1 && st.getQuestItemsCount(LIZARD_CAPTAIN_ORDER_ID) < 1 && st.getQuestItemsCount(HALF_OF_DAIRY_ID) < 1) {
                    ...
                    st.doGiveItems(HALF_OF_DAIRY_ID, 1);
                    st.setCond(4);
                    ...
                    st.doTakeItems(HALF_OF_DAIRY_ID, -1);
                    st.doGiveItems(DAIRY_OF_ALLANA_ID, 1);
                    st.setCond(7);
            ...
            if (st.getQuestItemsCount(CRYSTAL_MEDALLION_ID) > 0 && st.getQuestItemsCount(LIZARD_CAPTAIN_ORDER_ID) > 0)
                if (st.getQuestItemsCount(TAMATOS_NECKLACE_ID) > 0) {
                    ...
                    st.doTakeItems(TAMATOS_NECKLACE_ID, -1);
                    st.doGiveItems(MONEY_OF_SWINDLER_ID, 1);
                    st.setCond(6);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == LIZARDMAN_WARRIOR | npcId == LIZARDMAN_SCOUT | npcId == LIZARDMAN) {
            if (cond == 2 && st.getQuestItemsCount(LIZARD_CAPTAIN_ORDER_ID) < 1) {
                st.doGiveItems(LIZARD_CAPTAIN_ORDER_ID, 1);
                st.playSound(SOUND_MIDDLE);
                st.setCond(3);
            }
        } else if (npcId == TAMIL)
            if (cond == 4 && st.getQuestItemsCount(TAMATOS_NECKLACE_ID) < 1) {
                st.doGiveItems(TAMATOS_NECKLACE_ID, 1);
                st.playSound(SOUND_MIDDLE);
                st.setCond(5);
            }
        return null;
