...
public class _105_SkirmishWithOrcs extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if (event.equalsIgnoreCase("sentinel_kendnell_q0105_03.htm")) {
            ...
            if (st.getQuestItemsCount(Kendells1stOrder) + st.getQuestItemsCount(Kendells2stOrder) + st.getQuestItemsCount(Kendells3stOrder) + st.getQuestItemsCount(Kendells4stOrder) == 0) {
                int n = Rnd.get(4);
                if (n == 0)
                    st.doGiveItems(Kendells1stOrder, 1);
                else if (n == 1)
                    st.doGiveItems(Kendells2stOrder, 1);
                else if (n == 2)
                    st.doGiveItems(Kendells3stOrder, 1);
                else
                    st.doGiveItems(Kendells4stOrder, 1);
            }
        }
        return event;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            st.doTakeItems(Kendells1stOrder, -1);
            st.doTakeItems(Kendells2stOrder, -1);
            st.doTakeItems(Kendells3stOrder, -1);
            st.doTakeItems(Kendells4stOrder, -1);
            st.doTakeItems(KabooChiefs1stTorque, 1);
            int n = Rnd.get(4);
            if (n == 0)
                st.doGiveItems(Kendells5stOrder, 1);
            else if (n == 1)
                st.doGiveItems(Kendells6stOrder, 1);
            else if (n == 2)
                st.doGiveItems(Kendells7stOrder, 1);
            else
                st.doGiveItems(Kendells8stOrder, 1);
            st.setCond(3);
            st.setState(STARTED);
            ...
            st.doTakeItems(Kendells5stOrder, -1);
            st.doTakeItems(Kendells6stOrder, -1);
            st.doTakeItems(Kendells7stOrder, -1);
            st.doTakeItems(Kendells8stOrder, -1);
            st.doTakeItems(KabooChiefs2stTorque, -1);

            if (st.getPlayer().getPcClass().isMage())
                st.doGiveItems(RED_SUNSET_STAFF, 1);
            else
                st.doGiveItems(RED_SUNSET_SWORD, 1);

            st.doRewardItems(ADENA_ID, 17599);
            st.getPlayer().addExpAndSp(RATE_QUESTS_REWARD * 41478, RATE_QUESTS_REWARD * 3555);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1 && st.getQuestItemsCount(KabooChiefs1stTorque) == 0) {
            if (npcId == KabooChiefUoph && st.getQuestItemsCount(Kendells1stOrder) > 0)
                st.doGiveItems(KabooChiefs1stTorque, 1);
            ...
            if (npcId == KabooChiefTurel && st.getQuestItemsCount(Kendells5stOrder) > 0)
                st.doGiveItems(KabooChiefs2stTorque, 1);
