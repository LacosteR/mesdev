...
public class _281_HeadForTheHills extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("zerstorer_morsell_q0281_03.htm")) {
            ...
            st.doGiveItems(ADENA_ID, st.getQuestItemsCount(HillsOfGoldMonsterClaw) * 50);
            st.doTakeItems(HillsOfGoldMonsterClaw, -1);
            ...
            if (st.getQuestItemsCount(HillsOfGoldMonsterClaw) >= 50) {
                st.doTakeItems(HillsOfGoldMonsterClaw, 50);
                st.doGiveItems(ScrollOfEscape, 5);
                tryGiveOneTimeRevard(st);
                htmltext = "zerstorer_morsell_q0281_06.htm";
            } else
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        for (int[] element : DROPLIST)
            if (npcId == element[0]) {
                st.doGiveOnKillItems(HillsOfGoldMonsterClaw, 1, 1, element[1]);
                return null;
            }
        return null;
    }
