...
public class _066_CertifiedArbalester extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("32201-05.htm")) {
            ...
            st.set("id", "0");
            st.doTakeItems(EnmityCrystalCore, 1);
            st.setCond(5);
            st.setState(STARTED);
            ...
            st.doTakeItems(ManuscriptPage, -1);
            st.setCond(9);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("32214-03.htm")) {
            st.doGiveItems(KamaelInquisitorTraineeMark, 1);
            st.setCond(10);
            st.setState(STARTED);
            ...
            st.doTakeItems(FragmentofAttackOrders, -1);
            st.doTakeItems(KamaelInquisitorTraineeMark, -1); //возможно не тут забирают
            st.setCond(15);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30717-07.htm")) {
            st.doTakeItems(ManashenTalisman, -1);
            st.setCond(17);
            st.setState(STARTED);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Rindy) {
            if (st.getQuestItemsCount(KamaelInquisitorMark) > 0) {
                ...
                if (st.getPlayer().getPcClass().class_id == 0x7e) {
            ...
            if (cond == 2)
                ...
                if (st.getInt("id") == 0 && st.getQuestItemsCount(EnmityCrystal) >= 30) {
                    ...
                    st.doTakeItems(EnmityCrystal, -1);
                    st.set("id", "1");
            ...
            if (cond == 10)
                ...
                st.doTakeItems(ResearchOnTheGiantsAndTheAncientRace, -1);
                if (!st.getPlayer().getVarB("prof2.1")) {
                    st.addExpAndSp(429546, 29476);
                    st.doRewardItems(ADENA_ID, 77666);
                    st.getPlayer().setVar("prof2.1", "1", -1);
                }
                st.doGiveItems(KamaelInquisitorMark, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        for (int i = 0; i < DROPLIST_COND.length; i++) {
            if (cond == DROPLIST_COND[i][0] && npcId == DROPLIST_COND[i][2]) {
                if (st.doGiveOnKillItems(DROPLIST_COND[i][4], DROPLIST_COND[i][7], DROPLIST_COND[i][7], DROPLIST_COND[i][5], DROPLIST_COND[i][6])) {
        ...
        if (cond == 19 && (npcId == TimakOrcArcher || npcId == TimakOrcSoldier)) {
            ...
            st.doGiveItems(ResearchOnTheGiantsAndTheAncientRace, 1);
            st.setCond(20);
            st.setState(STARTED);
        }
        return null;
