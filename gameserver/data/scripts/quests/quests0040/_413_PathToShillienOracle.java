...
public class _413_PathToShillienOracle extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            ...
            st.doGiveItems(SIDRAS_LETTER1_ID, 1);
        } else if (event.equalsIgnoreCase("413_1")) {
            if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x26 && st.getQuestItemsCount(ORB_OF_ABYSS_ID) < 1)
                htmltext = "master_sidra_q0413_05.htm";
            else if (st.getPlayer().getPcClass().class_id != 0x26) {
                if (st.getPlayer().getPcClass().class_id == 0x2a)
                    htmltext = "master_sidra_q0413_02a.htm";
                else
                    htmltext = "master_sidra_q0413_03.htm";
            } else if (st.getPlayer().getLevel() < 18 && st.getPlayer().getPcClass().class_id == 0x26)
                htmltext = "master_sidra_q0413_02.htm";
            else if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x26 && st.getQuestItemsCount(ORB_OF_ABYSS_ID) > 0)
                htmltext = "master_sidra_q0413_04.htm";
        } else if (event.equalsIgnoreCase("30377_1")) {
            htmltext = "magister_talbot_q0413_02.htm";
            st.doTakeItems(SIDRAS_LETTER1_ID, -1);
            st.doGiveItems(BLANK_SHEET1_ID, 5);
            ...
            st.doTakeItems(PRAYER_OF_ADON_ID, -1);
            st.doGiveItems(PENITENTS_MARK_ID, 1);
            st.playSound(SOUND_ITEMGET);
            st.setCond(5);
        }
        return htmltext;
        ...
        if (npcId == SIDRA) {
            if (cond < 1)
                ...
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(ORB_OF_ABYSS_ID, 1);
            ...
            if (cond == 1 && st.getQuestItemsCount(SIDRAS_LETTER1_ID) > 0)
                ...
                st.doTakeItems(BLOODY_RUNE1_ID, -1);
                st.doGiveItems(GARMIEL_BOOK_ID, 1);
                st.doGiveItems(PRAYER_OF_ADON_ID, 1);
                st.playSound(SOUND_ITEMGET);
                st.setCond(4);
            } else if (cond > 3 && cond < 7)
            ...
            if (cond == 4 && st.getQuestItemsCount(PRAYER_OF_ADON_ID) > 0)
                ...
                st.doTakeItems(ASHEN_BONES_ID, -1);
                st.doTakeItems(PENITENTS_MARK_ID, -1);
                st.doGiveItems(ANDARIEL_BOOK_ID, 1);
                st.playSound(SOUND_ITEMGET);
                st.setCond(7);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == DARK_SUCCUBUS)
            if (cond == 2 && st.getQuestItemsCount(BLANK_SHEET1_ID) > 0) {
                st.doGiveItems(BLOODY_RUNE1_ID, 1);
                st.doTakeItems(BLANK_SHEET1_ID, 1);
        ...
        for (int i : ASHEN_BONES_DROP)
            if (npcId == i && cond == 5 && st.getQuestItemsCount(ASHEN_BONES_ID) < 10) {
                st.doGiveItems(ASHEN_BONES_ID, 1);
