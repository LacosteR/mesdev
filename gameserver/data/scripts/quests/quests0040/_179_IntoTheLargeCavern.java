...
public class _179_IntoTheLargeCavern extends Quest implements ScriptFile, OnDeathListener {
    ...
    public void onDeath(Creature cha, Creature killer) {
        ...
        switch (cha.getNpcId()) {
            case Kamael_Guard:
                st.doDropOnGroundItems(npc, Gate_Key_Kamael, 1);
                break;
            case Guardian_of_Records:
                st.doDropOnGroundItems(npc, Gate_Key_Archives, 1);
                break;
            case Guardian_of_Observation:
                st.doDropOnGroundItems(npc, Gate_Key_Observation, 1);
                break;
            case Spiculas_Guard:
                st.doDropOnGroundItems(npc, Gate_Key_Spicula, 1);
                break;
            case Harkilgameds_Gatekeeper:
                st.doDropOnGroundItems(npc, Gate_Key_Harkilgamed, 1);
                break;
            case Rodenpiculas_Gatekeeper:
                st.doDropOnGroundItems(npc, Gate_Key_Rodenpicula, 1);
                break;
            case Guardian_of_Arviterre:
                st.doDropOnGroundItems(npc, Gate_Key_Arviterre, 1);
                break;
            case Katenars_Gatekeeper:
                st.doDropOnGroundItems(npc, Gate_Key_Katenar, 1);
                break;
            case Guardian_of_Prediction:
                st.doDropOnGroundItems(npc, Gate_Key_Prediction, 1);
                break;
            case Guardian_of_Secrets:
                st.doDropOnGroundItems(npc, Gate_Key_Massive_Cavern, 1);
                break;
        }
    }
