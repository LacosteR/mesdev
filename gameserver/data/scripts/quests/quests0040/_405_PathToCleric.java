...
public class _405_PathToCleric extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1"))
            if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x0a && st.getQuestItemsCount(MARK_OF_FAITH) < 1) {
                st.setCond(1);
                st.setState(STARTED);
                st.playSound(SOUND_ACCEPT);
                st.doGiveItems(LETTER_OF_ORDER1, 1);
                htmltext = "gigon_q0405_05.htm";
            } else if (st.getPlayer().getPcClass().class_id != 0x0a) {
                if (st.getPlayer().getPcClass().class_id == 0x0f)
                    htmltext = "gigon_q0405_02a.htm";
                else
                    htmltext = "gigon_q0405_02.htm";
            } else if (st.getPlayer().getLevel() < 18 && st.getPlayer().getPcClass().class_id == 0x0a)
                htmltext = "gigon_q0405_03.htm";
            else if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x0a && st.getQuestItemsCount(MARK_OF_FAITH) > 0)
        ...
        if (npcId == ZIGAUNT) {
            ...
            if (cond < 1 && st.getQuestItemsCount(MARK_OF_FAITH) < 1)
                ...
                if (st.getQuestItemsCount(BOOK_OF_VIVI) > 0 && st.getQuestItemsCount(BOOK_OF_SIMLON) > 2 && st.getQuestItemsCount(BOOK_OF_PRAGA) > 0) {
                    ...
                    st.doTakeItems(BOOK_OF_PRAGA, -1);
                    st.doTakeItems(BOOK_OF_VIVI, -1);
                    st.doTakeItems(BOOK_OF_SIMLON, -1);
                    st.doTakeItems(LETTER_OF_ORDER1, -1);
                    st.doGiveItems(LETTER_OF_ORDER2, 1);
                    st.setCond(3);
                } else
                ...
                st.doTakeItems(LEMONIELLS_COVENANT, -1);
                st.doTakeItems(LETTER_OF_ORDER2, -1);
                if (!st.getPlayer().getVarB("q405"))
                    st.getPlayer().setVar("q405", "1", -1);
                st.exitCurrentQuest(true);
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(MARK_OF_FAITH, 1);
            ...
            if (st.getQuestItemsCount(BOOK_OF_SIMLON) < 1) {
                ...
                st.doGiveItems(BOOK_OF_SIMLON, 3);
                checkBooks(st);
            ...
            if (st.getQuestItemsCount(BOOK_OF_PRAGA) < 1 && st.getQuestItemsCount(NECKLACE_OF_MOTHER) < 1) {
                ...
                st.doGiveItems(NECKLACE_OF_MOTHER, 1);
                ...
                st.doTakeItems(NECKLACE_OF_MOTHER, -1);
                st.doTakeItems(PENDANT_OF_MOTHER, -1);
                st.doGiveItems(BOOK_OF_PRAGA, 1);
                checkBooks(st);
            ...
            if (st.getQuestItemsCount(LETTER_OF_ORDER2) < 1)
                ...
                st.doGiveItems(BOOK_OF_LEMONIELL, 1);
                st.setCond(4);
                ...
                st.doTakeItems(CERTIFICATE_OF_GALLINT, -1);
                st.doGiveItems(LEMONIELLS_COVENANT, 1);
                st.setCond(6);
            ...
            if (cond == 4 && st.getQuestItemsCount(BOOK_OF_LEMONIELL) > 0 && st.getQuestItemsCount(CERTIFICATE_OF_GALLINT) < 1) {
                ...
                st.doTakeItems(BOOK_OF_LEMONIELL, -1);
                st.doGiveItems(CERTIFICATE_OF_GALLINT, 1);
                st.setCond(5);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == RUIN_ZOMBIE | npcId == RUIN_ZOMBIE_LEADER)
            if (st.getCond() == 1 && st.getQuestItemsCount(PENDANT_OF_MOTHER) < 1) {
                st.doGiveItems(PENDANT_OF_MOTHER, 1);
