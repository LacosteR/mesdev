...
public class _369_CollectorOfJewels extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1)
            ...
            st.doTakeItems(FLARE_SHARD, -1);
            st.doTakeItems(FREEZING_SHARD, -1);
            if (cond == 2) {
                htmltext = "30376-05.htm";
                st.doGiveItems(ADENA_ID, 38810);
                st.playSound(SOUND_MIDDLE);
                st.setCond(3);
            } else {
                htmltext = "30376-10.htm";
                st.doGiveItems(ADENA_ID, 84415);
                st.playSound(SOUND_FINISH);
                st.exitCurrentQuest(true);
            }
        }
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        if (qs.getQuestItemsCount(drop[0]) < max_count && qs.doGiveOnKillItems(drop[0], 1, 1, max_count, drop[1]) && qs.getQuestItemsCount(FLARE_SHARD) >= max_count && qs.getQuestItemsCount(FREEZING_SHARD) >= max_count)
