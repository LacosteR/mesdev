...
public class _168_DeliverSupplies extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("1")) {
            ...
            st.doGiveItems(JENNIES_LETTER_ID, 1);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30349 && cond == 0) {
            ...
            st.doTakeItems(SENTRY_BLADE1_ID, 1);
            st.setCond(3);
            ...
            st.doTakeItems(OLD_BRONZE_SWORD_ID, 2);
            st.unset("cond");
            st.playSound(SOUND_FINISH);
            st.doRewardItems(ADENA_ID, 820);
            st.exitCurrentQuest(false);
        } else if (npcId == 30360 && cond == 1 && st.getQuestItemsCount(JENNIES_LETTER_ID) == 1) {
            htmltext = "30360-01.htm";
            st.doTakeItems(JENNIES_LETTER_ID, 1);
            st.doGiveItems(SENTRY_BLADE1_ID, 1);
            st.doGiveItems(SENTRY_BLADE2_ID, 1);
            st.doGiveItems(SENTRY_BLADE3_ID, 1);
            st.setCond(2);
            ...
            st.doTakeItems(SENTRY_BLADE2_ID, 1);
            st.doGiveItems(OLD_BRONZE_SWORD_ID, 1);
            ...
            st.doTakeItems(SENTRY_BLADE3_ID, 1);
            st.doGiveItems(OLD_BRONZE_SWORD_ID, 1);
