...
public class _212_TrialOfDuty extends Quest implements ScriptFile {
    ...
    private static final int RewardExp = RATE_QUESTS_REWARD * 762576;
    private static final int RewardSP = RATE_QUESTS_REWARD * 49458;
    private static final int RewardAdena = RATE_QUESTS_REWARD * 138968;
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            ...
            st.doTakeItems(TEAR_OF_LOYALTY_ID, 1);
            st.setCond(14);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30109 && id == CREATED) {
            if (st.getPlayer().getPcClass().ordinal() == 0x04 || st.getPlayer().getPcClass().ordinal() == 0x13 || st.getPlayer().getPcClass().ordinal() == 0x20)
            ...
            if (cond == 18 && st.getQuestItemsCount(LETTER_OF_DUSTIN_ID) > 0) {
                ...
                st.doTakeItems(LETTER_OF_DUSTIN_ID, -1);
                st.doGiveItems(MARK_OF_DUTY_ID, 1);
            ...
            if (cond == 1) {
                ...
                if (st.getQuestItemsCount(OLD_KNIGHT_SWORD_ID) == 0)
                    st.doGiveItems(OLD_KNIGHT_SWORD_ID, 1);
                st.setCond(2);
                ...
                st.doTakeItems(KNIGHTS_TEAR_ID, 1);
                st.doTakeItems(OLD_KNIGHT_SWORD_ID, 1);
                st.setCond(4);
            } else if (cond == 4)
            ...
            if (cond == 4) {
                ...
                st.setCond(7);
                st.doGiveItems(MIRROR_OF_ORPIC_ID, 1);
                ...
                st.doTakeItems(TEAR_OF_CONFESSION_ID, 1);
                st.setCond(10);
            } else if (cond == 10)
            ...
            st.doTakeItems(MIRROR_OF_ORPIC_ID, 1);
            st.doTakeItems(TALIANUSS_REPORT_ID, 1);
            st.doGiveItems(TEAR_OF_CONFESSION_ID, 1);
            st.setCond(9);
        } else if (npcId == 30655) {
            if (cond == 10)
                ...
                st.doTakeItems(MILITAS_ARTICLE_ID, st.getQuestItemsCount(MILITAS_ARTICLE_ID));
                st.doGiveItems(TEAR_OF_LOYALTY_ID, 1);
                st.setCond(13);
            } else if (cond == 13)
            ...
            if (cond == 13 && st.getQuestItemsCount(TEAR_OF_LOYALTY_ID) > 0)
                ...
                st.doTakeItems(ATEBALTS_SKULL_ID, 1);
                st.doTakeItems(ATEBALTS_RIBS_ID, 1);
                st.doTakeItems(ATEBALTS_SHIN_ID, 1);
                st.doGiveItems(SAINTS_ASHES_URN_ID, 1);
                st.setCond(16);
            } else if (cond == 17 && st.getQuestItemsCount(LETTER_OF_WINDAWOOD_ID) > 0) {
                htmltext = "dustin_q0212_08.htm";
                st.doTakeItems(LETTER_OF_WINDAWOOD_ID, 1);
                st.doGiveItems(LETTER_OF_DUSTIN_ID, 1);
                st.setCond(18);
            } else if (cond == 16)
            ...
            if (cond == 16 && st.getQuestItemsCount(SAINTS_ASHES_URN_ID) > 0) {
                ...
                st.doTakeItems(SAINTS_ASHES_URN_ID, 1);
                st.doGiveItems(LETTER_OF_WINDAWOOD_ID, 1);
                st.setCond(17);
            } else if (cond == 17)
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 20190 || npcId == 20191) {
            ...
            st.doGiveItems(KNIGHTS_TEAR_ID, 1);
            st.playSound(SOUND_MIDDLE);
            st.setCond(3);
        } else if ((npcId == 20200 || npcId == 20201) && cond == 5 && st.getQuestItemsCount(TALIANUSS_REPORT_ID) == 0) {
            if (Rnd.chance(50)) {
                st.doGiveItems(REPORT_PIECE_ID, 1);
                st.playSound(SOUND_ITEMGET);
            }
            if (st.getQuestItemsCount(REPORT_PIECE_ID) >= 10) {
                st.doTakeItems(REPORT_PIECE_ID, st.getQuestItemsCount(REPORT_PIECE_ID));
                st.doGiveItems(TALIANUSS_REPORT_ID, 1);
                st.setCond(6);
                st.playSound(SOUND_MIDDLE);
            }
        } else if (npcId == 20144 && cond == 7 && Rnd.chance(20)) {
            st.addSpawn(30656, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), 300000);
            st.setCond(8);
            st.playSound(SOUND_MIDDLE);
        } else if (npcId >= 20577 && npcId <= 20582 && cond == 11 && st.getQuestItemsCount(MILITAS_ARTICLE_ID) < 20) {
            st.doGiveItems(MILITAS_ARTICLE_ID, 1);
            ...
            if (st.getQuestItemsCount(ATEBALTS_SKULL_ID) == 0) {
                st.doGiveItems(ATEBALTS_SKULL_ID, 1);
                st.playSound(SOUND_ITEMGET);
            } else if (st.getQuestItemsCount(ATEBALTS_RIBS_ID) == 0) {
                st.doGiveItems(ATEBALTS_RIBS_ID, 1);
                st.playSound(SOUND_ITEMGET);
            } else if (st.getQuestItemsCount(ATEBALTS_SHIN_ID) == 0) {
                st.doGiveItems(ATEBALTS_SHIN_ID, 1);
                st.setCond(15);
                st.playSound(SOUND_ITEMGET);
            }
        return null;
