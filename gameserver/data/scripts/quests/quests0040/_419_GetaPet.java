...
public class _419_GetaPet extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("details"))
            ...
            switch (st.getPlayer().getRace()) {
                case human:
                    st.doGiveItems(ANIMAL_SLAYER_LIST1, 1);
                    htmltext = "419_slay_0.htm";
                    break;
                case elf:
                    st.doGiveItems(ANIMAL_SLAYER_LIST2, 1);
                    htmltext = "419_slay_1.htm";
                    break;
                case darkelf:
                    st.doGiveItems(ANIMAL_SLAYER_LIST3, 1);
                    htmltext = "419_slay_2.htm";
                    break;
                case orc:
                    st.doGiveItems(ANIMAL_SLAYER_LIST4, 1);
                    htmltext = "419_slay_3.htm";
                    break;
                case dwarf:
                    st.doGiveItems(ANIMAL_SLAYER_LIST5, 1);
                    htmltext = "419_slay_4.htm";
                    break;
                case kamael:
                    st.doGiveItems(ANIMAL_SLAYER_LIST6, 1);
            ...
            if (event.equalsIgnoreCase("tryme"))
                ...
                st.unset("answers");
                st.unset("question");
                st.doGiveItems(ANIMAL_LOVERS_LIST1, 1);
            ...
            if (npcId == PET_MANAGER_MARTIN) {
                if (StateId == 0) {
                    ...
                    if (counts == 0)
                        ...
                        switch (st.getPlayer().getRace()) {
                            case human:
                                st.doTakeItems(ANIMAL_SLAYER_LIST1, -1);
                                st.doTakeItems(SPIDER_LEG1, -1);
                                break;
                            case elf:
                                st.doTakeItems(ANIMAL_SLAYER_LIST2, -1);
                                st.doTakeItems(SPIDER_LEG2, -1);
                                break;
                            case darkelf:
                                st.doTakeItems(ANIMAL_SLAYER_LIST3, -1);
                                st.doTakeItems(SPIDER_LEG3, -1);
                                break;
                            case orc:
                                st.doTakeItems(ANIMAL_SLAYER_LIST4, -1);
                                st.doTakeItems(SPIDER_LEG4, -1);
                                break;
                            case dwarf:
                                st.doTakeItems(ANIMAL_SLAYER_LIST5, -1);
                                st.doTakeItems(SPIDER_LEG5, -1);
                                break;
                            case kamael:
                                st.doTakeItems(ANIMAL_SLAYER_LIST6, -1);
                                st.doTakeItems(SPIDER_LEG6, -1);
                        }
                        st.set("id", "1");
                        st.doGiveItems(ANIMAL_LOVERS_LIST1, 1);
                        htmltext = "Slayed.htm";
                    ...
                    if (st.getInt("progress") == 7) {
                        st.doTakeItems(ANIMAL_LOVERS_LIST1, -1);
