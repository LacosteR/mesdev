...
public class _355_FamilyHonor extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("galicbredo_q0355_03.htm"))
            ...
            st.doTakeItems(BUST_OF_ANCIENT_GODDESS, count);
            st.doGiveItems(WORK_OF_BERONA, count);
        ...
        if (npcId == GALIBREDO) {
            if (cond == 0)
                ...
                if (count > 0) {
                    ...
                    st.doTakeItems(GALFREDOS_BUST, count);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1) {
            if (Rnd.chance(CHANCE_FOR_GALFREDOS_BUST)) {
                st.doGiveItems(GALFREDOS_BUST, 1);
                st.playSound(SOUND_ITEMGET);
            }
            if (Rnd.chance(CHANCE_FOR_GODDESS_BUST))
                st.doGiveItems(BUST_OF_ANCIENT_GODDESS, 1);
        }
        return null;
    }
}