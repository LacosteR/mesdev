...
public class _167_DwarvenKinship extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30350-04.htm")) {
            st.doGiveItems(CarlonsLetter, 1);
            st.playSound(SOUND_ACCEPT);
            st.setCond(1);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30255-03.htm")) {
            st.doTakeItems(CarlonsLetter, -1);
            st.doGiveItems(ADENA_ID, 2000);
            st.doGiveItems(NormansLetter, 1);
            st.setCond(2);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30255-04.htm")) {
            st.doTakeItems(CarlonsLetter, -1);
            st.doGiveItems(ADENA_ID, 15000);
            st.playSound(SOUND_GIVEUP);
            st.exitCurrentQuest(false);
        } else if (event.equalsIgnoreCase("30210-02.htm")) {
            st.doTakeItems(NormansLetter, -1);
            st.doRewardItems(ADENA_ID, 22000);
