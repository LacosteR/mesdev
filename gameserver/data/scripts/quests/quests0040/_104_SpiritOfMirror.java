...
public class _104_SpiritOfMirror extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if (event.equalsIgnoreCase("gallin_q0104_03.htm")) {
            ...
            st.doGiveItems(GALLINS_OAK_WAND, 3);
        }
        return event;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30017) {
            if (cond == 0)
                ...
                st.doTakeItems(WAND_SPIRITBOUND1, -1);
                st.doTakeItems(WAND_SPIRITBOUND2, -1);
                st.doTakeItems(WAND_SPIRITBOUND3, -1);

                st.doGiveItems(WAND_OF_ADEPT, 1);
                st.doRewardItems(ADENA_ID, 16866);
                st.getPlayer().addExpAndSp(RATE_QUESTS_REWARD * 39750, RATE_QUESTS_REWARD * 3407);
        ...
        if ((cond == 1 || cond == 2) && st.getPlayer().getActiveWeaponInstance() != null && st.getPlayer().getActiveWeaponInstance().getItemId() == GALLINS_OAK_WAND) {
            ...
            if (npcId == 27003 && st.getQuestItemsCount(WAND_SPIRITBOUND1) == 0) {
                if (st.getPlayer().getInventory().destroyItem(weapon, 1L)) {
                    st.doGiveItems(WAND_SPIRITBOUND1, 1);
                ...
                if (st.getPlayer().getInventory().destroyItem(weapon, 1L)) {
                    st.doGiveItems(WAND_SPIRITBOUND2, 1);
                    st.getPlayer().sendPacket(CACHE_SYSMSG_GALLINS_OAK_WAND);
                    long Collect = st.getQuestItemsCount(WAND_SPIRITBOUND1) + st.getQuestItemsCount(WAND_SPIRITBOUND2) + st.getQuestItemsCount(WAND_SPIRITBOUND3);
                    if (Collect == 3) {
                        st.setCond(3);
                ...
                if (st.getPlayer().getInventory().destroyItem(weapon, 1L)) {
                    st.doGiveItems(WAND_SPIRITBOUND3, 1);
                    st.getPlayer().sendPacket(CACHE_SYSMSG_GALLINS_OAK_WAND);
                    long Collect = st.getQuestItemsCount(WAND_SPIRITBOUND1) + st.getQuestItemsCount(WAND_SPIRITBOUND2) + st.getQuestItemsCount(WAND_SPIRITBOUND3);
                    if (Collect == 3) {
                        st.setCond(3);
