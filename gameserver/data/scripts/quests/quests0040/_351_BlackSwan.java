...
public class _351_BlackSwan extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30916-03.htm")) {
            st.setState(STARTED);
            st.setCond(1);
            st.doGiveItems(ORDER_OF_GOSTA, 1);
            st.playSound(SOUND_ACCEPT);
        } else if (event.equalsIgnoreCase("30969-02a.htm") && amount > 0) {
            htmltext = "30969-02.htm";
            st.doGiveItems(ADENA_ID, amount * 30);
            st.doTakeItems(LIZARD_FANG, -1);
        } else if (event.equalsIgnoreCase("30969-03a.htm") && amount2 > 0) {
            htmltext = "30969-03.htm";
            st.setCond(2);
            st.doGiveItems(ADENA_ID, amount2 * 500);
            st.doGiveItems(BILL_OF_IASON_HEINE, amount2);
            st.doTakeItems(BARREL_OF_LEAGUE, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        st.doGiveOnKillItems(LIZARD_FANG, 1, 1, CHANCE * mod);
        st.doGiveOnKillItems(BARREL_OF_LEAGUE, 1, 1, CHANCE2 * mod);
        return null;
    }
}