...
public class _230_TestOfSummoner extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30634-08.htm")) { // start part for Galatea
            ...
            st.doGiveItems(CRYSTAL_OF_PROGRESS1_ID, 1); // give Starting Crystal
            st.doTakeItems(CRYSTAL_OF_FOUL1_ID, -1); // just in case he cheated or loses
            st.doTakeItems(CRYSTAL_OF_DEFEAT1_ID, -1);
            st.doTakeItems(BEGINNERS_ARCANA_ID, 1);
            ...
            st.doGiveItems(CRYSTAL_OF_PROGRESS2_ID, 1);
            st.doTakeItems(CRYSTAL_OF_FOUL2_ID, -1);
            st.doTakeItems(CRYSTAL_OF_DEFEAT2_ID, -1);
            st.doTakeItems(BEGINNERS_ARCANA_ID, 1);
            ...
            st.doGiveItems(CRYSTAL_OF_PROGRESS3_ID, 1);
            st.doTakeItems(CRYSTAL_OF_FOUL3_ID, -1);
            st.doTakeItems(CRYSTAL_OF_DEFEAT3_ID, -1);
            st.doTakeItems(BEGINNERS_ARCANA_ID, 1);
            ...
            st.doGiveItems(CRYSTAL_OF_PROGRESS4_ID, 1);
            st.doTakeItems(CRYSTAL_OF_FOUL4_ID, -1);
            st.doTakeItems(CRYSTAL_OF_DEFEAT4_ID, -1);
            st.doTakeItems(BEGINNERS_ARCANA_ID, 1);
            ...
            st.doGiveItems(CRYSTAL_OF_PROGRESS5_ID, 1);
            st.doTakeItems(CRYSTAL_OF_FOUL5_ID, -1);
            st.doTakeItems(CRYSTAL_OF_DEFEAT5_ID, -1);
            st.doTakeItems(BEGINNERS_ARCANA_ID, 1);
            ...
            st.doGiveItems(CRYSTAL_OF_PROGRESS6_ID, 1);
            st.doTakeItems(CRYSTAL_OF_FOUL6_ID, -1);
            st.doTakeItems(CRYSTAL_OF_DEFEAT6_ID, -1);
            st.doTakeItems(BEGINNERS_ARCANA_ID, 1);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (id == CREATED && npcId == 30634) { // start part, Galatea
            for (String var : STATS)
                st.set(var, "0");
            if (st.getPlayer().getPcClass() == PcClass.wizard || st.getPlayer().getPcClass() == PcClass.elven_wizard || st.getPlayer().getPcClass() == PcClass.dark_wizard)
            ...
            if (npcId == 30634) { // Start&&End Npc Galatea related stuff
                if (step == 1) // step 1 means just started
                    ...
                    if (Arcanas == 6) { // finished all battles... the player is able to earn the marks
                        ...
                        st.doTakeItems(LARS_LIST1_ID, -1);
                        st.doTakeItems(LARS_LIST2_ID, -1);
                        st.doTakeItems(LARS_LIST3_ID, -1);
                        st.doTakeItems(LARS_LIST4_ID, -1);
                        st.doTakeItems(LARS_LIST5_ID, -1);
                        st.doTakeItems(ALMORS_ARCANA_ID, -1);
                        st.doTakeItems(BASILLIA_ARCANA_ID, -1);
                        st.doTakeItems(CAMONIELL_ARCANA_ID, -1);
                        st.doTakeItems(CELESTIEL_ARCANA_ID, -1);
                        st.doTakeItems(BELTHUS_ARCANA_ID, -1);
                        st.doTakeItems(BRYNTHEA_ARCANA_ID, -1);
                        st.doGiveItems(MARK_OF_SUMMONER_ID, 1);
                        if (!st.getPlayer().getVarB("prof2.3")) {
                            st.addExpAndSp(RATE_QUESTS_REWARD * 1664494, RATE_QUESTS_REWARD * 114220);
                            st.doRewardItems(ADENA_ID, 300960);
                ...
                if (step == 1) // first talk to lara
                    ...
                    if (ItemCount1 < 30 || ItemCount2 < 30) // if( you have not enough materials, List 1 - 5
                        ...
                        st.doGiveItems(BEGINNERS_ARCANA_ID, 2);
                        st.doTakeItems(LISTS[LaraPart][0], 1);
                        st.doTakeItems(LISTS[LaraPart][1], -1);
                        st.doTakeItems(LISTS[LaraPart][2], -1);
                        st.setCond(3);
                        st.set("Lara_Part", "0");
                    }
                }
                    ...
                    if (i[0] == npcId) {
                        ...
                        if (step > 1)
                            if (st.getQuestItemsCount(k[0]) > 0) // ready to fight... already take the mission to kill his pet
                                ...
                                st.doTakeItems(SUMMONERS[npcId - 30635][2], -1); // take crystal of victory
                                st.doGiveItems(SUMMONERS[npcId - 30635][1], 1);// give arcana
    ...
    public String onAttack(NpcInstance npc, QuestState st) { // on the first attack, the stat is in battle... anytime gives crystal and set stat
        ...
        if (npcId >= 27102 && npcId <= 27107) {
            ...
            if (st.getInt(var) == 2) {
                st.set(var, "3");
                st.doGiveItems(progress, 1);
                st.doTakeItems(start, 1);
        ...
        if (DROPLIST_LARA.containsKey(npcId)) {
            ...
            if (st.getInt(var) == 3) {
                ...
                for (Integer item : DROPLIST_SUMMON.get(npcId)) { // take all crystal of this summoner away from the player
                    if (isName != 1)
                        st.doTakeItems(item, -1);
                    isName = 0;
                }

                st.doTakeItems(progress, -1);
                if (isFoul) {
                    st.set(var, "6");
                    st.doGiveItems(victory, 1); // if he wons without cheating, set stat won and give victory crystal
                    st.playSound(SOUND_MIDDLE);
                } else
