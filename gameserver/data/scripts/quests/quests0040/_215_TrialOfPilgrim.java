...
public class _215_TrialOfPilgrim extends Quest implements ScriptFile {
    ...
    private static final int RewardExp = (int) (RATE_QUESTS_REWARD * 1258250);
    private static final int RewardSP = (int) (RATE_QUESTS_REWARD * 81606);
    private static final int RewardAdena = (int) (RATE_QUESTS_REWARD * 229298);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("1")) {
            ...
            st.doGiveItems(VOUCHER_OF_TRIAL_ID, 1);
        }
        if (!st.getPlayer().getVarB("dd1")) {
            st.doGiveItems(7562, RATE_QUESTS_REWARD * 49);
            ...
            st.doGiveItems(SPIRIT_OF_FLAME_ID, 1);
            st.doTakeItems(ESSENSE_OF_FLAME_ID, 1);
            st.setCond(5);
        } else if (event.equals("30650_1")) {
            if (st.getQuestItemsCount(ADENA_ID) >= 100000) {
                htmltext = "gerald_priest_of_earth_q0215_02.htm";
                st.doGiveItems(BOOK_OF_GERALD_ID, 1);
                st.doTakeItems(ADENA_ID, 100000);
                st.setCond(7);
            } else
            ...
            st.doTakeItems(BOOK_OF_DARKNESS_ID, 1);
            st.setCond(16);
            ...
            st.doGiveItems(BOOK_OF_DARKNESS_ID, 1);
            st.doTakeItems(DEBRIS_OF_WILLOW_ID, 1);
            st.setCond(15);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30648 && st.getCond() == 0) {
            if (st.getPlayer().getPcClass().class_id == 0x0f || st.getPlayer().getPcClass().class_id == 0x1d || st.getPlayer().getPcClass().class_id == 0x2a || st.getPlayer().getPcClass().class_id == 0x32) {
            ...
            st.doTakeItems(BOOK_OF_SAGE_ID, -1);
            st.doGiveItems(MARK_OF_PILGRIM_ID, 1);
            ...
            st.doTakeItems(VOUCHER_OF_TRIAL_ID, 1);
            st.setCond(2);
            ...
            st.doGiveItems(TAG_OF_RUMOR_ID, 1);
            st.setCond(6);
            ...
            st.doGiveItems(PICTURE_OF_NAHIR_ID, 1);
            st.setCond(10);
            ...
            st.doGiveItems(STATUE_OF_EINHASAD_ID, 1);
            st.doTakeItems(PICTURE_OF_NAHIR_ID, 1);
            st.doTakeItems(HAIR_OF_NAHIR_ID, 1);
            st.setCond(12);
            ...
            st.doGiveItems(BOOK_OF_SAGE_ID, 1);
            ...
            if (st.getQuestItemsCount(BOOK_OF_GERALD_ID) > 0)
                st.doTakeItems(BOOK_OF_GERALD_ID, 1);
            st.setCond(17);
            st.doTakeItems(GREY_BADGE_ID, 1);
            st.doTakeItems(SPIRIT_OF_FLAME_ID, 1);
            st.doTakeItems(STATUE_OF_EINHASAD_ID, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        int npcId = npc.getNpcId();
        if (npcId == 27116) {
            if (st.getCond() == 3 && st.doGiveOnKillItems(ESSENSE_OF_FLAME_ID, 1, 1, 1, 30))
                st.setCond(4);
        } else if (npcId == 27117) {
            if (st.getCond() == 10 && st.doGiveOnKillItems(HAIR_OF_NAHIR_ID, 1, 1, 1, 100))
                st.setCond(11);
        } else if (npcId == 27118)
            if (st.getCond() == 13 && st.doGiveOnKillItems(DEBRIS_OF_WILLOW_ID, 1, 1, 1, 20))
                st.setCond(14);
