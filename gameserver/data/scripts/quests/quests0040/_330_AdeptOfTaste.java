...
public class _330_AdeptOfTaste extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30469_03.htm") && _state == CREATED) {
            if (st.getQuestItemsCount(Ingredient_List) == 0)
                st.doGiveItems(Ingredient_List, 1);
            st.setState(STARTED);
            st.setCond(1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Jonas) {
            if (Has_Ingredient_List) {
                ...
                st.doTakeItems(Ingredient_List, -1);
                st.doTakeItems(ingredients, -1);
                st.doTakeItems(spec_ingredients, -1);
                ...
                st.doGiveItems(Jonass_Steak_Dishes[(int) spec_ingredients_count], 1);
                ...
                if (Jonass_Steak_Dish_count == 0 && Miriens_Review_count > 0)
                    for (int i = Miriens_Reviews.length; i > 0; i--)
                        if (st.getQuestItemsCount(Miriens_Reviews[i - 1]) > 0) {
                            st.doTakeItems(Miriens_Reviews, -1);
                            if (adena_rewards[i - 1] > 0)
                                st.doGiveItems(ADENA_ID, adena_rewards[i - 1]);
                            if (rewards[i - 1] > 0)
                                st.doGiveItems(rewards[i - 1], 1);
            ...
            if (all_ingredients_count == 0) {
                ...
                for (int i = Jonass_Steak_Dishes.length; i > 0; i--)
                    if (st.getQuestItemsCount(Jonass_Steak_Dishes[i - 1]) > 0) {
                        st.doTakeItems(Jonass_Steak_Dishes, -1);
                        st.playSound(SOUND_MIDDLE);
                        st.doGiveItems(Miriens_Reviews[i - 1], 1);
        ...
        if (npcId == Sonia) {
            ...
            if (st.getQuestItemsCount(Sonias_Botany_Book) > 0) {
                ...
                st.doGiveItems(Sonias_Botany_Book, 1);
                return "30062_01.htm";
            }
        }
        ...
        if (npcId == Glyvka) {
            ...
            if (st.getQuestItemsCount(Glyvkas_Botany_Book) > 0) {
                ...
                return "30067_07.htm";
            {
                st.doGiveItems(Glyvkas_Botany_Book, 1);
                return "30067_01.htm";
            }
        }
            ...
            if (st.getQuestItemsCount(Rollants_Creature_Book) > 0) {
                if (!has_meat) {
                    if (st.getQuestItemsCount(Body_of_Monster_Eye) < 30)
                        return "30069_02.htm";
                    st.doTakeItems(Rollants_Creature_Book, -1);
                    st.doTakeItems(Body_of_Monster_Eye, -1);
                    st.playSound(SOUND_MIDDLE);
                    st.doGiveItems(Meat_of_Monster_Eye, 1);
                    return "30069_03.htm";
                }
            } else if (has_meat)
                return "30069_04.htm";
            else {
                st.doGiveItems(Rollants_Creature_Book, 1);
                return "30069_01.htm";
            }
        }
        ...
        if (npcId == Jacob) {
            ...
            if (st.getQuestItemsCount(Jacobs_Insect_Book) > 0) {
                ...
                st.doGiveItems(Jacobs_Insect_Book, 1);
                return "30073_01.htm";
            }
        }
            ...
            if (st.getQuestItemsCount(Panos_Contract) > 0) {
                if (!has_potato) {
                    if (st.getQuestItemsCount(Hobgoblin_Amulet) < 30)
                        return "30078_02.htm";
                    st.doTakeItems(Panos_Contract, -1);
                    st.doTakeItems(Hobgoblin_Amulet, -1);
                    st.playSound(SOUND_MIDDLE);
                    st.doGiveItems(Dionian_Potato, 1);
                    return "30078_03.htm";
                }
            } else if (has_potato)
                return "30078_04.htm";
            else {
                st.doGiveItems(Panos_Contract, 1);
                return "30078_01.htm";
            }
        }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Hobgoblin && st.getQuestItemsCount(Panos_Contract) > 0)
            st.doGiveOnKillItems(Hobgoblin_Amulet, 1, 1, 30, 100);
            ...
            st.doGiveOnKillItems(Body_of_Monster_Eye, 1, 3, 30, 97);
        else if (npcId == Monster_Eye_Gazer && st.getQuestItemsCount(Rollants_Creature_Book) > 0)
            st.doGiveOnKillItems(Body_of_Monster_Eye, 1, 2, 30, 100);
    ...
    private static void MandragoraDrop(QuestState st, int i1, int i2) {
        int i = Rnd.get(100);
        if (i < i1)
            st.doGiveOnKillItems(Red_Mandragora_Root, 1, 1, 40, 100);
        else if (i < i2)
            st.doGiveOnKillItems(White_Mandragora_Root, 1, 1, 40, 100);
    }

    private static void BeeDrop(QuestState st, int i1, int i2) {
        int i = Rnd.get(100);
        if (i < i1)
            st.doGiveOnKillItems(Nectar, 1, 1, 20, 100);
        else if (i < i2)
            st.doGiveOnKillItems(Royal_Jelly, 1, 1, 10, 100);
    }

    private static void AntDrop(QuestState st, int i1, int i2) {
        int i = Rnd.get(100);
        if (i < i1)
            st.doGiveOnKillItems(Green_Marsh_Moss, 1, 1, 20, 100);
        else if (i < i2)
            st.doGiveOnKillItems(Brown_Marsh_Moss, 1, 1, 20, 100);
    }

    private static void Root2Sap(QuestState st, int sap_id) {
        st.doTakeItems(Sonias_Botany_Book, -1);
        st.doTakeItems(White_Mandragora_Root, -1);
        st.doTakeItems(Red_Mandragora_Root, -1);
        st.playSound(SOUND_MIDDLE);
        st.doGiveItems(sap_id, 1);
    }

    private static void Moss2Bundle(QuestState st, int bundle_id) {
        st.doTakeItems(Glyvkas_Botany_Book, -1);
        st.doTakeItems(Brown_Marsh_Moss, -1);
        st.doTakeItems(Green_Marsh_Moss, -1);
        st.playSound(SOUND_MIDDLE);
        st.doGiveItems(bundle_id, 1);
    }

    private static void Nectar2Honey(QuestState st, int honey_id) {
        st.doTakeItems(Jacobs_Insect_Book, -1);
        st.doTakeItems(Nectar, -1);
        st.doTakeItems(Royal_Jelly, -1);
        st.playSound(SOUND_MIDDLE);
        st.doGiveItems(honey_id, 1);
    }
