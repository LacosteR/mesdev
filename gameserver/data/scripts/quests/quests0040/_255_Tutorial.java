...
public class _255_Tutorial extends Quest implements ScriptFile, OnPlayerEnterListener {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        int classId = player.getPcClass().class_id;
        int Ex = st.getInt("Ex");
        ...
        if (event.startsWith("UC")) {
            ...
            if (level < 6 && st.getInt("onlyone") == 0) {
                ...
                st.showQuestionMark(36);
                st.playSound(SOUND_TUTORIAL);
            } else if (level == 38 && player.getQuestState("_10278_MutatedKaneusHeine") == null) {
                st.showQuestionMark(36);
                st.playSound(SOUND_TUTORIAL);
            } else if (level == 48 && player.getQuestState("_10279_MutatedKaneusOren") == null) {
                st.showQuestionMark(36);
                st.playSound(SOUND_TUTORIAL);
            } else if (level == 58 && player.getQuestState("_10280_MutatedKaneusSchuttgart") == null) {
                st.showQuestionMark(36);
                st.playSound(SOUND_TUTORIAL);
            } else if (level == 68 && player.getQuestState("_10281_MutatedKaneusRune") == null) {
                st.showQuestionMark(36);
                st.playSound(SOUND_TUTORIAL);
            } else if (level == ConfigExternal.LEVEL_FOR_PRESENT && player.getQuestState("_192_SevenSignSeriesOfDoubt") == null) {
                st.showQuestionMark(36);
                st.playSound(SOUND_TUTORIAL);
                if (!AccountDAO.getInstance().checkIsPresentObtained(player.getAccountName())) {
                    AccountDAO.getInstance().setIsPresentObtained(player.getAccountName());
                    st.getPlayer().getInventory().addItem(ConfigExternal.ID_PRESENT, ConfigExternal.COUNT_PRESENT); // present
                }
            }
        }
        ...
        // Client Event
            ...
            if (event_id == 1 && player.getLevel() < 6) {
                ...
                if (player.getLevel() == 5)
                    if (st.getInt("lvl") < 5 && !player.getPcClass().isMage() || classId == 49) {
                ...
                if (player.getLevel() == 6) {
                    if (st.getInt("lvl") < 6 && player.getPcClass().getLevel() == 0) {
                    ...
                    if (st.getInt("lvl") < 7 && player.getPcClass().isMage() && classId != 49 && player.getPcClass().getLevel() == 0) {
                    ...
                    if (st.getInt("lvl") < 19 && player.getRace() != Race.kamael && player.getPcClass().getLevel() == 0)
                        switch (classId) {
                            case 0:
                            case 10:
                            case 18:
                    ...
                    if (st.getInt("lvl") < 35 && player.getRace() != Race.kamael && player.getPcClass().getLevel() == 1)
                        switch (classId) {
                            case 1:
                            case 4:
                            case 7:
                    ...
                    if (st.getInt("lvl") < 79) {
                        ...
                        if (!AccountDAO.getInstance().checkIsPresentObtained(player.getAccountName())) {
                            AccountDAO.getInstance().setIsPresentObtained(player.getAccountName());
                            st.getPlayer().getInventory().addItem(20638, 1); // present
                        }
                    }
                }
        ...
        // Question mark clicked
            ...
            if (MarkId == 1) {
                ...
                if (player.getPcClass().isMage() && classId != 49)
