    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == ARUJIEN) {
            if (cond == 0) {
                ...
                htmltext = "arujien_q0002_09.htm";
                st.doTakeItems(GREENIS_LETTER, -1);
                st.doGiveItems(MYSTICS_EARRING, 1);
                st.doRewardItems(ADENA_ID, 620 + 1850); // T2
                st.getPlayer().addExpAndSp(RATE_QUESTS_REWARD * 4254, RATE_QUESTS_REWARD * 335);
                if (st.getPlayer().getPcClass().getLevel() == 1 && !st.getPlayer().getVarB("ng1"))
                    st.getPlayer().sendPacket(new ExShowScreenMessage("  Delivery duty complete.\nGo find the Newbie Guide.", 5000, ScreenMessageAlign.TOP_CENTER, true));
                st.playSound(SOUND_FINISH);
            ...
            if (cond == 1 && st.getQuestItemsCount(ARUJIENS_LETTER1) > 0) {
                ...
                st.doTakeItems(ARUJIENS_LETTER1, -1);
                st.doGiveItems(ARUJIENS_LETTER2, 1);
                st.setCond(2);
                st.playSound(SOUND_MIDDLE);
            } else if (cond == 2)
            ...
            if (cond == 2 && st.getQuestItemsCount(ARUJIENS_LETTER2) > 0) {
                ...
                st.doTakeItems(ARUJIENS_LETTER2, -1);
                st.doGiveItems(ARUJIENS_LETTER3, 1);
                st.setCond(3);
                st.playSound(SOUND_MIDDLE);
            } else if (cond == 3)
            ...
            if (cond == 4 && st.getQuestItemsCount(POETRY_BOOK) > 0) {
                ...
                st.doTakeItems(POETRY_BOOK, -1);
                st.doGiveItems(GREENIS_LETTER, 1);
                st.setCond(5);
                st.playSound(SOUND_MIDDLE);
            } else if (cond == 5 && st.getQuestItemsCount(GREENIS_LETTER) > 0)
