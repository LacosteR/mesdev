...
public class _229_TestOfWitchcraft extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30630-08.htm")) {
            ...
            st.doGiveItems(AlexandriasBook, 1);
            st.doTakeItems(OrimsDiagram, 1);
            st.setCond(2);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30110-03.htm"))
            st.doGiveItems(IkersList, 1);
        else if (event.equalsIgnoreCase("30476-02.htm"))
            st.doGiveItems(Aklantoth_2stGem, 1);
        else if (event.equalsIgnoreCase("30063-02.htm"))
            st.doGiveItems(LarasMemo, 1);
        else if (event.equalsIgnoreCase("30314-02.htm"))
            st.doGiveItems(NestlesMemo, 1);
        else if (event.equalsIgnoreCase("30435-02.htm")) {
            st.doTakeItems(NestlesMemo, 1);
            st.doGiveItems(LeopoldsJournal, 1);
            ...
            if (isQuest != null && !isQuest.isDead())
                ...
                st.doTakeItems(AlexandriasBook, 1);
                st.doTakeItems(Aklantoth_1stGem, 1);
                st.doTakeItems(Aklantoth_2stGem, 1);
                st.doTakeItems(Aklantoth_3stGem, 1);
                st.doTakeItems(Aklantoth_4stGem, 1);
                st.doTakeItems(Aklantoth_5stGem, 1);
                st.doTakeItems(Aklantoth_6stGem, 1);
                if (st.getQuestItemsCount(Brimstone_1st) == 0)
                    st.doGiveItems(Brimstone_1st, 1);
                st.setCond(4);
                st.set("id", "1");
            ...
            st.doTakeItems(Brimstone_1st, -1);
            st.doGiveItems(OrimsInstructions, 1);
            st.doGiveItems(Orims1stLetter, 1);
            st.doGiveItems(Orims2stLetter, 1);
            st.setCond(6);
        } else if (event.equalsIgnoreCase("30110-08.htm")) {
            st.doTakeItems(Orims2stLetter, 1);
            st.doGiveItems(SoultrapCrystal, 1);
            st.doGiveItems(IkersAmulet, 1);
            if (st.getQuestItemsCount(SwordOfBinding) > 0) {
                st.setCond(7);
                st.setState(STARTED);
            }
        } else if (event.equalsIgnoreCase("30417-03.htm")) {
            st.doTakeItems(Orims1stLetter, 1);
            st.doGiveItems(SirVaspersLetter, 1);
            ...
            if (isQuest != null)
                ...
                if (st.getQuestItemsCount(Brimstone_2nd) == 0)
                    st.doGiveItems(Brimstone_2nd, 1);
            ...
            st.doTakeItems(ZeruelBindCrystal, 1);
        else if (event.equalsIgnoreCase("30630-21.htm"))
            st.doTakeItems(PurgatoryKey, 1);
        else if (event.equalsIgnoreCase("30630-22.htm")) {
            st.doTakeItems(SwordOfBinding, -1);
            st.doTakeItems(IkersAmulet, -1);
            st.doTakeItems(OrimsInstructions, -1);
            if (!st.getPlayer().getVarB("prof2.3")) {
                st.addExpAndSp(RATE_QUESTS_REWARD * 2058244, RATE_QUESTS_REWARD * 141240);
                st.doRewardItems(ADENA_ID, 372154);
                st.getPlayer().setVar("prof2.3", "1", -1);
            }
            st.doGiveItems(MarkOfWitchcraft, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Orim) {
            if (st.getQuestItemsCount(MarkOfWitchcraft) != 0) {
                ...
                if (st.getPlayer().getPcClass().class_id == 0x0b || st.getPlayer().getPcClass().class_id == 0x04 || st.getPlayer().getPcClass().class_id == 0x20)
                    if (st.getPlayer().getLevel() < 39) {
                        htmltext = "30630-02.htm";
                        st.exitCurrentQuest(true);
                    } else if (st.getPlayer().getPcClass().class_id == 0x0b)
            ...
            if (cond == 2) {
                if (st.getQuestItemsCount(Aklantoth_1stGem) == 0 && st.getQuestItemsCount(IkersList) == 0)
                    ...
                    st.doTakeItems(IkersList, -1);
                    st.doTakeItems(DireWyrmFang, -1);
                    st.doTakeItems(LetoLizardmanCharm, -1);
                    st.doTakeItems(EnchantedGolemHeartstone, -1);
                    st.doGiveItems(Aklantoth_1stGem, 1);
            ...
            if (cond == 6) {
                if (st.getQuestItemsCount(SirVaspersLetter) > 0 || st.getQuestItemsCount(VadinsCrucifix) > 0)
                    ...
                    st.doTakeItems(VadinsSanctions, 1);
                    st.doGiveItems(SwordOfBinding, 1);
            ...
            if (cond == 6) {
                if (st.getQuestItemsCount(SirVaspersLetter) != 0) {
                    ...
                    st.doTakeItems(SirVaspersLetter, 1);
                    st.doGiveItems(VadinsCrucifix, 1);
                    ...
                    st.doTakeItems(TamlinOrcAmulet, -1);
                    st.doTakeItems(VadinsCrucifix, -1);
                    st.doGiveItems(VadinsSanctions, 1);
                } else if (st.getQuestItemsCount(VadinsSanctions) > 0)
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 2 && st.getQuestItemsCount(LeopoldsJournal) > 0 && npcId == SkeletalMercenary) {
            ...
            if (st.getQuestItemsCount(Aklantoth_4stGem) != 0 && st.getQuestItemsCount(Aklantoth_5stGem) != 0 && st.getQuestItemsCount(Aklantoth_6stGem) != 0) {
                st.doTakeItems(LeopoldsJournal, -1);
            ...
            if (st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == SwordOfBinding) {
                st.doTakeItems(Brimstone_2nd, 1);
                st.doTakeItems(SoultrapCrystal, 1);
                st.doGiveItems(PurgatoryKey, 1);
                st.doGiveItems(ZeruelBindCrystal, 1);
                st.playSound(SOUND_MIDDLE);
                st.unset("id");
                st.setCond(10);
                st.setState(STARTED);
