...
public class _416_PathToOrcShaman extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("tataru_zu_hestui_q0416_06.htm")) {
            st.doGiveItems(FireCharm, 1);
            st.setState(STARTED);
            st.setCond(1);
            st.playSound(SOUND_ACCEPT);
        } else if (event.equalsIgnoreCase("hestui_totem_spirit_q0416_03.htm")) {
            st.doTakeItems(HestuiMask, -1);
            st.doTakeItems(FieryEgg2nd, -1);
            st.doGiveItems(TotemSpiritClaw, 1);
            st.setCond(4);
        } else if (event.equalsIgnoreCase("tataru_zu_hestui_q0416_11.htm")) {
            st.doTakeItems(TotemSpiritClaw, -1);
            st.doGiveItems(TatarusLetterOfRecommendation, 1);
            st.setCond(5);
            ...
            st.doTakeItems(BloodCauldron, -1);
            st.doGiveItems(SpiritNet, 1);
            st.setCond(9);
        } else if (event.equalsIgnoreCase("seer_umos_q0416_07.htm")) {
            st.doTakeItems(TotemSpiritBlood, -1);
            if (st.getPlayer().getPcClass().getLevel() == 1) {
                st.doGiveItems(MaskOfMedium, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Hestui) {
            if (st.getQuestItemsCount(MaskOfMedium) != 0) {
                ...
                if (st.getPlayer().getPcClass().class_id != 0x31) {
                ...
                st.doTakeItems(KashaBearPelt, -1);
                st.doTakeItems(KashaBladeSpiderHusk, -1);
                st.doTakeItems(FieryEgg1st, -1);
                st.doTakeItems(FireCharm, -1);
                st.doGiveItems(HestuiMask, 1);
                st.doGiveItems(FieryEgg2nd, 1);
                st.setCond(3);
            } else if (cond == 3)
            ...
            if (cond == 5) {
                st.doTakeItems(TatarusLetterOfRecommendation, -1);
                st.doGiveItems(FlameCharm, 1);
                htmltext = "seer_umos_q0416_01.htm";
                st.setCond(6);
            } else if (cond == 6)
                htmltext = "seer_umos_q0416_02.htm";
            else if (cond == 7) {
                st.doTakeItems(GrizzlyBlood, -1);
                st.doTakeItems(FlameCharm, -1);
                st.doGiveItems(BloodCauldron, 1);
            ...
            if (cond == 8)
                ...
                st.doTakeItems(BoundDurkaSpirit, -1);
                st.doGiveItems(TotemSpiritBlood, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.getQuestItemsCount(KashaBearPelt) != 0 && st.getQuestItemsCount(KashaBladeSpiderHusk) != 0 && st.getQuestItemsCount(FieryEgg1st) != 0)
            ...
            if (st.getQuestItemsCount(DurkaParasite) < 8) {
                st.doGiveItems(DurkaParasite, 1);
            ...
            if (st.getQuestItemsCount(DurkaParasite) >= 8 || st.getQuestItemsCount(DurkaParasite) >= 5 && Rnd.chance(st.getQuestItemsCount(DurkaParasite) * 10))
                if (GameObjectsStorage.getByNpcId(QuestMonsterDurkaSpirit) == null) {
                    st.doTakeItems(DurkaParasite, -1);
            ...
            for (NpcInstance qnpc : GameObjectsStorage.getAllByNpcId(QuestMonsterDurkaSpirit, false))
                qnpc.deleteMe();
            if (cond == 9) {
                st.doTakeItems(SpiritNet, -1);
                st.doTakeItems(DurkaParasite, -1);
                st.doGiveItems(BoundDurkaSpirit, 1);
