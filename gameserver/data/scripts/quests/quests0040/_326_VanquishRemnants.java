            ...
            if (st.getPlayer().getLevel() < 21) {
                ...
                if (st.getQuestItemsCount(RedCrossBadge) + st.getQuestItemsCount(BlueCrossBadge) + st.getQuestItemsCount(BlackCrossBadge) >= 100) {
                    if (st.getQuestItemsCount(BlackLionMark) == 0) {
                        ...
                        st.doGiveItems(BlackLionMark, 1);
                    } else
                        htmltext = "leopold_q0326_06.htm";
                } else
                    htmltext = "leopold_q0326_05.htm";
                st.doGiveItems(ADENA_ID, (st.getQuestItemsCount(RedCrossBadge) * 46) + (st.getQuestItemsCount(BlueCrossBadge) * 54) + (st.getQuestItemsCount(BlackCrossBadge) * 68));
                st.doTakeItems(RedCrossBadge, -1);
                st.doTakeItems(BlueCrossBadge, -1);
                st.doTakeItems(BlackCrossBadge, -1);
            }
        return htmltext;
    }
        ...
        if (st.getState() == STARTED)
            for (int i = 0; i < DROPLIST_COND.length; i++)
                if (npc.getNpcId() == DROPLIST_COND[i][0])
                    st.doGiveOnKillItems(DROPLIST_COND[i][1], 1, 1, 100);
        return null;
    }
}