...
public class _135_TempleExecutor extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("shegfield_q0135_03.htm") && _state == CREATED) {
            ...
            st.unset("Report");
            st.doRewardItems(ADENA_ID, 16294);
            st.addExpAndSp(RATE_QUESTS_REWARD * 30000, RATE_QUESTS_REWARD * 2000);
            st.doGiveItems(Badge_Temple_Executor, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Shegfield && _state == STARTED) {
            ...
            if (cond == 5) {
                ...
                if (st.getQuestItemsCount(Sonins_Credentials) > 0 && st.getQuestItemsCount(Panos_Credentials) > 0 && st.getQuestItemsCount(Alexs_Credentials) > 0) {
                    st.doTakeItems(Panos_Credentials, -1);
                    st.doTakeItems(Sonins_Credentials, -1);
                    st.doTakeItems(Alexs_Credentials, -1);
                    st.set("Report", "1");
        ...
        if (npcId == Alex && _state == STARTED) {
            ...
            if (cond == 4) {
                if (st.getQuestItemsCount(Sonins_Credentials) > 0 && st.getQuestItemsCount(Panos_Credentials) > 0) {
                    st.setCond(5);
                    st.doTakeItems(Old_Treasure_Map, -1);
                    st.doGiveItems(Alexs_Credentials, 1);
                    st.playSound(SOUND_MIDDLE);
        ...
        if (npcId == Sonin && _state == STARTED) {
            ...
            st.doTakeItems(Stolen_Cargo, -1);
            st.doGiveItems(Sonins_Credentials, 1);
            st.playSound(SOUND_MIDDLE);
        ...
        if (npcId == Pano && _state == STARTED && cond == 4) {
            ...
            st.doTakeItems(Hate_Crystal, -1);
            st.doGiveItems(Panos_Credentials, 1);
            st.playSound(SOUND_MIDDLE);
