...
public class _103_SpiritOfCraftsman extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("blacksmith_karoyd_q0103_05.htm")) {
            st.doGiveItems(KAROYDS_LETTER_ID, 1);
            st.setCond(1);
            st.setState(STARTED);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30307 && st.getCond() == 0) {
            ...
            if (npcId == 30307 && st.getCond() >= 1 && (st.getQuestItemsCount(KAROYDS_LETTER_ID) >= 1 || st.getQuestItemsCount(CECKTINONS_VOUCHER1_ID) >= 1 || st.getQuestItemsCount(CECKTINONS_VOUCHER2_ID) >= 1))
                ...
                st.setCond(2);
                st.doTakeItems(KAROYDS_LETTER_ID, 1);
                st.doGiveItems(CECKTINONS_VOUCHER1_ID, 1);
                ...
                st.setCond(3);
                st.doTakeItems(CECKTINONS_VOUCHER1_ID, 1);
                st.doGiveItems(CECKTINONS_VOUCHER2_ID, 1);
                ...
                st.setCond(5);
                st.doTakeItems(CECKTINONS_VOUCHER2_ID, 1);
                st.doTakeItems(BONE_FRAGMENT1_ID, 10);
                st.doGiveItems(SOUL_CATCHER_ID, 1);
                ...
                st.setCond(6);
                st.doTakeItems(SOUL_CATCHER_ID, 1);
                st.doGiveItems(PRESERVE_OIL_ID, 1);
                ...
                st.setCond(8);
                st.doTakeItems(ZOMBIE_HEAD_ID, 1);
                st.doGiveItems(STEELBENDERS_HEAD_ID, 1);
                ...
                st.doTakeItems(STEELBENDERS_HEAD_ID, 1);

                st.doGiveItems(BLOODSABER_ID, 1);
                st.doRewardItems(ADENA_ID, 19799);
                st.getPlayer().addExpAndSp(RATE_QUESTS_REWARD * 46663, RATE_QUESTS_REWARD * 3999);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if ((npcId == 20517 || npcId == 20518 || npcId == 20455) && st.getCond() == 3) {
            ...
            if (st.doGiveOnKillItems(BONE_FRAGMENT1_ID, 1, 1, 10, 33))
                st.setCond(4);
        } else if ((npcId == 20015 || npcId == 20020) && st.getCond() == 6) {
            if (st.getQuestItemsCount(PRESERVE_OIL_ID) != 1)
                return null;

            if (st.doGiveOnKillItems(ZOMBIE_HEAD_ID, 1, 1, 10, 33)) {
                st.setCond(7);
                st.doTakeItems(PRESERVE_OIL_ID, 1);
            }
        }
