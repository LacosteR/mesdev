...
public class _223_TestOfChampion extends Quest implements ScriptFile {
    ...
    private static final int RewardExp = (int) (RATE_QUESTS_REWARD * 1270742);
    private static final int RewardSP = (int) (RATE_QUESTS_REWARD * 87200);
    private static final int RewardAdena = (int) (229764);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("1")) {
            ...
            if (!st.getPlayer().getVarB("dd3")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 64);
                st.getPlayer().setVar("dd3", "1", -1);
            }
            st.playSound(SOUND_ACCEPT);
            st.doGiveItems(ASCALONS_LETTER1, 1);
        } else if (event.equals("30624_1"))
            htmltext = "30624-05.htm";
        else if (event.equals("30624_2")) {
            htmltext = "30624-10.htm";
            st.setCond(COND5);
            st.doTakeItems(MASONS_LETTER, -1);
            st.doGiveItems(ASCALONS_LETTER2, 1);
        } else if (event.equals("30624_3")) {
            htmltext = "30624-14.htm";
            st.setCond(COND9);
            st.doTakeItems(GROOTS_LETTER, -1);
            st.doGiveItems(ASCALONS_LETTER3, 1);
        } else if (event.equals("30625_1"))
            htmltext = "30625-02.htm";
        else if (event.equals("30625_2")) {
            htmltext = "30625-03.htm";
            st.setCond(COND2);
            st.doTakeItems(ASCALONS_LETTER1, -1);
            st.doGiveItems(IRON_ROSE_RING, 1);
        } else if (event.equals("30093_1")) {
            htmltext = "30093-02.htm";
            st.setCond(COND6);
            st.doTakeItems(ASCALONS_LETTER2, -1);
            st.doGiveItems(WHITE_ROSE_INSIGNIA, 1);
        } else if (event.equals("30196_1"))
            htmltext = "30196-02.htm";
        else if (event.equals("30196_2")) {
            htmltext = "30196-03.htm";
            st.setCond(COND10);
            st.doTakeItems(ASCALONS_LETTER3, -1);
            st.doGiveItems(MOUENS_ORDER1, 1);
        } else if (event.equals("30196_3")) {
            htmltext = "30196-06.htm";
            st.setCond(COND12);
            st.doTakeItems(MOUENS_ORDER1, -1);
            st.doTakeItems(ROAD_RATMAN_HEAD, -1);
            st.doGiveItems(MOUENS_ORDER2, 1);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Ascalon) {
            if (cond == 0) {
                PcClass class_id = st.getPlayer().getPcClass();
                if (class_id != PcClass.warrior && class_id != PcClass.orc_raider) {
                    st.exitCurrentQuest(true);
                    return "30624-01.htm";
                }
                ...
                return class_id == PcClass.warrior ? "30624-03.htm" : "30624-04.htm";
            } else if (cond == COND1)
                ...
                st.doTakeItems(MOUENS_LETTER, -1);
                st.doGiveItems(MARK_OF_CHAMPION, 1);
            ...
            if (cond == COND1)
                ...
                st.doTakeItems(BLOODY_AXE_HEAD, -1);
                st.doTakeItems(IRON_ROSE_RING, -1);
                st.doGiveItems(MASONS_LETTER, 1);
                st.setCond(COND4);
            } else if (cond == COND4)
            ...
            if (cond == COND5)
                ...
                st.doTakeItems(WHITE_ROSE_INSIGNIA, -1);
                st.doTakeItems(HARPYS_EGG, -1);
                st.doTakeItems(MEDUSA_VENOM, -1);
                st.doTakeItems(WINDSUS_BILE, -1);
                st.doGiveItems(GROOTS_LETTER, 1);
                st.setCond(COND8);
            } else if (cond == COND8)
            ...
            if (cond == COND9)
                ...
                st.doTakeItems(MOUENS_ORDER2, -1);
                st.doTakeItems(LETO_LIZARDMAN_FANG, -1);
                st.doGiveItems(MOUENS_LETTER, 1);
                st.setCond(COND14);
            } else if (cond == COND13)
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        for (int[] drop : DROPLIST)
            if (drop[2] == npcId && drop[0] == cond) {
                st.doGiveOnKillItems(drop[3], 1, 1, drop[5], drop[4]);
