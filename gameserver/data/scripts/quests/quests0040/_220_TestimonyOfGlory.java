...
public class _220_TestimonyOfGlory extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("RETURN"))
            ...
            st.setCond(1);
            st.setState(STARTED);
            st.doGiveItems(VokiansOrder, 1);
            if (!st.getPlayer().getVarB("dd2")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 109);
                st.getPlayer().setVar("dd2", "1", -1);
            }
            st.playSound(SOUND_ACCEPT);
        } else if (event.equalsIgnoreCase("30642-03.htm")) {
            st.doTakeItems(VokiansOrder2, -1);
            st.doGiveItems(ChiantaOrder1st, 1);
            st.setCond(4);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30571-03.htm")) {
            st.doTakeItems(ScepterBox, -1);
            st.doGiveItems(TanapisOrder, 1);
            st.setCond(9);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30642-07.htm")) {
            st.doTakeItems(ScepterOfBreka, -1);
            st.doTakeItems(ScepterOfEnku, -1);
            st.doTakeItems(ScepterOfVuku, -1);
            st.doTakeItems(ScepterOfTurek, -1);
            st.doTakeItems(ScepterOfTunath, -1);
            st.doTakeItems(ChiantaOrder1st, -1);
            if (st.getPlayer().getLevel() >= 38) {
                st.doGiveItems(ChiantasOrder3rd, 1);
                st.setCond(6);
                st.setState(STARTED);
            } else {
                htmltext = "30642-06.htm";
                st.doGiveItems(ChiantasOrder2rd, 1);
            }
            ...
            if (st.getQuestItemsCount(ScepterOfBreka) > 0)
                ...
                st.doGiveItems(ManakiaLetter1st, 1);
            }
            ...
            if (st.getQuestItemsCount(ScepterOfEnku) > 0)
                ...
                st.doGiveItems(ManakiaLetter2st, 1);
            }
            ...
            if (st.getQuestItemsCount(ScepterOfVuku) > 0)
                ...
                st.doGiveItems(KasmansLetter1rd, 1);
            }
            ...
            if (st.getQuestItemsCount(ScepterOfTurek) > 0)
                ...
                st.doGiveItems(KasmansLetter2rd, 1);
            }
            ...
            if (st.getQuestItemsCount(ScepterOfTunath) > 0)
                ...
                st.doGiveItems(KasmansLetter3rd, 1);
            }
            ...
            if (spawn == 1) //если хоть один моб есть в мире, ставим таймер на удаление их(ня всякий) + говорим игроку подождать.
            {
                ...
                st.doTakeItems(ManakiaLetter1st, -1);
                st.doGiveItems(GloveOfVoltar, 1);
            ...
            if (isQuest != null) {
                ...
                st.doTakeItems(ManakiaLetter2st, -1);
                st.doGiveItems(GloveOfKepra, 1);
            ...
            if (isQuest != null) {
                ...
                st.doTakeItems(KasmansLetter2rd, -1);
                st.doGiveItems(GloveOfBurai, 1);
            ...
            st.doTakeItems(KasmansLetter3rd, -1);
            st.doGiveItems(ScepterOfTunath, 1);
            ...
            st.doTakeItems(KasmansLetter1rd, -1);
            st.doGiveItems(DrikosContract, 1);
        }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Vokian) {
            if (st.getQuestItemsCount(MarkOfGlory) != 0) {
                ...
                if (st.getPlayer().getPcClass().class_id == 45 || st.getPlayer().getPcClass().class_id == 47 || st.getPlayer().getPcClass().class_id == 50) {
                ...
                st.doTakeItems(VokiansOrder, -1);
                st.doTakeItems(ManashenShard, -10);
                st.doTakeItems(TyrantTalon, -10);
                st.doTakeItems(GuardianBasiliskFang, -10);
                st.doGiveItems(VokiansOrder2, 1);
                st.doGiveItems(NecklaceOfAuthority, 1);
            ...
            if (cond == 3)
                ...
                if (st.getQuestItemsCount(ChiantaOrder1st) > 0)
                    ...
                    if (st.getPlayer().getLevel() >= 38) {
                        st.doTakeItems(ChiantasOrder2rd, -1);
                        st.doGiveItems(ChiantasOrder3rd, 1);
                ...
                st.doTakeItems(NecklaceOfAuthority, -1);
                st.doTakeItems(ChiantasOrder3rd, -1);
                st.doTakeItems(TamlinOrcSkull, -1);
                st.doTakeItems(TimakOrcHead, -1);
                st.doGiveItems(ScepterBox, 1);
            ...
            if (cond == 4)
                if (st.getQuestItemsCount(ManakiaLetter1st) > 0)
                    ...
                    st.doTakeItems(PashikasHead, -1);
                    st.doTakeItems(VultusHead, -1);
                    st.doTakeItems(GloveOfVoltar, -1);
                    st.doGiveItems(ScepterOfBreka, 1);
            ...
            if (cond == 4)
                if (st.getQuestItemsCount(ManakiaLetter2st) > 0)
                    ...
                    st.doTakeItems(EnkuOverlordHead, -1);
                    st.doTakeItems(GloveOfKepra, -1);
                    st.doGiveItems(ScepterOfEnku, 1);
                    if (st.getQuestItemsCount(ScepterOfBreka) > 0 && st.getQuestItemsCount(ScepterOfEnku) > 0 && st.getQuestItemsCount(ScepterOfVuku) > 0 && st.getQuestItemsCount(ScepterOfTurek) > 0 && st.getQuestItemsCount(ScepterOfTunath) > 0) {
                        st.setCond(5);
                        st.setState(STARTED);
                        st.playSound(SOUND_MIDDLE);
            ...
            if (cond == 4)
                if (st.getQuestItemsCount(KasmansLetter2rd) > 0)
                    ...
                    st.doTakeItems(MakumBugbearHead, -1);
                    st.doTakeItems(GloveOfBurai, -1);
                    st.doGiveItems(ScepterOfTurek, 1);
                    if (st.getQuestItemsCount(ScepterOfBreka) > 0 && st.getQuestItemsCount(ScepterOfEnku) > 0 && st.getQuestItemsCount(ScepterOfVuku) > 0 && st.getQuestItemsCount(ScepterOfTurek) > 0 && st.getQuestItemsCount(ScepterOfTunath) > 0) {
                        st.setCond(5);
                        st.setState(STARTED);
                        st.playSound(SOUND_MIDDLE);
            ...
            if (cond == 4)
                if (st.getQuestItemsCount(KasmansLetter1rd) > 0)
                    ...
                    if (st.getQuestItemsCount(StakatoDroneHusk) >= 30) {
                        ...
                        st.doTakeItems(StakatoDroneHusk, -1);
                        st.doTakeItems(DrikosContract, -1);
                        st.doGiveItems(ScepterOfVuku, 1);
                        if (st.getQuestItemsCount(ScepterOfBreka) > 0 && st.getQuestItemsCount(ScepterOfEnku) > 0 && st.getQuestItemsCount(ScepterOfVuku) > 0 && st.getQuestItemsCount(ScepterOfTurek) > 0 && st.getQuestItemsCount(ScepterOfTunath) > 0) {
                            st.setCond(5);
                            st.setState(STARTED);
                            st.playSound(SOUND_MIDDLE);
            ...
            if (cond == 8)
                ...
                st.doTakeItems(ScepterOfTantos, -1);
                st.doTakeItems(TanapisOrder, -1);
                st.doGiveItems(RitualBox, 1);
        ...
        if (npcId == Kakai && cond == 11) {
            st.doTakeItems(RitualBox, -1);
            st.doGiveItems(MarkOfGlory, 1);
            if (!st.getPlayer().getVarB("prof2.2")) {
                st.addExpAndSp(RATE_QUESTS_REWARD * 1448226, RATE_QUESTS_REWARD * 96648);
                st.doRewardItems(ADENA_ID, 262720);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1 && st.getQuestItemsCount(TyrantTalon) >= 10 && st.getQuestItemsCount(GuardianBasiliskFang) >= 10 && st.getQuestItemsCount(ManashenShard) >= 10) {
            ...
            if (npcId == RagnaOrcOverlord || npcId == RagnaOrcSeer) {
                ...
                if (isQuest != null)
                    isQuest.deleteMe();
                st.doGiveItems(ScepterOfTantos, 1);
                st.setCond(10);
                st.setState(STARTED);
                st.playSound(SOUND_MIDDLE);
            }
