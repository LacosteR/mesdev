    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Vellior)
            if (cond == 0) {
                ...
                st.doTakeItems(NightmareCrystal, -1);
                st.doRewardItems(ADENA_ID, 102680);
                st.addExpAndSp(RATE_QUESTS_REWARD * 38607, RATE_QUESTS_REWARD * 4018);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1 && npcId == Merkenis) {
            if (st.getQuestItemsCount(NightmareCrystal) == 0)
                st.doGiveItems(NightmareCrystal, 1);
