...
public class _137_TempleChampionPart1 extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("sylvain_q0137_04.htm")) {
            st.doTakeItems(BadgeTempleExecutor, -1);
            st.doTakeItems(BadgeTempleMissionary, -1);
            st.setCond(1);
            st.setState(STARTED);
            st.set("talk", "0");
            ...
            st.doRewardItems(ADENA_ID, 69146);
            st.playSound(SOUND_FINISH);
            st.addExpAndSp(RATE_QUESTS_REWARD * 219975, RATE_QUESTS_REWARD * 13047);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.doGiveOnKillItems(FRAGMENT, 1, 1, 30, 100))
            st.setCond(3);
