...
public class _038_DragonFangs extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("guard_luis_q0038_0201.htm"))
            if (cond == 2) {
                st.setCond(3);
                st.doTakeItems(FEATHER_ORNAMENT, 100);
                st.doGiveItems(TOOTH_OF_TOTEM, 1);
                st.playSound(SOUND_MIDDLE);
            }
        if (event.equals("iris_q0038_0301.htm"))
            if (cond == 3) {
                st.setCond(4);
                st.doTakeItems(TOOTH_OF_TOTEM, 1);
                st.doGiveItems(LETTER_OF_IRIS, 1);
                st.playSound(SOUND_MIDDLE);
            }
        if (event.equals("magister_roh_q0038_0401.htm"))
            if (cond == 4) {
                st.setCond(5);
                st.doTakeItems(LETTER_OF_IRIS, 1);
                st.doGiveItems(LETTER_OF_ROHMER, 1);
                st.playSound(SOUND_MIDDLE);
            }
        if (event.equals("iris_q0038_0501.htm"))
            if (cond == 5) {
                st.setCond(6);
                st.doTakeItems(LETTER_OF_ROHMER, 1);
                st.playSound(SOUND_MIDDLE);
            }
        if (event.equals("iris_q0038_0601.htm"))
            if (cond == 7) {
                st.doTakeItems(TOOTH_OF_DRAGON, 50);
                int luck = Rnd.get(3);
                if (luck == 0) {
                    st.doRewardItems(BLUE_BUCKSKIN_BOOTS, 1);
                    st.doRewardItems(ADENA_ID, 1500);
                }
                if (luck == 1) {
                    st.doRewardItems(BONE_HELMET, 1);
                    st.doRewardItems(ADENA_ID, 5200);
                }
                if (luck == 2) {
                    st.doRewardItems(ASSAULT_BOOTS, 1);
                    st.doRewardItems(ADENA_ID, 1500);
                }
                st.addExpAndSp(RATE_QUESTS_REWARD * 435117, RATE_QUESTS_REWARD * 23977);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 20357 || npcId == 21100) {
            ...
            if (st.doGiveOnKillItems(FEATHER_ORNAMENT, 1, 1, 100, CHANCE_FOR_QUEST_ITEMS))
                st.setCond(2);
        } else if (npcId == 20356 || npcId == 21101) {
            if (cond != 6)
                return null;

            if (st.doGiveOnKillItems(TOOTH_OF_DRAGON, 1, 1, 50, CHANCE_FOR_QUEST_ITEMS))
                st.setCond(7);
        }
