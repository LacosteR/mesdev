...
public class _224_TestOfSagittarius extends Quest implements ScriptFile {
    ...
    private static final int RewardExp = (int) (RATE_QUESTS_REWARD * 894888);
    private static final int RewardSP = (int) (RATE_QUESTS_REWARD * 61408);
    private static final int RewardAdena = (int) (161806);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("1")) {
            ...
            st.doGiveItems(BERNARDS_INTRODUCTION_ID, 1);
        } else if (event.equals("30626_1"))
            htmltext = "30626-02.htm";
        else if (event.equals("30626_2")) {
            htmltext = "30626-03.htm";
            st.doTakeItems(BERNARDS_INTRODUCTION_ID, st.getQuestItemsCount(BERNARDS_INTRODUCTION_ID));
            st.doGiveItems(LETTER_OF_HAMIL1_ID, 1);
            st.setCond(2);
        } else if (event.equals("30626_3"))
            htmltext = "30626-06.htm";
        else if (event.equals("30626_4")) {
            htmltext = "30626-07.htm";
            st.doTakeItems(HUNTERS_RUNE1_ID, st.getQuestItemsCount(HUNTERS_RUNE1_ID));
            st.doGiveItems(LETTER_OF_HAMIL2_ID, 1);
            st.setCond(5);
        } else if (event.equals("30653_1")) {
            htmltext = "30653-02.htm";
            st.doTakeItems(LETTER_OF_HAMIL1_ID, st.getQuestItemsCount(LETTER_OF_HAMIL1_ID));
            st.setCond(3);
        } else if (event.equals("30514_1")) {
            htmltext = "30514-02.htm";
            st.doTakeItems(LETTER_OF_HAMIL2_ID, st.getQuestItemsCount(LETTER_OF_HAMIL2_ID));
            st.setCond(6);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30702 && st.getCond() == 0) {
            if (st.getPlayer().getPcClass().class_id == 0x07 || st.getPlayer().getPcClass().class_id == 0x16 || st.getPlayer().getPcClass().class_id == 0x23) {
            ...
            st.doGiveItems(LETTER_OF_HAMIL3_ID, 1);
            st.setCond(9);
            ...
            st.doTakeItems(CRESCENT_MOON_BOW_ID, -1);
            st.doTakeItems(TALISMAN_OF_KADESH_ID, -1);
            st.doTakeItems(BLOOD_OF_LIZARDMAN_ID, -1);
            st.doGiveItems(MARK_OF_SAGITTARIUS_ID, 1);
            ...
            st.doTakeItems(TALISMAN_OF_SNAKE_ID, st.getQuestItemsCount(TALISMAN_OF_SNAKE_ID));
            st.setCond(8);
            ...
            st.doTakeItems(LETTER_OF_HAMIL3_ID, st.getQuestItemsCount(LETTER_OF_HAMIL3_ID));
            st.setCond(10);
            ...
            st.doTakeItems(MITHRIL_CLIP_ID, st.getQuestItemsCount(MITHRIL_CLIP_ID));
            st.doTakeItems(STAKATO_CHITIN_ID, st.getQuestItemsCount(STAKATO_CHITIN_ID));
            st.doTakeItems(ST_BOWSTRING_ID, st.getQuestItemsCount(ST_BOWSTRING_ID));
            st.doTakeItems(MANASHENS_HORN_ID, st.getQuestItemsCount(MANASHENS_HORN_ID));
            st.doGiveItems(CRESCENT_MOON_BOW_ID, 1);
            st.doGiveItems(WOODEN_ARROW_ID, 10);
            st.setCond(12);
        }
        return htmltext;
    }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 20079 || npcId == 20080 || npcId == 20081 || npcId == 20084 || npcId == 20086 || npcId == 20089 || npcId == 20090) {
            if (st.getCond() == 3 && st.getQuestItemsCount(HUNTERS_RUNE1_ID) < 10 && Rnd.chance(50)) {
                st.doGiveItems(HUNTERS_RUNE1_ID, 1);
            ...
            if (st.getCond() == 6 && st.getQuestItemsCount(HUNTERS_RUNE2_ID) < 10 && Rnd.chance(50)) {
                st.doGiveItems(HUNTERS_RUNE2_ID, 1);
                if (st.getQuestItemsCount(HUNTERS_RUNE2_ID) >= 10) {
                    st.doTakeItems(HUNTERS_RUNE2_ID, 10);
                    st.doGiveItems(TALISMAN_OF_SNAKE_ID, 1);
                    st.setCond(7);
                    st.playSound(SOUND_MIDDLE);
                } else
                    st.playSound(SOUND_ITEMGET);
            ...
            if (st.getCond() == 10 && st.getQuestItemsCount(STAKATO_CHITIN_ID) == 0 && Rnd.chance(10)) {
                st.doGiveItems(STAKATO_CHITIN_ID, 1);
            ...
            if (st.getCond() == 10 && st.getQuestItemsCount(MANASHENS_HORN_ID) == 0 && Rnd.chance(10)) {
                st.doGiveItems(MANASHENS_HORN_ID, 1);
            ...
            if (st.getCond() == 10 && st.getQuestItemsCount(ST_BOWSTRING_ID) == 0 && Rnd.chance(10)) {
                st.doGiveItems(ST_BOWSTRING_ID, 1);
            ...
            if (st.getCond() == 10 && st.getQuestItemsCount(MITHRIL_CLIP_ID) == 0 && Rnd.chance(10)) {
                st.doGiveItems(MITHRIL_CLIP_ID, 1);
            ...
            if (st.getCond() == 13)
                if (Rnd.chance((st.getQuestItemsCount(BLOOD_OF_LIZARDMAN_ID) - 120) * 5)) {
                    st.addSpawn(27090);
                    st.doTakeItems(BLOOD_OF_LIZARDMAN_ID, st.getQuestItemsCount(BLOOD_OF_LIZARDMAN_ID));
                    st.playSound(SOUND_BEFORE_BATTLE);
                } else {
                    st.doGiveItems(BLOOD_OF_LIZARDMAN_ID, 1);
                    st.playSound(SOUND_ITEMGET);
                }
        } else if (npcId == 27090)
            if (st.getCond() == 13 && st.getQuestItemsCount(TALISMAN_OF_KADESH_ID) == 0)
                if (st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == CRESCENT_MOON_BOW_ID) {
                    st.doGiveItems(TALISMAN_OF_KADESH_ID, 1);
                    st.setCond(14);
                    st.playSound(SOUND_MIDDLE);
                } else
                    st.addSpawn(27090);
