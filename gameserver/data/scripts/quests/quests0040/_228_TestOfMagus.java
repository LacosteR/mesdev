...
public class _228_TestOfMagus extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30629-04.htm")) {
            ...
            st.doTakeItems(RukalsLetter, -1);
            st.doGiveItems(ParinasLetter, 1);
            st.setCond(2);
            st.setState(STARTED);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("30612-02.htm")) {
            st.doTakeItems(ParinasLetter, -1);
            st.doGiveItems(LilacCharm, 1);
            st.setCond(3);
            st.setState(STARTED);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("30629-10.htm")) {
            st.doTakeItems(LilacCharm, -1);
            st.doTakeItems(GoldenSeed1st, -1);
            st.doTakeItems(GoldenSeed2st, -1);
            st.doTakeItems(GoldenSeed3st, -1);
            st.doGiveItems(ScoreOfElements, 1);
            st.setCond(5);
            st.setState(STARTED);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("30412-02.htm")) {
            st.doGiveItems(SylphCharm, 1);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("30409-03.htm")) {
            st.doGiveItems(SerpentCharm, 1);
            st.playSound(SOUND_MIDDLE);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Rukal) {
            if (st.getQuestItemsCount(MarkOfMagus) != 0) {
                ...
                if (st.getPlayer().getPcClass().class_id == 0x0b || st.getPlayer().getPcClass().class_id == 0x1a || st.getPlayer().getPcClass().class_id == 0x27) {
                ...
                st.doTakeItems(ScoreOfElements, -1);
                st.doTakeItems(ToneOfWater, -1);
                st.doTakeItems(ToneOfFire, -1);
                st.doTakeItems(ToneOfWind, -1);
                st.doTakeItems(ToneOfEarth, -1);
                st.doGiveItems(MarkOfMagus, 1);
            ...
            if (st.getQuestItemsCount(ToneOfFire) == 0) {
                if (st.getQuestItemsCount(SalamanderCharm) == 0) {
                    ...
                    st.doGiveItems(SalamanderCharm, 1);
                    st.playSound(SOUND_MIDDLE);
                } else if (st.getQuestItemsCount(FlameCrystal) < 5)
                    htmltext = "30411-02.htm";
                else {
                    st.doTakeItems(SalamanderCharm, -1);
                    st.doTakeItems(FlameCrystal, -1);
                    st.doGiveItems(ToneOfFire, 1);
            ...
            if (st.getQuestItemsCount(ToneOfWind) == 0) {
                if (st.getQuestItemsCount(SylphCharm) == 0)
                    ...
                    st.doTakeItems(SylphCharm, -1);
                    st.doTakeItems(HarpysFeather, -1);
                    st.doTakeItems(WyrmsWingbone, -1);
                    st.doTakeItems(WindsusMane, -1);
                    st.doGiveItems(ToneOfWind, 1);
            ...
            if (st.getQuestItemsCount(ToneOfEarth) == 0) {
                if (st.getQuestItemsCount(SerpentCharm) == 0)
                    ...
                    st.doTakeItems(SerpentCharm, -1);
                    st.doTakeItems(EnchantedMonstereye, -1);
                    st.doTakeItems(EnchantedStoneGolemPowder, -1);
                    st.doTakeItems(EnchantedIronGolemScrap, -1);
                    st.doGiveItems(ToneOfEarth, 1);
            ...
            if (st.getQuestItemsCount(ToneOfWater) == 0) {
                if (st.getQuestItemsCount(UndineCharm) == 0) {
                    ...
                    st.doGiveItems(UndineCharm, 1);
                    st.playSound(SOUND_MIDDLE);
                } else if (st.getQuestItemsCount(DazzlingDrop) < 20)
                    htmltext = "30413-02.htm";
                else {
                    st.doTakeItems(UndineCharm, -1);
                    st.doTakeItems(DazzlingDrop, -1);
                    st.doGiveItems(ToneOfWater, 1);
