    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("jager_bixon_q0280_03.htm") && _state == CREATED) {
            ...
            if (event.equalsIgnoreCase("ADENA")) {
                st.doTakeItems(Grey_Keltir_Tooth, -1);
                st.doTakeItems(Black_Wolf_Tooth, -1);
                st.doGiveItems(ADENA_ID, (Grey_Keltir_Tooth_count + Black_Wolf_Tooth_count) * 2);
                ...
                if (take_Grey_Keltir_Tooth > 0)
                    st.doTakeItems(Grey_Keltir_Tooth, take_Grey_Keltir_Tooth);
                if (take_Black_Wolf_Tooth > 0)
                    st.doTakeItems(Black_Wolf_Tooth, take_Black_Wolf_Tooth);
                int rew_count = (int) st.getRateQuestsReward();
                while (rew_count > 0) {
                    rew_count--;
                    st.doGiveItems(REWARDS[Rnd.get(REWARDS.length)], 1);
                }
                st.playSound(SOUND_MIDDLE);
                return "jager_bixon_q0280_06.htm";
            }
