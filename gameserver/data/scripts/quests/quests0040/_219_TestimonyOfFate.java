...
public class _219_TestimonyOfFate extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30476-05.htm")) {
            ...
            st.doGiveItems(KairasLetter, 1);
            if (!st.getPlayer().getVarB("dd2")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 98);
                st.getPlayer().setVar("dd2", "1", -1);
            }
        } else if (event.equalsIgnoreCase("30114-04.htm")) {
            st.doTakeItems(AldersSkull2, 1);
            st.doGiveItems(AldersReceipt, 1);
            st.setCond(12);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30476-12.htm")) {
            if (st.getPlayer().getLevel() >= 38) {
                st.doTakeItems(RevelationsManuscript, -1);
                st.doGiveItems(KairasRecommendation, 1);
                st.setCond(15);
                st.setState(STARTED);
            } else {
                htmltext = "30476-13.htm";
                st.doTakeItems(RevelationsManuscript, -1);
                st.doGiveItems(KairasInstructions, 1);
                st.setCond(14);
                st.setState(STARTED);
            }
        } else if (event.equalsIgnoreCase("30419-02.htm")) {
            st.doTakeItems(ThifiellsLetter, -1);
            st.doGiveItems(ArkeniasNote, 1);
            st.setCond(17);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("31845-02.htm"))
            st.doGiveItems(PixyGarnet, 1);
        else if (event.equalsIgnoreCase("31850-02.htm"))
            st.doGiveItems(BlightTreantSeed, 1);
        else if (event.equalsIgnoreCase("30419-05.htm")) {
            st.doTakeItems(ArkeniasNote, -1);
            st.doTakeItems(RedFairyDust, -1);
            st.doTakeItems(BlightTreantSap, -1);
            st.doGiveItems(ArkeniasLetter, 1);
            st.setCond(18);
            st.setState(STARTED);
        }
        ...
        if (npcId == Kaira) {
            if (st.getQuestItemsCount(MarkofFate) != 0) {
                ...
                if (AldersSpiritObject == null) {
                    st.doTakeItems(AldersSkull1, -1);
                    if (st.getQuestItemsCount(AldersSkull2) == 0)
                        st.doGiveItems(AldersSkull2, 1);
                ...
                if (st.getQuestItemsCount(KairasInstructions) != 0 && st.getPlayer().getLevel() < 38)
                    ...
                    st.doGiveItems(KairasRecommendation, 1);
                    st.doTakeItems(KairasInstructions, 1);
            ...
            if (cond == 1) {
                ...
                st.doTakeItems(KairasLetter, -1);
                st.doGiveItems(MetheussFuneralJar, 1);
                st.setCond(2);
                st.setState(STARTED);
            } else if (cond == 2)
                htmltext = "30614-02.htm";
            else if (cond == 3) {
                st.doTakeItems(KasandrasRemains, -1);
                st.doGiveItems(HerbalismTextbook, 1);
                htmltext = "30614-03.htm";
                st.setCond(5);
                st.setState(STARTED);
            } else if (cond == 8) {
                st.doTakeItems(Belladonna, -1);
                st.doGiveItems(AldersSkull1, 1);
            ...
            if (cond == 5) {
                st.doTakeItems(HerbalismTextbook, -1);
                st.doGiveItems(IxiasList, 1);
                ...
                st.doTakeItems(MedusasIchor, -1);
                st.doTakeItems(MarshSpiderFluids, -1);
                st.doTakeItems(DeadSeekerDung, -1);
                st.doTakeItems(TyrantsBlood, -1);
                st.doTakeItems(NightshadeRoot, -1);
                st.doTakeItems(IxiasList, -1);
                st.doGiveItems(Belladonna, 1);
            ...
            if (cond == 12) {
                st.doTakeItems(AldersReceipt, -1);
                st.doGiveItems(RevelationsManuscript, 1);
            ...
            if (cond == 15) {
                st.doTakeItems(KairasRecommendation, -1);
                st.doGiveItems(ThifiellsLetter, 1);
                st.doGiveItems(PalusCharm, 1);
                ...
                if (!st.getPlayer().getVarB("prof2.2")) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 1365470, RATE_QUESTS_REWARD * 91124);
                    st.doRewardItems(ADENA_ID, 247708);
                    st.getPlayer().setVar("prof2.2", "1", -1);
                }
                st.doTakeItems(ArkeniasLetter, -1);
                st.doTakeItems(PalusCharm, -1);
                st.doGiveItems(MarkofFate, 1);
            ...
            if (st.getQuestItemsCount(RedFairyDust) == 0 && st.getQuestItemsCount(PixyGarnet) == 0)
                ...
                st.doTakeItems(GrandissSkull, -1);
                st.doTakeItems(KarulBugbearSkull, -1);
                st.doTakeItems(BrekaOverlordSkull, -1);
                st.doTakeItems(LetoOverlordSkull, -1);
                st.doTakeItems(PixyGarnet, -1);
                st.doGiveItems(RedFairyDust, 1);
            ...
            if (st.getQuestItemsCount(BlightTreantSap) == 0 && st.getQuestItemsCount(BlightTreantSeed) == 0)
                ...
                st.doTakeItems(BlackWillowLeaf, -1);
                st.doTakeItems(BlightTreantSeed, -1);
                st.doGiveItems(BlightTreantSap, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 2 && npcId == HangmanTree) {
            st.doTakeItems(MetheussFuneralJar, -1);
            st.doGiveItems(KasandrasRemains, 1);
