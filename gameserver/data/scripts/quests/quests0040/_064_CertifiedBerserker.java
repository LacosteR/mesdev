    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("32207-01a.htm")) {
            ...
            if (!st.getPlayer().getVarB("dd1")) {
                st.doGiveItems(Dimenional_Diamonds, 48);
            ...
            st.doTakeItems(MESSAGE_PLATE, -1);
            st.setCond(8);
        } else if (event.equalsIgnoreCase("32236-01a.htm")) {
            st.setCond(13);
            st.doGiveItems(H_LETTER, 1);
            st.cancelQuestTimer("HARKILGAMED_Fail");
            DESPAWN_HARKILGAMED();
        } else if (event.equalsIgnoreCase("32215-05a.htm")) {
            st.setCond(14);
            st.doTakeItems(H_LETTER, -1);
            st.doGiveItems(T_REC, 1);
        } else if (event.equalsIgnoreCase("32207-03a.htm")) {
            if (!st.getPlayer().getVarB("prof2.1")) {
                st.addExpAndSp(RATE_QUESTS_REWARD * 349006, RATE_QUESTS_REWARD * 23948);
                st.doRewardItems(ADENA_ID, 63104);
                st.getPlayer().setVar("prof2.1", "1", -1);
            }
            st.doGiveItems(OrkurusRecommendation, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == ORKURUS) {
            if (st.getQuestItemsCount(OrkurusRecommendation) != 0) {
                ...
                if (st.getPlayer().getPcClass().class_id == 0x7D) {
            ...
            if (cond == 1)
                ...
                st.doTakeItems(BREKA_ORC_HEAD, -1);
                st.setCond(4);
            ...
            if (cond == 8) {
                ...
                st.setCond(11);
                st.doTakeItems(REPORT1, -1);
                st.doTakeItems(REPORT2, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 2) {
            if (npcId == BREKA_ORC || npcId == BREKA_ORC_ARCHER || npcId == BREKA_ORC_SHAMAN || npcId == BREKA_ORC_OVERLORD || npcId == BREKA_ORC_WARRIOR) {
                if (st.doGiveOnKillItems(BREKA_ORC_HEAD, 1, 1, 20, 100))
                    st.setCond(3);
            }
        }

        if (cond == 5 && npcId == ROAD_SCAVENGER) {
            if (st.doGiveOnKillItems(MESSAGE_PLATE, 1, 1, 1, 20))
                st.setCond(6);
        }

        if (cond == 9) {
            if (npcId == DEAD_SEEKER)
                st.doGiveOnKillItems(REPORT1, 1, 1, 1, 30, false);

            if (npcId == STAKATO)
                st.doGiveOnKillItems(REPORT2, 1, 1, 1, 30, false);
