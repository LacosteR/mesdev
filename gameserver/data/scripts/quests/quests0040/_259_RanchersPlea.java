...
public class _259_RanchersPlea extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30497 && st.getCond() == 0) {
            ...
            st.doGiveItems(ADENA_ID, st.getQuestItemsCount(GIANT_SPIDER_SKIN_ID) * 25);
            st.doTakeItems(GIANT_SPIDER_SKIN_ID, st.getQuestItemsCount(GIANT_SPIDER_SKIN_ID));
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() > 0)
            st.doGiveOnKillItems(GIANT_SPIDER_SKIN_ID, 1, 1, 100);
        return null;
    }
}