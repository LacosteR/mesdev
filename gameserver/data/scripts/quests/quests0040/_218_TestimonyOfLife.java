...
public class _218_TestimonyOfLife extends Quest implements ScriptFile {
    ...
    private static final int RewardExp = (int) (RATE_QUESTS_REWARD * 1886832);
    private static final int RewardSP = (int) (RATE_QUESTS_REWARD * 125918);
    private static final int RewardAdena = (int) (RATE_QUESTS_REWARD * 342288);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            ...
            st.doGiveItems(CARDIENS_LETTER, 1);
            if (!st.getPlayer().getVarB("dd2")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 102);
            ...
            st.doTakeItems(CARDIENS_LETTER, 1);
            st.doGiveItems(MOONFLOWER_CHARM, 1);
            st.doGiveItems(HIERARCHS_LETTER, 1);
        } else if (event.equalsIgnoreCase("30371_1"))
            htmltext = "30371-02.htm";
        else if (event.equalsIgnoreCase("30371_2")) {
            htmltext = "30371-03.htm";
            st.doTakeItems(HIERARCHS_LETTER, 1);
            st.doGiveItems(GRAIL_DIAGRAM, 1);
        } else if (event.equalsIgnoreCase("30371_3"))
            if (st.getPlayer().getLevel() < 38) {
                htmltext = "30371-10.htm";
                st.doTakeItems(STARDUST, 1);
                st.doGiveItems(THALIAS_INSTRUCTIONS, 1);
            } else {
                htmltext = "30371-11.htm";
                st.doTakeItems(STARDUST, 1);
                st.doGiveItems(THALIAS_LETTER2, 1);
            }
            ...
            st.doTakeItems(GRAIL_DIAGRAM, 1);
            st.doGiveItems(PUSHKINS_LIST, 1);
        } else if (event.equalsIgnoreCase("30300_6"))
            htmltext = "30300-09.htm";
        else if (event.equalsIgnoreCase("30300_7")) {
            htmltext = "30300-10.htm";
            st.doTakeItems(PURE_MITHRIL_ORE, -1);
            st.doTakeItems(ANT_SOLDIER_ACID, -1);
            st.doTakeItems(WYRMS_TALON1, -1);
            st.doTakeItems(PUSHKINS_LIST, 1);
            st.doGiveItems(PURE_MITHRIL_CUP, 1);
            ...
            st.doTakeItems(THALIAS_LETTER1, 1);
            st.doGiveItems(ARKENIAS_CONTRACT, 1);
            st.doGiveItems(ARKENIAS_INSTRUCTIONS, 1);
        } else if (event.equalsIgnoreCase("30375_1")) {
            htmltext = "30375-02.htm";
            st.doTakeItems(ARKENIAS_INSTRUCTIONS, 1);
            st.doGiveItems(ADONIUS_LIST, 1);
        } else if (event.equalsIgnoreCase("30655_1")) {
            htmltext = "30655-02.htm";
            st.doTakeItems(THALIAS_LETTER2, 1);
            st.doGiveItems(ISAELS_INSTRUCTIONS, 1);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30460 && cond == 0) {
            ...
            st.doTakeItems(CAMOMILE_CHARM, -1);
            st.doGiveItems(MARK_OF_LIFE, 1);
            if (!st.getPlayer().getVarB("prof2.2")) {
                st.addExpAndSp(RewardExp, RewardSP);
                st.doRewardItems(ADENA_ID, RewardAdena);
            ...
            st.doTakeItems(WATER_OF_LIFE, 1);
            st.doTakeItems(MOONFLOWER_CHARM, 1);
            st.doGiveItems(CAMOMILE_CHARM, 1);
            ...
            st.doTakeItems(PURE_MITHRIL_CUP, 1);
            st.doGiveItems(THALIAS_LETTER1, 1);
            ...
            if (st.getPlayer().getLevel() < 38)
                ...
                st.doTakeItems(THALIAS_INSTRUCTIONS, 1);
                st.doGiveItems(THALIAS_LETTER2, 1);
            }
            ...
            st.doTakeItems(ISAELS_LETTER, 1);
            st.doGiveItems(GRAIL_OF_PURITY, 1);
            ...
            st.doTakeItems(TEARS_OF_UNICORN, 1);
            st.doGiveItems(WATER_OF_LIFE, 1);
            ...
            st.doTakeItems(ARKENIAS_CONTRACT, 1);
            st.doTakeItems(ANDARIEL_SCRIPTURE_COPY, 1);
            st.doGiveItems(STARDUST, 1);
            ...
            if (st.getQuestItemsCount(SPIDER_ICHOR) >= 20 && st.getQuestItemsCount(HARPYS_DOWN) >= 20) {
                ...
                st.doTakeItems(SPIDER_ICHOR, st.getQuestItemsCount(SPIDER_ICHOR));
                st.doTakeItems(HARPYS_DOWN, st.getQuestItemsCount(HARPYS_DOWN));
                st.doTakeItems(ADONIUS_LIST, 1);
                st.doGiveItems(ANDARIEL_SCRIPTURE_COPY, 1);
            } else
            ...
            if (st.getQuestItemsCount(TALINS_SPEAR_BLADE) > 0 && st.getQuestItemsCount(TALINS_SPEAR_SHAFT) > 0 && st.getQuestItemsCount(TALINS_RUBY) > 0 && st.getQuestItemsCount(TALINS_AQUAMARINE) > 0 && st.getQuestItemsCount(TALINS_AMETHYST) > 0 && st.getQuestItemsCount(TALINS_PERIDOT) > 0) {
                ...
                st.doTakeItems(TALINS_SPEAR_BLADE, 1);
                st.doTakeItems(TALINS_SPEAR_SHAFT, 1);
                st.doTakeItems(TALINS_RUBY, 1);
                st.doTakeItems(TALINS_AQUAMARINE, 1);
                st.doTakeItems(TALINS_AMETHYST, 1);
                st.doTakeItems(TALINS_PERIDOT, 1);
                st.doTakeItems(ISAELS_INSTRUCTIONS, 1);
                st.doGiveItems(ISAELS_LETTER, 1);
                st.doGiveItems(TALINS_SPEAR, 1);
            } else
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 20550) {
            ...
            if (cond > 0 && st.getQuestItemsCount(MOONFLOWER_CHARM) >= 1 && st.getQuestItemsCount(PUSHKINS_LIST) >= 1 && st.getQuestItemsCount(PURE_MITHRIL_ORE) < 10)
                if (Rnd.chance(50)) {
                    st.doGiveItems(PURE_MITHRIL_ORE, 1);
            ...
            st.set("id", "0");
            if (cond > 0 && st.getQuestItemsCount(MOONFLOWER_CHARM) >= 1 && st.getQuestItemsCount(PUSHKINS_LIST) >= 1)
                st.doGiveOnKillItems(WYRMS_TALON1, 1, 1, 20, 50);
            ...
            if (cond > 0 && st.getQuestItemsCount(MOONFLOWER_CHARM) >= 1 && st.getQuestItemsCount(PUSHKINS_LIST) >= 1 && st.getQuestItemsCount(ANT_SOLDIER_ACID) < 20)
                st.doGiveOnKillItems(ANT_SOLDIER_ACID, 1, 1, 20, 80);
        } else if (npcId == 20233) {
            st.set("id", "0");
            if (cond > 0 && st.getQuestItemsCount(MOONFLOWER_CHARM) >= 1 && st.getQuestItemsCount(ADONIUS_LIST) >= 1)
                st.doGiveOnKillItems(SPIDER_ICHOR, 1, 1, 20, 50);
        } else if (npcId == 20145) {
            st.set("id", "0");
            if (cond > 0 && st.getQuestItemsCount(MOONFLOWER_CHARM) >= 1 && st.getQuestItemsCount(ADONIUS_LIST) >= 1)
                st.doGiveOnKillItems(HARPYS_DOWN, 1, 1, 20, 50);
        } else if (npcId == 27077) {
            if (cond > 0 && st.getQuestItemsCount(MOONFLOWER_CHARM) >= 1 && st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == TALINS_SPEAR && st.getQuestItemsCount(GRAIL_OF_PURITY) >= 1 && st.getQuestItemsCount(TEARS_OF_UNICORN) == 0)
                if (st.getQuestItemsCount(TALINS_SPEAR) > 0) {
                    st.doTakeItems(GRAIL_OF_PURITY, 1);
                    st.doTakeItems(TALINS_SPEAR, 1);
                    st.doGiveItems(TEARS_OF_UNICORN, 1);
                }
