    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (st.getPlayer().isClanLeader())
            ...
            // WITCH_KALIS
                ...
                st.doTakeItems(SYMBOL_OF_LOYALTY, -1);
                st.doGiveItems(ANTIDOTE_RECIPE, 1);
        ...
        // STATUE_OF_OFFERING
            ...
            st.doGiveItems(SYMBOL_OF_LOYALTY, 1);
            st.playSound(SOUND_ACCEPT);
            ...
            if (st.getQuestItemsCount(ADENA_ID) < RETRY_PRICE)
                ...
                st.doTakeItems(ADENA_ID, RETRY_PRICE);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == SIR_KRISTOF_RODEMAI) {
            if (!st.getPlayer().isClanLeader()) {
                ...
                st.doTakeItems(VOUCHER_OF_FAITH, -1);
                st.doGiveItems(PROOF_OF_ALLIANCE, 1);
            ...
            if (st.getPlayer().isClanLeader()) {
                if (cond == 1)
                    ...
                    if (st.getQuestItemsCount(HERB_OF_HARIT) > 0 && st.getQuestItemsCount(HERB_OF_VANOR) > 0 && st.getQuestItemsCount(HERB_OF_OEL_MAHUM) > 0 && st.getQuestItemsCount(BLOOD_OF_EVA) > 0 && st.getQuestItemsCount(ANTIDOTE_RECIPE) > 0) {
                        st.doTakeItems(ANTIDOTE_RECIPE, 1);
                        st.doTakeItems(HERB_OF_HARIT, 1);
                        st.doTakeItems(HERB_OF_VANOR, 1);
                        st.doTakeItems(HERB_OF_OEL_MAHUM, 1);
                        st.doTakeItems(BLOOD_OF_EVA, 1);
                        st.doGiveItems(POTION_OF_RECOVERY, 1);
                        st.doGiveItems(VOUCHER_OF_FAITH, 1);
