...
public class _001_LettersOfLove extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState qs, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("quest_accept")) {
            ...
            qs.setCond(1);
            qs.setState(STARTED);
            qs.doGiveItems(DARINGS_LETTER, 1);
        ...
        switch (npcId) {
            case DARIN:
                if (cond == 0) {
                    ...
                    st.doTakeItems(ROXXY_KERCHIEF, -1);
                    st.doGiveItems(DARINGS_RECEIPT, 1);
                    st.setCond(3);
                    ...
                    st.doRewardItems(ADENA_ID, 3666); // T2
            ...
            case ROXXY:
                if (cond == 1 && st.getQuestItemsCount(ROXXY_KERCHIEF) == 0 && st.getQuestItemsCount(DARINGS_LETTER) > 0) {
                    ...
                    st.doTakeItems(DARINGS_LETTER, -1);
                    st.doGiveItems(ROXXY_KERCHIEF, 1);
                    st.setCond(2);
                    st.playSound(SOUND_MIDDLE);
            ...
            case BAULRO:
                if (cond == 3 && st.getQuestItemsCount(DARINGS_RECEIPT) == 1) {
                    ...
                    st.doTakeItems(DARINGS_RECEIPT, -1);
                    st.doGiveItems(BAULS_POTION, 1);
                    st.setCond(4);
                    st.playSound(SOUND_MIDDLE);
                } else if (cond == 4)
