    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("elder_filaur_q0116_0104.htm")) {
            ...
            if (st.getQuestItemsCount(Bandage) >= 20 && st.getQuestItemsCount(Energy_Stone) >= 5 && st.getQuestItemsCount(Thief_Key) >= 10) {
                st.doTakeItems(Bandage, 20);
                st.doTakeItems(Energy_Stone, 5);
                st.doTakeItems(Thief_Key, 10);
                st.doGiveItems(Supplying_Goods_for_Railroad_Worker, 1);
                st.setCond(2);
                st.setState(STARTED);
            } else
            ...
            st.doTakeItems(Supplying_Goods_for_Railroad_Worker, 1);
            st.doGiveItems(SSD, 1740);
            st.addExpAndSp(RATE_QUESTS_REWARD * 82792, RATE_QUESTS_REWARD * 4981);
            st.exitCurrentQuest(false);
        } else if (event.equalsIgnoreCase("adena")) {
            htmltext = "railman_obi_q0116_0302.htm";
            st.doTakeItems(Supplying_Goods_for_Railroad_Worker, 1);
            st.doRewardItems(ADENA_ID, 16500);
            st.addExpAndSp(RATE_QUESTS_REWARD * 82792, RATE_QUESTS_REWARD * 4981);
            st.exitCurrentQuest(false);
        }
        return htmltext;
