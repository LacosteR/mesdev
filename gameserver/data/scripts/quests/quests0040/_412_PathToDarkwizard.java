...
public class _412_PathToDarkwizard extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x26 && st.getQuestItemsCount(JEWEL_OF_DARKNESS_ID) < 1) {
                st.setCond(1);
                st.setState(STARTED);
                st.playSound(SOUND_ACCEPT);
                st.doGiveItems(SEEDS_OF_DESPAIR_ID, 1);
                htmltext = "varika_q0412_05.htm";
            } else if (st.getPlayer().getPcClass().class_id != 0x26) {
                if (st.getPlayer().getPcClass().class_id == 0x27)
            ...
            else if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x26 && st.getQuestItemsCount(JEWEL_OF_DARKNESS_ID) > 0)
        ...
        if (npcId == VARIKA) {
            if (cond < 1) {
                ...
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(JEWEL_OF_DARKNESS_ID, 1);
            ...
            if (st.getQuestItemsCount(HUB_SCENT_ID) < 1 && st.getQuestItemsCount(HEART_OF_LUNACY_ID) < 1) {
                ...
                st.doGiveItems(SEEDS_OF_LUNACY_ID, 1);
                st.doTakeItems(HEART_OF_LUNACY_ID, -1);
                st.doTakeItems(HUB_SCENT_ID, -1);
            }
            ...
            if (st.getQuestItemsCount(SEEDS_OF_ANGER_ID) < 1) {
                if (st.getQuestItemsCount(SEEDS_OF_DESPAIR_ID) > 0 && st.getQuestItemsCount(FAMILYS_ASHES_ID) < 1 && st.getQuestItemsCount(LUCKY_KEY_ID) < 1)
                    ...
                    st.doGiveItems(SEEDS_OF_ANGER_ID, 1);
                    st.doTakeItems(FAMILYS_ASHES_ID, -1);
                    st.doTakeItems(LUCKY_KEY_ID, -1);
                }
            } else
            ...
            if (st.getQuestItemsCount(SEEDS_OF_DESPAIR_ID) > 0 && st.getQuestItemsCount(CANDLE_ID) < 1 && st.getQuestItemsCount(KNEE_BONE_ID) < 1)
                ...
                st.doGiveItems(SEEDS_OF_HORROR_ID, 1);
                st.doTakeItems(CANDLE_ID, -1);
                st.doTakeItems(KNEE_BONE_ID, -1);
            }
        return htmltext;
    }
