...
public class _258_BringWolfPelts extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            st.doTakeItems(WOLF_PELT, 40);
            int n = Rnd.get(16);
            if (n == 0) {
                st.doGiveItems(Cotton_Shirt, 1);
                st.playSound(SOUND_JACKPOT);
            } else if (n < 6)
                st.doGiveItems(Leather_Pants, 1);
            else if (n < 9)
                st.doGiveItems(Leather_Shirt, 1);
            else if (n < 13)
                st.doGiveItems(Short_Leather_Gloves, 1);
            else
                st.doGiveItems(Tunic, 1);
