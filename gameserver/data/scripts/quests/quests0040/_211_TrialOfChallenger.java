...
public class _211_TrialOfChallenger extends Quest implements ScriptFile {
    ...
    private void Spawn_Raldo(QuestState st) {
        ...
        Raldo_Spawn = addSpawn(Raldo, st.getPlayer().getPositionComponent().getLoc(), 100, 300000);
    }
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            ...
            if (!st.getPlayer().getVarB("dd1")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 61);
            ...
            st.doTakeItems(LETTER_OF_KASH_ID, 1);
            st.setCond(4);
        } else if (event.equalsIgnoreCase("30647_1")) {
            if (st.getQuestItemsCount(BROKEN_KEY_ID) > 0) {
                st.doGiveItems(SCROLL_OF_SHYSLASSY_ID, 1);
                if (Rnd.chance(22)) {
                    htmltext = "chest_of_shyslassys_q0211_03.htm";
                    st.doTakeItems(BROKEN_KEY_ID, 1);
                    st.playSound(SOUND_JACKPOT);
                    int n = Rnd.get(100);
                    if (n > 90) {
                        st.doGiveItems(MITHRIL_SCALE_GAITERS_MATERIAL_ID, 1);
                        st.doGiveItems(BRIGANDINE_GAUNTLET_PATTERN_ID, 1);
                        st.doGiveItems(MANTICOR_SKIN_GAITERS_PATTERN_ID, 1);
                        st.doGiveItems(GAUNTLET_OF_REPOSE_OF_THE_SOUL_PATTERN_ID, 1);
                        st.doGiveItems(IRON_BOOTS_DESIGN_ID, 1);
                    } else if (n > 70) {
                        st.doGiveItems(TOME_OF_BLOOD_PAGE_ID, 1);
                        st.doGiveItems(ELVEN_NECKLACE_BEADS_ID, 1);
                    } else if (n > 40)
                        st.doGiveItems(WHITE_TUNIC_PATTERN_ID, 1);
                    else
                        st.doGiveItems(IRON_BOOTS_DESIGN_ID, 1);
                } else {
                    htmltext = "chest_of_shyslassys_q0211_02.htm";
                    st.doTakeItems(BROKEN_KEY_ID, -1);
                    st.doRewardItems(ADENA_ID, Rnd.get(1000) + 1);
                }
            } else
            ...
            st.setCond(8);
            st.doTakeItems(WATCHERS_EYE2_ID, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (id == CREATED) {
            st.setCond(0);
            if (npcId == Kash)
                if (st.getPlayer().getPcClass().ordinal() == 0x01 || st.getPlayer().getPcClass().ordinal() == 0x13 || st.getPlayer().getPcClass().ordinal() == 0x20 || st.getPlayer().getPcClass().ordinal() == 0x2d || st.getPlayer().getPcClass().ordinal() == 0x2f)
            ...
            st.doTakeItems(SCROLL_OF_SHYSLASSY_ID, 1);
            st.doGiveItems(LETTER_OF_KASH_ID, 1);
            st.setCond(3);
            ...
            st.doTakeItems(WATCHERS_EYE1_ID, 1);
            st.setCond(6);
            ...
            st.doTakeItems(BROKEN_KEY_ID, -1);
            st.doGiveItems(MARK_OF_CHALLENGER_ID, 1);
            if (!st.getPlayer().getVarB("prof2.1")) {
                st.addExpAndSp(RATE_QUESTS_REWARD * RewardExp, RATE_QUESTS_REWARD * RewardSP);
