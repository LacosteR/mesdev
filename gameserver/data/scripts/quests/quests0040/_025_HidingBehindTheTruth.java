...
public class _025_HidingBehindTheTruth extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState qs, NpcInstance npc) {
        if (event.equalsIgnoreCase("StartQuest")) {
            ...
            if (qs.getCond() == 4) {
                qs.setCond(5);
                qs.doTakeItems(SUSPICIOUS_TOTEM_DOLL_1, -1);
                qs.doTakeItems(SUSPICIOUS_TOTEM_DOLL_2, -1);
                if (qs.getQuestItemsCount(GEMSTONE_KEY) == 0)
                    qs.doGiveItems(GEMSTONE_KEY, 1);
            ...
            if (qs.getQuestItemsCount(MAP_FOREST_OF_DEADMAN) == 0)
                qs.doGiveItems(MAP_FOREST_OF_DEADMAN, 1);
        } else if (event.equalsIgnoreCase("31534-07.htm")) {
            Player player = qs.getPlayer();
            qs.addSpawn(TRIOLS_PAWN, player.getPositionComponent().getX() + 50, player.getPositionComponent().getY() + 50, player.getPositionComponent().getZ());
            qs.setCond(7);
        } else if (event.equalsIgnoreCase("31534-11.htm")) {
            qs.set("id", "8");
            qs.doGiveItems(CONTRACT, 1);
            ...
            COFFIN_SPAWN = qs.addSpawn(COFFIN, player.getPositionComponent().getX() + 50, player.getPositionComponent().getY() + 50, player.getPositionComponent().getZ());
            ...
            if (qs.getCond() != 17)
                return "31532-24.htm";
            qs.doRewardItems(RING_OF_BLESSING, 2);
            qs.doRewardItems(EARRING_OF_BLESSING, 1);
            qs.addExpAndSp(RATE_QUESTS_REWARD * 572277, RATE_QUESTS_REWARD * 53750);
            qs.exitCurrentQuest(false);
            return "31532-25.htm";
        } else if (event.equalsIgnoreCase("End2")) {
            if (qs.getCond() != 18)
                return "31522-15a.htm";
            qs.doRewardItems(NECKLACE_OF_BLESSING, 1);
            qs.doRewardItems(EARRING_OF_BLESSING, 1);
            qs.addExpAndSp(RATE_QUESTS_REWARD * 572277, RATE_QUESTS_REWARD * 53750);
            qs.exitCurrentQuest(false);
            return "31522-16.htm";
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        switch (npcId) {
            ...
            case MYSTERIOUS_WIZARD:
                if (cond == 2) {
                    st.setCond(3);
                    st.doGiveItems(SUSPICIOUS_TOTEM_DOLL_2, 1);
                    return "31522-01.htm";
                }
                if (cond == 3)
                    return "31522-02.htm";
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        if (npcId == TRIOLS_PAWN && cond == 7) {
            qs.doGiveOnKillItems(SUSPICIOUS_TOTEM_DOLL_3, 1, 1, 1, 100);
            qs.setCond(8);
        }
