...
public class _166_DarkMass extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("1")) {
            ...
            st.doGiveItems(UNDRES_LETTER_ID, 1);
            st.setCond(1);
            st.setState(STARTED);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30130) {
            if (id == CREATED) {
                ...
                st.doTakeItems(UNDRES_LETTER_ID, -1);
                st.doTakeItems(CEREMONIAL_DAGGER_ID, -1);
                st.doTakeItems(DREVIANT_WINE_ID, -1);
                st.doTakeItems(GARMIELS_SCRIPTURE_ID, -1);
                st.doGiveItems(ADENA_ID, 2966);
