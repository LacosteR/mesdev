...
public class _422_RepentYourSins extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            if (st.getQuestItemsCount(PENITENTS_MANACLES1) >= 1)
                st.doTakeItems(PENITENTS_MANACLES1, -1);
            if (st.getQuestItemsCount(PENITENTS_MANACLES) >= 1)
                st.doTakeItems(PENITENTS_MANACLES, -1);
            st.setCond(16);
            st.set("level", String.valueOf(st.getPlayer().getLevel()));
            st.doGiveItems(PENITENTS_MANACLES, 1);
            ...
            if (pet != null) {
                ...
                if (st.getPlayer().getPkKills() <= Pk_remove) {
                    ...
                    if (st.getQuestItemsCount(PENITENTS_MANACLES2) < 1)
                        st.doGiveItems(PENITENTS_MANACLES2, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Black_Judge) {
            ...
            if (cond <= 13 && cond > 9 && st.getQuestItemsCount(MANUAL_OF_MANACLES) < 1) {
                if (st.getQuestItemsCount(PENITENTS_MANACLES2) < 1) {
                    st.setCond(14);
                    st.doGiveItems(MANUAL_OF_MANACLES, 1);
                    return "black_judge_q0422_08.htm";
                }
                st.doTakeItems(PENITENTS_MANACLES2, -1);
                if (st.getQuestItemsCount(PENITENTS_MANACLES) < 1)
                    st.doGiveItems(PENITENTS_MANACLES, 1);
                st.setCond(16);
                cond = 16;
        ...
        if (npcId == Katari) {
            ...
            st.setCond(10);
            st.doTakeItems(SCAVENGER_WERERAT_SKULL, -1);
        ...
        if (npcId == Piotur) {
            ...
            st.setCond(11);
            st.doTakeItems(TUREK_WARHOUND_TAIL, -1);
        ...
        if (npcId == Casian) {
            ...
            if (cond == 8) {
                ...
                st.setCond(12);
                st.doTakeItems(TYRANT_KINGPIN_HEART, -1);
        ...
        if (npcId == Joan) {
            ...
            if (st.getQuestItemsCount(TRISALIM_TARANTULAS_VENOM_SAC) >= 3) {
                st.setCond(13);
                st.doTakeItems(TRISALIM_TARANTULAS_VENOM_SAC, -1);
        ...
        if (npcId == Pushkin)
            if (cond >= 14) {
                if (st.getQuestItemsCount(MANUAL_OF_MANACLES) == 1) {
                    ...
                    if (st.getQuestItemsCount(SILVER_NUGGET) >= 10 && st.getQuestItemsCount(STEEL) >= 5 && st.getQuestItemsCount(ADAMANTINE_NUGGET) >= 2 && st.getQuestItemsCount(COKES) >= 10 && st.getQuestItemsCount(BLACKSMITHS_FRAME) >= 1) {
                        st.setCond(15);
                        st.doTakeItems(MANUAL_OF_MANACLES, 1);
                        st.doTakeItems(SILVER_NUGGET, 10);
                        st.doTakeItems(ADAMANTINE_NUGGET, 2);
                        st.doTakeItems(COKES, 10);
                        st.doTakeItems(STEEL, 5);
                        st.doTakeItems(BLACKSMITHS_FRAME, 1);
                        st.doGiveItems(PENITENTS_MANACLES1, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == SCAVENGER_WERERAT)
            if (cond == 6 && skulls < 10) {
                st.doGiveItems(SCAVENGER_WERERAT_SKULL, 1);
                if (skulls == 10)
                    st.playSound(SOUND_MIDDLE);
                else
        ...
        if (npcId == TUREK_WARHOUND)
            if (cond == 7 && tails < 10) {
                st.doGiveItems(TUREK_WARHOUND_TAIL, 1);
                if (tails == 10)
                    st.playSound(SOUND_MIDDLE);
                else
                    st.playSound(SOUND_ITEMGET);
        ...
        if (npcId == TYRANT_KINGPIN)
            if (cond == 8 && heart < 1) {
                st.doGiveItems(TYRANT_KINGPIN_HEART, 1);
                st.playSound(SOUND_MIDDLE);
            }
        if (npcId == TRISALIM_TARANTULA)
            if (cond == 9 && sacs < 3) {
                st.doGiveItems(TRISALIM_TARANTULAS_VENOM_SAC, 1);
                if (skulls == 3)
                    st.playSound(SOUND_MIDDLE);
                else
                    st.playSound(SOUND_ITEMGET);
