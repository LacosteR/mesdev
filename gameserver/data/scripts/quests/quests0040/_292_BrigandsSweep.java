    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Spiron) {
            if (cond == 0) {
                ...
                st.doTakeItems(GoblinNecklace, -1);
                st.doTakeItems(GoblinPendant, -1);
                st.doTakeItems(GoblinLordPendant, -1);
                st.doTakeItems(SuspiciousContract, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.getQuestItemsCount(SuspiciousContract) == 0 && Rnd.chance(Chance))
            if (st.getQuestItemsCount(SuspiciousMemo) < 3) {
                st.doGiveItems(SuspiciousMemo, 1);
                st.playSound(SOUND_ITEMGET);
            } else {
                st.doTakeItems(SuspiciousMemo, -1);
                st.doGiveItems(SuspiciousContract, 1);
