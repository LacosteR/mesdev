...
public class _415_PathToOrcMonk extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Urutu) {
            if (st.getQuestItemsCount(KhavatariTotem) != 0) {
                ...
                if (st.getPlayer().getPcClass().class_id != 0x2c) {
                    if (st.getPlayer().getPcClass().class_id == 0x2f)
                ...
                st.doTakeItems(RosheeksLetter, 1);
                st.doGiveItems(GantakisLetterOfRecommendation, 1);
            ...
            if (cond == 1) {
                st.doTakeItems(Pomegranate, -1);
                st.doGiveItems(LeatherPouch1st, 1);
                ...
                st.doTakeItems(LeatherPouchFull1st, -1);
                st.doGiveItems(LeatherPouch2st, 1);
                st.setCond(4);
                st.setState(STARTED);
            } else if (cond == 4)
                htmltext = "khavatari_rosheek_q0415_04.htm";
            else if (cond == 5) {
                st.doTakeItems(LeatherPouchFull2st, -1);
                st.doGiveItems(LeatherPouch3st, 1);
                ...
                st.doTakeItems(LeatherPouchFull3st, -1);
                st.doGiveItems(FierySpiritScroll, 1);
                st.doGiveItems(RosheeksLetter, 1);
            ...
            if (cond == 9) {
                st.doTakeItems(GantakisLetterOfRecommendation, -1);
                st.doGiveItems(Fig, 1);
                ...
                st.doTakeItems(FierySpiritScroll, -1);
                st.doTakeItems(IronWillScroll, -1);
                st.doTakeItems(TorukusLetter, -1);
                htmltext = "prefect_kasman_q0415_04.htm";
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(KhavatariTotem, 1);
            ...
            if (cond == 10) {
                st.doTakeItems(Fig, -1);
                st.doGiveItems(LeatherPouch4st, 1);
                ...
                st.doTakeItems(LeatherPouchFull4st, -1);
                st.doGiveItems(IronWillScroll, 1);
                st.doGiveItems(TorukusLetter, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 3 && st.getQuestItemsCount(LeatherPouchFull1st) == 0) {
            st.doTakeItems(KashaBearClaw, -1);
            st.doTakeItems(LeatherPouch1st, -1);
            st.doGiveItems(LeatherPouchFull1st, 1);
        } else if (cond == 5 && st.getQuestItemsCount(LeatherPouchFull2st) == 0) {
            st.doTakeItems(KashaBladeSpiderTalon, -1);
            st.doTakeItems(LeatherPouch2st, -1);
            st.doGiveItems(LeatherPouchFull2st, 1);
        } else if (cond == 7 && st.getQuestItemsCount(LeatherPouchFull3st) == 0) {
            st.doTakeItems(ScarletSalamanderScale, -1);
            st.doTakeItems(LeatherPouch3st, -1);
            st.doGiveItems(LeatherPouchFull3st, 1);
            ...
            st.doTakeItems(VukuOrcTusk, -1);
            st.doTakeItems(RatmanFang, -1);
            st.doTakeItems(LangkLizardmanTooth, -1);
            st.doTakeItems(FelimLizardmanTooth, -1);
            st.doTakeItems(LeatherPouch4st, -1);
            st.doGiveItems(LeatherPouchFull4st, 1);
            st.setCond(12);
            st.setState(STARTED);
        }
        return null;
