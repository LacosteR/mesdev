...
public class _232_TestOfLord extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (state == CREATED) {
            if (event.equalsIgnoreCase("30565-05.htm")) {
                st.doGiveItems(ORDEAL_NECKLACE, 1);
                if (!st.getPlayer().getVarB("dd3")) {
                    st.doGiveItems(Dimensional_Diamond, RATE_QUESTS_REWARD * 92);
            ...
            if (event.equalsIgnoreCase("30565-12.htm") && st.getQuestItemsCount(IMMORTAL_FLAME) > 0) {
                st.doTakeItems(IMMORTAL_FLAME, -1);
                st.doGiveItems(MARK_OF_LORD, 1);
                if (!st.getPlayer().getVarB("prof2.3")) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 894888, RATE_QUESTS_REWARD * 61408);
                    st.doRewardItems(ADENA_ID, 161806);
                ...
                st.doTakeItems(SWORD_INTO_SKULL, -1);
                st.doTakeItems(AXE_OF_CEREMONY, -1);
                st.doTakeItems(MONSTEREYE_WOODCARVING, -1);
                st.doTakeItems(HANDIWORK_SPIDER_BROOCH, -1);
                st.doTakeItems(ORDEAL_NECKLACE, -1);
                st.doTakeItems(HUGE_ORC_FANG, -1);
                st.doGiveItems(BEAR_FANG_NECKLACE, 1);
                st.playSound(SOUND_MIDDLE);
                st.setCond(3);
            } else if (event.equalsIgnoreCase("30566-02.htm"))
                st.doGiveItems(VARKEES_CHARM, 1);
            else if (event.equalsIgnoreCase("30567-02.htm"))
                st.doGiveItems(TANTUS_CHARM, 1);
            else if (event.equalsIgnoreCase("30558-02.htm") && st.getQuestItemsCount(ADENA_ID) >= 1000) {
                st.doTakeItems(ADENA_ID, 1000);
                st.doGiveItems(NERUGA_AXE_BLADE, 1);
                st.playSound(SOUND_MIDDLE);
            } else if (event.equalsIgnoreCase("30568-02.htm"))
                st.doGiveItems(HATOS_CHARM, 1);
            else if (event.equalsIgnoreCase("30641-02.htm"))
                st.doGiveItems(TAKUNA_CHARM, 1);
            else if (event.equalsIgnoreCase("30642-02.htm"))
                st.doGiveItems(CHIANTA_CHARM, 1);
            else if (event.equalsIgnoreCase("30649-04.htm") && st.getQuestItemsCount(BEAR_FANG_NECKLACE) > 0) {
                st.doTakeItems(BEAR_FANG_NECKLACE, -1);
                st.doGiveItems(MARTANKUS_CHARM, 1);
                st.playSound(SOUND_MIDDLE);
                st.setCond(4);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (_state == CREATED) {
            ...
            if (st.getPlayer().getPcClass().class_id != 0x32) {
                st.exitCurrentQuest(true);
                return "30565-02.htm";
            }
        ...
        if (npcId == Varkees && ORDEAL_NECKLACE_COUNT > 0) {
            ...
            if (MANAKIAS_AMULET_COUNT == 0)
                return "30566-03.htm";
            st.doTakeItems(VARKEES_CHARM, -1);
            st.doTakeItems(MANAKIAS_AMULET, -1);
            st.doGiveItems(HUGE_ORC_FANG, 1);
            if (cond1Complete(st)) {
            ...
            if (VARKEES_CHARM_COUNT > 0 && HUGE_ORC_FANG_COUNT == 0) {
                if (MANAKIAS_AMULET_COUNT == 0) {
                    if (MANAKIAS_ORDERS_COUNT == 0) {
                        st.doGiveItems(MANAKIAS_ORDERS, 1);
                        return "30515-01.htm";
                    }
                    if (BREKA_ORC_FANG_COUNT < 20)
                        return "30515-02.htm";
                    st.doTakeItems(MANAKIAS_ORDERS, -1);
                    st.doTakeItems(BREKA_ORC_FANG, -1);
                    st.doGiveItems(MANAKIAS_AMULET, 1);
                    st.playSound(SOUND_MIDDLE);
                    return "30515-03.htm";
        ...
        if (npcId == Tantus) {
            if (AXE_OF_CEREMONY_COUNT == 0) {
                ...
                if (NERUGA_AXE_BLADE_COUNT == 0 || st.getQuestItemsCount(BONE_ARROW) < 1000)
                    return "30567-03.htm";
                st.doTakeItems(TANTUS_CHARM, -1);
                st.doTakeItems(NERUGA_AXE_BLADE, -1);
                st.doTakeItems(BONE_ARROW, 1000);
                st.doGiveItems(AXE_OF_CEREMONY, 1);
                if (cond1Complete(st)) {
                    st.playSound(SOUND_JACKPOT);
                    st.setCond(2);
                } else
        ...
        if (npcId == Hatos) {
            if (SWORD_INTO_SKULL_COUNT == 0) {
                ...
                if (URUTU_BLADE_COUNT == 0 || TIMAK_ORC_SKULL_COUNT < 10)
                    return "30568-03.htm";
                st.doTakeItems(HATOS_CHARM, -1);
                st.doTakeItems(URUTU_BLADE, -1);
                st.doTakeItems(TIMAK_ORC_SKULL, -1);
                st.doGiveItems(SWORD_INTO_SKULL, 1);
                if (cond1Complete(st)) {
                    st.playSound(SOUND_JACKPOT);
                    st.setCond(2);
                } else
            ...
            if (HATOS_CHARM_COUNT > 0 && SWORD_INTO_SKULL_COUNT == 0) {
                if (URUTU_BLADE_COUNT == 0) {
                    if (SUMARIS_LETTER_COUNT > 0)
                        return "30564-02.htm";
                    st.doGiveItems(SUMARIS_LETTER, 1);
                    st.playSound(SOUND_MIDDLE);
                    return "30564-01.htm";
        ...
        if (npcId == Somak)
            if (SWORD_INTO_SKULL_COUNT == 0) {
                if (URUTU_BLADE_COUNT == 0 && HATOS_CHARM_COUNT > 0 && SUMARIS_LETTER_COUNT > 0) {
                    st.doTakeItems(SUMARIS_LETTER, -1);
                    st.doGiveItems(URUTU_BLADE, 1);
                    st.playSound(SOUND_MIDDLE);
                    return "30510-01.htm";
                }
                if (URUTU_BLADE_COUNT > 0 && HATOS_CHARM_COUNT > 0 && SUMARIS_LETTER_COUNT == 0)
        ...
        if (npcId == Takuna) {
            if (HANDIWORK_SPIDER_BROOCH_COUNT == 0) {
                ...
                if (st.getQuestItemsCount(MARSH_SPIDER_FEELER) < 10 || st.getQuestItemsCount(MARSH_SPIDER_FEET) < 10)
                    return "30641-03.htm";
                st.doTakeItems(MARSH_SPIDER_FEELER, -1);
                st.doTakeItems(MARSH_SPIDER_FEET, -1);
                st.doTakeItems(TAKUNA_CHARM, -1);
                st.doGiveItems(HANDIWORK_SPIDER_BROOCH, 1);
                if (cond1Complete(st)) {
                    st.playSound(SOUND_JACKPOT);
                    st.setCond(2);
                } else
        ...
        if (npcId == Chianta) {
            ...
            if (MONSTEREYE_WOODCARVING_COUNT == 0) {
                ...
                if (st.getQuestItemsCount(CORNEA_OF_EN_MONSTEREYE) < 20)
                    return "30642-03.htm";
                st.doTakeItems(CORNEA_OF_EN_MONSTEREYE, -1);
                st.doTakeItems(CHIANTA_CHARM, -1);
                st.doGiveItems(MONSTEREYE_WOODCARVING, 1);
                if (cond1Complete(st)) {
                    st.playSound(SOUND_JACKPOT);
                    st.setCond(2);
                } else
        ...
        if (npcId == Ancestor_Martankus) {
            ...
            if (MARTANKUS_CHARM_COUNT > 0) {
                if (cond == 5 || st.getQuestItemsCount(RAGNA_CHIEF_NOTICE) > 0 && st.getQuestItemsCount(RAGNA_ORC_HEAD) > 0) {
                    st.doTakeItems(MARTANKUS_CHARM, -1);
                    st.doTakeItems(RAGNA_ORC_HEAD, -1);
                    st.doTakeItems(RAGNA_CHIEF_NOTICE, -1);
                    st.doGiveItems(IMMORTAL_FLAME, 1);
                    st.playSound(SOUND_MIDDLE);
                    return "30649-06.htm";
                }
                return "30649-05.htm";
