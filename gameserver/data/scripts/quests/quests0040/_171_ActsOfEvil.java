...
public class _171_ActsOfEvil extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("30381-02.htm") && cond == 0) {
            ...
            st.setCond(7);
            st.setState(STARTED);
            st.doTakeItems(WeaponsTradeContract, -1);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equals("30437-03.htm") && cond == 8) {
            st.doGiveItems(RolentoCargobox, 1);
            st.doGiveItems(CertificateOfTheSilverScaleGuild, 1);
            st.setCond(9);
            st.setState(STARTED);
        } else if (event.equals("30617-04.htm") && cond == 9) {
            st.doTakeItems(CertificateOfTheSilverScaleGuild, -1);
            st.doTakeItems(AttackDirectives, -1);
            st.doTakeItems(RolentoCargobox, -1);
            st.setCond(10);
            st.setState(STARTED);
        ...
        if (npcId == Alvah) {
            if (cond == 0) {
                ...
                if (st.getQuestItemsCount(RangerReportPart1) > 0 && st.getQuestItemsCount(RangerReportPart2) > 0 && st.getQuestItemsCount(RangerReportPart3) > 0 && st.getQuestItemsCount(RangerReportPart4) > 0) {
                    ...
                    st.doTakeItems(RangerReportPart1, -1);
                    st.doTakeItems(RangerReportPart2, -1);
                    st.doTakeItems(RangerReportPart3, -1);
                    st.doTakeItems(RangerReportPart4, -1);
                    st.setCond(6);
                    st.setState(STARTED);
                } else
                ...
                st.doRewardItems(ADENA_ID, 95000);
                st.addExpAndSp(RATE_QUESTS_REWARD * 159820, RATE_QUESTS_REWARD * 9182);
            ...
            if (cond == 2) {
                if (st.getQuestItemsCount(BladeMold) >= 20) {
                    st.doTakeItems(BladeMold, -1);
                    st.doGiveItems(TyrasBill, 1);
            ...
            if (cond == 10)
                if (st.getQuestItemsCount(OlMahumCaptainHead) >= 30) {
                    ...
                    st.doGiveItems(ADENA_ID, 8000);
                    st.doTakeItems(OlMahumCaptainHead, -1);
                    st.setCond(11);
                    st.setState(STARTED);
