...
public class _226_TestOfHealer extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            ...
            if (!st.getPlayer().getVarB("dd3")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 104);
                st.getPlayer().setVar("dd3", "1", -1);
            }
            st.setCond(COND1);
            st.setState(STARTED);
            st.playSound(SOUND_ACCEPT);
            st.doGiveItems(REPORT_OF_PERRIN_ID, 1);
        } else if (event.equalsIgnoreCase("30473_1"))
            htmltext = "30473-08.htm";
        else if (event.equalsIgnoreCase("30473_2")) {
            htmltext = "30473-09.htm";
            st.doTakeItems(GOLDEN_STATUE_ID, -1);
            st.doRewardItems(ADENA_ID, 266980);
            st.doGiveItems(MARK_OF_HEALER_ID, 1);
            st.addExpAndSp(RATE_QUESTS_REWARD * 1476566, RATE_QUESTS_REWARD * 101324);
            ...
            if (st.getQuestItemsCount(ADENA_ID) >= 100000) {
                ...
                st.doGiveItems(PICTURE_OF_WINDY_ID, 1);
                st.setCond(COND7);
            } else
            ...
            st.doTakeItems(PICTURE_OF_WINDY_ID, 1);
            st.doGiveItems(WINDYS_PEBBLES_ID, 1);
            st.setCond(COND8);
        } else if (event.equalsIgnoreCase("30674_1")) {
            htmltext = "30674-02.htm";
            st.setCond(COND11);
            st.doTakeItems(ORDER_OF_SORIUS_ID, 1);
            st.addSpawn(27122);
            st.addSpawn(27122);
            st.addSpawn(27123);
            ...
            st.doTakeItems(SECRET_LETTER1_ID, 1);
            st.doTakeItems(SECRET_LETTER2_ID, 1);
            st.doTakeItems(SECRET_LETTER3_ID, 1);
            st.doTakeItems(SECRET_LETTER4_ID, 1);
            st.doGiveItems(CRISTINAS_LETTER_ID, 1);
            st.setCond(COND22);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Bandellos) {
            if (cond == 0) {
                if (st.getPlayer().getPcClass() == PcClass.knight || st.getPlayer().getPcClass() == PcClass.cleric || st.getPlayer().getPcClass() == PcClass.oracle || st.getPlayer().getPcClass() == PcClass.elven_knight)
                ...
                if (st.getQuestItemsCount(GOLDEN_STATUE_ID) == 0) {
                    ...
                    if (!st.getPlayer().getVarB("prof2.3")) {
                        ...
                        st.doRewardItems(ADENA_ID, 133490);
            ...
            if (cond == COND1)
                ...
                st.doTakeItems(REPORT_OF_PERRIN_ID, 1);
                st.setCond(COND4);
            } else if (cond != COND2)
            ...
            if (cond == COND5)
                ...
                st.doGiveItems(GOLDEN_STATUE_ID, 1);
                st.doTakeItems(WINDYS_PEBBLES_ID, 1);
                st.setCond(COND9);
            } else if (cond == COND6) {
                st.setCond(COND9);
