    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("1")) {
            ...
            if (st.getQuestItemsCount(ADENA_ID) >= 1000) {
                ...
                st.doGiveItems(POWDER_TO_SUMMON_DEAD_SOULS, 1);
                st.setCond(3);
            ...
            st.doTakeItems(POWDER_TO_SUMMON_DEAD_SOULS, -1);
            st.doTakeItems(VICTIMS_ARM_BONE, -1);
            st.doTakeItems(VICTIMS_THIGH_BONE, -1);
            st.doTakeItems(VICTIMS_SKULL, -1);
            st.doTakeItems(VICTIMS_RIB_BONE, -1);
            st.doTakeItems(VICTIMS_SPINE, -1);
            st.setCond(6);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30970)
            if (id == CREATED) {
                ...
                st.setCond(1);
                st.doGiveItems(ADENA_ID, amount * 238);
                st.doGiveItems(BILL_OF_IASON_HEINE, Rnd.get(7) + 1);
                st.doTakeItems(USELESS_BONE_PIECES, -1);
            }
        if (npcId == 30912)
            if (cond == 2) {
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (random <= CHANCE)
            if (st.getQuestItemsCount(VICTIMS_ARM_BONE) == 0)
                st.doGiveItems(VICTIMS_ARM_BONE, 1);
            else if (st.getQuestItemsCount(VICTIMS_THIGH_BONE) == 0)
                st.doGiveItems(VICTIMS_THIGH_BONE, 1);
            else if (st.getQuestItemsCount(VICTIMS_SKULL) == 0)
                st.doGiveItems(VICTIMS_SKULL, 1);
            else if (st.getQuestItemsCount(VICTIMS_RIB_BONE) == 0)
                st.doGiveItems(VICTIMS_RIB_BONE, 1);
            else if (st.getQuestItemsCount(VICTIMS_SPINE) == 0)
                st.doGiveItems(VICTIMS_SPINE, 1);
        if (random <= CHANCE2)
            st.doGiveItems(USELESS_BONE_PIECES, (Rnd.get(8) + 1));
        return null;
    }
}