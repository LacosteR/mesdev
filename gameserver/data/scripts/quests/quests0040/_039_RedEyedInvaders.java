...
public class _039_RedEyedInvaders extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("guard_babenco_q0039_0104.htm")) {
            ...
            if (st.getQuestItemsCount(BBN) >= 100 && st.getQuestItemsCount(RBN) >= 100) {
                st.setCond(4);
                st.doTakeItems(BBN, -1);
                st.doTakeItems(RBN, -1);
                st.playSound(SOUND_ACCEPT);
            } else
            ...
            if (st.getQuestItemsCount(IP) >= 30 && st.getQuestItemsCount(GML) >= 30) {
                st.doTakeItems(IP, -1);
                st.doTakeItems(GML, -1);
                st.doRewardItems(REW[0], 60);
                st.doRewardItems(REW[1], 1);
                st.doRewardItems(REW[2], 500);
                st.addExpAndSp(RATE_QUESTS_REWARD * 62236, RATE_QUESTS_REWARD * 2783);
                st.setCond(0);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 2) {
            if (npcId == 20919 || npcId == 20920)
                st.doGiveOnKillItems(BBN, 1, 1, 100, 100, false);

            else if (npcId == 20921)
                st.doGiveOnKillItems(RBN, 1, 1, 100, 100, false);
        ...
        if (cond == 4) {
            if (npcId == 20920 || npcId == 20921)
                st.doGiveOnKillItems(IP, 1, 1, 30, 100, false);

            else if (npcId == 20925)
                st.doGiveOnKillItems(GML, 1, 1, 30, 100, false);
