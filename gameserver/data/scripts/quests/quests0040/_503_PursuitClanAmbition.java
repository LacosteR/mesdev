...
public class _503_PursuitClanAmbition extends Quest implements ScriptFile {
    ...
    public void giveItem(int item, long maxcount, QuestState st) {
        ...
        if (leader == null)
            return;
        if (PositionUtils.getDistance(player, leader) > ConfigOthers.PARTY_DISTRIBUTION_RANGE)
            return;
    ...
    public String exit503(boolean completed, QuestState st) {
        if (completed) {
            st.doGiveItems(Proof_Aspiration, 1);
        ...
        st.doTakeItems(Scepter_Judgement, -1);
        try {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30760-08.htm")) {
            st.doGiveItems(G_Let_Martien, 1);
            st.setCond(1);
            st.set("Fritz", "1");
            st.set("Lutz", "1");
            st.set("Kurtz", "1");
            ...
            st.doGiveItems(G_Let_Balthazar, 1);
            st.setCond(4);
        } else if (event.equalsIgnoreCase("30760-16.htm")) {
            st.doGiveItems(G_Let_Rodemai, 1);
            st.setCond(7);
            ...
            st.doTakeItems(G_Let_Martien, -1);
            st.setCond(2);
            suscribe_members(st);
        ...
        // Events Kurtz
            ...
            if (st.getInt("Kurtz") == 1) {
                ...
                st.doGiveItems(Mi_Drake_Eggs, 6);
                st.doGiveItems(Brooch, 1);
                st.set("Kurtz", "2");
            }
        }
        // Events Lutz
            ...
            if (lutz == 1) {
                ...
                st.doGiveItems(Mi_Drake_Eggs, 4);
                st.doGiveItems(Bl_Wyrm_Eggs, 3);
                st.set("Lutz", "2");
            }
            st.addSpawn(BlitzWyrm, npc.getPositionComponent().getLoc().x, npc.getPositionComponent().getLoc().y, npc.getPositionComponent().getLoc().z, Location.getRandomHeading(), 300, 120000);
            st.addSpawn(BlitzWyrm, npc.getPositionComponent().getLoc().x, npc.getPositionComponent().getLoc().y, npc.getPositionComponent().getLoc().z, Location.getRandomHeading(), 300, 120000);
        }
        // Events Fritz
        ...
        // Events Kusto
            ...
            st.doTakeItems(Brooch, -1);
            st.doGiveItems(Bl_Anvil_Coin, 1);
            st.set("Kurtz", "3");
        }
        // Events Balthazar
        else if (event.equalsIgnoreCase("30764-03.htm")) {
            st.doTakeItems(G_Let_Balthazar, -1);
            st.setCond(5);
            st.set("Kurtz", "3");
            ...
            st.doTakeItems(Bl_Anvil_Coin, -1);
            st.set("Kurtz", "4");
            st.doGiveItems(Recipe_Spiteful_Soul_Energy, 1);
        }
        // Events Rodemai
        else if (event.equalsIgnoreCase("30868-04.htm")) {
            st.doTakeItems(G_Let_Rodemai, -1);
            st.setCond(8);
                ...
                if (npcId == Gustaf) {
                    ...
                    if (cond == 4)
                        ...
                        if (st.getQuestItemsCount(Spiteful_Soul_Energy) > 9) {
                            ...
                            st.doTakeItems(Spiteful_Soul_Energy, -1);
                            st.doTakeItems(Brooch, -1);
                            st.setCond(6);
                        } else
                    ...
                    if (st.getCond() == 10)
                        if (st.getQuestItemsCount(Imp_Keys) < 6)
                            ...
                            st.setCond(11);
                            st.doTakeItems(Imp_Keys, 6);
                            st.doGiveItems(Scepter_Judgement, 1);
                        } else
