...
public class _225_TestOfTheSearcher extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30690-05.htm")) {
            st.doGiveItems(LuthersLetter, 1);
            st.setCond(1);
            st.setState(STARTED);
            if (!st.getPlayer().getVarB("dd3")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 82);
                st.getPlayer().setVar("dd3", "1", -1);
            }
            st.playSound(SOUND_ACCEPT);
        } else if (event.equalsIgnoreCase("30291-07.htm")) {
            st.doTakeItems(LeirynnsReport, -1);
            st.doTakeItems(StrangeMap, -1);
            st.doGiveItems(LambertsMap, 1);
            st.doGiveItems(AlexsLetter, 1);
            st.doGiveItems(AlexsOrder, 1);
            st.setCond(8);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30420-01a.htm")) {
            st.doTakeItems(WineCatalog, -1);
            st.doGiveItems(TyrasContract, 1);
            st.setCond(10);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30730-01d.htm")) {
            st.doTakeItems(OldOrder, -1);
            st.doGiveItems(JaxsDiary, 1);
            st.setCond(14);
            st.setState(STARTED);
            ...
            st.doTakeItems(RustedKey, -1);
            st.doGiveItems(GoldBar, 20);
            st.setCond(18);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Luther) {
            if (st.getQuestItemsCount(MarkOfSearcher) != 0) {
                ...
                if (st.getPlayer().getPcClass().class_id == 0x07 || st.getPlayer().getPcClass().class_id == 0x16 || st.getPlayer().getPcClass().class_id == 0x23 || st.getPlayer().getPcClass().class_id == 0x36) {
                    if (st.getPlayer().getLevel() >= 39) {
                        if (st.getPlayer().getPcClass().class_id == 0x36)
                ...
                st.doTakeItems(AlexsRecommend, -1);
                st.doGiveItems(MarkOfSearcher, 1);
            ...
            if (cond == 1) {
                ...
                st.doTakeItems(LuthersLetter, -1);
                st.doGiveItems(AlexsWarrant, 1);
                st.setCond(2);
                st.setState(STARTED);
            } else if (cond == 2)
                ...
                st.doTakeItems(AlexsOrder, -1);
                st.doTakeItems(CombinedMap, -1);
                st.doTakeItems(GoldBar, -1);
                st.doGiveItems(AlexsRecommend, 1);
            ...
            if (cond == 2) {
                ...
                st.doTakeItems(AlexsWarrant, -1);
                st.doGiveItems(Leirynns1stOrder, 1);
                st.setCond(3);
                st.setState(STARTED);
            } else if (cond == 3)
                ...
                st.doTakeItems(DeluTotem, -1);
                st.doTakeItems(Leirynns1stOrder, -1);
                st.doGiveItems(Leirynns2ndOrder, 1);
                st.setCond(5);
                st.setState(STARTED);
            } else if (cond == 5)
                htmltext = "30728-04.htm";
            else if (cond == 6) {
                st.doTakeItems(ChiefKalkisFang, -1);
                st.doTakeItems(Leirynns2ndOrder, -1);
                st.doGiveItems(LeirynnsReport, 1);
            ...
            if (cond == 8) {
                st.doTakeItems(AlexsLetter, -1);
                st.doGiveItems(WineCatalog, 1);
                ...
                st.doTakeItems(WineCatalog, -1);
                st.doTakeItems(MalrukianWine, -1);
                st.doGiveItems(OldOrder, 1);
            ...
            if (cond == 9)
                ...
                st.doTakeItems(TyrasContract, -1);
                st.doTakeItems(RedSporeDust, -1);
                st.doGiveItems(MalrukianWine, 1);
            ...
            if (cond == 13)
                ...
                st.doTakeItems(SoltsMap, -1);
                st.doTakeItems(MakelsMap, -1);
                st.doTakeItems(LambertsMap, -1);
                st.doTakeItems(JaxsDiary, -1);
                st.doGiveItems(CombinedMap, 1);
