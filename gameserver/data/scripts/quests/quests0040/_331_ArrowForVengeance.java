...
public class _331_ArrowForVengeance extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            if (st.getQuestItemsCount(HARPY_FEATHER) + st.getQuestItemsCount(MEDUSA_VENOM) + st.getQuestItemsCount(WYRMS_TOOTH) > 0) {
                st.doGiveItems(ADENA_ID, (80 * st.getQuestItemsCount(HARPY_FEATHER)) + (90 * st.getQuestItemsCount(MEDUSA_VENOM)) + (100 * st.getQuestItemsCount(WYRMS_TOOTH)));
                st.doTakeItems(HARPY_FEATHER, -1);
                st.doTakeItems(MEDUSA_VENOM, -1);
                st.doTakeItems(WYRMS_TOOTH, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() > 0)
            switch (npc.getNpcId()) {
                case 20145:
                    st.doGiveOnKillItems(HARPY_FEATHER, 1, 1, 33);
                    break;
                case 20158:
                    st.doGiveOnKillItems(MEDUSA_VENOM, 1, 1, 33);
                    break;
                case 20176:
                    st.doGiveOnKillItems(WYRMS_TOOTH, 1, 1, 33);
                    break;
            }
        return null;
    }
