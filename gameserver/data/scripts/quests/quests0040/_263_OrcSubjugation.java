    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            if (st.getQuestItemsCount(ORC_AMULET) == 0 && st.getQuestItemsCount(ORC_NECKLACE) == 0)
                ...
                st.doGiveItems(ADENA_ID, RATE_QUESTS_REWARD * st.getQuestItemsCount(ORC_AMULET) * 20 + st.getQuestItemsCount(ORC_NECKLACE) * 30 + 1100);
                st.doTakeItems(ORC_AMULET, -1);
                st.doTakeItems(ORC_NECKLACE, -1);
            } else {
                htmltext = "sentry_kayleen_q0263_05.htm";
                st.doGiveItems(ADENA_ID, st.getQuestItemsCount(ORC_AMULET) * 20 + st.getQuestItemsCount(ORC_NECKLACE) * 30);
                st.doTakeItems(ORC_AMULET, -1);
                st.doTakeItems(ORC_NECKLACE, -1);
            }
        return htmltext;
