...
public class _295_DreamsOfTheSkies extends Quest implements ScriptFile {
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1 && st.getQuestItemsCount(FLOATING_STONE) < 50)
            if (Rnd.chance(25)) {
                st.doGiveItems(FLOATING_STONE, 1);
                ...
                st.doGiveItems(FLOATING_STONE, 50 - st.getQuestItemsCount(FLOATING_STONE));
                st.playSound(SOUND_MIDDLE);
                st.setCond(2);
            } else {
                st.doGiveItems(FLOATING_STONE, 2);
                st.playSound(SOUND_ITEMGET);
            }
        return null;
    }
