    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == NATOOLS) {
            ...
            if (cond == 2)
                ...
                st.doRewardItems(ADENA_ID, 92676);
                st.addExpAndSp(RATE_QUESTS_REWARD * 223056, RATE_QUESTS_REWARD * 13901);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npc.getNpcId() == FallenAngel) {
            if (cond == 5) {
                ...
                st.setState(STARTED);
                st.playSound(SOUND_MIDDLE);
                st.doGiveItems(BLOOD, 1);
            }
        } else if (cond == 4 && st.doGiveOnKillItems(FRAGMENT, 1, 1, 30, 20)) {
            st.setCond(5);
            st.setState(STARTED);
        }
        return null;
