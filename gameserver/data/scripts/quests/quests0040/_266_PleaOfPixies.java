...
public class _266_PleaOfPixies extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (st.getCond() == 0) {
            ...
            st.doTakeItems(PREDATORS_FANG, -1);
            int n = Rnd.get(100);
            if (n < 2) {
                st.doGiveItems(EMERALD, 1);
                st.doGiveItems(REC_SPIRITSHOT, 1);
                st.playSound(SOUND_JACKPOT);
            } else if (n < 20) {
                st.doGiveItems(BLUE_ONYX, 1);
                st.doGiveItems(REC_LEATHER_BOOT, 1);
            } else if (n < 45)
                st.doGiveItems(ONYX, 1);
            else
                st.doGiveItems(GLASS_SHARD, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1)
            st.doGiveOnKillItems(PREDATORS_FANG, 1, 1, 100, 60 + npc.getLevel() * 5);
        return null;
    }
}