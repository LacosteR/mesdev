...
public class _141_ShadowFoxPart3 extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30894-02.htm")) {
            ...
            if (st.getInt("reward") != 1) {
                ...
                st.doRewardItems(ADENA_ID, 88888);
                st.addExpAndSp(RATE_QUESTS_REWARD * 278005, RATE_QUESTS_REWARD * 17058);
                st.set("reward", "1");
                htmltext = "select.htm";
            } else
                htmltext = "select.htm";
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 2 && st.doGiveOnKillItems(REPORT, 1, 1, 30, 80 * npc.getTemplate().rateHp))
            st.setCond(3);
        return null;
    }
}