...
public class _118_ToLeadAndBeLed extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if (event.equals("30298-02.htm")) {
            ...
            if (st.getQuestItemsCount(D_CRY) < need_dcry)
                return "30298-07.htm";
            st.setCond(7);
            st.doTakeItems(D_CRY, need_dcry);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 2 && _state == STARTED) {
            ...
            st.doTakeItems(BLOOD_OF_MAILLE_LIZARDMAN, -1);
            return "30298-04.htm";
        }
        ...
        if (cond == 8 && _state == STARTED) {
            ...
            st.doTakeItems(KING_OF_THE_ARANEID_LEG, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        int cond = st.getCond();
        if (npcId == MAILLE_LIZARDMAN && cond == 1) {
            if (st.doGiveOnKillItems(BLOOD_OF_MAILLE_LIZARDMAN, 1, 1, 10, 50))
                st.setCond(2);
        } else if (npcId == KING_OF_THE_ARANEID && cond == 7) {
            if (st.doGiveOnKillItems(KING_OF_THE_ARANEID_LEG, 1, 1, 8, 50))
                st.setCond(8);
        }
        return null;
    }
