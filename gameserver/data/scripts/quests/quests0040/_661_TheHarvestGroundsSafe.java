    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("warehouse_keeper_norman_q0661_0103.htm") || event.equalsIgnoreCase("warehouse_keeper_norman_q0661_0201.htm")) {
            ...
            if (STING + GEM + TALON >= 10) {
                st.doRewardItems(ADENA_ID, (STING * 56) + (GEM * 58) + (TALON * 60) + 2800);
                st.doTakeItems(STING_OF_GIANT_POISON, -1);
                st.doTakeItems(TALON_OF_YOUNG_ARANEID, -1);
                st.doTakeItems(CLOUDY_GEM, -1);
            } else {
                st.doRewardItems(ADENA_ID, (STING * 56) + (GEM * 58) + (TALON * 60));
                st.doTakeItems(STING_OF_GIANT_POISON, -1);
                st.doTakeItems(TALON_OF_YOUNG_ARANEID, -1);
                st.doTakeItems(CLOUDY_GEM, -1);
            }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.getCond() == 1) {
            if (npcId == GIANT_POISON_BEE && Rnd.chance(75)) {
                st.doGiveItems(STING_OF_GIANT_POISON, 1);
                st.playSound(SOUND_ITEMGET);
            }
            if (npcId == CLOYDY_BEAST && Rnd.chance(71)) {
                st.doGiveItems(CLOUDY_GEM, 1);
                st.playSound(SOUND_ITEMGET);
            }
            if (npcId == YOUNG_ARANEID && Rnd.chance(67)) {
                st.doGiveItems(TALON_OF_YOUNG_ARANEID, 1);
                st.playSound(SOUND_ITEMGET);
            }
        }
        return null;
