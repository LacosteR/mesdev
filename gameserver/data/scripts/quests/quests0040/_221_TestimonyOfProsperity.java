    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30104-04.htm")) {
            if (!st.getPlayer().getVarB("dd2")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 50);
                st.getPlayer().setVar("dd2", "1", -1);
            }
            st.playSound(SOUND_ACCEPT);
            st.doGiveItems(RingOfTestimony1st, 1);
            st.setCond(1);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30466-03.htm"))
            st.doGiveItems(BrightsList, 1);
        else if (event.equalsIgnoreCase("30620-03.htm")) {
            st.doTakeItems(MandragoraBouquet, -1);
            st.doGiveItems(EmilysRecipe, 1);
            ...
            st.doGiveItems(BlessedSeed, 1);
            if (st.getQuestItemsCount(OldAccountBook) > 0 && st.getQuestItemsCount(BlessedSeed) > 0 && st.getQuestItemsCount(EmilysRecipe) > 0 && st.getQuestItemsCount(LilithsElvenWafer) > 0) {
                st.setCond(2);
                st.setState(STARTED);
            }
        } else if (event.equalsIgnoreCase("30005-04.htm"))
            st.doGiveItems(CrystalBrooch, 1);
        else if (event.equalsIgnoreCase("30368-03.htm")) {
            st.doTakeItems(CrystalBrooch, -1);
            st.doGiveItems(LilithsElvenWafer, 1);
            if (st.getQuestItemsCount(OldAccountBook) > 0 && st.getQuestItemsCount(BlessedSeed) > 0 && st.getQuestItemsCount(EmilysRecipe) > 0 && st.getQuestItemsCount(LilithsElvenWafer) > 0) {
                st.setCond(2);
                st.setState(STARTED);
            }
        } else if (event.equalsIgnoreCase("30531-03.htm")) {
            st.doGiveItems(CollectionLicense, 1);
            st.doGiveItems(Lockirins1stNotice, 1);
            st.doGiveItems(Lockirins2stNotice, 1);
            st.doGiveItems(Lockirins3stNotice, 1);
            st.doGiveItems(Lockirins4stNotice, 1);
            st.doGiveItems(Lockirins5stNotice, 1);
        } else if (event.equalsIgnoreCase("30555-02.htm"))
            st.doGiveItems(ProcurationOfTorocco, 1);
        else if (event.equalsIgnoreCase("30534-03a.htm") && st.getQuestItemsCount(ADENA_ID) >= 5000) {
            htmltext = "30534-03b.htm";
            st.doTakeItems(ADENA_ID, 5000);
            st.doTakeItems(ProcurationOfTorocco, -1);
            st.doGiveItems(ReceiptOfContribution3st, 1);
        } else if (event.equalsIgnoreCase("30104-07.htm")) {
            st.doTakeItems(RingOfTestimony1st, -1);
            st.doTakeItems(OldAccountBook, -1);
            st.doTakeItems(BlessedSeed, -1);
            st.doTakeItems(EmilysRecipe, -1);
            st.doTakeItems(LilithsElvenWafer, -1);
            if (st.getPlayer().getLevel() < 38) {
                st.doGiveItems(ParmansInstructions, 1);
                st.setCond(3);
                st.setState(STARTED);
            } else {
                st.doGiveItems(ParmansLetter, 1);
                st.doGiveItems(RingOfTestimony2st, 1);
                htmltext = "30104-08.htm";
                st.setCond(4);
                st.setState(STARTED);
            }
        } else if (event.equalsIgnoreCase("30621-04.htm")) {
            st.doGiveItems(ClayDough, 1);
            st.setCond(5);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30622-02.htm")) {
            st.doTakeItems(ClayDough, -1);
            st.doGiveItems(PatternOfKeyhole, 1);
            st.setCond(6);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30622-04.htm")) {
            st.doTakeItems(NikolasList, -1);
            st.doTakeItems(KeyOfTitan, 1);
            st.doGiveItems(MaphrTabletFragment, 1);
            st.setCond(9);
            st.setState(STARTED);
        }
        return htmltext;
        ...
        if (npcId == Parman) {
            if (st.getQuestItemsCount(MarkOfProsperity) != 0) {
                ...
                if (st.getPlayer().getLevel() < 38)
                    ...
                    st.doTakeItems(ParmansInstructions, -1);
                    st.doGiveItems(RingOfTestimony2st, 1);
                    st.doGiveItems(ParmansLetter, 1);
                    st.setCond(4);
                    st.setState(STARTED);
                ...
                if (!st.getPlayer().getVarB("prof2.2")) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 1199958, RATE_QUESTS_REWARD * 80080);
                    st.doRewardItems(ADENA_ID, 217682);
                    st.getPlayer().setVar("prof2.2", "1", -1);
                }
                st.doTakeItems(RingOfTestimony2st, -1);
                st.doTakeItems(MaphrTabletFragment, -1);
                st.doGiveItems(MarkOfProsperity, 1);
            ...
            if (st.getQuestItemsCount(CollectionLicense) == 0)
                ...
                if (st.getQuestItemsCount(ReceiptOfContribution1st) > 0 && st.getQuestItemsCount(ReceiptOfContribution2st) > 0 && st.getQuestItemsCount(ReceiptOfContribution3st) > 0 && st.getQuestItemsCount(ReceiptOfContribution4st) > 0 && st.getQuestItemsCount(ReceiptOfContribution5st) > 0) {
                    ...
                    st.doTakeItems(CollectionLicense, -1);
                    st.doTakeItems(ReceiptOfContribution1st, -1);
                    st.doTakeItems(ReceiptOfContribution2st, -1);
                    st.doTakeItems(ReceiptOfContribution3st, -1);
                    st.doTakeItems(ReceiptOfContribution4st, -1);
                    st.doTakeItems(ReceiptOfContribution5st, -1);
                    st.doGiveItems(OldAccountBook, 1);
                    if (st.getQuestItemsCount(OldAccountBook) > 0 && st.getQuestItemsCount(BlessedSeed) > 0 && st.getQuestItemsCount(EmilysRecipe) > 0 && st.getQuestItemsCount(LilithsElvenWafer) > 0)
                        st.setCond(2);
                } else
            ...
            if (st.getQuestItemsCount(Lockirins1stNotice) > 0) {
                ...
                st.doTakeItems(Lockirins1stNotice, -1);
                ...
                st.doTakeItems(ContributionOfShari, -1);
                st.doGiveItems(ReceiptOfContribution1st, 1);
            ...
            if (st.getQuestItemsCount(ContributionOfShari) == 0) {
                st.doGiveItems(ContributionOfShari, 1);
            ...
            if (st.getQuestItemsCount(Lockirins2stNotice) > 0) {
                ...
                st.doTakeItems(Lockirins2stNotice, -1);
                ...
                st.doTakeItems(ContributionOfMaryse, -1);
                st.doTakeItems(ContributionOfMion, -1);
                st.doGiveItems(ReceiptOfContribution2st, 1);
            ...
            if (st.getQuestItemsCount(ContributionOfMion) == 0) {
                ...
                st.doGiveItems(ContributionOfMion, 1);
            } else
            ...
            if (st.getQuestItemsCount(MarysesRequest) == 0 && st.getQuestItemsCount(ContributionOfMaryse) == 0) {
                ...
                st.doGiveItems(MarysesRequest, 1);
                ...
                if (st.getQuestItemsCount(AnimalSkin) < 100)
                    ...
                    st.doTakeItems(AnimalSkin, 100);
                    st.doTakeItems(MarysesRequest, -1);
                    st.doGiveItems(ContributionOfMaryse, 1);
                }
            ...
            if (st.getQuestItemsCount(Lockirins3stNotice) > 0) {
                ...
                st.doTakeItems(Lockirins3stNotice, -1);
            ...
            if (st.getQuestItemsCount(Lockirins4stNotice) > 0) {
                ...
                st.doTakeItems(Lockirins4stNotice, -1);
                ...
                st.doTakeItems(ReceiptOfBolter, -1);
                st.doGiveItems(ReceiptOfContribution4st, 1);
            ...
            if (st.getQuestItemsCount(ReceiptOfBolter) == 0) {
                ...
                st.doGiveItems(ReceiptOfBolter, 1);
            } else
            ...
            if (st.getQuestItemsCount(Lockirins5stNotice) > 0) {
                ...
                st.doTakeItems(Lockirins5stNotice, -1);
                ...
                st.doTakeItems(ContributionOfToma, -1);
                st.doGiveItems(ReceiptOfContribution5st, 1);
            } else
            ...
            if (st.getQuestItemsCount(ContributionOfToma) == 0) {
                ...
                st.doGiveItems(ContributionOfToma, 1);
            } else
            ...
            if (cond == 1) {
                if (st.getQuestItemsCount(BrightsList) == 0 && st.getQuestItemsCount(EmilysRecipe) == 0 && st.getQuestItemsCount(MandragoraBouquet) == 0)
                    ...
                    st.doTakeItems(BrightsList, -1);
                    st.doTakeItems(MandragoraPetal, -1);
                    st.doTakeItems(CrimsonMoss, -1);
                    st.doGiveItems(MandragoraBouquet, 1);
            ...
            if (cond == 4)
                ...
                st.doTakeItems(PatternOfKeyhole, -1);
                st.doGiveItems(NikolasList, 1);
                st.doGiveItems(RecipeTitanKey, 1);
