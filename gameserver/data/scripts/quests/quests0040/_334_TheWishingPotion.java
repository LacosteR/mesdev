...
public class _334_TheWishingPotion extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30738-03.htm")) {
            ...
            if (checkIngr(st)) {
                ...
                st.doTakeItems(ALCHEMY_TEXT_ID, -1);
                st.doTakeItems(SECRET_BOOK_ID, -1);
                st.doTakeItems(POTION_RECIPE_1_ID, -1);
                st.doTakeItems(POTION_RECIPE_2_ID, -1);
                st.doTakeItems(AMBER_SCALE_ID, -1);
                st.doTakeItems(WIND_SOULSTONE_ID, -1);
                st.doTakeItems(GLASS_EYE_ID, -1);
                st.doTakeItems(HORROR_ECTOPLASM_ID, -1);
                st.doTakeItems(SILENOS_HORN_ID, -1);
                st.doTakeItems(ANT_SOLDIER_APHID_ID, -1);
                st.doTakeItems(TYRANTS_CHITIN_ID, -1);
                st.doTakeItems(BUGBEAR_BLOOD_ID, -1);
                if (st.getQuestItemsCount(MATILDS_ORB_ID) == 0)
                    st.doGiveItems(MATILDS_ORB_ID, 1);
                st.doGiveItems(WISH_POTION_ID, 1);
                st.setCond(0);
            } else
        ...
        //WISH 1
            ...
            if (st.getQuestItemsCount(WISH_POTION_ID) >= 1) {
                st.doTakeItems(WISH_POTION_ID, 1);
                if (Rnd.chance(50)) {
        ...
        //WISH 2
            ...
            if (st.getQuestItemsCount(WISH_POTION_ID) >= 1) {
                ...
                if (WISH_CHANCE <= 33) {
                    ...
                    st.doGiveItems(ADENA_ID, 10000);
                else if (Rnd.chance(2))
                    st.doGiveItems(ADENA_ID, ((Rnd.get(10) + 1) * 1000000L));
                else {
                    st.addSpawn(GRIMA);
                    st.addSpawn(GRIMA);
                    st.addSpawn(GRIMA);
        ...
        //WISH 3
            ...
            if (st.getQuestItemsCount(WISH_POTION_ID) >= 1) {
                ...
                int WISH_CHANCE = Rnd.get(100) + 1;
                if (WISH_CHANCE <= 33)
                    st.doGiveItems(CERTIFICATE_OF_ROYALTY_ID, 1);
                else if (WISH_CHANCE >= 66)
                    st.doGiveItems(ANCIENT_CROWN_ID, 1);
                else
                    st.addSpawn(SANCHES);
            } else
                htmltext = "30738-14.htm";
        ...
        //WISH 4
            ...
            if (st.getQuestItemsCount(WISH_POTION_ID) >= 1) {
                ...
                int WISH_CHANCE = Rnd.get(100) + 1;
                if (WISH_CHANCE <= 33)
                    st.doGiveItems(SPELLBOOK_ICEBOLT_ID, 1);
                else if (WISH_CHANCE <= 66)
                    st.doGiveItems(SPELLBOOK_BATTLEHEAL_ID, 1);
                else
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == ALCHEMIST_MATILD) {
            ...
            if (st.getQuestItemsCount(FORBIDDEN_LOVE_SCROLL_ID) >= 1) {
                st.doTakeItems(FORBIDDEN_LOVE_SCROLL_ID, 1);
                st.doGiveItems(ADENA_ID, RATE_QUESTS_DROP * 500000);
            ...
            if (DROP_CHANCE <= 20) {
                ...
                st.doGiveItems(HEART_OF_PAAGRIO_ID, 1);
                st.getPlayer().getTarget().decayMe();
                htmltext = "30743-06.htm";
            } else {
                st.getPlayer().getTarget().decayMe();
            ...
            if (DROP_CHANCE <= DROP_CHANCE_NECKLACE_OF_GRACE_ID)
                st.doGiveItems(NECKLACE_OF_GRACE_ID, 1);
            else
                st.doGiveItems(SCROLL_OF_ESCAPE_ID, 1);
            st.getPlayer().getTarget().decayMe();
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == SUCCUBUS_OF_SEDUCTION && DROP_CHANCE <= DROP_CHANCE_FORBIDDEN_LOVE_SCROLL_ID) {
            ...
            st.doGiveItems(FORBIDDEN_LOVE_SCROLL_ID, 1);
        } else if (npcId == GRIMA && DROP_CHANCE <= DROP_CHANCE_GOLD_BAR_ID) {
            st.playSound(SOUND_ITEMGET);
            st.doGiveItems(GOLD_BAR_ID, Rnd.get(5) + 1);
            ...
            if (DROP_CHANCE <= 25)
                st.doGiveItems(DEMONS_BOOTS_ID, 1);
            else if (DROP_CHANCE <= 50)
                st.doGiveItems(DEMONS_GLOVES_ID, 1);
            else if (DROP_CHANCE <= 75)
                st.doGiveItems(DEMONS_STOCKINGS_ID, 1);
            else
                st.doGiveItems(DEMONS_TUNIC_ID, 1);
            st.playSound(SOUND_ITEMGET);
        }
        return null;
    }
