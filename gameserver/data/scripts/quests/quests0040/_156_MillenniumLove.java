...
public class _156_MillenniumLove extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("30368-06.htm")) {
            st.doGiveItems(LILITHS_LETTER, 1);
            st.setCond(1);
            st.setState(STARTED);
            st.playSound(SOUND_ACCEPT);
        } else if (event.equals("156_1")) {
            st.doTakeItems(LILITHS_LETTER, -1);
            if (st.getQuestItemsCount(THEONS_DIARY) == 0) {
                st.doGiveItems(THEONS_DIARY, 1);
                st.setCond(2);
            }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30368) {
            if (cond == 0) {
                ...
                st.doTakeItems(THEONS_DIARY, -1);
                if (st.getPlayer().getPcClass().isMage())
                    st.doGiveItems(GR_COMP_PACKAGE_SPS, 1);
                else
                    st.doGiveItems(GR_COMP_PACKAGE_SS, 1);
                st.doRewardItems(ADENA_ID, 3000);
