...
public class _325_GrimCollector extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("guard_curtiz_q0325_03.htm")) {
            ...
            st.doGiveItems(ANATOMY_DIAGRAM, 1);
        else if (event.equalsIgnoreCase("samed_q0325_06.htm")) {
            if (pieces(st) > 0) {
                st.doGiveItems(ADENA_ID, 30 * st.getQuestItemsCount(ZOMBIE_HEAD) + 20 * st.getQuestItemsCount(ZOMBIE_HEART) + 20 * st.getQuestItemsCount(ZOMBIE_LIVER) + 50 * st.getQuestItemsCount(SKULL) + 15 * st.getQuestItemsCount(RIB_BONE) + 10 * st.getQuestItemsCount(SPINE) + 10 * st.getQuestItemsCount(ARM_BONE) + 10 * st.getQuestItemsCount(THIGH_BONE) + 2000 * st.getQuestItemsCount(COMPLETE_SKELETON));
                st.doTakeItems(ZOMBIE_HEAD, -1);
                st.doTakeItems(ZOMBIE_HEART, -1);
                st.doTakeItems(ZOMBIE_LIVER, -1);
                st.doTakeItems(SKULL, -1);
                st.doTakeItems(RIB_BONE, -1);
                st.doTakeItems(SPINE, -1);
                st.doTakeItems(ARM_BONE, -1);
                st.doTakeItems(THIGH_BONE, -1);
                st.doTakeItems(COMPLETE_SKELETON, -1);
            }
            st.doTakeItems(ANATOMY_DIAGRAM, -1);
            ...
            st.doGiveItems(ADENA_ID, 2000 * st.getQuestItemsCount(COMPLETE_SKELETON));
            st.doTakeItems(COMPLETE_SKELETON, -1);
            ...
            if (st.getQuestItemsCount(SPINE) != 0 && st.getQuestItemsCount(ARM_BONE) != 0 && st.getQuestItemsCount(SKULL) != 0 && st.getQuestItemsCount(RIB_BONE) != 0 && st.getQuestItemsCount(THIGH_BONE) != 0) {
                ...
                st.doTakeItems(THIGH_BONE, 1);
                if (Rnd.chance(80))
                    st.doGiveItems(COMPLETE_SKELETON, 1);
                else
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 20026) {
            if (n < 90) {
                ...
                if (n < 40)
                    st.doGiveItems(ZOMBIE_HEAD, 1);
                else if (n < 60)
                    st.doGiveItems(ZOMBIE_HEART, 1);
                else
                    st.doGiveItems(ZOMBIE_LIVER, 1);
            }
        } else if (npcId == 20029) {
            st.playSound(SOUND_ITEMGET);
            if (n < 44)
            ...
            if (n < 79) {
                st.playSound(SOUND_ITEMGET);
                if (n < 5)
                    st.doGiveItems(SKULL, 1);
                else if (n < 15)
                    st.doGiveItems(RIB_BONE, 1);
                else if (n < 29)
                    st.doGiveItems(SPINE, 1);
                else
                    st.doGiveItems(THIGH_BONE, 1);
            }
        } else if (npcId == 20042) {
            if (n < 86) {
                st.playSound(SOUND_ITEMGET);
