...
public class _417_PathToScavenger extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("1")) {
            st.set("id", "0");
            if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x35 && st.getQuestItemsCount(RING_OF_RAVEN) == 0) {
                st.setCond(1);
                st.setState(STARTED);
                st.playSound(SOUND_ACCEPT);
                st.doGiveItems(PIPIS_LETTER, 1);
                htmltext = "collector_pipi_q0417_05.htm";
            } else if (st.getPlayer().getPcClass().class_id != 0x35) {
                if (st.getPlayer().getPcClass().class_id == 0x36)
                    htmltext = "collector_pipi_q0417_02a.htm";
                else
                    htmltext = "collector_pipi_q0417_08.htm";
            } else if (st.getPlayer().getLevel() < 18 && st.getPlayer().getPcClass().class_id == 0x35)
                htmltext = "collector_pipi_q0417_02.htm";
            else if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x35 && st.getQuestItemsCount(RING_OF_RAVEN) >= 1)
                htmltext = "collector_pipi_q0417_04.htm";
        } else if (event.equals("30519_1")) {
            if (st.getQuestItemsCount(PIPIS_LETTER) > 0) {
                st.doTakeItems(PIPIS_LETTER, 1);
                st.setCond(2);
                int n = Rnd.get(3);
                if (n == 0) {
                    htmltext = "trader_mion_q0417_02.htm";
                    st.doGiveItems(ZIMENFS_POTION, 1);
                } else if (n == 1) {
                    htmltext = "trader_mion_q0417_03.htm";
                    st.doGiveItems(CHARIS_AXE, 1);
                } else if (n == 2) {
                    htmltext = "trader_mion_q0417_04.htm";
                    st.doGiveItems(BRONKS_INGOT, 1);
                }
            } else
                htmltext = "noquest";
            ...
            if (st.getQuestItemsCount(ZIMENFS_POTION, CHARIS_AXE, BRONKS_INGOT) > 0) {
                if (st.getInt("id") / 10 < 2) {
                    ...
                    st.doGiveItems(MIONS_LETTER, 1);
                    st.doTakeItems(CHARIS_AXE, 1);
                    st.doTakeItems(ZIMENFS_POTION, 1);
                    st.doTakeItems(BRONKS_INGOT, 1);
                }
            } else
                htmltext = "noquest";
            ...
            if (st.getQuestItemsCount(ZIMENFS_PAY) > 0 || st.getQuestItemsCount(CHALIS_PAY) > 0 || st.getQuestItemsCount(BRONKS_PAY) > 0) {
                int n = Rnd.get(3);
                st.doTakeItems(ZIMENFS_PAY, 1);
                st.doTakeItems(CHALIS_PAY, 1);
                st.doTakeItems(BRONKS_PAY, 1);
                if (n == 0) {
                    htmltext = "trader_mion_q0417_02.htm";
                    st.doGiveItems(ZIMENFS_POTION, 1);
                } else if (n == 1) {
            ...
            if (st.getQuestItemsCount(BEAD_PARCEL) > 0) {
                ...
                st.doTakeItems(BEAD_PARCEL, 1);
                st.doGiveItems(ROUTS_TP_SCROLL, 1);
                st.setCond(10);
            } else
                htmltext = "noquest";
            ...
            if (st.getQuestItemsCount(ROUTS_TP_SCROLL) > 0) {
                ...
                st.doTakeItems(ROUTS_TP_SCROLL, 1);
                st.doGiveItems(SUCCUBUS_UNDIES, 1);
                st.setCond(11);
            } else
                htmltext = "noquest";
        return htmltext;
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Pippi) {
            ...
            if (st.getQuestItemsCount(MIONS_LETTER) >= 1) {
                ...
                st.doTakeItems(MIONS_LETTER, -1);
                st.doGiveItems(BEAR_PIC, 1);
                st.setCond(5);
                ...
                st.doTakeItems(HONEY_JAR, st.getQuestItemsCount(HONEY_JAR));
                st.doTakeItems(BEAR_PIC, 1);
                st.doGiveItems(TARANTULA_PIC, 1);
                st.setCond(7);
                ...
                st.doTakeItems(BEAD, st.getQuestItemsCount(BEAD));
                st.doTakeItems(TARANTULA_PIC, 1);
                st.doGiveItems(BEAD_PARCEL, 1);
                st.setCond(9);
            ...
            if (st.getQuestItemsCount(BEAD_PARCEL) >= 1)
                ...
                st.doTakeItems(SUCCUBUS_UNDIES, 1);
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(RING_OF_RAVEN, 1);
                    if (!st.getPlayer().getVarB("prof1")) {
                        st.getPlayer().setVar("prof1", "1", -1);
                        st.addExpAndSp(228064, 16455);
                        st.doGiveItems(ADENA_ID, 163800);
                    }
                }
