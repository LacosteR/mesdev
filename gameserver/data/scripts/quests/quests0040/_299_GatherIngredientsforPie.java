...
public class _299_GatherIngredientsforPie extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("emilly_q0299_0104.htm") && _state == CREATED) {
            ...
            st.doTakeItems(Fruit_Basket, -1);
            if (Rnd.chance(Reward_Varnish_Chance))
                st.doGiveItems(Varnish, 50);
            else
                st.doGiveItems(ADENA_ID, RATE_QUESTS_REWARD * 25000);
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        int npcId = npc.getNpcId();
        if (npcId == Wasp_Worker && qs.doGiveOnKillItems(Honey_Pouch, 1, 1, 100, Wasp_Worker_Chance) || npcId == Wasp_Leader && qs.doGiveOnKillItems(Honey_Pouch, 1, 1, 100, Wasp_Leader_Chance))
            qs.setCond(2);
