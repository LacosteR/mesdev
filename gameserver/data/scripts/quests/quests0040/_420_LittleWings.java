...
public class _420_LittleWings extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30829-02.htm") && _state == CREATED) {
            ...
            st.setCond(2);
            st.doTakeItems(Fairy_Stone, -1);
            st.doTakeItems(Deluxe_Fairy_Stone, -1);
            st.doTakeItems(Fairy_Stone_List, -1);
            st.doTakeItems(Deluxe_Fairy_Stone_List, -1);
            st.doGiveItems(Fairy_Stone_List, 1);
            ...
            st.doTakeItems(Fairy_Stone_List, -1);
            st.doTakeItems(Deluxe_Fairy_Stone_List, -1);
            st.doGiveItems(Deluxe_Fairy_Stone_List, 1);
            st.playSound(SOUND_MIDDLE);
            ...
            if (CheckFairyStoneItems(st, Delux_Fairy_Stone_Items)) {
                ...
                st.doGiveItems(Deluxe_Fairy_Stone, 1);
                st.playSound(SOUND_MIDDLE);
            } else
            ...
            if (egg_id == 0)
                return "noquest";
            st.doTakeItems(egg_id, -1);
            st.doGiveItems(Rnd.get(Dragonflute_of_Wind, Dragonflute_of_Twilight), 1);
            ...
            if (Rnd.chance(Pet_Armor_Chance)) {
                ...
                st.doGiveItems(armor_id, 1);
            } else
                st.doGiveItems(Food_For_Hatchling, 20);
        }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId >= Exarion && npcId <= Shamhai) {
            ...
            if (cond == 6 && st.getQuestItemsCount(getWyrmScale(npcId)) > 0) {
                ...
                st.doTakeItems(getWyrmScale(npcId), -1);
                st.doTakeItems(egg_id, -1);
                st.doGiveItems(egg_id, 1);
                st.setCond(7);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 2 && npcId == Toad_Lord) {
            ...
            if (st.getQuestItemsCount(Toad_Lord_Back_Skin) < needed_skins && Rnd.chance(Toad_Lord_Back_Skin_Chance)) {
                st.doGiveItems(Toad_Lord_Back_Skin, 1);
        ...
        if (cond == 6) {
            ...
            if (wyrm_id > 0 && st.getQuestItemsCount(getWyrmScale(wyrm_id)) > 0 && st.getQuestItemsCount(getWyrmEgg(wyrm_id)) < 20 && Rnd.chance(Egg_Chance)) {
                st.doGiveItems(getWyrmEgg(wyrm_id), 1);
    ...
    public static void TakeFairyStoneItems(QuestState st, int[][] item_list) {
        for (int[] _item : item_list)
            st.doTakeItems(_item[0], _item[1]);
    }
}