...
public class _217_TestimonyOfTrust extends Quest implements ScriptFile {
    ...
    private static final int RewardExp = (int) (RATE_QUESTS_REWARD * 1390298);
    private static final int RewardSP = (int) (RATE_QUESTS_REWARD * 92782);
    private static final int RewardAdena = (int) (RATE_QUESTS_REWARD * 252212);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            if (!st.getPlayer().getVarB("dd2")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 96);
            ...
            st.doGiveItems(LETTER_TO_ELF_ID, 1);
            st.doGiveItems(LETTER_TO_DARKELF_ID, 1);
            ...
            st.doTakeItems(LETTER_TO_ELF_ID, 1);
            st.doGiveItems(ORDER_OF_OZZY_ID, 1);
            st.setCond(2);
        } else if (event.equalsIgnoreCase("30358_1")) {
            htmltext = "tetrarch_thifiell_q0217_02.htm";
            st.doTakeItems(LETTER_TO_DARKELF_ID, 1);
            st.doGiveItems(LETTER_OF_THIFIELL_ID, 1);
            st.setCond(5);
        } else if (event.equalsIgnoreCase("30657_1"))
            if (st.getPlayer().getLevel() >= 38) {
                htmltext = "cardinal_seresin_q0217_03.htm";
                st.doTakeItems(LETTER_TO_SERESIN_ID, 1);
                st.doGiveItems(LETTER_TO_ORC_ID, 1);
                st.doGiveItems(LETTER_TO_DWARF_ID, 1);
                st.setCond(12);
            } else
            ...
            st.doTakeItems(LETTER_TO_ORC_ID, 1);
            st.doGiveItems(LETTER_TO_MANAKIA_ID, 1);
            st.setCond(13);
        } else if (event.equalsIgnoreCase("30515_1")) {
            htmltext = "seer_manakia_q0217_02.htm";
            st.doTakeItems(LETTER_TO_MANAKIA_ID, 1);
            st.setCond(14);
        } else if (event.equalsIgnoreCase("30531_1")) {
            htmltext = "first_elder_lockirin_q0217_02.htm";
            st.doTakeItems(LETTER_TO_DWARF_ID, 1);
            st.doGiveItems(LETTER_TO_NICHOLA_ID, 1);
            st.setCond(18);
        } else if (event.equalsIgnoreCase("30621_1")) {
            htmltext = "maestro_nikola_q0217_02.htm";
            st.doTakeItems(LETTER_TO_NICHOLA_ID, 1);
            st.doGiveItems(ORDER_OF_NICHOLA_ID, 1);
            st.setCond(19);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30191) {
            if (cond == 0) {
                ...
                st.doTakeItems(SCROLL_OF_DARKELF_TRUST_ID, 1);
                st.doTakeItems(SCROLL_OF_ELF_TRUST_ID, 1);
                st.doGiveItems(LETTER_TO_SERESIN_ID, 1);
                st.setCond(10);
                ...
                st.doTakeItems(SCROLL_OF_DWARF_TRUST_ID, 1);
                st.doTakeItems(SCROLL_OF_ORC_TRUST_ID, 1);
                st.doGiveItems(RECOMMENDATION_OF_HOLLIN_ID, 1);
                st.setCond(23);
            } else if (cond == 19)
            ...
            if (cond == 1 && st.getQuestItemsCount(LETTER_TO_ELF_ID) > 0)
                ...
                st.doTakeItems(BREATH_OF_WINDS_ID, 1);
                st.doTakeItems(SEED_OF_VERDURE_ID, 1);
                st.doTakeItems(ORDER_OF_OZZY_ID, 1);
                st.doGiveItems(SCROLL_OF_ELF_TRUST_ID, 1);
                st.setCond(4);
            } else if (cond == 4)
            ...
            if (cond == 4 && st.getQuestItemsCount(LETTER_TO_DARKELF_ID) > 0)
                ...
                st.doTakeItems(BASILISK_PLASMA_ID, 1);
                st.doTakeItems(STAKATO_ICHOR_ID, 1);
                st.doTakeItems(HONEY_DEW_ID, 1);
                st.doGiveItems(SCROLL_OF_DARKELF_TRUST_ID, 1);
                st.setCond(9);
            ...
            if (cond == 5 && st.getQuestItemsCount(LETTER_OF_THIFIELL_ID) > 0) {
                ...
                st.doTakeItems(LETTER_OF_THIFIELL_ID, 1);
                st.doGiveItems(ORDER_OF_CLAYTON_ID, 1);
                st.setCond(6);
                ...
                st.doTakeItems(ORDER_OF_CLAYTON_ID, 1);
                st.setCond(8);
            ...
            if (cond == 12 && st.getQuestItemsCount(LETTER_TO_ORC_ID) > 0)
                ...
                st.doTakeItems(LETTER_OF_MANAKIA_ID, 1);
                st.doGiveItems(SCROLL_OF_ORC_TRUST_ID, 1);
                st.setCond(17);
            } else if (cond >= 17)
            ...
            if (cond == 13 && st.getQuestItemsCount(LETTER_TO_MANAKIA_ID) > 0)
                ...
                st.doTakeItems(PARASITE_OF_LOTA_ID, -1);
                st.doGiveItems(LETTER_OF_MANAKIA_ID, 1);
                st.setCond(16);
            } else if (cond == 16)
            ...
            if (cond == 17 && st.getQuestItemsCount(LETTER_TO_DWARF_ID) > 0)
                ...
                st.doGiveItems(SCROLL_OF_DWARF_TRUST_ID, 1);
                st.setCond(22);
            } else if (cond == 22)
            ...
            if (cond == 18 && st.getQuestItemsCount(LETTER_TO_NICHOLA_ID) > 0)
                ...
                st.doTakeItems(HEART_OF_PORTA_ID, -1);
                st.doTakeItems(ORDER_OF_NICHOLA_ID, 1);
                st.setCond(21);
            } else if (cond == 21)
            ...
            st.doTakeItems(RECOMMENDATION_OF_HOLLIN_ID, -1);
            st.doGiveItems(MARK_OF_TRUST_ID, 1);
        ...
        if (npcId == 20036 || npcId == 20044) {
            ...
            if (cond == 2 && st.getQuestItemsCount(BREATH_OF_WINDS_ID) == 0)
                if (st.getQuestItemsCount(SEED_OF_VERDURE_ID) > 0) {
                    st.doGiveItems(BREATH_OF_WINDS_ID, 1);
                    st.setCond(3);
            ...
            if (cond == 2 && st.getQuestItemsCount(SEED_OF_VERDURE_ID) == 0)
                if (st.getQuestItemsCount(BREATH_OF_WINDS_ID) > 0) {
                    st.doGiveItems(SEED_OF_VERDURE_ID, 1);
                    st.setCond(3);
                    st.playSound(SOUND_MIDDLE);
                } else {
                    st.doGiveItems(SEED_OF_VERDURE_ID, 1);
            ...
            if (cond == 6 && st.getQuestItemsCount(BLOOD_OF_GUARDIAN_BASILISK_ID) < 10 && st.getQuestItemsCount(ORDER_OF_CLAYTON_ID) > 0 && st.getQuestItemsCount(BASILISK_PLASMA_ID) == 0)
                if (st.getQuestItemsCount(BLOOD_OF_GUARDIAN_BASILISK_ID) > 9) {
                    st.doTakeItems(BLOOD_OF_GUARDIAN_BASILISK_ID, -1);
                    st.doGiveItems(BASILISK_PLASMA_ID, 1);
                    if (st.getQuestItemsCount(STAKATO_ICHOR_ID) + st.getQuestItemsCount(BASILISK_PLASMA_ID) + st.getQuestItemsCount(HONEY_DEW_ID) >= 3)
                        st.setCond(7);
                    st.playSound(SOUND_MIDDLE);
                } else {
                    st.doGiveItems(BLOOD_OF_GUARDIAN_BASILISK_ID, 1);
                    st.playSound(SOUND_ITEMGET);
                }
            ...
            if (cond == 6 && st.getQuestItemsCount(STAKATOS_FLUIDS_ID) < 10 && st.getQuestItemsCount(ORDER_OF_CLAYTON_ID) > 0 && st.getQuestItemsCount(STAKATO_ICHOR_ID) == 0)
                if (st.getQuestItemsCount(STAKATOS_FLUIDS_ID) > 9) {
                    st.doTakeItems(STAKATOS_FLUIDS_ID, -1);
                    st.doGiveItems(STAKATO_ICHOR_ID, 1);
                    if (st.getQuestItemsCount(STAKATO_ICHOR_ID) + st.getQuestItemsCount(BASILISK_PLASMA_ID) + st.getQuestItemsCount(HONEY_DEW_ID) >= 3)
                        st.setCond(7);
                    st.playSound(SOUND_MIDDLE);
                } else {
                    st.doGiveItems(STAKATOS_FLUIDS_ID, 1);
                    st.playSound(SOUND_ITEMGET);
                }
            ...
            if (cond == 6 && st.getQuestItemsCount(GIANT_APHID_ID) < 10 && st.getQuestItemsCount(ORDER_OF_CLAYTON_ID) > 0 && st.getQuestItemsCount(HONEY_DEW_ID) == 0)
                if (st.getQuestItemsCount(GIANT_APHID_ID) > 9) {
                    st.doTakeItems(GIANT_APHID_ID, -1);
                    st.doGiveItems(HONEY_DEW_ID, 1);
                    if (st.getQuestItemsCount(STAKATO_ICHOR_ID) + st.getQuestItemsCount(BASILISK_PLASMA_ID) + st.getQuestItemsCount(HONEY_DEW_ID) >= 3)
                        st.setCond(7);
                    st.playSound(SOUND_MIDDLE);
                } else {
                    st.doGiveItems(GIANT_APHID_ID, 1);
                    st.playSound(SOUND_ITEMGET);
                }
        } else if (npcId == 20553) {
            if (cond == 14 && st.doGiveOnKillItems(PARASITE_OF_LOTA_ID, 1, 1, 10, 50))
                st.setCond(15);
        } else if (npcId == 20213 && cond == 19 && st.getQuestItemsCount(HEART_OF_PORTA_ID) < 1) {
            st.doGiveItems(HEART_OF_PORTA_ID, 1);
            st.setCond(20);
            st.playSound(SOUND_MIDDLE);
        }
        return null;
