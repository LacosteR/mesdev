...
public class _291_RevengeOfTheRedbonnet extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            st.doTakeItems(BlackWolfPelt, -1);
            if (random < 3)
                st.doGiveItems(GrandmasPearl, 1);
            else if (random < 21)
                st.doGiveItems(GrandmasMirror, 1);
            else if (random < 46)
                st.doGiveItems(GrandmasNecklace, 1);
            else {
                st.doGiveItems(ScrollOfEscape, 1);
                st.doGiveItems(GrandmasHairpin, 1);
            }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1 && st.getQuestItemsCount(BlackWolfPelt) < 40) {
            st.doGiveItems(BlackWolfPelt, 1);
