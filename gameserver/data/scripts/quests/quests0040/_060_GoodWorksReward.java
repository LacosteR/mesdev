...
import l2p.technical.text.PrintfFormat;
...
public class _060_GoodWorksReward extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("31435-03.htm") && _state == CREATED) {
            ...
            st.doTakeItems(Bloody_Cloth_Fragment, -1);
            st.setCond(5);
            st.playSound(SOUND_MIDDLE);
            ...
            if (profs.containsKey(st.getPlayer().getPcClass().class_id))
                return Mammon_dialog.sprintf(profs.get(st.getPlayer().getPcClass().class_id));
        } else if (event.equalsIgnoreCase("31092-05.htm") && cond == 10) {
            st.doRewardItems(ADENA_ID, 3000000);
            ...
            for (int mark : _marks)
                if (st.getQuestItemsCount(mark) > 0)
                    ...
                    st.doGiveItems(mark, 1);
            if (return_adena > 0)
                st.doRewardItems(ADENA_ID, return_adena);
            st.playSound(SOUND_FINISH);
            st.exitCurrentQuest(false);
            return "31092-05.htm";
        }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (_state == CREATED) {
            if (npcId != Daeger)
                return "noquest";
            if (st.getPlayer().getLevel() < 39 || st.getPlayer().getRace() == Race.kamael || st.getPlayer().getPcClass().getLevel() != 1) {
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        if (qs.getState() == STARTED && qs.getCond() == 1) {
            ...
            qs.setCond(2);
            qs.doGiveOnKillItems(Bloody_Cloth_Fragment, 1, 1, 1, 100);
        }
        return null;
    }
