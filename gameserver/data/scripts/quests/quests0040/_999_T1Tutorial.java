    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("TimerEx_NewbieHelper")) {
            ...
            if (st.getQuestItemsCount(e.item) > 0 && st.getInt("onlyone") == 0) {
                ...
                st.doTakeItems(e.item, 1);
                if (Ex <= 3)
                    qs.set("Ex", "4");
                if (classId == e.classId1) {
                    st.doGiveItems(e.gift1, e.count1);
                    ...
                    if (e.gift2 != 0) {
                        st.doGiveItems(e.gift2, e.count2);
                        st.playTutorialVoice("tutorial_voice_026");
                    }
                }
                st.set("step", "3");
            ...
            if (t.npcTyp == 1) {
                if (step == 0 && Ex < 0) {
                    ...
                    if (st.getQuestItemsCount(BLUE_GEM) > 0) {
                        st.doTakeItems(BLUE_GEM, st.getQuestItemsCount(BLUE_GEM));
                        st.doGiveItems(t.item, 1);
                        st.set("step", "2");
                        qs.set("Ex", "3");
                        st.startQuestTimer("TimerEx_NewbieHelper", 30000);
                        qs.set("ucMemo", "3");
                        if (isMage) {
                            st.playTutorialVoice("tutorial_voice_027");
                            st.doGiveItems(SPIRITSHOT_NOVICE, 100);
                            ...
                            st.playTutorialVoice("tutorial_voice_026");
                            st.doGiveItems(SOULSHOT_NOVICE, 200);
