...
public class _411_PathToAssassin extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x1f && st.getQuestItemsCount(IRON_HEART_ID) < 1) {
                st.setCond(1);
                st.setState(STARTED);
                st.playSound(SOUND_ACCEPT);
                st.doGiveItems(SHILENS_CALL_ID, 1);
                htmltext = "triskel_q0411_05.htm";
            } else if (st.getPlayer().getPcClass().class_id != 0x1f) {
                if (st.getPlayer().getPcClass().class_id == 0x23)
                ...
                st.exitCurrentQuest(true);
            } else if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x1f && st.getQuestItemsCount(IRON_HEART_ID) > 0)
                htmltext = "triskel_q0411_04.htm";
        } else if (event.equalsIgnoreCase("30419_1")) {
            htmltext = "arkenia_q0411_05.htm";
            st.doTakeItems(SHILENS_CALL_ID, -1);
            st.doGiveItems(ARKENIAS_LETTER_ID, 1);
            st.setCond(2);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("30382_1")) {
            htmltext = "guard_leikan_q0411_03.htm";
            st.doTakeItems(ARKENIAS_LETTER_ID, -1);
            st.doGiveItems(LEIKANS_NOTE_ID, 1);
            st.setCond(3);
            st.playSound(SOUND_MIDDLE);
        }
        return htmltext;
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == TRISKEL) {
            if (cond < 1) {
                ...
                st.doTakeItems(ARKENIA_RECOMMEND_ID, -1);
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(IRON_HEART_ID, 1);
            ...
            if (cond == 1 && st.getQuestItemsCount(SHILENS_CALL_ID) > 0)
                ...
                st.doTakeItems(SHILENS_TEARS_ID, -1);
                st.doTakeItems(LEIKANS_KNIFE_ID, -1);
                st.doGiveItems(ARKENIA_RECOMMEND_ID, 1);
                st.setCond(7);
                st.playSound(SOUND_MIDDLE);
            } else if (cond == 7)
            ...
            if (cond == 2 && st.getQuestItemsCount(ARKENIAS_LETTER_ID) > 0)
                ...
                st.doTakeItems(ONYX_BEASTS_MOLAR_ID, -1);
                st.doTakeItems(LEIKANS_NOTE_ID, -1);
                st.doGiveItems(LEIKANS_KNIFE_ID, 1);
                st.setCond(5);
                st.playSound(SOUND_MIDDLE);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == CALPICO) {
            if (cond == 5 && st.getQuestItemsCount(LEIKANS_KNIFE_ID) > 0 && st.getQuestItemsCount(SHILENS_TEARS_ID) < 1) {
                st.doGiveItems(SHILENS_TEARS_ID, 1);
                st.playSound(SOUND_MIDDLE);
                st.setCond(6);
            }
            ...
            if (cond == 3 && st.getQuestItemsCount(LEIKANS_NOTE_ID) > 0 && st.getQuestItemsCount(ONYX_BEASTS_MOLAR_ID) < 10) {
                st.doGiveItems(ONYX_BEASTS_MOLAR_ID, 1);
