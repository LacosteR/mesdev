...
public class _404_PathToWizard extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            if (st.getPlayer().getPcClass().class_id == 0x0a) {
            ...
            if (st.getQuestItemsCount(WIND_FEATHER_ID) < 1) {
                ...
                st.doGiveItems(WIND_FEATHER_ID, 1);
                st.setCond(6);
            }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == PARINA) {
            if (cond == 0)
                ...
                st.doTakeItems(FLAME_EARING_ID, st.getQuestItemsCount(FLAME_EARING_ID));
                st.doTakeItems(WIND_BANGEL_ID, st.getQuestItemsCount(WIND_BANGEL_ID));
                st.doTakeItems(WATER_NECKLACE_ID, st.getQuestItemsCount(WATER_NECKLACE_ID));
                st.doTakeItems(EARTH_RING_ID, st.getQuestItemsCount(EARTH_RING_ID));
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    if (st.getQuestItemsCount(BEAD_OF_SEASON_ID) < 1)
                        st.doGiveItems(BEAD_OF_SEASON_ID, 1);
            ...
            if (cond > 0 && st.getQuestItemsCount(MAP_OF_LUSTER_ID) < 1 && st.getQuestItemsCount(FLAME_EARING_ID) < 1) {
                st.doGiveItems(MAP_OF_LUSTER_ID, 1);
                ...
                st.doTakeItems(KEY_OF_FLAME_ID, -1);
                st.doTakeItems(MAP_OF_LUSTER_ID, -1);
                if (st.getQuestItemsCount(FLAME_EARING_ID) < 1)
                    st.doGiveItems(FLAME_EARING_ID, 1);
            ...
            if (cond == 4 && st.getQuestItemsCount(FLAME_EARING_ID) > 0 && st.getQuestItemsCount(BROKEN_BRONZE_MIRROR_ID) < 1 && st.getQuestItemsCount(WIND_BANGEL_ID) < 1) {
                st.doGiveItems(BROKEN_BRONZE_MIRROR_ID, 1);
                ...
                st.doTakeItems(WIND_FEATHER_ID, st.getQuestItemsCount(WIND_FEATHER_ID));
                st.doTakeItems(BROKEN_BRONZE_MIRROR_ID, st.getQuestItemsCount(BROKEN_BRONZE_MIRROR_ID));
                if (st.getQuestItemsCount(WIND_BANGEL_ID) < 1)
                    st.doGiveItems(WIND_BANGEL_ID, 1);
            ...
            if (cond == 7 && st.getQuestItemsCount(WIND_BANGEL_ID) > 0 && st.getQuestItemsCount(RAMAS_DIARY_ID) < 1 && st.getQuestItemsCount(WATER_NECKLACE_ID) < 1) {
                st.doGiveItems(RAMAS_DIARY_ID, 1);
                ...
                st.doTakeItems(SPARKLE_PEBBLE_ID, -1);
                st.doTakeItems(RAMAS_DIARY_ID, -1);
                if (st.getQuestItemsCount(WATER_NECKLACE_ID) < 1)
                    st.doGiveItems(WATER_NECKLACE_ID, 1);
            ...
            if (cond > 0 && st.getQuestItemsCount(WATER_NECKLACE_ID) > 0 && st.getQuestItemsCount(RUST_GOLD_COIN_ID) < 1 && st.getQuestItemsCount(EARTH_RING_ID) < 1) {
                st.doGiveItems(RUST_GOLD_COIN_ID, 1);
                ...
                st.doTakeItems(RED_SOIL_ID, st.getQuestItemsCount(RED_SOIL_ID));
                st.doTakeItems(RUST_GOLD_COIN_ID, st.getQuestItemsCount(RUST_GOLD_COIN_ID));
                if (st.getQuestItemsCount(EARTH_RING_ID) < 1)
                    st.doGiveItems(EARTH_RING_ID, 1);
