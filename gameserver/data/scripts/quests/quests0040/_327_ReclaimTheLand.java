...
public class _327_ReclaimTheLand extends Quest implements ScriptFile {
    ...
    private static boolean ExpReward(QuestState st, int item_id) {
        ...
        if (exp_reward == 0)
            return false;
        st.doTakeItems(item_id, -1);
        st.addExpAndSp(RATE_QUESTS_REWARD * exp_reward, 0);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("piotur_q0327_03.htm") && _state == CREATED) {
            ...
            st.doTakeItems(CLAY_URN_FRAGMENT, 5);
            if (!Rnd.chance(Exchange_Chance))
                return "trader_acellopy_q0327_10.htm";
            st.doGiveItems(ANCIENT_CLAY_URN, 1);
            st.playSound(SOUND_MIDDLE);
            ...
            st.doTakeItems(BRASS_TRINKET_PIECE, 5);
            if (!Rnd.chance(Exchange_Chance))
                return "trader_acellopy_q0327_10.htm";
            st.doGiveItems(ANCIENT_BRASS_TIARA, 1);
            st.playSound(SOUND_MIDDLE);
            ...
            st.doTakeItems(BRONZE_MIRROR_PIECE, 5);
            if (!Rnd.chance(Exchange_Chance))
                return "trader_acellopy_q0327_10.htm";
            st.doGiveItems(ANCIENT_BRONZE_MIRROR, 1);
            st.playSound(SOUND_MIDDLE);
            ...
            st.doTakeItems(JADE_NECKLACE_BEAD, 5);
            if (!Rnd.chance(Exchange_Chance))
                return "trader_acellopy_q0327_09.htm";
            st.doGiveItems(ANCIENT_JADE_NECKLACE, 1);
            st.playSound(SOUND_MIDDLE);
            return "trader_acellopy_q0327_07.htm";
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Piotur) {
            ...
            st.doTakeItems(TUREK_DOGTAG, -1);
            st.doTakeItems(TUREK_MEDALLION, -1);
            st.doGiveItems(ADENA_ID, reward);
            st.playSound(SOUND_MIDDLE);
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        if (Rnd.chance(_drop.chance)) {
            int n = Rnd.get(100);
            if (n < 25)
                qs.doGiveItems(CLAY_URN_FRAGMENT, 1);
            else if (n < 50)
                qs.doGiveItems(BRASS_TRINKET_PIECE, 1);
            else if (n < 75)
                qs.doGiveItems(BRONZE_MIRROR_PIECE, 1);
            else
                qs.doGiveItems(JADE_NECKLACE_BEAD, 1);
        }
        qs.doGiveItems(_drop.itemList[0], 1);
