...
public class _402_PathToKnight extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        Integer classid = st.getPlayer().getPcClass().class_id;
        ...
        if (event.equalsIgnoreCase("sir_karrel_vasper_q0402_02a.htm")) {
            ...
            if (squire > 0 && coin1 + coin2 + coin3 + coin4 + coin5 + coin6 >= 3) {
                for (int i = 1162; i < 1179; i++)
                    st.doTakeItems(i, -1);
                st.doTakeItems(MARK_OF_ESQUIRE, -1);
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(SWORD_OF_RITUAL, 1);
        ...
        if (npcId == SIR_KLAUS_VASPER) {
            ...
            if (guards_mark1 < 1 && coin1 < 1)
                ...
                if (st.getQuestItemsCount(BUGBEAR_NECKLACE) < 10)
                    ...
                    st.doTakeItems(BUGBEAR_NECKLACE, -1);
                    st.doTakeItems(GLUDIO_GUARDS_MARK1, 1);
                    st.doGiveItems(COIN_OF_LORDS1, 1);
            ...
            if (church_mark1 < 1 && coin2 < 1)
                ...
                if (st.getQuestItemsCount(EINHASAD_CRUCIFIX) < 12)
                    ...
                    st.doTakeItems(EINHASAD_CRUCIFIX, -1);
                    st.doTakeItems(EINHASAD_CHURCH_MARK1, 1);
                    st.doGiveItems(COIN_OF_LORDS2, 1);
                    st.playSound(SOUND_MIDDLE);
                }
            } else if (coin2 > 0)
            ...
            if (coin3 < 1 && guards_mark2 < 1)
                ...
                if (st.getQuestItemsCount(POISON_SPIDER_LEG1) < 20)
                    ...
                    st.doTakeItems(POISON_SPIDER_LEG1, -1);
                    st.doTakeItems(GLUDIO_GUARDS_MARK2, 1);
                    st.doGiveItems(COIN_OF_LORDS3, 1);
                    st.playSound(SOUND_MIDDLE);
                }
            } else if (coin3 > 0)
            ...
            if (coin4 < 1 && church_mark2 < 1)
                ...
                if (st.getQuestItemsCount(LIZARDMAN_TOTEM) < 20)
                    ...
                    st.doTakeItems(LIZARDMAN_TOTEM, -1);
                    st.doTakeItems(EINHASAD_CHURCH_MARK2, 1);
                    st.doGiveItems(COIN_OF_LORDS4, 1);
                    st.playSound(SOUND_MIDDLE);
                }
            } else if (coin4 > 0)
            ...
            if (guards_mark3 < 1 && coin5 < 1)
                ...
                if (st.getQuestItemsCount(GIANT_SPIDER_HUSK) < 20)
                    ...
                    st.doTakeItems(GIANT_SPIDER_HUSK, -1);
                    st.doTakeItems(GLUDIO_GUARDS_MARK3, 1);
                    st.doGiveItems(COIN_OF_LORDS5, 1);
                    st.playSound(SOUND_MIDDLE);
                }
            } else if (coin5 > 0)
            ...
            if (church_mark3 < 1 && coin6 < 1)
                ...
                if (st.getQuestItemsCount(HORRIBLE_SKULL) < 10)
                    ...
                    st.doTakeItems(HORRIBLE_SKULL, -1);
                    st.doTakeItems(EINHASAD_CHURCH_MARK3, 1);
                    st.doGiveItems(COIN_OF_LORDS6, 1);
                    st.playSound(SOUND_MIDDLE);
                }
            } else if (coin6 > 0)
