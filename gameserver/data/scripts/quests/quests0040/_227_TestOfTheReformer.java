...
public class _227_TestOfTheReformer extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30118-04.htm")) {
            st.doGiveItems(BookOfReform, 1);
            if (!st.getPlayer().getVarB("dd3")) {
                st.doGiveItems(7562, RATE_QUESTS_REWARD * 60);
            ...
            st.doTakeItems(HugeNail, -1);
            st.doTakeItems(BookOfReform, -1);
            st.doGiveItems(LetterOfIntroduction, 1);
            st.setCond(4);
            st.setState(STARTED);
        } else if (event.equalsIgnoreCase("30666-04.htm")) {
            st.doTakeItems(LetterOfIntroduction, -1);
            st.doGiveItems(SlasLetter, 1);
            st.setCond(5);
            st.setState(STARTED);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Pupina) {
            if (st.getQuestItemsCount(MarkOfReformer) != 0) {
                ...
                if (st.getPlayer().getPcClass().class_id == 0x0f || st.getPlayer().getPcClass().class_id == 0x2a) {
            ...
            if (cond == 4)
                ...
                st.doTakeItems(OlMahumMoney, -1);
                st.doGiveItems(Greetings, 3);
                htmltext = "30666-06.htm";
                st.setCond(11);
                st.setState(STARTED);
            } else if (cond == 20) {
                st.doTakeItems(KatarisLetter, -1);
                st.doTakeItems(KakansLetter, -1);
                st.doTakeItems(NyakurisLetter, -1);
                st.doTakeItems(RamussLetter, -1);
                st.doGiveItems(MarkOfReformer, 1);
                if (!st.getPlayer().getVarB("prof2.3")) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 1252844, RATE_QUESTS_REWARD * 85972);
                    st.doRewardItems(ADENA_ID, 226528);
            ...
            if (cond == 5 || cond == 6) {
                ...
                st.doTakeItems(LetterOfBetrayer, -1);
                st.doGiveItems(KatarisLetter, 1);
            ...
            if (cond == 11 || cond == 12)
                ...
                st.doTakeItems(Greetings, 1);
                st.doGiveItems(KakansLetter, 1);
            ...
            if (cond == 14 || cond == 15)
                ...
                st.doTakeItems(Greetings, 1);
                st.doGiveItems(NyakurisLetter, 1);
            ...
            if (cond == 17) {
                st.doTakeItems(Greetings, -1);
                st.doGiveItems(UndeadList, 1);
                htmltext = "30667-01.htm";
                st.setCond(18);
                st.setState(STARTED);
            } else if (cond == 19) {
                st.doTakeItems(BoneFragment1, -1);
                st.doTakeItems(BoneFragment2, -1);
                st.doTakeItems(BoneFragment3, -1);
                st.doTakeItems(BoneFragment4, -1);
                st.doTakeItems(BoneFragment5, -1);
                st.doTakeItems(UndeadList, -1);
                st.doGiveItems(RamussLetter, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 18 && st.getQuestItemsCount(BoneFragment1) != 0 && st.getQuestItemsCount(BoneFragment2) != 0 && st.getQuestItemsCount(BoneFragment3) != 0 && st.getQuestItemsCount(BoneFragment4) != 0 && st.getQuestItemsCount(BoneFragment5) != 0) {
            ...
            if (st.getQuestItemsCount(RippedDiary) < 6)
                st.doGiveItems(RippedDiary, 1);
            else if (GameObjectsStorage.getByNpcId(Aruraune) == null) {
                st.doTakeItems(RippedDiary, -1);
                st.setCond(2);
                st.setState(STARTED);
            ...
            if (cond == 8) {
                if (st.getQuestItemsCount(LetterOfBetrayer) == 0)
                    st.doGiveItems(LetterOfBetrayer, 1);
                st.setCond(9);
                st.setState(STARTED);
            }
