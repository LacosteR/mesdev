...
public class _274_SkirmishWithTheWerewolves extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (id == CREATED)
            ...
            if (st.getQuestItemsCount(MARAKU_WEREWOLF_HEAD) < 40)
                ...
                st.doTakeItems(MARAKU_WEREWOLF_HEAD, -1);
                st.doGiveItems(ADENA_ID, 3500);
                if (st.getQuestItemsCount(MARAKU_WOLFMEN_TOTEM) >= 1) {
                    st.doGiveItems(ADENA_ID, st.getQuestItemsCount(MARAKU_WOLFMEN_TOTEM) * 600);
                    st.doTakeItems(MARAKU_WOLFMEN_TOTEM, -1);
                }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1 && st.getQuestItemsCount(MARAKU_WEREWOLF_HEAD) < 40) {
            ...
            st.doGiveItems(MARAKU_WEREWOLF_HEAD, 1);
        }
        if (Rnd.chance(5))
            st.doGiveItems(MARAKU_WOLFMEN_TOTEM, 1);
        return null;
    }
}