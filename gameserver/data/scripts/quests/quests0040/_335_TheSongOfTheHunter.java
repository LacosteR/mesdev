...
public class _335_TheSongOfTheHunter extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30744_03.htm") && _state == CREATED) {
            if (st.getQuestItemsCount(_1st_Test_Instructions) == 0)
                st.doGiveItems(_1st_Test_Instructions, 1);
            st.setState(STARTED);
            st.setCond(1);
            ...
            if (st.getQuestItemsCount(Cybellins_Dagger) == 0)
                st.doGiveItems(Cybellins_Dagger, 1);
            if (st.getQuestItemsCount(Cybellins_Request) == 0)
                st.doGiveItems(Cybellins_Request, 1);
            st.doTakeItems(q_blood_crystal, -1);
            st.playSound(SOUND_MIDDLE);
            st.doGiveItems(q_blood_crystal[1], 1);
            ...
            st.doTakeItems(Cybellins_Dagger, -1);
            st.doTakeItems(Cybellins_Request, -1);
            st.doTakeItems(q_blood_crystal, -1);
            ...
            if (st.getQuestItemsCount(Laurel_Leaf_Pin) > 0)
                st.doTakeItems(Laurel_Leaf_Pin, 1);
            for (Request r : Requests1) {
                st.doTakeItems(r.request_id, -1);
                st.doTakeItems(r.request_item, -1);
            }
            for (Request r : Requests2) {
                st.doTakeItems(r.request_id, -1);
                st.doTakeItems(r.request_item, -1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Grey) {
            if (st.getQuestItemsCount(_1st_Test_Instructions) > 0) {
                ...
                st.doTakeItems(_1st_Test_Instructions, -1);
                st.playSound(SOUND_MIDDLE);
                st.doGiveItems(_1st_Circle_Hunter_License, 1);
                st.setCond(2);
                return "30744_06.htm";
            }
            ...
            if (st.getQuestItemsCount(_2nd_Test_Instructions) > 0) {
                ...
                st.doTakeItems(_2nd_Test_Instructions, -1);
                st.doTakeItems(_1st_Circle_Hunter_License, -1);
                st.playSound(SOUND_MIDDLE);
                st.doGiveItems(_2nd_Circle_Hunter_License, 1);
                st.setCond(3);
                return "30744_12.htm";
            }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
            ...
            for (int[][] ItemsCond : Items_Circle)
                for (int i = 2; i < ItemsCond.length; i++)
                    if (npcId == ItemsCond[i][0])
                        st.doGiveOnKillItems(ItemsCond[0][0], 1, 1, ItemsCond[1][0], ItemsCond[i][1]);
            if (st.getQuestItemsCount(_1st_Test_Instructions) > 0) {
                ...
                if (npcId == Breka_Orc_Warrior) {
                    if (Hakas_Head_count == 0 && Rnd.chance(10))
                        st.addSpawn(Breka_Overlord_Haka, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
                    else if (Jakas_Head_count == 0 && Rnd.chance(10))
                        st.addSpawn(Breka_Overlord_Jaka, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
                    else if (Markas_Head_count == 0 && Rnd.chance(10))
                        st.addSpawn(Breka_Overlord_Marka, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
                } else if (npcId == Breka_Overlord_Haka) {
                    if (Hakas_Head_count == 0)
                        st.doGiveOnKillItems(Hakas_Head, 1, 1, 1, 100);
                } else if (npcId == Breka_Overlord_Jaka) {
                    if (Jakas_Head_count == 0)
                        st.doGiveOnKillItems(Jakas_Head, 1, 1, 1, 100);
                } else if (npcId == Breka_Overlord_Marka) {
                    if (Markas_Head_count == 0)
                        st.doGiveOnKillItems(Markas_Head, 1, 1, 1, 100);
                } else if (npcId == Windsus)
                    if (st.getQuestItemsCount(Windsus_Aleph_Skin) == 0 && Rnd.chance(10))
                        st.addSpawn(Windsus_Aleph, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
            } else if (st.getQuestItemsCount(_2nd_Test_Instructions) > 0) {
                ...
                if (npcId == Tarlk_Bugbear_Warrior) {
                    if (Athus_Head_count == 0 && Rnd.chance(10))
                        st.addSpawn(Tarlk_Raider_Athu, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
                    else if (Lankas_Head_count == 0 && Rnd.chance(10))
                        st.addSpawn(Tarlk_Raider_Lanka, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
                    else if (Triskas_Head_count == 0 && Rnd.chance(10))
                        st.addSpawn(Tarlk_Raider_Triska, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
                    else if (Moturas_Head_count == 0 && Rnd.chance(10))
                        st.addSpawn(Tarlk_Raider_Motura, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
                    else if (Kalaths_Head_count == 0 && Rnd.chance(10))
                        st.addSpawn(Tarlk_Raider_Kalath, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
                } else if (npcId == Tarlk_Raider_Athu) {
                    if (Athus_Head_count == 0)
                        st.doGiveOnKillItems(Athus_Head, 1, 1, 1, 100);
                } else if (npcId == Tarlk_Raider_Lanka) {
                    if (Lankas_Head_count == 0)
                        st.doGiveOnKillItems(Lankas_Head, 1, 1, 1, 100);
                } else if (npcId == Tarlk_Raider_Triska) {
                    if (Triskas_Head_count == 0)
                        st.doGiveOnKillItems(Triskas_Head, 1, 1, 1, 100);
                } else if (npcId == Tarlk_Raider_Motura) {
                    if (Moturas_Head_count == 0)
                        st.doGiveOnKillItems(Moturas_Head, 1, 1, 1, 100);
                } else if (npcId == Tarlk_Raider_Kalath)
                    if (Kalaths_Head_count == 0)
                        st.doGiveOnKillItems(Kalaths_Head, 1, 1, 1, 100);
            }
        }
        ...
        if (st.getQuestItemsCount(_1st_Circle_Hunter_License) > 0 || st.getQuestItemsCount(_2nd_Circle_Hunter_License) > 0) {
                ...
                if (Blood_Crystal_Level > 0 && Blood_Crystal_Level < 10)
                    for (int lizardmen_id : q_blood_crystal_lizardmen)
                        if (npcId == lizardmen_id)
                            if (Rnd.chance(50)) {
                                st.doTakeItems(q_blood_crystal[Blood_Crystal_Level], -1);
                                st.playSound(Blood_Crystal_Level < 6 ? SOUND_MIDDLE : SOUND_JACKPOT);
                                st.doGiveItems(q_blood_crystal[Blood_Crystal_Level + 1], 1);
                            } else {
                                st.doTakeItems(q_blood_crystal, -1);
                                st.doGiveItems(q_blood_crystal[0], 1);
                            }
            }
            ...
            if (request != null) {
                if (request.droplist.containsKey(npcId))
                    st.doGiveOnKillItems(request.request_item, 1, 1, request.request_count, request.droplist.get(npcId));
                ...
                if (request.spawnlist.containsKey(npcId) && st.getQuestItemsCount(request.request_item) < request.request_count) {
                    ...
                    if (Rnd.chance(spawn_n_chance[1])) {
                        st.addSpawn(spawn_n_chance[0], npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000);
        ...
        if ((npcId == 27160 || npcId == 27162 || npcId == 27164) && Rnd.chance(50)) {
            ...
            st.addSpawn(27150, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000); // b_legion_stormtrooper
            st.addSpawn(27150, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 300000); // b_legion_stormtrooper
        }
    ...
    private static boolean Blood_Crystal2Adena(QuestState st, int Blood_Crystal_Level) {
        if (Blood_Crystal_Level < 2)
            return false;
        st.doTakeItems(q_blood_crystal, -1);
        st.doGiveItems(ADENA_ID, 3400 * (int) Math.pow(2, Blood_Crystal_Level - 2));
        return true;
    }
    ...
    public static class Request {
        ...
        public boolean Complete(QuestState st) {
            if (st.getQuestItemsCount(request_item) < request_count)
                return false;
            st.doTakeItems(request_id, -1);
            st.doTakeItems(request_item, -1);
            st.playSound(SOUND_MIDDLE);
            st.doGiveItems(Laurel_Leaf_Pin, 1);
            st.doGiveItems(ADENA_ID, reward_adena);
            st.unset("list");
            return true;
        }
    }
