...
public class _140_ShadowFoxPart2 extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30895-02.htm")) {
            ...
            st.doRewardItems(ADENA_ID, 18775);
            ...
            st.doTakeItems(CRYSTAL, 5);
            if (Rnd.chance(60)) {
                st.doGiveItems(OXYDE, 1);
                if (st.getQuestItemsCount(OXYDE) >= 3) {
                    htmltext = "30912-09b.htm";
                    st.setCond(4);
                    st.setState(STARTED);
                    st.playSound(SOUND_MIDDLE);
                    st.doTakeItems(CRYSTAL, -1);
                    st.doTakeItems(OXYDE, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 3)
            st.doGiveOnKillItems(CRYSTAL, 1, 1, 1, 80 * npc.getTemplate().rateHp);
        return null;
    }
}