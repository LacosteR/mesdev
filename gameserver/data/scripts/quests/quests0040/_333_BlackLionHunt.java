...
public class _333_BlackLionHunt extends Quest implements ScriptFile {
    ...
    public void giveRewards(QuestState st, int item, long count) {
        st.doGiveItems(ADENA_ID, 35 * count);
        st.doTakeItems(item, count);
        if (count >= 20)
            st.doGiveItems(LIONS_CLAW, count / 20 * (long) st.getRateQuestsReward());
    }
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("start")) {
            ...
            st.set("part", "1");
            st.doGiveItems(SOPHIAS_LETTER1, 1);
            return "30735-02.htm";
        } else if (event.equalsIgnoreCase("p2_t")) {
            st.set("part", "2");
            st.doGiveItems(SOPHIAS_LETTER2, 1);
            return "30735-03.htm";
        } else if (event.equalsIgnoreCase("p3_t")) {
            st.set("part", "3");
            st.doGiveItems(SOPHIAS_LETTER3, 1);
            return "30735-04.htm";
        } else if (event.equalsIgnoreCase("p4_t")) {
            st.set("part", "4");
            st.doGiveItems(SOPHIAS_LETTER4, 1);
            return "30735-05.htm";
            ...
            if (claw > 0) {
                st.doGiveItems(LIONS_EYE, claw);
                long eye = st.getQuestItemsCount(LIONS_EYE);
                st.doTakeItems(LIONS_CLAW, claw * 10);
                int ala_count = 3;
                int soul_count = 100;
                int soe_count = 20;
                int heal_count = 20;
                ...
                while (claw > 0) {
                    int n = Rnd.get(5);
                    if (n == 0)
                        st.doGiveItems(ALACRITY_POTION, Math.round(ala_count * st.getRateQuestsReward()));
                    else if (n == 1)
                        st.doGiveItems(SOULSHOT_D, Math.round(soul_count * st.getRateQuestsReward()));
                    else if (n == 2)
                        st.doGiveItems(SCROLL_ESCAPE, Math.round(soe_count * st.getRateQuestsReward()));
                    else if (n == 3)
                        st.doGiveItems(SPIRITSHOT_D, Math.round(spir_count * st.getRateQuestsReward()));
                    else if (n == 4)
                        st.doGiveItems(HEALING_POTION, Math.round(heal_count * st.getRateQuestsReward()));
                    claw -= 1;
                }
                if (check_eye > 0)
                    return "30735-06.htm";
            ...
            if (st.getQuestItemsCount(CARGO_BOX1) > 0) {
                if (st.getQuestItemsCount(ADENA_ID) >= OPEN_BOX_PRICE) {
                    st.doTakeItems(CARGO_BOX1, 1);
                    st.doTakeItems(ADENA_ID, OPEN_BOX_PRICE);
                    int rand = Rnd.get(1, 162);
                    if (rand < 21) {
                        st.doGiveItems(GLUDIO_APPLE, 1);
                        return "red_foor-02.htm";
                    } else if (rand < 41) {
                        st.doGiveItems(CORN_MEAL, 1);
                        return "red_foor-03.htm";
                    } else if (rand < 61) {
                        st.doGiveItems(WOLF_PELTS, 1);
                        return "red_foor-04.htm";
                    } else if (rand < 74) {
                        st.doGiveItems(MONNSTONE, 1);
                        return "red_foor-05.htm";
                    } else if (rand < 86) {
                        st.doGiveItems(GLUDIO_WEETS_FLOWER, 1);
                        return "red_foor-06.htm";
                    } else if (rand < 98) {
                        st.doGiveItems(SPIDERSILK_ROPE, 1);
                        return "red_foor-07.htm";
                    } else if (rand < 99) {
                        st.doGiveItems(ALEXANDRIT, 1);
                        return "red_foor-08.htm";
                    } else if (rand < 109) {
                        st.doGiveItems(SILVER_TEA, 1);
                        return "red_foor-09.htm";
                    } else if (rand < 119) {
                        st.doGiveItems(GOLEM_PART, 1);
                        return "red_foor-10.htm";
                    } else if (rand < 123) {
                        st.doGiveItems(FIRE_EMERALD, 1);
                        return "red_foor-11.htm";
                    } else if (rand < 127) {
                        st.doGiveItems(SILK_FROCK, 1);
                        return "red_foor-12.htm";
                    } else if (rand < 131) {
                        st.doGiveItems(PORCELAN_URN, 1);
                        return "red_foor-13.htm";
                    } else if (rand < 132) {
                        st.doGiveItems(IMPERIAL_DIAMOND, 1);
                        return "red_foor-13.htm";
                    } else if (rand < 147) {
                        int random_stat = Rnd.get(4);
                        if (random_stat == 3) {
                            st.doGiveItems(STATUE_SHILIEN_HEAD, 1);
                            return "red_foor-14.htm";
                        } else if (random_stat == 0) {
                            st.doGiveItems(STATUE_SHILIEN_TORSO, 1);
                            return "red_foor-14.htm";
                        } else if (random_stat == 1) {
                            st.doGiveItems(STATUE_SHILIEN_ARM, 1);
                            return "red_foor-14.htm";
                        } else if (random_stat == 2) {
                            st.doGiveItems(STATUE_SHILIEN_LEG, 1);
                            return "red_foor-14.htm";
                        }
                    } else if (rand <= 162) {
                        int random_tab = Rnd.get(4);
                        if (random_tab == 0) {
                            st.doGiveItems(FRAGMENT_ANCIENT_TABLE1, 1);
                            return "red_foor-15.htm";
                        } else if (random_tab == 1) {
                            st.doGiveItems(FRAGMENT_ANCIENT_TABLE2, 1);
                            return "red_foor-15.htm";
                        } else if (random_tab == 2) {
                            st.doGiveItems(FRAGMENT_ANCIENT_TABLE3, 1);
                            return "red_foor-15.htm";
                        } else if (random_tab == 3) {
                            st.doGiveItems(FRAGMENT_ANCIENT_TABLE4, 1);
                            return "red_foor-15.htm";
                        }
                    }
                } else
            ...
            if (st.getQuestItemsCount(COMPLETE_TABLET) > 0) {
                st.doTakeItems(COMPLETE_TABLET, 1);
                st.doGiveItems(ADENA_ID, 30000);
            ...
            if (st.getQuestItemsCount(COMPLETE_STATUE) > 0) {
                st.doTakeItems(COMPLETE_STATUE, 1);
                st.doGiveItems(ADENA_ID, 30000);
            ...
            if (st.getQuestItemsCount(CARGO_BOX1) > 0) {
                ...
                if (count > 2)
                    count = 2;
                st.doGiveItems(GUILD_COIN, 1);
                st.doGiveItems(ADENA_ID, (1 + count) * 100);
                st.doTakeItems(CARGO_BOX1, 1);
        ...
        if (on_npc) {
            ...
            if (allowDrop == 1 && st.getInt("part") == part)
                if (rand < chancePartItem) {
                    st.doGiveItems(partItem, npcId == 27152 ? 8 : 1);
                    st.playSound(SOUND_ITEMGET);
                    if (rand2 < chanceBox) {
                        st.doGiveItems(CARGO_BOX1, 1);
                        if (rand > chancePartItem)
                            st.playSound(SOUND_ITEMGET);
                    }
                }
