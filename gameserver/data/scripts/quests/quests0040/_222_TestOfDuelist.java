...
public class _222_TestOfDuelist extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30623-04.htm") && st.getPlayer().getRace() == Race.orc)
            ...
            st.setCond(2);
            st.setState(STARTED);
            st.doGiveItems(OrderGludio, 1);
            st.doGiveItems(OrderDion, 1);
            st.doGiveItems(OrderGiran, 1);
            st.doGiveItems(OrderOren, 1);
            st.doGiveItems(OrderAden, 1);
            ...
            st.doTakeItems(PunchersShard, -1);
            st.doTakeItems(NobleAntsFeeler, -1);
            st.doTakeItems(DronesChitin, -1);
            st.doTakeItems(DeadSeekerFang, -1);
            st.doTakeItems(OverlordNecklace, -1);
            st.doTakeItems(FetteredSoulsChain, -1);
            st.doTakeItems(ChiefsAmulet, -1);
            st.doTakeItems(EnchantedEyeMeat, -1);
            st.doTakeItems(TamrinOrcsRing, -1);
            st.doTakeItems(TamrinOrcsArrow, -1);
            st.doTakeItems(OrderGludio, -1);
            st.doTakeItems(OrderDion, -1);
            st.doTakeItems(OrderGiran, -1);
            st.doTakeItems(OrderOren, -1);
            st.doTakeItems(OrderAden, -1);
            st.doGiveItems(FinalOrder, 1);
            st.setCond(4);
            st.setState(STARTED);
        }
        return htmltext;
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Kaien)
            if (st.getQuestItemsCount(MarkOfDuelist) != 0) {
                ...
                if (st.getPlayer().getPcClass().class_id == 0x01 || st.getPlayer().getPcClass().class_id == 0x2f || st.getPlayer().getPcClass().class_id == 0x13 || st.getPlayer().getPcClass().class_id == 0x20) {
                ...
                st.doGiveItems(MarkOfDuelist, 1);
                if (!st.getPlayer().getVarB("prof2.3")) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 594888, RATE_QUESTS_REWARD * 61408);
