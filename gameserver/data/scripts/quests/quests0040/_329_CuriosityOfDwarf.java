...
public class _329_CuriosityOfDwarf extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (st.getCond() == 0) {
            ...
            if (broken + heart > 0) {
                st.doGiveItems(ADENA_ID, (50 * broken) + (1000 * heart));
                st.doTakeItems(BROKEN_HEARTSTONE, -1);
                st.doTakeItems(GOLEM_HEARTSTONE, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 20085) {
            if (n < 5) {
                st.doGiveItems(GOLEM_HEARTSTONE, 1);
                st.playSound(SOUND_ITEMGET);
            } else if (n < 58) {
                st.doGiveItems(BROKEN_HEARTSTONE, 1);
                st.playSound(SOUND_ITEMGET);
            }
        } else if (npcId == 20083)
            if (n < 6) {
