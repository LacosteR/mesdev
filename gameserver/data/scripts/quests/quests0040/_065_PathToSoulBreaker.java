...
public class _065_PathToSoulBreaker extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("32138-09.htm")) {
            st.doTakeItems(Wyrm_Heart, 10);
            st.doGiveItems(Kekropus_Rec, 1);
            st.setCond(17);
            st.setState(STARTED);
        }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Vitus) {
            if (st.getQuestItemsCount(SB_Certificate) > 0) {
                ...
                if (st.getPlayer().getPcClass().class_id == 0x7e || st.getPlayer().getPcClass().class_id == 0x7d) {
                ...
                st.doTakeItems(Kekropus_Rec, 1);
                if (!st.getPlayer().getVarB("prof2.1")) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 393750, RATE_QUESTS_REWARD * 27020);
                    st.doRewardItems(ADENA_ID, 71194);
                    st.getPlayer().setVar("prof2.1", "1", -1);
                }
                st.doGiveItems(SB_Certificate, 1);
                st.exitCurrentQuest(true);
            }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 15 && npcId == Wyrm && st.doGiveOnKillItems(Wyrm_Heart, 1, 1, 10, 40)) {
            st.setState(STARTED);
            st.setCond(16);
        }
        return null;
