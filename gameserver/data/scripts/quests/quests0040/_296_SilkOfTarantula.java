...
public class _296_SilkOfTarantula extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("trader_mion_q0296_03.htm")) {
            ...
            st.doTakeItems(TARANTULA_SPINNERETTE, -1);
            ...
            if (st.getQuestItemsCount(TARANTULA_SPINNERETTE) >= 1) {
                ...
                st.doGiveItems(TARANTULA_SPIDER_SILK, 17);
                st.doTakeItems(TARANTULA_SPINNERETTE, -1);
            } else
        ...
        if (npcId == 30519) {
            if (cond == 0) {
                ...
                if (st.getQuestItemsCount(TARANTULA_SPIDER_SILK) < 1)
                    ...
                    st.doGiveItems(ADENA_ID, st.getQuestItemsCount(TARANTULA_SPIDER_SILK) * 23);
                    st.doTakeItems(TARANTULA_SPIDER_SILK, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1)
            if (Rnd.chance(50))
                st.doGiveOnKillItems(TARANTULA_SPINNERETTE, 1, 1, 45);
            else
                st.doGiveOnKillItems(TARANTULA_SPIDER_SILK, 1, 1, 45);
        return null;
    }
}