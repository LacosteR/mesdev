...
public class _233_TestOfWarspirit extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30510-05.htm") && _state == CREATED) {
            ...
            st.doGiveItems(ORIMS_CONTRACT, 1);
        else if (event.equalsIgnoreCase("30682-02.htm") && _state == STARTED)
            st.doGiveItems(PEKIRONS_TOTEM, 1);
        else if (event.equalsIgnoreCase("30515-02.htm") && _state == STARTED)
            st.doGiveItems(MANAKIAS_TOTEM, 1);
        else if (event.equalsIgnoreCase("30507-02.htm") && _state == STARTED)
            st.doGiveItems(RACOYS_TOTEM, 1);
        else if (event.equalsIgnoreCase("30030-04.htm") && _state == STARTED)
            st.doGiveItems(VIVIANTES_LETTER, 1);
        else if (event.equalsIgnoreCase("30649-03.htm") && _state == STARTED && st.getQuestItemsCount(WARSPIRIT_TOTEM) > 0) {
            st.doTakeItems(WARSPIRIT_TOTEM, -1);
            st.doTakeItems(BRAKIS_REMAINS2, -1);
            st.doTakeItems(HERMODTS_REMAINS2, -1);
            st.doTakeItems(KIRUNAS_REMAINS2, -1);
            st.doTakeItems(TAMLIN_ORC_HEAD, -1);
            st.doTakeItems(TONARS_REMAINS2, -1);
            st.doGiveItems(MARK_OF_WARSPIRIT, 1);
        ...
        if (npcId == Somak) {
            if (st.getQuestItemsCount(VENDETTA_TOTEM) > 0) {
                if (st.getQuestItemsCount(TAMLIN_ORC_HEAD) < 13)
                    return "30510-08.htm";
                st.doTakeItems(VENDETTA_TOTEM, -1);
                st.doGiveItems(WARSPIRIT_TOTEM, 1);
                st.doGiveItems(BRAKIS_REMAINS2, 1);
                st.doGiveItems(HERMODTS_REMAINS2, 1);
                st.doGiveItems(KIRUNAS_REMAINS2, 1);
                st.doGiveItems(TONARS_REMAINS2, 1);
            ...
            if (st.getQuestItemsCount(BRAKIS_REMAINS1) == 0 || st.getQuestItemsCount(HERMODTS_REMAINS1) == 0 || st.getQuestItemsCount(KIRUNAS_REMAINS1) == 0 || st.getQuestItemsCount(TONARS_REMAINS1) == 0)
                return "30510-06.htm";
            st.doTakeItems(BRAKIS_REMAINS1, -1);
            st.doTakeItems(HERMODTS_REMAINS1, -1);
            st.doTakeItems(KIRUNAS_REMAINS1, -1);
            st.doTakeItems(TONARS_REMAINS1, -1);
            st.doGiveItems(VENDETTA_TOTEM, 1);
            st.playSound(SOUND_MIDDLE);
            return "30510-07.htm";
        }
        ...
        if (npcId == Orim) {
            if (st.getQuestItemsCount(ORIMS_CONTRACT) > 0) {
                if (st.getQuestItemsCount(PORTAS_EYE) < 10 || st.getQuestItemsCount(EXCUROS_SCALE) < 10 || st.getQuestItemsCount(MORDEOS_TALON) < 10)
                    return "30630-05.htm";
                st.doTakeItems(ORIMS_CONTRACT, -1);
                st.doTakeItems(PORTAS_EYE, -1);
                st.doTakeItems(EXCUROS_SCALE, -1);
                st.doTakeItems(MORDEOS_TALON, -1);
                st.doGiveItems(BRAKIS_REMAINS1, 1);
                st.playSound(SOUND_MIDDLE);
                return "30630-06.htm";
            }
            ...
            if (st.getQuestItemsCount(PEKIRONS_TOTEM) > 0) {
                for (int drop_id : Leto_Lizardman_Drops)
                    if (st.getQuestItemsCount(drop_id) == 0)
                        return "30682-03.htm";
                st.doTakeItems(PEKIRONS_TOTEM, -1);
                for (int drop_id : Leto_Lizardman_Drops)
                    if (st.getQuestItemsCount(drop_id) == 0)
                        st.doTakeItems(drop_id, -1);
                st.doGiveItems(TONARS_REMAINS1, 1);
                st.playSound(SOUND_MIDDLE);
                return "30682-04.htm";
            }
        ...
        if (npcId == Manakia) {
            if (st.getQuestItemsCount(MANAKIAS_TOTEM) > 0) {
                ...
                for (int drop_id : Medusa_Drops)
                    if (st.getQuestItemsCount(drop_id) == 0)
                        return "30515-03.htm";
                st.doTakeItems(MANAKIAS_TOTEM, -1);
                st.doTakeItems(HERMODTS_SKULL, -1);
                for (int drop_id : Medusa_Drops)
                    if (st.getQuestItemsCount(drop_id) == 0)
                        st.doTakeItems(drop_id, -1);
                st.doGiveItems(HERMODTS_REMAINS1, 1);
                st.playSound(SOUND_MIDDLE);
                return "30515-04.htm";
            }
            ...
            if (st.getQuestItemsCount(RACOYS_TOTEM) > 0) {
                ...
                if (st.getQuestItemsCount(VIVIANTES_LETTER) == 0) {
                    for (int drop_id : Noble_Ant_Drops)
                        if (st.getQuestItemsCount(drop_id) == 0)
                            return "30507-05.htm";
                    st.doTakeItems(RACOYS_TOTEM, -1);
                    st.doTakeItems(INSECT_DIAGRAM_BOOK, -1);
                    for (int drop_id : Noble_Ant_Drops)
                        if (st.getQuestItemsCount(drop_id) == 0)
                            st.doTakeItems(drop_id, -1);
                    st.doGiveItems(KIRUNAS_REMAINS1, 1);
                    st.playSound(SOUND_MIDDLE);
                    return "30507-06.htm";
                }
            } else {
        ...
        if (npcId == Sarien)
            if (st.getQuestItemsCount(RACOYS_TOTEM) > 0) {
                if (st.getQuestItemsCount(INSECT_DIAGRAM_BOOK) == 0 && st.getQuestItemsCount(VIVIANTES_LETTER) > 0) {
                    st.doTakeItems(VIVIANTES_LETTER, -1);
                    st.doGiveItems(INSECT_DIAGRAM_BOOK, 1);
                    st.playSound(SOUND_MIDDLE);
                    return "30436-01.htm";
                }
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        if (npcId == Porta && qs.getQuestItemsCount(ORIMS_CONTRACT) > 0 && qs.getQuestItemsCount(PORTAS_EYE) < 10) {
            ...
            qs.doGiveItems(EXCUROS_SCALE, 1);
            ...
            qs.doGiveItems(MORDEOS_TALON, 1);
            ...
            qs.doGiveItems(HERMODTS_SKULL, 1);
            ...
            if (Rnd.chance(npcId == Tamlin_Orc ? 30 : 50)) {
                qs.doGiveItems(TAMLIN_ORC_HEAD, 1);
