    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30608-04.htm")) {
            ...
            if (EYES + SCALE + SHARD >= 45) {
                st.doRewardItems(ADENA_ID, (EYES * 100) + (SCALE * 100) + (SHARD * 100) + 9000);
                st.doTakeItems(WATCHING_EYES, -1);
                st.doTakeItems(DELU_LIZARDMAN_SCALE, -1);
                st.doTakeItems(ROUGHLY_HEWN_ROCK_GOLEM_SHARD, -1);
            } else {
                st.doRewardItems(ADENA_ID, (EYES * 100) + (SCALE * 100) + (SHARD * 100));
                st.doTakeItems(WATCHING_EYES, -1);
                st.doTakeItems(DELU_LIZARDMAN_SCALE, -1);
                st.doTakeItems(ROUGHLY_HEWN_ROCK_GOLEM_SHARD, -1);
            }
            ...
            if (EYES + SCALE + SHARD >= 99) {
                ...
                if (EYES >= 100)
                    st.doTakeItems(WATCHING_EYES, 100);
                else {
                    st.doTakeItems(WATCHING_EYES, -1);
                    if (SCALE >= n)
                        st.doTakeItems(DELU_LIZARDMAN_SCALE, n);
                    else {
                        st.doTakeItems(DELU_LIZARDMAN_SCALE, -1);
                        st.doTakeItems(ROUGHLY_HEWN_ROCK_GOLEM_SHARD, t);
                    }
                }
                if (Rnd.chance(80)) {
                    st.doRewardItems(ADENA_ID, 13000);
                    st.doGiveItems(SCROLL_ENCANT_ARMOR, 1);
                } else
                    st.doRewardItems(ADENA_ID, 1000);
                st.playSound(SOUND_ITEMGET);
            } else
            ...
            if (EYES + SCALE + SHARD >= 199) {
                ...
                int luck = Rnd.get(15);
                if (EYES >= 200)
                    st.doTakeItems(WATCHING_EYES, 200);
                else
                    st.doTakeItems(WATCHING_EYES, -1);
                if (SCALE >= n)
                    st.doTakeItems(DELU_LIZARDMAN_SCALE, n);
                ...
                if (luck < 9) {
                    ...
                    st.doGiveItems(SCROLL_ENCHANT_WEAPON, 1);
                else
                    st.doRewardItems(ADENA_ID, 2000);
                st.playSound(SOUND_ITEMGET);
            } else
                htmltext = "30291-14.htm";
            ...
            if (EYES + SCALE + SHARD >= 499) {
                ...
                if (EYES >= 500)
                    st.doTakeItems(WATCHING_EYES, 500);
                else
                    st.doTakeItems(WATCHING_EYES, -1);
                if (SCALE >= n)
                    st.doTakeItems(DELU_LIZARDMAN_SCALE, n);
                ...
                if (Rnd.chance(80)) {
                    st.doRewardItems(ADENA_ID, 45000);
                    st.doGiveItems(SCROLL_ENCHANT_WEAPON, 1);
                } else
                    st.doRewardItems(ADENA_ID, 5000);
                st.playSound(SOUND_ITEMGET);
            } else
                htmltext = "30291-14.htm";
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.getCond() == 2)
            if (npcId == 21106 | npcId == 21102 && chance < 79) {
                st.doGiveItems(WATCHING_EYES, 1);
                st.playSound(SOUND_ITEMGET);
            } else if (npcId == ROUGH_HEWN_ROCK_GOLEM && chance < 75) {
                st.doGiveItems(ROUGHLY_HEWN_ROCK_GOLEM_SHARD, 1);
                st.playSound(SOUND_ITEMGET);
            } else if (npcId == 20781 | npcId == 21104 | npcId == 21107 | npcId == 21105 && chance < 67) {
                st.doGiveItems(DELU_LIZARDMAN_SCALE, 1);
                st.playSound(SOUND_ITEMGET);
            }
        return null;
    }
