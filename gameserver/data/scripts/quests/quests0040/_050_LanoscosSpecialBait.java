...
public class _050_LanoscosSpecialBait extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("fisher_lanosco_q0050_0104.htm")) {
            ...
            if (st.getQuestItemsCount(EssenceofWind) < 100)
                ...
                st.unset("cond");
                st.doTakeItems(EssenceofWind, -1);
                st.doRewardItems(WindFishingLure, 4);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() != 1 || npc.getNpcId() != SingingWind)
            return null;

        if (st.doGiveOnKillItems(EssenceofWind, 1, 1, 100, 30))
            st.setCond(2);
