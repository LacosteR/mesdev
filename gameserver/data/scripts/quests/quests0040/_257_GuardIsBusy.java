...
public class _257_GuardIsBusy extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("gilbert_q0257_03.htm")) {
            ...
            st.doTakeItems(GLUDIO_LORDS_MARK, -1);
            st.doGiveItems(GLUDIO_LORDS_MARK, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            st.doGiveItems(ADENA, 12 * st.getQuestItemsCount(ORC_AMULET) + 20 * st.getQuestItemsCount(ORC_NECKLACE) + 25 * st.getQuestItemsCount(WEREWOLF_FANG));
            ...
            st.doTakeItems(ORC_AMULET, -1);
            st.doTakeItems(ORC_NECKLACE, -1);
            st.doTakeItems(WEREWOLF_FANG, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.getQuestItemsCount(GLUDIO_LORDS_MARK) > 0 && st.getCond() > 0)
            if (npcId == 20130 || npcId == 20131 || npcId == 20006)
                st.doGiveOnKillItems(ORC_AMULET, 1, 1, 50);
            else if (npcId == 20093 || npcId == 20096 || npcId == 20098)
                st.doGiveOnKillItems(ORC_NECKLACE, 1, 1, 50);
            else if (npcId == 20132)
                st.doGiveOnKillItems(WEREWOLF_FANG, 1, 1, 33);
            else if (npcId == 20343)
                st.doGiveOnKillItems(WEREWOLF_FANG, 1, 1, 50);
            else if (npcId == 20342)
                st.doGiveOnKillItems(WEREWOLF_FANG, 1, 1, 75);
        return null;
    }
}