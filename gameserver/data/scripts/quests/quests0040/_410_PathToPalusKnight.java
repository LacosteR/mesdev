...
public class _410_PathToPalusKnight extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            ...
            st.doGiveItems(PALLUS_TALISMAN_ID, 1);
        } else if (event.equalsIgnoreCase("410_1")) {
            if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x1f && st.getQuestItemsCount(GAZE_OF_ABYSS_ID) == 0)
                htmltext = "master_virgil_q0410_05.htm";
            else if (st.getPlayer().getPcClass().class_id != 0x1f) {
                if (st.getPlayer().getPcClass().class_id == 0x20)
                    htmltext = "master_virgil_q0410_02a.htm";
                else
                    htmltext = "master_virgil_q0410_03.htm";
            } else if (st.getPlayer().getLevel() < 18 && st.getPlayer().getPcClass().class_id == 0x1f)
                htmltext = "master_virgil_q0410_02.htm";
            else if (st.getPlayer().getLevel() >= 18 && st.getPlayer().getPcClass().class_id == 0x1f && st.getQuestItemsCount(GAZE_OF_ABYSS_ID) >= 1)
                htmltext = "master_virgil_q0410_04.htm";
        } else if (event.equalsIgnoreCase("30329_2")) {
            htmltext = "master_virgil_q0410_10.htm";
            st.doTakeItems(PALLUS_TALISMAN_ID, -1);
            st.doTakeItems(LYCANTHROPE_SKULL_ID, -1);
            st.doGiveItems(VIRGILS_LETTER_ID, 1);
            st.setCond(3);
        } else if (event.equalsIgnoreCase("30422_1")) {
            htmltext = "kalinta_q0410_02.htm";
            st.doTakeItems(VIRGILS_LETTER_ID, -1);
            st.doGiveItems(MORTE_TALISMAN_ID, 1);
            st.setCond(4);
        } else if (event.equalsIgnoreCase("30422_2")) {
            htmltext = "kalinta_q0410_06.htm";
            st.doTakeItems(MORTE_TALISMAN_ID, -1);
            st.doTakeItems(TRIMDEN_SILK_ID, -1);
            st.doTakeItems(PREDATOR_CARAPACE_ID, -1);
            st.doGiveItems(COFFIN_ETERNAL_REST_ID, 1);
            st.setCond(6);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == VIRGIL) {
            if (cond < 1)
                ...
                st.doTakeItems(COFFIN_ETERNAL_REST_ID, -1);
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(GAZE_OF_ABYSS_ID, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == LYCANTHROPE) {
            if (cond == 1 && st.getQuestItemsCount(PALLUS_TALISMAN_ID) > 0 && st.getQuestItemsCount(LYCANTHROPE_SKULL_ID) < 13) {
                st.doGiveItems(LYCANTHROPE_SKULL_ID, 1);
            ...
            if (cond == 4 && st.getQuestItemsCount(MORTE_TALISMAN_ID) > 0 && st.getQuestItemsCount(PREDATOR_CARAPACE_ID) < 1) {
                st.doGiveItems(PREDATOR_CARAPACE_ID, 1);
                st.playSound(SOUND_MIDDLE);
            ...
            if (cond == 4 && st.getQuestItemsCount(MORTE_TALISMAN_ID) > 0 && st.getQuestItemsCount(TRIMDEN_SILK_ID) < 5) {
                st.doGiveItems(TRIMDEN_SILK_ID, 1);
                if (st.getQuestItemsCount(TRIMDEN_SILK_ID) > 4) {
                    st.playSound(SOUND_MIDDLE);
