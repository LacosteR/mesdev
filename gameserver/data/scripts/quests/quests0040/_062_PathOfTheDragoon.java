...
public class _062_PathOfTheDragoon extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Gwain) {
            if (id == CREATED) {
                if (st.getPlayer().getPcClass() != PcClass.kamael_m_soldier) {
            ...
            if (cond == 1)
                ...
                st.doTakeItems(FelimHead, -1);
                st.setCond(3);
                htmltext = "master_shubain_q0062_04.htm";
            } else if (cond == 3 && st.getQuestItemsCount(VenomousSpiderLeg) >= 10) {
                st.doTakeItems(VenomousSpiderLeg, -1);
                st.doGiveItems(ShubainsRecommendation, 1);
                st.setCond(4);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (id == FelimLizardmanWarrior && cond == 2)
            st.doGiveOnKillItems(FelimHead, 1, 1, 5, 100);

        if (id == VenomousSpider && cond == 3)
            st.doGiveOnKillItems(VenomousSpiderLeg, 1, 1, 10, 100);

        if (id == TumranBugbear && cond == 5)
            st.doGiveOnKillItems(TumranBugbearHeart, 1, 1, 1, 100);
