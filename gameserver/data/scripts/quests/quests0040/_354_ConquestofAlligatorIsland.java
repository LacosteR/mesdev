...
public class _354_ConquestofAlligatorIsland extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30895-00a.htm"))
            ...
            if (amount > 0)
                if (amount > 99) {
                    st.doGiveItems(ADENA_ID, amount * 300);
                    st.doTakeItems(ALLIGATOR_TOOTH, -1);
                    st.playSound(SOUND_ITEMGET);
                    int random = Rnd.get(RANDOM_REWARDS.length);
                    st.doGiveItems(RANDOM_REWARDS[random][0], RANDOM_REWARDS[random][1]);
                    htmltext = "30895-05b.htm";
                } else {
                    st.doGiveItems(ADENA_ID, amount * 100);
                    st.doTakeItems(ALLIGATOR_TOOTH, -1);
                    st.playSound(SOUND_ITEMGET);
                    htmltext = "30895-05a.htm";
                }
        } else if (event.equalsIgnoreCase("30895-08.htm")) {
            st.doGiveItems(PIRATES_TREASURE_MAP, 1);
            st.doTakeItems(TORN_MAP_FRAGMENT, -1);
            st.playSound(SOUND_ITEMGET);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (Rnd.chance(CHANCE)) {
            st.doGiveItems(ALLIGATOR_TOOTH, 1);
            st.playSound(SOUND_ITEMGET);
        }
        if (Rnd.chance(CHANCE2) && st.getQuestItemsCount(TORN_MAP_FRAGMENT) < 10) {
            st.doGiveItems(TORN_MAP_FRAGMENT, 1);
