...
public class _158_SeedOfEvil extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 30031 && st.getCond() == 0) {
            ...
            st.doTakeItems(CLAY_TABLET_ID, st.getQuestItemsCount(CLAY_TABLET_ID));
            st.playSound(SOUND_FINISH);
            st.doRewardItems(ADENA_ID, 1495);
            st.addExpAndSp(RATE_QUESTS_REWARD * 17818, RATE_QUESTS_REWARD * 927);
            st.doGiveItems(ENCHANT_ARMOR_D, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getQuestItemsCount(CLAY_TABLET_ID) == 0) {
            st.doGiveItems(CLAY_TABLET_ID, 1);
