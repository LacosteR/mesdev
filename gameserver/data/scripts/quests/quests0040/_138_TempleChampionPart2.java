...
public class _138_TempleChampionPart2 extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("sylvain_q0138_04.htm")) {
            ...
            st.doGiveItems(MANIFESTO, 1);
        } else if (event.equalsIgnoreCase("sylvain_q0138_09.htm")) {
            st.addExpAndSp(RATE_QUESTS_REWARD * 187062, RATE_QUESTS_REWARD * 11307);
            st.doRewardItems(ADENA_ID, 84593);
        ...
        if (npcId == SYLVAIN) {
            ...
            if (cond == 3)
                ...
                if (st.getQuestItemsCount(RELIC) >= 10) {
                    ...
                    st.doTakeItems(RELIC, -1);
                    st.doGiveItems(ANGUS_REC, 1);
                    st.setCond(5);
                    st.playSound(SOUND_MIDDLE);
                } else
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        st.doGiveOnKillItems(RELIC, 1, 1, 10, 100);
        return null;
    }
}