...
public class _407_PathToElvenScout extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            if (st.getPlayer().getPcClass().class_id == 0x12) {
                if (st.getPlayer().getLevel() >= 18) {
                    if (st.getQuestItemsCount(REORIA_RECOMMENDATION_ID) > 0) {
                        ...
                        st.doGiveItems(REORIA_LETTER2_ID, 1);
                        st.setCond(1);
                        st.setState(STARTED);
            ...
            } else if (st.getPlayer().getPcClass().class_id == 0x16) {
            ...
            st.doTakeItems(REORIA_LETTER2_ID, 1);
            st.setCond(2);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == REISA) {
            if (cond == 0)
                ...
                st.doTakeItems(HONORARY_GUARD_ID, 1);
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(REORIA_RECOMMENDATION_ID, 1);
            ...
            if (cond == 1)
                ...
                if (st.getQuestItemsCount(PRIGUNS_TEAR_LETTER1_ID) >= 1 && st.getQuestItemsCount(PRIGUNS_TEAR_LETTER2_ID) >= 1 && st.getQuestItemsCount(PRIGUNS_TEAR_LETTER3_ID) >= 1 && st.getQuestItemsCount(PRIGUNS_TEAR_LETTER4_ID) >= 1) {
                    ...
                    st.doTakeItems(PRIGUNS_TEAR_LETTER1_ID, 1);
                    st.doTakeItems(PRIGUNS_TEAR_LETTER2_ID, 1);
                    st.doTakeItems(PRIGUNS_TEAR_LETTER3_ID, 1);
                    st.doTakeItems(PRIGUNS_TEAR_LETTER4_ID, 1);
                    st.doGiveItems(MORETTIS_HERB_ID, 1);
                    st.doGiveItems(MORETTIS_LETTER_ID, 1);
                    st.setCond(4);
                } else
                ...
                st.doTakeItems(PRIGUNS_LETTER_ID, 1);
                st.doGiveItems(HONORARY_GUARD_ID, 1);
                st.setCond(8);
            } else if (cond > 8)
            ...
            if (cond == 4) {
                ...
                st.doTakeItems(RUSTED_KEY_ID, 1);
                st.doTakeItems(MORETTIS_HERB_ID, 1);
                st.doTakeItems(MORETTIS_LETTER_ID, 1);
                st.doGiveItems(PRIGUNS_LETTER_ID, 1);
                st.setCond(7);
            } else if (cond == 7)
