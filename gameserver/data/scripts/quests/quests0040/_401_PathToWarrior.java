...
public class _401_PathToWarrior extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("401_1")) {
            if (st.getPlayer().getPcClass().class_id == 0x00) {
            ...
            } else if (st.getPlayer().getPcClass().class_id == 0x01)
            ...
            st.doTakeItems(SIMPLONS_LETTER_ID, 1);
            st.doTakeItems(RUSTED_BRONZE_SWORD2_ID, 1);
            st.doGiveItems(RUSTED_BRONZE_SWORD3_ID, 1);
            st.setCond(5);
            ...
            if (st.getQuestItemsCount(EINS_LETTER_ID) == 0) {
                ...
                st.doGiveItems(EINS_LETTER_ID, 1);
                htmltext = "ein_q0401_06.htm";
            }
        } else if (event.equalsIgnoreCase("30253_1")) {
            htmltext = "trader_simplon_q0401_02.htm";
            st.doTakeItems(EINS_LETTER_ID, 1);
            st.doGiveItems(WARRIOR_GUILD_MARK_ID, 1);
            st.setCond(2);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == AURON && cond == 0)
            ...
            if (st.getQuestItemsCount(RUSTED_BRONZE_SWORD1_ID) < 1)
                ...
                st.doTakeItems(WARRIOR_GUILD_MARK_ID, -1);
                st.doTakeItems(RUSTED_BRONZE_SWORD1_ID, -1);
                st.doGiveItems(RUSTED_BRONZE_SWORD2_ID, 1);
                st.doGiveItems(SIMPLONS_LETTER_ID, 1);
                st.setCond(4);
            ...
            if (st.getQuestItemsCount(POISON_SPIDER_LEG2_ID) < 20)
                ...
                st.doTakeItems(POISON_SPIDER_LEG2_ID, -1);
                st.doTakeItems(RUSTED_BRONZE_SWORD3_ID, -1);
                if (st.getPlayer().getPcClass().getLevel() == 1) {
                    st.doGiveItems(MEDALLION_OF_WARRIOR_ID, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == TRACKER_SKELETON || npcId == TRACKER_SKELETON_LD) {
            if (cond == 2 && st.getQuestItemsCount(RUSTED_BRONZE_SWORD1_ID) < 10) {
                st.doGiveItems(RUSTED_BRONZE_SWORD1_ID, 1);
            ...
            if (st.getQuestItemsCount(POISON_SPIDER_LEG2_ID) < 20 && st.getQuestItemsCount(RUSTED_BRONZE_SWORD3_ID) == 1 && st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == RUSTED_BRONZE_SWORD3_ID) {
                st.doGiveItems(POISON_SPIDER_LEG2_ID, 1);
                if (st.getQuestItemsCount(POISON_SPIDER_LEG2_ID) >= 20) {
                    st.playSound(SOUND_MIDDLE);
                    st.setCond(6);
                } else
