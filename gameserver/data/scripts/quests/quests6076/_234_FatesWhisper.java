...
public class _234_FatesWhisper extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("31002-03.htm")) {
            ...
            st.doTakeItems(REIRIAS_SOUL_ORB, -1);
            st.setCond(COND2);
        } else if (event.equalsIgnoreCase("31030-02.htm"))
            st.doGiveItems(HALLATES_INFERNIUM_SCEPTER, 1);
        else if (event.equalsIgnoreCase("31028-02.htm"))
            st.doGiveItems(KERNONS_INFERNIUM_SCEPTER, 1);
        else if (event.equalsIgnoreCase("31029-02.htm"))
            st.doGiveItems(GOLCONDAS_INFERNIUM_SCEPTER, 1);
        else if (event.equalsIgnoreCase("31002-06a.htm")) {
            st.doTakeItems(HALLATES_INFERNIUM_SCEPTER, -1);
            st.doTakeItems(KERNONS_INFERNIUM_SCEPTER, -1);
            st.doTakeItems(GOLCONDAS_INFERNIUM_SCEPTER, -1);
            st.setCond(COND3);
        } else if (event.equalsIgnoreCase("30182-01c.htm")) {
            st.doTakeItems(INFERNIUM_VARNISH, -1);
            st.doGiveItems(INFERNIUM_VARNISH, 1);
        } else if (event.equalsIgnoreCase("31002-07a.htm"))
            st.setCond(COND4);
        else if (event.equalsIgnoreCase("31002-08a.htm")) {
            st.doTakeItems(REORINS_HAMMER, -1);
            st.setCond(COND5);
        } else if (event.equalsIgnoreCase("30178-01a.htm"))
            st.setCond(COND6);
        else if (event.equalsIgnoreCase("30833-01a.htm"))
            return ConfigCharacter.ALT_ALLOW_SUBCLASS_WITHOUT_BAIUM ? "30833-01a.htm" : "30833-01n.htm";
        else if (event.equalsIgnoreCase("30833-01b.htm")) {
            st.setCond(COND7);
            st.doGiveItems(PIPETTE_KNIFE, 1);
        } else if (event.equalsIgnoreCase("30833-01c.htm")) {
            st.setCond(COND8);
            st.doGiveItems(WhiteCloth, 30);
        ...
        if (newweapon != 0) {
            ...
            if (st.getPlayer().getInventory().destroyItem(itemtotake, 1L)) {
                st.doGiveItems(newweapon, 1);
                st.doGiveItems(STAR_OF_DESTINY, 1);
                st.unset("cond");
                st.unset("oldweapon");
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == REORIN) {
            ...
            st.doGiveItems(REIRIAS_SOUL_ORB, 1);
            ...
            st.doGiveItems(REORINS_HAMMER, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == COND8 && (npcId == GUARDIAN_ANGEL || npcId == SEAL_ANGEL)) {
            ...
            if (st.getQuestItemsCount(WhiteCloth) > 0 && count < 30 && Rnd.chance(RATE_QUESTS_DROP * 33)) {
                st.doGiveItems(BloodStainedCloth, 1);
                if (count >= 29) {
                    st.doTakeItems(WhiteCloth, -1);
                    st.setCond(COND9);
                } else
    ...
    public String onAttack(NpcInstance npc, QuestState st) {
        ...
        if ((cond == COND7 || cond == COND9) && npc.getNpcId() == BAIUM && st.getQuestItemsCount(PIPETTE_KNIFE) >= 1 && st.getQuestItemsCount(RED_PIPETTE_KNIFE) == 0 && st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == PIPETTE_KNIFE) {
            ...
            st.doTakeItems(PIPETTE_KNIFE, -1);
            st.doGiveItems(RED_PIPETTE_KNIFE, 1);
