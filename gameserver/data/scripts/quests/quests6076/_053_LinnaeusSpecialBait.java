...
public class _053_LinnaeusSpecialBait extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("fisher_linneaus_q0053_0104.htm")) {
            ...
            if (st.getQuestItemsCount(HeartOfCrimsonDrake) < 100)
                ...
                st.unset("cond");
                st.doTakeItems(HeartOfCrimsonDrake, -1);
                st.doRewardItems(FlameFishingLure, 4);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() != 1 || npc.getNpcId() != CrimsonDrake)
            return null;

        if (st.doGiveOnKillItems(HeartOfCrimsonDrake, 1, 1, 100, 30))
            st.setCond(2);
