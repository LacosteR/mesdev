...
public class _110_ToThePrimevalIsle extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("1")) {
            ...
            st.setCond(1);
            st.doGiveItems(ANCIENT_BOOK, 1);
            st.setState(STARTED);
            st.playSound(SOUND_ACCEPT);
        } else if (event.equals("2") && st.getQuestItemsCount(ANCIENT_BOOK) > 0) {
            htmltext = "marquez_q0110_05.htm";
            st.playSound(SOUND_FINISH);
            st.doRewardItems(ADENA_ID, 191678);
            st.addExpAndSp(RATE_QUESTS_REWARD * 251602, RATE_QUESTS_REWARD * 25245);
            st.doTakeItems(ANCIENT_BOOK, -1);
