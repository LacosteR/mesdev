...
public class _235_MimirsElixir extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == LADD) {
            ...
            // Finally, all is done...time to learn how to use the Elixir...
                ...
                // st.doTakeItems(MIMIRS_ELIXIR,-1) #disabled for compatibility with the way java expects things at Grand Master.
            ...
            if (cond == 2)
                ...
                st.doTakeItems(SAGES_STONE, -1);
                st.doGiveItems(TRUE_GOLD, 1);
                st.setCond(5);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        int npcId = npc.getNpcId();
        int cond = st.getCond();
        if (npcId == Chimera_Piece && cond == 3 && st.getQuestItemsCount(SAGES_STONE) == 0 && Rnd.chance(RATE_QUESTS_DROP * chance)) {
            st.doGiveItems(SAGES_STONE, 1);
            st.playSound(SOUND_ITEMGET);
            st.setCond(cond + 1);
        }
        if (npcId == Bloody_Guardian && cond == 6 && st.getQuestItemsCount(BLOOD_FIRE) == 0 && Rnd.chance(RATE_QUESTS_DROP * chance)) {
            st.doGiveItems(BLOOD_FIRE, 1);
            st.playSound(SOUND_ITEMGET);
            st.setCond(cond + 1);
        }
        return null;
