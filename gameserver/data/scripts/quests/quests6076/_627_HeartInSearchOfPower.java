...
public class _627_HeartInSearchOfPower extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("dark_necromancer_q0627_0104.htm")) {
            ...
            st.doTakeItems(GEM_OF_SUBMISSION, 300);
            st.doGiveItems(SEAL_OF_LIGHT, 1);
            st.setCond(3);
        } else if (event.equals("enfeux_q0627_0301.htm")) {
            st.doTakeItems(SEAL_OF_LIGHT, 1);
            st.doGiveItems(GEM_OF_SAINTS, 1);
            st.setCond(4);
        } else if (event.equals("dark_necromancer_q0627_0401.htm"))
            st.doTakeItems(GEM_OF_SAINTS, 1);
        else {
            if (event.equals("627_11")) {
                ...
                htmltext = "dark_necromancer_q0627_0402.htm";
                st.doGiveItems(ASOFE, RATE_QUESTS_REWARD * 13);
                st.doRewardItems(ADENA_ID, 6400);
            } else if (event.equals("627_13")) {
                htmltext = "dark_necromancer_q0627_0402.htm";
                st.doGiveItems(THONS, RATE_QUESTS_REWARD * 13);
                st.doRewardItems(ADENA_ID, 6400);
            } else if (event.equals("627_14")) {
                htmltext = "dark_necromancer_q0627_0402.htm";
                st.doGiveItems(ENRIA, RATE_QUESTS_REWARD * 6);
                st.doRewardItems(ADENA_ID, 13600);
            } else if (event.equals("627_15")) {
                htmltext = "dark_necromancer_q0627_0402.htm";
                st.doGiveItems(MOLD_HARDENER, RATE_QUESTS_REWARD * 3);
                st.doRewardItems(ADENA_ID, 17200);
            }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.doGiveOnKillItems(GEM_OF_SUBMISSION, 1, 1, 300, 100))
            st.setCond(2);
