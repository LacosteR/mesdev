...
public class _463_IMustBeaGenius extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (npc.getNpcId() == GUTENHAGEN) {
            if (event.equalsIgnoreCase("collecter_gutenhagen_q0463_05.htm")) {
                ...
                if (diff == 0) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 198725, RATE_QUESTS_REWARD * 15892);
                    htmltext = "collecter_gutenhagen_q0463_09.htm";
                } else if (diff >= 1 && diff < 5) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 278216, RATE_QUESTS_REWARD * 22249);
                    htmltext = "collecter_gutenhagen_q0463_10.htm";
                } else if (diff >= 5 && diff < 10) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 317961, RATE_QUESTS_REWARD * 25427);
                    htmltext = "collecter_gutenhagen_q0463_11.htm";
                } else if (diff >= 10 && diff < 25) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 357706, RATE_QUESTS_REWARD * 28606);
                    htmltext = "collecter_gutenhagen_q0463_11.htm";
                } else if (diff >= 25 && diff < 40) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 397451, RATE_QUESTS_REWARD * 31784);
                    htmltext = "collecter_gutenhagen_q0463_12.htm";
                } else if (diff >= 40 && diff < 60) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 596176, RATE_QUESTS_REWARD * 47677);
                    htmltext = "collecter_gutenhagen_q0463_13.htm";
                } else if (diff >= 60 && diff < 72) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 715411, RATE_QUESTS_REWARD * 57212);
                    htmltext = "collecter_gutenhagen_q0463_14.htm";
                } else if (diff >= 72 && diff < 81) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 794901, RATE_QUESTS_REWARD * 63569);
                    htmltext = "collecter_gutenhagen_q0463_14.htm";
                } else if (diff >= 81 && diff < 89) {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 914137, RATE_QUESTS_REWARD * 73104);
                    htmltext = "collecter_gutenhagen_q0463_15.htm";
                } else {
                    st.addExpAndSp(RATE_QUESTS_REWARD * 1192352, RATE_QUESTS_REWARD * 95353);
                    htmltext = "collecter_gutenhagen_q0463_15.htm";
                }
                st.unset("cond");
                st.unset("number");
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getState() == STARTED && st.getCond() == 1) {
            ...
            if (_number > 0) {
                st.doGiveItems(CORPSE_LOG, _number);
                ...
                st.doTakeItems(CORPSE_LOG, Math.abs(_number));
                st.playSound(SOUND_ITEMGET);
                Functions.npcSay(npc, NpcString.ATT__ATTACK__S1__RO__ROGUE__S2, st.getPlayer().getName(), String.valueOf(_number));
            }

            if (st.getQuestItemsCount(CORPSE_LOG) >= _day_number) {
                st.doTakeItems(CORPSE_LOG, -1);
                st.doGiveItems(COLLECTION, 1);
                st.setCond(2);
            }
