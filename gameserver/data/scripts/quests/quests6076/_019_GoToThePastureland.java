...
public class _019_GoToThePastureland extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("trader_vladimir_q0019_0104.htm")) {
            st.doGiveItems(BEAST_MEAT, 1);
            st.setCond(1);
            st.setState(STARTED);
            st.playSound(SOUND_ACCEPT);
        }
        if (event.equals("beast_herder_tunatun_q0019_0201.htm")) {
            st.doTakeItems(BEAST_MEAT, -1);
            st.addExpAndSp(RATE_QUESTS_REWARD * 385040, RATE_QUESTS_REWARD * 75250);
            st.doRewardItems(ADENA_ID, 147200);
