...
public class _605_AllianceWithKetraOrcs extends Quest implements ScriptFile {
    ...
    private static void takeAllMarks(QuestState st) {
        st.doTakeItems(MARK_OF_KETRA_ALLIANCE1, -1);
        st.doTakeItems(MARK_OF_KETRA_ALLIANCE2, -1);
        st.doTakeItems(MARK_OF_KETRA_ALLIANCE3, -1);
        st.doTakeItems(MARK_OF_KETRA_ALLIANCE4, -1);
        st.doTakeItems(MARK_OF_KETRA_ALLIANCE5, -1);
    }
    ...
    private static boolean CheckNextLevel(QuestState st, int soilder_count, int capitan_count, int general_count, int other_item, boolean take) {
        ...
        if (take) {
            if (soilder_count > 0)
                st.doTakeItems(VB_SOLDIER, soilder_count);
            if (capitan_count > 0)
                st.doTakeItems(VB_CAPTAIN, capitan_count);
            if (general_count > 0)
                st.doTakeItems(VB_GENERAL, general_count);
            if (other_item > 0)
                st.doTakeItems(other_item, 1);
            takeAllMarks(st);
        }
        return true;
    }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == RECRUIT || npcId == FOOTMAN || npcId == SCOUT || npcId == HUNTER || npcId == SHAMAN) {
            ...
            if (cond > 2)
                st.doGiveOnKillItems(VB_GENERAL, 1, 1, 80);
        return null;
    }
}