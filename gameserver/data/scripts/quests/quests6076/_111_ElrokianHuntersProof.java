...
public class _111_ElrokianHuntersProof extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("marquez_q111_2.htm") && cond == 0) {
            ...
            st.setCond(6);
            st.doTakeItems(DiaryFragment, -1);
            st.playSound(SOUND_MIDDLE);
            ...
            st.doTakeItems(OrnithomimusClaw, -1);
            st.doTakeItems(DeinonychusBone, -1);
            st.doTakeItems(PachycephalosaurusSkin, -1);
            st.setCond(12);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("asamah_q111_8.htm")) {
            st.doRewardItems(ADENA_ID, 1071691);
            st.addExpAndSp(RATE_QUESTS_REWARD * 553524, RATE_QUESTS_REWARD * 55538);
            st.doGiveItems(ElrokianTrap, 1);
            st.doGiveItems(TrapStone, 100);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 4) {
            for (int i : Velociraptor) {
                ...
                if (st.doGiveOnKillItems(DiaryFragment, 1, 1, 50, 100))
                    st.setCond(5);
            }
        } else if (cond == 10) {
            for (int i : Ornithomimus) {
                ...
                st.doGiveOnKillItems(OrnithomimusClaw, 1, 1, 10, 100);
                return null;
            }
            for (int i : Deinonychus) {
                ...
                st.doGiveOnKillItems(DeinonychusBone, 1, 1, 10, 100);
                return null;
            }
            for (int i : Pachycephalosaurus) {
                ...
                st.doGiveOnKillItems(PachycephalosaurusSkin, 1, 1, 10, 100);
                return null;
            }
