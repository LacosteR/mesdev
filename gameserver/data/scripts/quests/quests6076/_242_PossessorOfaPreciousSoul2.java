...
import quests.quests4060._241_PossessorOfaPreciousSoul1;
...
public class _242_PossessorOfaPreciousSoul2 extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if (event.equalsIgnoreCase("31742-2.htm")) {
            st.setCond(1);
            st.set("CoRObjId", "0");
            st.doTakeItems(VIRGILS_LETTER_1_PART, 1);
            st.setState(STARTED);
            ...
            st.doTakeItems(BLONDE_STRAND, 1);
            st.setCond(7);
            st.playSound(SOUND_MIDDLE);
            ...
            st.setCond(8);
            st.doGiveItems(SORCERY_INGREDIENT, 1);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("31748-2.htm")) {
            st.doTakeItems(ORB_OF_BINDING, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == 31742) {
            ...
            if (cond == 1)
                ...
                st.doGiveItems(CARADINE_LETTER, 1);
                st.addExpAndSp(RATE_QUESTS_REWARD * 455764, 0);
                st.unset("cond");
                st.unset("CoRObjId");
                st.unset("prog");
                st.unset("talk");
            ...
            if (cond == 4) {
                st.doGiveItems(BLONDE_STRAND, 1);
            ...
            if (st.getCond() == 9)
                ...
                if (PureWhiteUnicornSpawn == null || !st.getPlayer().knowsObject(PureWhiteUnicornSpawn) || !PureWhiteUnicornSpawn.isVisible())
                    PureWhiteUnicornSpawn = st.addSpawn(PureWhiteUnicorn, npc.getPositionComponent().getX() + 10, npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), 120000);
            } else
                htmltext = "noquest";
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.getCond() == 9 && st.getQuestItemsCount(ORB_OF_BINDING) < 4)
            st.doGiveItems(ORB_OF_BINDING, 1);
