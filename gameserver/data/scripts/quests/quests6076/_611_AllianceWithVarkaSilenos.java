...
public class _611_AllianceWithVarkaSilenos extends Quest implements ScriptFile {
    ...
    private static void takeAllMarks(QuestState st) {
        st.doTakeItems(MARK_OF_VARKA_ALLIANCE1, -1);
        st.doTakeItems(MARK_OF_VARKA_ALLIANCE2, -1);
        st.doTakeItems(MARK_OF_VARKA_ALLIANCE3, -1);
        st.doTakeItems(MARK_OF_VARKA_ALLIANCE4, -1);
        st.doTakeItems(MARK_OF_VARKA_ALLIANCE5, -1);
    }
    ...
    private static boolean CheckNextLevel(QuestState st, int soilder_count, int capitan_count, int general_count, int other_item, boolean take) {
        ...
        if (take) {
            if (soilder_count > 0)
                st.doTakeItems(KB_SOLDIER, soilder_count);
            if (capitan_count > 0)
                st.doTakeItems(KB_CAPTAIN, capitan_count);
            if (general_count > 0)
                st.doTakeItems(KB_GENERAL, general_count);
            if (other_item > 0)
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == RAIDER || npcId == FOOTMAN || npcId == SCOUT || npcId == WAR_HOUND || npcId == SHAMAN) {
            if (cond > 0)
                st.doGiveOnKillItems(KB_SOLDIER, 1, 1, 60);
            ...
            if (cond > 1)
                st.doGiveOnKillItems(KB_CAPTAIN, 1, 1, 70);
            ...
            if (cond > 2)
                st.doGiveOnKillItems(KB_GENERAL, 1, 1, 80);
        return null;
    }
}