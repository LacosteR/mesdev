...
public class _144_PailakaInjuredDragon extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("Enter")) {
            ...
            st.doGiveItems(SPEAR, 1);
        } else if (event.equalsIgnoreCase("32512-02.htm")) {
            st.doTakeItems(SPEAR, 1);
            st.doTakeItems(ENCHSPEAR, 1);
            st.doTakeItems(LASTSPEAR, 1);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == KETRAOSHAMAN) {
            ...
            if (st.getQuestItemsCount(SPEAR) > 0 && st.getQuestItemsCount(STAGE1) > 0) {
                st.doTakeItems(SPEAR, 1);
                st.doTakeItems(STAGE1, 1);
                st.doGiveItems(ENCHSPEAR, 1);
                htmltext = "32509-02.htm";
            }
            if (st.getQuestItemsCount(ENCHSPEAR) > 0 && st.getQuestItemsCount(STAGE2) > 0) {
                st.doTakeItems(ENCHSPEAR, 1);
                st.doTakeItems(STAGE2, 1);
                st.doGiveItems(LASTSPEAR, 1);
            ...
            if (cond == 4) {
                ...
                st.doGiveItems(PSHIRT, 1);
                st.addExpAndSp(RATE_QUESTS_REWARD * 28000000, RATE_QUESTS_REWARD * 2850000);
                st.setCond(5);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        switch (npcId) {
            ...
            case LATANA:
                st.setCond(4);
                st.playSound(SOUND_MIDDLE);
                addSpawnToInstance(KOSUPPORTER2, npc.getPositionComponent().getLoc(), 0, refId);
                break;
        }

        if (ArrayUtils.contains(Pailaka3rd, npcId))
            if (Rnd.get(100) < 30)
                st.doDropOnGroundItems(npc, PAILAKA3DROP[Rnd.get(PAILAKA3DROP.length)], 1);

        if (ArrayUtils.contains(Antelopes, npcId))
            st.doDropOnGroundItems(npc, ANTELOPDROP[Rnd.get(ANTELOPDROP.length)], Rnd.get(1, 10));
