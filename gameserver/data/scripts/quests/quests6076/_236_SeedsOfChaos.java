...
public class _236_SeedsOfChaos extends Quest implements ScriptFile {
    ...
    // Chances
    private final static int BLACK_ECHO_CRYSTAL_CHANCE = (int) (15 * ConfigRates.RATE_QUESTS_DROP);
    private final static int SHINING_MEDALLION_CHANCE = (int) (20 * ConfigRates.RATE_QUESTS_DROP);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("32138_02b.htm") && _state == CREATED) {
            ...
            st.doTakeItems(BLACK_ECHO_CRYSTAL, -1);
            st.set("echo", "1");
            ...
            st.doGiveItems(SCROLL_ENCHANT_WEAPON_A, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (_state == CREATED) {
            ...
            if (st.getQuestItemsCount(STAR_OF_DESTINY) > 0) {
                st.doTakeItems(STAR_OF_DESTINY, -1);
                st.setCond(0);
                return "32138_02.htm";
            }
        ...
        if (npcId == HARKILGAMED) {
            ...
            if (cond == 13) {
                ...
                st.doTakeItems(SHINING_MEDALLION, -1);
                return "32236_07.htm";
            }
            if (cond > 13)
                return "32236_09.htm";
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        if (IsInIntArray(npcId, NEEDLE_STAKATO_DRONES)) {
            if (cond == 2 && qs.getQuestItemsCount(BLACK_ECHO_CRYSTAL) == 0 && Rnd.chance(RATE_QUESTS_DROP * BLACK_ECHO_CRYSTAL_CHANCE)) {
                qs.doGiveItems(BLACK_ECHO_CRYSTAL, 1);
                qs.setCond(3);
                qs.playSound(SOUND_MIDDLE);
            }
        } else if (IsInIntArray(npcId, SPLENDOR_MOBS))
            if (cond == 12 && qs.getQuestItemsCount(SHINING_MEDALLION) < 62 && Rnd.chance(RATE_QUESTS_DROP * SHINING_MEDALLION_CHANCE)) {
                qs.doGiveItems(SHINING_MEDALLION, 1);
