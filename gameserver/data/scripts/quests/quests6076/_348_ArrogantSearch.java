...
public class _348_ArrogantSearch extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        {
            st.setCond(4);
            st.doTakeItems(SHELL_OF_MONSTERS, -1);
        ...
        if (event.equals("30864-09a.htm")) {
            st.setCond(29);
            st.doGiveItems(WHITE_FABRIC_2, 10);
        }
        if (event.equals("30864-10a.htm")) {
            if (st.getQuestItemsCount(WHITE_FABRIC_2) < 10)
                st.doGiveItems(WHITE_FABRIC_2, 10 - st.getQuestItemsCount(WHITE_FABRIC_2));
            htmltext = "30864-10.htm";
        }
        if (event.equals("30864-10b.htm")) {
            if (st.getQuestItemsCount(BLOODED_FABRIC) > 1) {
                long count = st.getQuestItemsCount(BLOODED_FABRIC);
                st.doTakeItems(BLOODED_FABRIC, -1);
                st.doGiveItems(ADENA_ID, count * 5000);
                htmltext = "30864-10.htm";
            } else
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == HANELLIN) {
            ...
            // Player abandoned in the middle of last dialog...repeat the dialog.
                ...
                st.setCond(5);
                st.doGiveItems(HANELLINS_FIRST_LETTER, 1);
                st.doGiveItems(HANELLINS_SECOND_LETTER, 1);
                st.doGiveItems(HANELLINS_THIRD_LETTER, 1);
                ...
                st.doTakeItems(BOOK_OF_SAINT, -1);
                st.doTakeItems(BLOOD_OF_SAINT, -1);
                st.doTakeItems(BRANCH_OF_SAINT, -1);
                st.setCond(22);
                ...
                st.doTakeItems(ANTIDOTE, 5);
                st.doTakeItems(HEALING_POTION, 1);
                if (st.getInt("companions") == 0) {
                    st.setCond(25);
                    htmltext = "30864-07.htm";// go get platinum tribe blood...
                    st.doGiveItems(WHITE_FABRIC_1, 1);
                } else {
                    st.setCond(23);
                    htmltext = "not implemented yet";
                    st.doGiveItems(WHITE_FABRIC_0, 3);
                }
                ...
                if (st.getQuestItemsCount(WHITE_FABRIC_2) < 1)
                    st.doGiveItems(WHITE_FABRIC_2, 1);
                htmltext = "30864-07a.htm";
        ...
        // Other NPCs follow:
            ...
            if (ARK_OWNERS.containsKey(npcId)) {
                ...
                if (st.getQuestItemsCount(ARK_OWNERS.get(npcId)[0]) == 1) {
                    st.doTakeItems(ARK_OWNERS.get(npcId)[0], 1);
                ...
                // the player has the key and doesn't have the contents, give the contents
                {
                    ...
                    st.doTakeItems(ARKS.get(npcId)[0], 1);
                    st.doGiveItems(ARKS.get(npcId)[2], 1);
                }
                ...
                if (st.getQuestItemsCount(FIRST_KEY_OF_ARK) == 0 && st.getInt("angelKillerIsDefeated") == 0) {
                    ...
                    st.doGiveItems(FIRST_KEY_OF_ARK, 1);
