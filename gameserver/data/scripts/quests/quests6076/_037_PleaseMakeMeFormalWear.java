...
public class _037_PleaseMakeMeFormalWear extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("30842-1.htm")) {
            ...
            st.doGiveItems(SIGNET_RING, 1);
            st.setCond(2);
        } else if (event.equals("31521-1.htm")) {
            st.doTakeItems(SIGNET_RING, 1);
            st.doGiveItems(ICE_WINE, 1);
            st.setCond(3);
        } else if (event.equals("31627-1.htm")) {
            if (st.getQuestItemsCount(ICE_WINE) > 0) {
                st.doTakeItems(ICE_WINE, 1);
                st.setCond(4);
            } else
                htmltext = "You don't have enough materials";
        } else if (event.equals("31521-3.htm")) {
            st.doGiveItems(BOX_OF_COOKIES, 1);
            st.setCond(5);
        } else if (event.equals("31520-3.htm")) {
            st.doTakeItems(BOX_OF_COOKIES, 1);
            st.setCond(6);
        } else if (event.equals("31520-5.htm")) {
            st.doTakeItems(MYSTERIOUS_CLOTH, 1);
            st.doTakeItems(JEWEL_BOX, 1);
            st.doTakeItems(SEWING_KIT, 1);
            st.setCond(7);
        } else if (event.equals("31520-7.htm"))
            if (st.getQuestItemsCount(DRESS_SHOES_BOX) > 0) {
                st.doTakeItems(DRESS_SHOES_BOX, 1);
                st.doRewardItems(ItemTemplate.ITEM_ID_FORMAL_WEAR, 1);
                st.unset("cond");
