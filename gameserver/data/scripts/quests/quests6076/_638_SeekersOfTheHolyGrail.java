...
public class _638_SeekersOfTheHolyGrail extends Quest implements ScriptFile {
    ...
    private String tryRevard(QuestState st) {
        ...
        while (st.getQuestItemsCount(TOTEM) >= 2000) {
            st.doTakeItems(TOTEM, 2000);
            int rnd = Rnd.get(100);
            if (rnd < 50)
                st.doRewardItems(ADENA_ID, 3576000);
            else if (rnd < 85) {
                st.doGiveItems(EAS, 1);
            } else {
                st.doGiveItems(EWS, 1);
            }
            ok = true;
        }
        if (ok) {
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        st.doGiveOnKillItems(TOTEM, 1, 1, DROP_CHANCE * npc.getTemplate().rateHp);
