...
public class _129_PailakaDevilsLegacy extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == DISURVIVOR) {
            ...
            if (st.getQuestItemsCount(SWORD) > 0 && st.getQuestItemsCount(KDROP) > 0) {
                st.doTakeItems(SWORD, 1);
                st.doTakeItems(KDROP, 1);
                st.doGiveItems(ENCHSWORD, 1);
                htmltext = "32508-02.htm";
            }
            if (st.getQuestItemsCount(ENCHSWORD) > 0 && st.getQuestItemsCount(ADROP) > 0) {
                st.doTakeItems(ENCHSWORD, 1);
                st.doTakeItems(ADROP, 1);
                st.doGiveItems(LASTSWORD, 1);
            ...
            if (cond == 4) {
                if (player.getPet() != null)
                    ...
                    st.doGiveItems(PBRACELET, 1);
                    st.addExpAndSp(RATE_QUESTS_REWARD * 10810000, RATE_QUESTS_REWARD * 950000);
                    st.setCond(5);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == KAMS && st.getQuestItemsCount(KDROP) == 0)
            ...
            if (Rnd.get(100) < 80)
                st.doDropOnGroundItems(npc, HERBS[Rnd.get(HERBS.length)], Rnd.get(1, 2));
        } else if (npcId == CHEST)
            if (Rnd.get(100) < 80)
                st.doDropOnGroundItems(npc, CHESTDROP[Rnd.get(CHESTDROP.length)], Rnd.get(1, 10));
