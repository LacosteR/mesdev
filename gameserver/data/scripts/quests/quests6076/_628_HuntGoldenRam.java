...
public class _628_HuntGoldenRam extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("31554-03a.htm")) {
            if (st.getQuestItemsCount(CHITIN) >= 100 && st.getCond() == 1) {
                st.setCond(2);
                st.doTakeItems(CHITIN, 100);
                st.doGiveItems(RECRUIT, 1);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (st.isCompleted())
            ...
            if (chitin1 >= 100 && chitin2 >= 100) {
                ...
                st.doTakeItems(CHITIN, -1);
                st.doTakeItems(CHITIN2, -1);
                st.doTakeItems(RECRUIT, -1);
                st.doGiveItems(SOLDIER, 1);
                st.getPlayer().updateRam();
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond >= 1 && 21507 < npcId && npcId < 21513)
            st.doGiveOnKillItems(CHITIN, 1, 1, 100, CHANCE + (npcId - 21506) * 2);
        else if (cond == 2 && 21513 <= npcId && npcId <= 21518)
            st.doGiveOnKillItems(CHITIN2, 1, 1, 100, CHANCE + (npcId - 21512) * 3);
