...
public class _601_WatchingEyes extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("eye_of_argos_q0601_0104.htm"))
            ...
            st.doGiveItems(ADENA_ID, adena);
            if (item != 0) {
                st.doGiveItems(item, 5);
                st.addExpAndSp(RATE_QUESTS_REWARD * 120000, RATE_QUESTS_REWARD * 10000);
            }
            st.doTakeItems(PROOF_OF_AVENGER, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1) {
            if (st.doGiveOnKillItems(PROOF_OF_AVENGER, 1, 1, 100, DROP_CHANCE))
                st.setCond(2);
        }
        return null;
    }
