...
public class _622_DeliveryofSpecialLiquor extends Quest implements ScriptFile {
    ...
    private static void takeDrink(QuestState st, int setcond) {
        ...
        st.doTakeItems(SpecialDrink, 1);
        st.doGiveItems(FeeOfSpecialDrink, 1);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("jeremy_q0622_0104.htm") && _state == CREATED) {
            ...
            st.setCond(1);
            st.doTakeItems(SpecialDrink, -1);
            st.doTakeItems(FeeOfSpecialDrink, -1);
            st.doGiveItems(SpecialDrink, 5);
            ...
            if (Rnd.chance(Tateossian_CHANCE)) {
                if (Rnd.chance(40))
                    st.doGiveItems(RecipeSealedTateossianRing, 1);
                else if (Rnd.chance(40))
                    st.doGiveItems(RecipeSealedTateossianEarring, 1);
                else
                    st.doGiveItems(RecipeSealedTateossianNecklace, 1);
            } else {
                st.doRewardItems(ADENA_ID, 18800);
                st.doGiveItems(HastePotion, 1);
            }
