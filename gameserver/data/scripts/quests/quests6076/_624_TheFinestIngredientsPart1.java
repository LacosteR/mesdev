...
public class _624_TheFinestIngredientsPart1 extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("jeremy_q0624_0104.htm"))
            ...
            if (st.getQuestItemsCount(TRUNK_OF_NEPENTHES) == 50 && st.getQuestItemsCount(FOOT_OF_BANDERSNATCHLING) == 50 && st.getQuestItemsCount(SECRET_SPICE) == 50) {
                st.doTakeItems(TRUNK_OF_NEPENTHES, -1);
                st.doTakeItems(FOOT_OF_BANDERSNATCHLING, -1);
                st.doTakeItems(SECRET_SPICE, -1);
                st.playSound(SOUND_FINISH);
                st.doGiveItems(SAUCE, 1);
                st.doGiveItems(CRYOLITE, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.getCond() == 1) {
            if (npcId == HOT_SPRINGS_NEPENTHES)
                st.doGiveOnKillItems(TRUNK_OF_NEPENTHES, 1, 1, 50, 100);
            else if (npcId == HOT_SPRINGS_BANDERSNATCHLING)
                st.doGiveOnKillItems(FOOT_OF_BANDERSNATCHLING, 1, 1, 50, 100);
            else if ((npcId == HOT_SPRINGS_ATROX || npcId == HOT_SPRINGS_ATROXSPAWN))
                st.doGiveOnKillItems(SECRET_SPICE, 1, 1, 50, 100);
            onKillCheck(st);
        }
        return null;
    }
