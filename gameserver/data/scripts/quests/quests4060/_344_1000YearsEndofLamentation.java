...
public class _344_1000YearsEndofLamentation extends Quest implements ScriptFile {
        ...
        if (event.equalsIgnoreCase("30754-04.htm")) {
            ...
            if (amount == 0)
                ...
                if (Rnd.get((int) (SPECIAL / st.getRateQuestsReward())) >= amount)
                    st.doGiveItems(ADENA_ID, amount * 60);
                else {
                    htmltext = "30754-10.htm";
                    st.set("ok", "1");
                    st.set("amount", str(amount));
                }
                st.doTakeItems(ARTICLES_DEAD_HEROES, -1);
            }
            ...
            if (st.getInt("ok") != 1)
                ...
                if (random < 25) {
                    ...
                    st.doGiveItems(OLD_KEY, 1);
                } else if (random < 50) {
                    htmltext = "30754-13.htm";
                    st.doGiveItems(OLD_HILT, 1);
                } else if (random < 75) {
                    htmltext = "30754-14.htm";
                    st.doGiveItems(OLD_TOTEM, 1);
                } else
                    st.doGiveItems(CRUCIFIX, 1);
            }
        return htmltext;
    }
        ...
        if (id == CREATED) {
            ...
            if (npcId == GILMORE) {
                ...
                if (mission == 1)
                    ...
                    st.doGiveItems(4044, RATE_QUESTS_REWARD * 1);
                else if (mission == 3)
                    st.doGiveItems(4043, RATE_QUESTS_REWARD * 1);
                else if (mission == 4)
                    st.doGiveItems(4042, RATE_QUESTS_REWARD * 1);
                if (amt > 0) {
                    st.unset("amount");
                    st.doGiveItems(ADENA_ID, (amt * 50) + bonus);
                }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1)
            st.doGiveOnKillItems(ARTICLES_DEAD_HEROES, 1, 1, (CHANCE + (npc.getNpcId() - 20234) * 2));
        return null;
    }
    ...
    private boolean rewards(QuestState st, int npcId) {
        ...
        if (npcId == ORVEN && st.getQuestItemsCount(CRUCIFIX) > 0) {
            st.set("mission", "1");
            st.doTakeItems(CRUCIFIX, -1);
            state = true;
            if (chance < 50)
                st.doGiveItems(1875, 19);
            else if (chance < 70)
                st.doGiveItems(952, 5);
            else
                st.doGiveItems(2437, 1);
        } else if (npcId == GARVARENTZ && st.getQuestItemsCount(OLD_TOTEM) > 0) {
            st.set("mission", "2");
            st.doTakeItems(OLD_TOTEM, -1);
            state = true;
            if (chance < 45)
                st.doGiveItems(1882, 70);
            else if (chance < 95)
                st.doGiveItems(1881, 50);
            else
                st.doGiveItems(191, 1);
        } else if (npcId == KAIEN && st.getQuestItemsCount(OLD_HILT) > 0) {
            st.set("mission", "3");
            st.doTakeItems(OLD_HILT, -1);
            state = true;
            if (chance < 50)
                st.doGiveItems(1874, 25);
            else if (chance < 75)
                st.doGiveItems(1887, 10);
            else if (chance < 99)
                st.doGiveItems(951, 1);
            else
                st.doGiveItems(133, 1);
        } else if (npcId == RODEMAI && st.getQuestItemsCount(OLD_KEY) > 0) {
            st.set("mission", "4");
            st.doTakeItems(OLD_KEY, -1);
            state = true;
            if (chance < 40)
                st.doGiveItems(1879, 55);
            else if (chance < 90)
                st.doGiveItems(951, 1);
            else
                st.doGiveItems(885, 1);
        }
        return state;
    }
}