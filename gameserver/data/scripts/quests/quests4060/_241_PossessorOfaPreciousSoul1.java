...
public class _241_PossessorOfaPreciousSoul1 extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if (event.equalsIgnoreCase("31739-02.htm")) {
            ...
            st.doTakeItems(CRIMSON_MOSS, -1);
            st.doGiveItems(MEDICINE, 1);
            st.setCond(16);
        } else if (event.equalsIgnoreCase("31743-02.htm")) {
            st.doTakeItems(MEDICINE, -1);
            st.setCond(17);
            ...
            st.doGiveItems(VIRGILS_LETTER, 1);
            st.addExpAndSp(RATE_QUESTS_REWARD * 263043, 0);
            st.unset("cond");
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 3) {
            if (npcId == 21154 && Rnd.chance(10))
                ...
                st.doGiveItems(LEGENG_OF_SEVENTEEN, 1);
                st.setCond(4);
                st.playSound(SOUND_ITEMGET);
            }
        } else if (cond == 6) {
            if ((npcId == 20244 || npcId == 20245)) {
                if (st.doGiveOnKillItems(MALRUK_SUCCUBUS_CLAW, 1, 1, 10, 10))
                    st.setCond(7);
            }
        } else if (cond == 14) {
            if (npcId == 20669) {
                if (st.doGiveOnKillItems(CRIMSON_MOSS, 1, 1, 5, 10))
                    st.setCond(15);
            }
        }
        return null;
