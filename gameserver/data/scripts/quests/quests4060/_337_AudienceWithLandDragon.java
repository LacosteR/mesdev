...
public class _337_AudienceWithLandDragon extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("1")) {
            ...
            st.set("guard", "0");
            st.setState(STARTED);
            st.doGiveItems(FEATHER_OF_GABRIELLE_ID, 1);
            ...
            st.doTakeItems(MARK_OF_WATCHMAN_ID, -1);
            st.set("step", "10");
            st.setCond(2);
            ...
            st.doTakeItems(MARK_OF_WATCHMAN_ID, -1);
            st.doTakeItems(FEATHER_OF_GABRIELLE_ID, -1);
            st.doGiveItems(HERALD_OF_SLAYER_ID, 1);
            st.playSound(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("16")) {
            st.set("step", "16");
            st.setCond(4);
            htmltext = "30754-02.htm";
            st.doTakeItems(HERALD_OF_SLAYER_ID, -1);
            st.playSound(SOUND_MIDDLE);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == GABRIELLE) {
            ...
            if (st.getQuestItemsCount(STALKER_HORN_ID) < 1 && st.getQuestItemsCount(DRAKE_TALON_ID) < 1 && step == 1)
                ...
                st.doTakeItems(STALKER_HORN_ID, -1);
                st.doTakeItems(DRAKE_TALON_ID, -1);
                st.doGiveItems(MARK_OF_WATCHMAN_ID, 1);
                st.set("step", "3");
                st.playSound(SOUND_MIDDLE);
            } else if (step == 2)
                htmltext = "30720-02.htm";
            ...
            if (st.getQuestItemsCount(REMAINS_OF_SACRIFICED_ID) < 1 && step == 3)
                ...
                st.doTakeItems(REMAINS_OF_SACRIFICED_ID, -1);
                st.doGiveItems(MARK_OF_WATCHMAN_ID, 1);
                st.set("step", "5");
                st.playSound(SOUND_MIDDLE);
            } else if (step == 4)
            ...
            if (st.getQuestItemsCount(TOTEM_OF_LAND_DRAGON_ID) < 1 && step == 5)
                ...
                st.doTakeItems(TOTEM_OF_LAND_DRAGON_ID, -1);
                st.doGiveItems(MARK_OF_WATCHMAN_ID, 1);
                st.set("step", "7");
                st.playSound(SOUND_MIDDLE);
            } else if (step == 6)
            ...
            if (st.getQuestItemsCount(HAMRUT_LEG_ID) < 1 && st.getQuestItemsCount(KRANROT_SKIN_ID) < 1 && step == 7)
                ...
                st.doTakeItems(HAMRUT_LEG_ID, -1);
                st.doTakeItems(KRANROT_SKIN_ID, -1);
                st.doGiveItems(MARK_OF_WATCHMAN_ID, 1);
                st.set("step", "9");
                st.playSound(SOUND_MIDDLE);
            } else if (step == 8)
            ...
            if (st.getQuestItemsCount(MARA_FANG_ID) < 1 && st.getQuestItemsCount(FIRST_ABYSS_FRAGMENT_ID) < 1 && step == 10)
                ...
                st.doTakeItems(MARA_FANG_ID, -1);
                st.doTakeItems(FIRST_ABYSS_FRAGMENT_ID, -1);
                st.doGiveItems(MARK_OF_WATCHMAN_ID, 1);
                st.set("step", "12");
                st.playSound(SOUND_MIDDLE);
            } else if (step == 11)
            ...
            if (st.getQuestItemsCount(MUSFEL_FANG_ID) < 1 && st.getQuestItemsCount(SECOND_ABYSS_FRAGMENT_ID) < 1 && step == 12)
                ...
                st.doTakeItems(MUSFEL_FANG_ID, -1);
                st.doTakeItems(SECOND_ABYSS_FRAGMENT_ID, -1);
                st.doGiveItems(MARK_OF_WATCHMAN_ID, 1);
                st.set("step", "14");
                st.playSound(SOUND_MIDDLE);
            } else if (step == 13)
            ...
            if (st.getQuestItemsCount(THIRD_ABYSS_FRAGMENT_ID) < 1)
                ...
                st.doTakeItems(THIRD_ABYSS_FRAGMENT_ID, -1);
                st.unset("step");
                st.unset("cond");
                st.unset("guard");
                st.exitCurrentQuest(true);
                st.doGiveItems(PORTAL_STONE_ID, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        for (int[] element : DROPLIST)
            ...
            if (npcId == element[1] && step == element[0] && st.getQuestItemsCount(element[2]) < element[3] && Rnd.chance(element[4])) {
                st.doGiveItems(element[2], element[5]);
