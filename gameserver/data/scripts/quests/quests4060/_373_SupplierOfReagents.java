...
import quests.quests6076._235_MimirsElixir;
...
public class _373_SupplierOfReagents extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30166-4.htm")) {
            ...
            st.set("temp", "0");
            st.set("mixing", "0");
            st.doGiveItems(6317, 1);
            st.doGiveItems(5904, 1);
            ...
            if (s_event[1].equals("M")) {
                if (s_event[2].equals("Insert")) {
                    if (st.getQuestItemsCount(MIXING_STONE1) != 0) {
                        st.doTakeItems(MIXING_STONE1, -1);
                        st.set("mixing", "1");
                    ...
                    if (st.getInt("mixing") != 0) {
                        st.set("mixing", "0");
                        st.set("temp", "0");
                        st.doGiveItems(MIXING_STONE1, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        for (int[] i : DROPLIST_COND)
            if (npcId == i[2])
                st.doGiveOnKillItems(i[4], i[7], i[7], i[6]);
        return null;
    }
}