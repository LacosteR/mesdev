...
public class _421_LittleWingAdventures extends Quest implements ScriptFile {
    ...
    // Items
    private static int Dragonflute_of_Wind = PetDataHolder.getInstance().getControlItemId("hatchling_of_wind");
    private static int Dragonflute_of_Star = PetDataHolder.getInstance().getControlItemId("hatchling_of_star");
    private static int Dragonflute_of_Twilight = PetDataHolder.getInstance().getControlItemId("hatchling_of_twilight");
    private static int Dragon_Bugle_of_Wind = PetDataHolder.getInstance().getControlItemId("wind_strider");
    private static int Dragon_Bugle_of_Star = PetDataHolder.getInstance().getControlItemId("star_strider");
    private static int Dragon_Bugle_of_Twilight = PetDataHolder.getInstance().getControlItemId("twilight_strider");
    // Quest Items
    ...
    private static boolean HatchlingSummoned(QuestState st, boolean CheckObjID) {
        ...
        if (dragonflute == null)
            return false;
        if (PetDataHolder.getInstance().getControlItemId(_pet.getNpcId()) != dragonflute.getItemId())
            return false;
        return true;
    }
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30610_05.htm") && _state == CREATED) {
            ...
            if (dragonflute.getObjectId() != dragonflute_id) {
                if (Rnd.chance(10)) {
                    st.doTakeItems(dragonflute.getItemId(), 1);
            ...
            if (event.equalsIgnoreCase("30747_04.htm")) {
                st.setCond(2);
                st.doTakeItems(Fairy_Leaf, -1);
                st.doGiveItems(Fairy_Leaf, 4);
    ...
    /*
     * благодаря skills.Quest421FairyTree вызовется только при атаке от L2PetInstance
     */
    @Override
