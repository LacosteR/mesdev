...
public class _648_AnIceMerchantsDream extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("repre_q0648_14.htm")) {
            ...
            if (reward > 0) {
                st.doTakeItems(Silver_Ice_Crystal, -1);
                st.doTakeItems(Black_Ice_Crystal, -1);
                st.doGiveItems(ADENA_ID, RATE_QUESTS_REWARD * reward);
            } else
            ...
            if (Rnd.chance(Silver2Black_Chance)) {
                st.doGiveItems(Black_Ice_Crystal, 1);
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        int cond = qs.getCond();
        if (cond > 0) {
            qs.doGiveOnKillItems(Silver_Ice_Crystal, 1, 1, npc.getNpcId() - 22050);
            if (cond == 2)
                qs.doGiveOnKillItems(Silver_Hemocyte, 1, 1, Silver_Hemocyte_Chance);
        }
