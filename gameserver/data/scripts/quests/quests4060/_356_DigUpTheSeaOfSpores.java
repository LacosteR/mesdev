...
public class _356_DigUpTheSeaOfSpores extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("magister_gauen_q0356_06.htm")) {
            ...
            st.doTakeItems(CARNIVORE_SPORE, -1);
            st.doTakeItems(HERBIBOROUS_SPORE, -1);
            st.playSound(SOUND_FINISH);
            st.exitCurrentQuest(true);
            if (event.equalsIgnoreCase("magister_gauen_q0356_17.htm"))
                st.doGiveItems(ADENA_ID, 20950);
            else
                st.addExpAndSp(RATE_QUESTS_REWARD * 45500, RATE_QUESTS_REWARD * 2600);
        }
        return htmltext;
    }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == SPORE_ZOMBIE) {
            if (st.getQuestItemsCount(CARNIVORE_SPORE) < 50) {
                st.doGiveItems(CARNIVORE_SPORE, 1);
                if (st.getQuestItemsCount(CARNIVORE_SPORE) >= 50) {
            ...
            if (st.getQuestItemsCount(HERBIBOROUS_SPORE) < 50) {
                st.doGiveItems(HERBIBOROUS_SPORE, 1);
                if (st.getQuestItemsCount(HERBIBOROUS_SPORE) >= 50) {
                    st.playSound(SOUND_MIDDLE);
                    if (st.getQuestItemsCount(CARNIVORE_SPORE) >= 50) {
                        st.setCond(3);
