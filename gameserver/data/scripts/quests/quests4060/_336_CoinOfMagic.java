...
public class _336_CoinOfMagic extends Quest implements ScriptFile {
    ...
    private String promote(QuestState st) {
        ...
        if (grade == 1)
            ...
            if (h == 6) {
                ...
                st.doTakeItems(3812 + grade, -1);
                st.doGiveItems(3811 + grade, 1);
        ...
        if (npcId == SORINT) {
            if (id == CREATED) {
                ...
                if (st.getQuestItemsCount(KALDIS_COIN) > 0) {
                    st.doTakeItems(KALDIS_COIN, -1);
                    st.doTakeItems(COIN_DIAGRAM, -1);
                    st.doGiveItems(MEMBERSHIP_3, 1);
                    st.set("grade", "3");
                    st.setCond(4);
                    st.playSound(SOUND_FANFARE_MIDDLE);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == HaritLizardmanMatriarch || npcId == HaritLizardmanShaman) {
            if (cond == 2)
                if (st.doGiveOnKillItems(KALDIS_COIN, 1, RATE_QUESTS_DROP * 1, RATE_QUESTS_DROP * 1, RATE_QUESTS_DROP * 10 * npc.getTemplate().rateHp))
                    st.setCond(3);
            return null;
        }
        for (int[] e : DROPLIST)
            if (e[0] == npcId) {
                st.doGiveOnKillItems(e[1], 1, 1, chance);
                return null;
            }
        for (int u : UNKNOWN)
            if (u == npcId) {
                st.doGiveOnKillItems(BASIC_COINS[Rnd.get(BASIC_COINS.length)], 1, 1, chance * npc.getTemplate().rateHp);
                return null;
            }
        return null;
    }
