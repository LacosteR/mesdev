...
public class _338_AlligatorHunter extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30892-02.htm")) {
            ...
            st.doTakeItems(AlligatorLeather, -1);
            st.doGiveItems(ADENA_ID, AdenaCount);
            ...
            if (cond == DROPLIST_COND[i][0] && npcId == DROPLIST_COND[i][2])
                if (DROPLIST_COND[i][3] == 0 || st.getQuestItemsCount(DROPLIST_COND[i][3]) > 0)
                    if (DROPLIST_COND[i][5] == 0)
                        st.doGiveOnKillItems(DROPLIST_COND[i][4], RATE_QUESTS_DROP * DROPLIST_COND[i][7], RATE_QUESTS_DROP * DROPLIST_COND[i][7], RATE_QUESTS_DROP * DROPLIST_COND[i][6]);
                    else if (st.doGiveOnKillItems(DROPLIST_COND[i][4], DROPLIST_COND[i][7], RATE_QUESTS_DROP * DROPLIST_COND[i][7], DROPLIST_COND[i][5], RATE_QUESTS_DROP * DROPLIST_COND[i][6]))
