...
public class _343_UndertheShadowoftheIvoryTower extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30834-03.htm")) {
            ...
            if (orbs > 0) {
                st.doGiveItems(ADENA_ID, orbs * 120);
                st.doTakeItems(ORB, -1);
            } else
            ...
            if (st.getInt("playing") > 0) {
                if (random1 == 0) {
                    ...
                    st.doGiveItems(ORB, 10);
                } else if (random1 == 1)
                    htmltext = "30934-06.htm";
                else {
                    htmltext = "30934-04.htm";
                    st.doGiveItems(ORB, 20);
                }
                st.unset("playing");
            } else {
            ...
            if (orbs >= 10) {
                ...
                if (toss == 1)
                    ...
                    st.doGiveItems(ORB, 30);
                else if (toss == 3)
                    st.doGiveItems(ORB, 70);
                else if (toss == 4)
                    st.doGiveItems(ORB, 150);
            } else
                htmltext = "noorbs.htm";
            ...
            if (st.getQuestItemsCount(ECTOPLASM) > 0) {
                ...
                if (random <= 119)
                    st.doGiveItems(955, 1);
                else if (random <= 169)
                    st.doGiveItems(951, 1);
                else if (random <= 329)
                    st.doGiveItems(2511, Rnd.get(200) + 401);
                else if (random <= 559)
                    st.doGiveItems(2510, Rnd.get(200) + 401);
                else if (random <= 561)
                    st.doGiveItems(316, 1);
                else if (random <= 578)
                    st.doGiveItems(630, 1);
                else if (random <= 579)
                    st.doGiveItems(188, 1);
                else if (random <= 581)
                    st.doGiveItems(885, 1);
                else if (random <= 582)
                    st.doGiveItems(103, 1);
                else if (random <= 584)
                    st.doGiveItems(917, 1);
                else
                    st.doGiveItems(736, 1);
            } else
        ...
        if (npcId == CEMA) {
            if (id != STARTED) {
                for (int i : AllowClass)
                    if (st.getPlayer().getPcClass().class_id == i && st.getPlayer().getLevel() >= 40)
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        st.doGiveOnKillItems(ORB, 1, 1, CHANCE);
        return null;
    }
}