...
public class _185_NikolasCooperationConsideration extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30621-01.htm")) {
            ...
            st.setCond(1);
            st.doGiveItems(NicolasMap, 1);
        } else if (event.equalsIgnoreCase("30673-03.htm")) {
            st.playSound(SOUND_MIDDLE);
            st.setCond(2);
            st.doTakeItems(NicolasMap, -1);
            ...
            if (st.getQuestItemsCount(BrokenMetal) > 0)
                ...
                st.doGiveItems(Certificate, 1);
            st.doRewardItems(ADENA_ID, 72527);
            st.addExpAndSp(RATE_QUESTS_REWARD * 203717, RATE_QUESTS_REWARD * 14032);
            ...
            st.playSound(SOUND_MIDDLE);
            st.setCond(5);
            st.doGiveItems(BrokenMetal, 1);
        } else if (event.equalsIgnoreCase("32366-06.htm")) {
            st.unset("step");
            st.playSound(SOUND_MIDDLE);
            st.setCond(4);
            st.doGiveItems(Metal, 1);
