...
public abstract class SagasSuperclass extends Quest {
    ...
    private void FinishQuest(QuestState st, Player player) {
        st.addExpAndSp(RATE_QUESTS_REWARD * 2299404, 0);
        st.doRewardItems(ADENA_ID, 5000000);
        st.doGiveItems(6622, RATE_QUESTS_REWARD);
        st.exitCurrentQuest(true);
        player.setClassId(getPcClass(player), false, true);
        if (!player.isSubClassActive() && player.getBaseClassId() == getPrevClass(player))
            player.setBaseClass(getPcClass(player));
    ...
    protected int getPcClass(Player player) {
        return classid;
    }
    ...
    public void giveHallishaMark(QuestState st) {
        ...
        if (st.getQuestItemsCount(Items[3]) < 700)
            st.doGiveOnKillItems(Items[3], 1, 4, 700, 100);
        else {
            st.doTakeItems(Items[3], 20);
            NpcInstance Archon = spawn(Mob[1], st.getPlayer().getPositionComponent().getLoc());
    ...
    public static QuestState findQuest(Player player) {
        ...
        for (Integer q : Quests.keySet()) {
            ...
            if (st != null) {
                ...
                for (int c : qc)
                    if (player.getPcClass().class_id == c)
                        return st;
            }
        }
        return null;
    ...
    public static void process_step_15to16(QuestState st) {
        ...
        st.doTakeItems(Halishas_Mark, -1);
        if (st.getQuestItemsCount(Resonance_Amulet) == 0)
            st.doGiveItems(Resonance_Amulet, 1);
        st.setCond(16);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("0-011.htm") || event.equalsIgnoreCase("0-012.htm") || event.equalsIgnoreCase("0-013.htm") || event.equalsIgnoreCase("0-014.htm") || event.equalsIgnoreCase("0-015.htm"))
            ...
            st.setCond(4);
            st.doTakeItems(Items[0], 1);
            if (Items[11] != 0)
                st.doTakeItems(Items[11], 1);
            st.doGiveItems(Items[1], 1);
            htmltext = "1-06.htm";
            ...
            st.setCond(5);
            st.doTakeItems(Items[1], 1);
            st.doGiveItems(Items[4], 1);
            htmltext = "2-06.htm";
            ...
            st.setCond(13);
            st.doTakeItems(Items[2], 1);
            st.doGiveItems(Items[7], 1);
            htmltext = "3-08.htm";
        } else if (event.equalsIgnoreCase("4-1"))
            htmltext = "4-010.htm";
        else if (event.equalsIgnoreCase("4-2")) {
            st.doGiveItems(Items[9], 1);
            st.setCond(18);
            st.playSound(SOUND_MIDDLE);
            htmltext = "4-011.htm";
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        int cond = st.getCond();
        Player player = st.getPlayer();
        if (player.getPcClass().class_id != getPrevClass(player)) {
            st.exitCurrentQuest(true);
            return htmltext;
        }
        ...
        if (cond == 0) {
            ...
            if (npcId == NPC[0])
                if (player.getLevel() >= 76) {
                    htmltext = "0-09.htm";
                    if (getPcClass(player) < 131 || getPcClass(player) > 135)
                        FinishQuest(st, player);
                } else
                    htmltext = "0-010.htm";
        return htmltext;
        ...
        if (isArchonMinions(npcId)) {
            ...
            if (party != null) {
                for (Player player1 : party.getPartyMembers())
                    if (PositionUtils.getDistance(player, player1) <= ConfigOthers.PARTY_DISTRIBUTION_RANGE) {
            ...
            if (st1 != null)
                if (st1.getCond() == 6)
                    if (st1.getInt("kills") < 9)
                        ...
                        st1.doGiveItems(((SagasSuperclass) st1.getQuest()).Items[5], 1);
                        st1.setCond(7);
                    }
        } else {
            int cond = st.getCond();
