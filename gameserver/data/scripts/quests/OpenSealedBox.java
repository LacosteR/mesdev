...
import quests.quests6076._620_FourGoblets;
...
public class OpenSealedBox {
    ...
    public String apply() {
        if (takecount > 0) {
            ...
            st.doTakeItems(_620_FourGoblets.Sealed_Box, takecount);
            for (Integer itemId : rewards.keySet())
                st.doGiveItems(itemId, rewards.get(itemId)); //не применять рейты тут
        }
