...
public class _309_ForAGoodCause extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("32647-05.htm")) {
            ...
            if (st.getQuestItemsCount(MucrokianHide) >= 180) {
                st.doTakeItems(MucrokianHide, 180);
                st.doGiveItems(MoiraiRecipes[Rnd.get(MoiraiRecipes.length - 1)], 1);
                return null;
            } else
                htmltext = "32646-14.htm";
        } else if (event.equalsIgnoreCase("moiraimat")) {
            if (st.getQuestItemsCount(MucrokianHide) >= 100) {
                st.doTakeItems(MucrokianHide, 100);
                st.doGiveItems(Moiraimaterials[Rnd.get(Moiraimaterials.length - 1)], 1);
                return null;
            } else
                htmltext = "32646-14.htm";
        }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Atra)
            if (id == CREATED) {
                ...
                st.doTakeItems(FallenMucrokianHide, -1);
                if (fallen > 0)
                    st.doGiveItems(MucrokianHide, fallen * 2);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        st.doGiveOnKillItems(npc.getNpcId() == ContaminatedMucrokian ? FallenMucrokianHide : MucrokianHide, 1, 1, 60);
        return null;
    }
}