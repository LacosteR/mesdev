...
public class _311_ExpulsionOfEvilSpirits extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("chairen_q311_03.htm")) {
            ...
            if (st.getQuestItemsCount(SoulCoreContainingEvilSpirit) >= 10) {
                st.doTakeItems(SoulCoreContainingEvilSpirit, 10);
                st.doGiveItems(ProtectionSoulsPendant, 1);
            ...
            if (id > 0) {
                ...
                switch (id) {
                    ...
                    case 9628:
                        count = 24;
                        reward = Rnd.get(1, RATE_QUESTS_REWARD); // Leonard
                        break;
                    case 9629:
                        count = 43;
                        reward = Rnd.get(1, RATE_QUESTS_REWARD); // Adamantite
                        break;
                    case 9630:
                        count = 36;
                        reward = Rnd.get(1, RATE_QUESTS_REWARD); // Orichalcum
                        break;
                    case 9625:
                        count = 667;
                        reward = 1;
                ...
                if (count > 0) {
                    if (st.getQuestItemsCount(RagnaOrcAmulet) >= count) {
                        st.doGiveItems(id, reward);
                        st.doTakeItems(RagnaOrcAmulet, count);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1 && ArrayUtils.contains(MOBS, npcId)) {
            if (st.doGiveOnKillItems(SoulCoreContainingEvilSpirit, 1, 1, 10, DROP_CHANCE1, false))
                st.playSound(SOUND_FANFARE2);

            st.doGiveOnKillItems(RagnaOrcAmulet, 1, 1, DROP_CHANCE2);
        }
        return null;
    }
