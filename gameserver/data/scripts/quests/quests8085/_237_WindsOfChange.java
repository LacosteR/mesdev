...
public class _237_WindsOfChange extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30899-06.htm")) {
            st.doGiveItems(FlauensLetter, 1);
            st.setCond(1);
            st.setState(STARTED);
            ...
            st.doGiveItems(LetterToHelvetica, 1);
            st.setCond(5);
        } else if (event.equalsIgnoreCase("30969-10.htm")) {
            st.doGiveItems(LetterToAthenia, 1);
            st.setCond(6);
        } else if (event.equalsIgnoreCase("32641-02.htm")) {
            st.doTakeItems(LetterToHelvetica, -1);
            st.doRewardItems(ADENA_ID, 213876);
            st.addExpAndSp(RATE_QUESTS_REWARD * 892773, RATE_QUESTS_REWARD * 60012);
            st.doGiveItems(VicinityOfTheFieldOfSilenceResearchCenter, 1);
            st.setState(COMPLETED);
            st.exitCurrentQuest(false);
        } else if (event.equalsIgnoreCase("32643-02.htm")) {
            st.doTakeItems(LetterToAthenia, -1);
            st.doRewardItems(ADENA_ID, 213876);
            st.addExpAndSp(892773, 60012);
            st.doGiveItems(CertificateOfSupport, 1);
            st.setState(COMPLETED);
            st.exitCurrentQuest(false);
        }
        return htmltext;
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Flauen) {
            ...
            if (cond == 1) {
                st.doTakeItems(FlauensLetter, -1);
