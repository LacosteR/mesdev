...
public class _903_TheCallofAntharas extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("theodric_q903_03.htm")) {
            ...
            st.doTakeItems(BehemothDragonLeather);
            st.doTakeItems(TaraskDragonsLeatherFragment);
            st.doGiveItems(21897, 1); // Scroll: Antharas Call
            ...
            switch (npc.getNpcId()) {
                case TaraskDragon:
                    if (st.getQuestItemsCount(TaraskDragonsLeatherFragment) < 1)
                        st.doGiveItems(TaraskDragonsLeatherFragment, 1);
                    break;
                case BehemothDragon:
                    if (st.getQuestItemsCount(BehemothDragonLeather) < 1)
                        st.doGiveItems(BehemothDragonLeather, 1);
                    break;
                default:
                    break;
            }
