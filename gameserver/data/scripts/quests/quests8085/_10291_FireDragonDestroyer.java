...
public class _10291_FireDragonDestroyer extends Quest implements ScriptFile {
        ...
        if (npcId == Klein) {
            if (cond == 0) {
                ...
                if (st.getQuestItemsCount(ValorNecklace) >= 1) {
                    ...
                    st.doTakeItems(ValorNecklace);
                    st.doGiveItems(8567, 1);
                    st.doRewardItems(ADENA_ID, 126549);
                    st.addExpAndSp(RATE_QUESTS_REWARD * 717291, RATE_QUESTS_REWARD * 77397);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1 && npcId == Valakas) {
            st.doTakeItems(PoorNecklace);
            st.doGiveItems(ValorNecklace, 1);
            st.setCond(2);
        }
        return null;
    }
