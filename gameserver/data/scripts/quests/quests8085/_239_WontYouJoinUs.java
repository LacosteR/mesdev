...
public class _239_WontYouJoinUs extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("32643-07.htm")) {
            st.doTakeItems(DestroyedMachinePiece, -1);
            st.setCond(3);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Athenia)
            if (id == CREATED) {
                ...
                st.doTakeItems(CertificateOfSupport, -1);
                st.doTakeItems(EnchantedGolemFragment, -1);
                st.doRewardItems(ADENA_ID, 283346);
