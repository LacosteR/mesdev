...
public class _906_TheCallofValakas extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("klein_q906_04.htm")) {
            ...
            st.doTakeItems(LavasaurusAlphaFragment);
            st.doGiveItems(21895, 1); // Scroll: Valakas Call
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        int cond = st.getCond();
        if (cond == 1) {
            if (npc.getNpcId() == ValakasMinion && st.doGiveOnKillItems(LavasaurusAlphaFragment, 1, 1, 40))
                st.setCond(2);
        }
        return null;
    }
