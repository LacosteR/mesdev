...
public class _238_SuccessFailureOfBusiness extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("32641-06.htm")) {
            st.doTakeItems(BrokenPieveOfMagicForce, -1);
            st.setCond(3);
        }
        return htmltext;
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Helvetica)
            if (id == CREATED) {
                ...
                st.doTakeItems(VicinityOfTheFieldOfSilenceResearchCenter, -1);
                st.doTakeItems(GuardianSpiritFragment, -1);
                st.doGiveItems(ADENA_ID, RATE_QUESTS_REWARD * 283346);
