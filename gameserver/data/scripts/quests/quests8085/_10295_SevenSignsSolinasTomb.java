...
public class _10295_SevenSignsSolinasTomb extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("eris_q10295_5.htm")) {
            ...
            if (st.getQuestItemsCount(StaffofBlessing) > 0) {
                st.doTakeItems(StaffofBlessing);
            ...
            if (st.getQuestItemsCount(ScrollofAbstinence) > 0) {
                st.doTakeItems(ScrollofAbstinence);
                // TODO: remove glow from NPC
            ...
            if (st.getQuestItemsCount(SwordofHolySpirit) > 0) {
                st.doTakeItems(SwordofHolySpirit);
                // TODO: remove glow from NPC
            ...
            if (st.getQuestItemsCount(ShieldofSacrifice) > 0) {
                st.doTakeItems(ShieldofSacrifice);
                // TODO: remove glow from NPC
            ...
            if (!player.getPositionComponent().getReflection().getDoor(21100101).isOpen())
        ...
        if (npcId == ErisEvilThoughts) {
            if (cond == 0) {
                ...
                if (player.getLevel() >= 81) {
                    ...
                    st.addExpAndSp(ConfigQuests.SEVEN_SIGN_QUEST_RATE * 125000000, ConfigQuests.SEVEN_SIGN_QUEST_RATE * 12500000);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (ArrayUtils.contains(TombGuardians, npcId)) {
            if (checkGuardians(player, TombGuardians))
                player.getPositionComponent().getReflection().openDoor(21100018);
            switch (npcId) {
                case 18956:
                    player.getPositionComponent().getReflection().despawnByGroup("tombguards3");
                    break;
                case 18957:
                    player.getPositionComponent().getReflection().despawnByGroup("tombguards2");
                    break;
                case 18958:
                    player.getPositionComponent().getReflection().despawnByGroup("tombguards1");
                    break;
                case 18959:
                    player.getPositionComponent().getReflection().despawnByGroup("tombguards4");
                    break;
            }
        }
        return null;
    ...
    private void teleportElcardia(Player player) {
        for (NpcInstance n : player.getPositionComponent().getReflection().getNpcs())
    ...
    private void activateTombGuards(Player player) {
        Reflection r = player.getPositionComponent().getReflection();
