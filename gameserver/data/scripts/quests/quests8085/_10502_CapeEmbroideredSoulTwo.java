...
public class _10502_CapeEmbroideredSoulTwo extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            if (st.getQuestItemsCount(SOUL_FREYA) < 20) {
                ...
                st.doTakeItems(SOUL_FREYA, -1);
                st.doGiveItems(CLOAK_FREYA, 1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1 && (npcId == FREYA_NORMAL || npcId == FREYA_HARD)) {
            if (st.getQuestItemsCount(SOUL_FREYA) < 20)
                st.doGiveItems(SOUL_FREYA, Rnd.get(ConfigQuests.CLOAK_QUEST_RATE, ConfigQuests.CLOAK_QUEST_RATE * 3));
            if (st.getQuestItemsCount(SOUL_FREYA) >= 20) {
                st.setCond(2);
