            ...
            switch (st.getState()) {
                ...
                case STARTED:
                    if (cond == 1)
                        ...
                        st.doTakeItems(ShoesStringofSelMahum);
                        st.doTakeItems(ShinySalmon);
                        st.addExpAndSp(RATE_QUESTS_REWARD * 224784, RATE_QUESTS_REWARD * 342528);
        ...
        if (cond == 1) {
            if (st.getQuestItemsCount(ShoesStringofSelMahum) < 10 && st.getQuestItemsCount(ShinySalmon) < 5) {
                if (st.getQuestItemsCount(ShoesStringofSelMahum) < 10 && ArrayUtils.contains(SelMahums, npcId))
                    st.doGiveOnKillItems(ShoesStringofSelMahum, 1, 1, 20);
                if (st.getQuestItemsCount(ShinySalmon) < 5 && npcId == SelChef)
                    st.doGiveOnKillItems(ShinySalmon, 1, 1, 10);
            } else
                st.setCond(2);
        }
        return null;
