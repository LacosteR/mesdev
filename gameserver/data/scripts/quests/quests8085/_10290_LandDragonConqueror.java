...
public class _10290_LandDragonConqueror extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("theodric_q10290_04.htm")) {
            ...
            st.doGiveItems(ShabbyNecklace, 1);
        }
        return htmltext;
    }
        ...
        if (npcId == Theodric) {
            if (cond == 0) {
                ...
                if (st.getQuestItemsCount(MiracleNecklace) >= 1) {
                    ...
                    st.doTakeItems(MiracleNecklace);
                    st.doGiveItems(8568, 1);
                    st.doRewardItems(ADENA_ID, 131236);
                    st.addExpAndSp(RATE_QUESTS_REWARD * 702557, RATE_QUESTS_REWARD * 76334);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1 && npcId == UltimateAntharas) {
            st.doTakeItems(ShabbyNecklace);
            st.doGiveItems(MiracleNecklace, 1);
            st.setCond(2);
        }
        return null;
    }
