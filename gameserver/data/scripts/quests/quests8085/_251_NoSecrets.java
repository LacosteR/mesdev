    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npc.getNpcId() == GuardPinaps) {
            if (cond == 0) {
                ...
                st.doTakeItems(SelMahumTrainingDiary, -1);
                st.doTakeItems(SelMahumTrainingTimetable, -1);
                htmltext = "pinaps_q251_05.htm";
                st.setState(COMPLETED);
                st.doRewardItems(ADENA_ID, 313355);
                st.addExpAndSp(RATE_QUESTS_REWARD * 56787, RATE_QUESTS_REWARD * 160578);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1) {
            if (st.getQuestItemsCount(SelMahumTrainingDiary) < 10 && ArrayUtils.contains(SelMahumTrainers, npc.getNpcId()))
                st.doGiveOnKillItems(SelMahumTrainingDiary, 1, 1, 10, 40);
            else if (st.getQuestItemsCount(SelMahumTrainingTimetable) < 5 && ArrayUtils.contains(SelMahumRecruits, npc.getNpcId()))
                st.doGiveOnKillItems(SelMahumTrainingTimetable, 1, 1, 5, 25);
