...
public class _10284_AcquisionOfDivineSword extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("rafforty_q10284_02.htm")) {
            ...
            if (!st.getPlayer().getPositionComponent().getReflection().isDefault()) {
                st.getPlayer().getPositionComponent().getReflection().startCollapseTimer(60 * 1000L);
            ...
            st.getPlayer().getPositionComponent().getReflection().collapse();
            return null;
        } else if (event.equalsIgnoreCase("entermines")) {
            st.setCond(4);
            if (st.getQuestItemsCount(ColdResistancePotion) < 1)
                st.doGiveItems(ColdResistancePotion, 1);
            enterInstance(st.getPlayer(), 138);
            return null;
        } else if (event.equalsIgnoreCase("leavemines")) {
            st.doRewardItems(ADENA_ID, 296425);
            st.addExpAndSp(RATE_QUESTS_REWARD * 921805, RATE_QUESTS_REWARD * 82230);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Rafforty) {
            ...
            if (cond == 4) {
                st.doTakeItems(ColdResistancePotion);
                st.setCond(5);
                htmltext = "kegor_q10284_01.htm";
                for (int i = 0; i < 4; i++) {
                    NpcInstance mob = st.getPlayer().getPositionComponent().getReflection().addSpawnWithoutRespawn(MithrilMillipede, Location.findPointToStay(st.getPlayer(), 50, 100), st.getPlayer().getPositionComponent().getGeoIndex());
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 5 && npcId == MithrilMillipede) {
            if (_count < 3)
                ...
                st.setCond(6);
                st.getPlayer().getPositionComponent().getReflection().startCollapseTimer(3 * 60 * 1000L);
