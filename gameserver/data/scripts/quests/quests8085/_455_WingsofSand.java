...
public class _455_WingsofSand extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("sepsoul_q455_05.htm")) {
            ...
            st.doTakeItems(LargeDragon);
            StringTokenizer tokenizer = new StringTokenizer(event);
            tokenizer.nextToken();
            switch (Integer.parseInt(tokenizer.nextToken())) {
                case 1:
                    st.doGiveItems(reward_mats_w[Rnd.get(reward_mats_w.length)], RATE_QUESTS_REWARD * Rnd.get(1, 2));
                    break;
                case 2:
                    st.doGiveItems(reward_mats_a[Rnd.get(reward_mats_a.length)], RATE_QUESTS_REWARD * Rnd.get(1, 2));
                    break;
                case 3:
                    st.doGiveItems(reward_mats_acc[Rnd.get(reward_mats_acc.length)], RATE_QUESTS_REWARD * Rnd.get(1, 2));
                    break;
                case 4:
            ...
            switch (Integer.parseInt(tokenizer.nextToken())) {
                case 1:
                    st.doGiveItems(reward_resipes_w[Rnd.get(reward_resipes_w.length)], 1);
                    break;
                case 2:
                    st.doGiveItems(reward_resipes_a[Rnd.get(reward_resipes_a.length)], 1);
                    break;
                case 3:
                    st.doGiveItems(reward_resipes_acc[Rnd.get(reward_resipes_acc.length)], 1);
                    break;
                case 4:
                    st.doGiveItems(reward_attr_crystal[Rnd.get(reward_attr_crystal.length)], RATE_QUESTS_REWARD * 2);
                    break;
                default:
                    break;
            }
            if (Rnd.chance(25))
                st.doGiveItems(reward_ench_scroll[Rnd.get(reward_ench_scroll.length)], 1);
