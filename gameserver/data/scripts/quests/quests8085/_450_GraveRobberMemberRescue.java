    ...
    public String onTalk(final NpcInstance npc, QuestState st) {
        ...
        if (npcId == KANEMIKA) {
            if (id == CREATED) {
                ...
                st.doRewardItems(ADENA_ID, 65000);
                st.doTakeItems(EVIDENCE_OF_MIGRATION, -1);
                st.exitCurrentQuest(true);
            ...
            if (Rnd.chance(50)) {
                ...
                st.doGiveItems(EVIDENCE_OF_MIGRATION, 1);
                st.playSound(SOUND_ITEMGET);
                npc.moveToLocation(new Location(npc.getPositionComponent().getX() + 200, npc.getPositionComponent().getY() + 200, npc.getPositionComponent().getZ()), 0, false);
                ...
                NpcInstance warrior = st.addSpawn(WARRIOR_MON, npc.getPositionComponent().getX(), npc.getPositionComponent().getY(), npc.getPositionComponent().getZ(), npc.getHeading(), 100, 120000);
