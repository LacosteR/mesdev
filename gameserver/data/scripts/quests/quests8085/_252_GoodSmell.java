...
public class _252_GoodSmell extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("stan_q252_03.htm")) {
            ...
            st.doTakeItems(SelMahumDiary, -1);
            st.doTakeItems(SelMahumCookbookPage, -1);
            st.setState(COMPLETED);
            st.doRewardItems(ADENA_ID, 147656);
            st.addExpAndSp(RATE_QUESTS_REWARD * 716238, RATE_QUESTS_REWARD * 78324);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1) {
            if (st.getQuestItemsCount(SelMahumDiary) < 10 && ArrayUtils.contains(SelMahums, npc.getNpcId()))
                st.doGiveOnKillItems(SelMahumDiary, 1, 1, 10, 15);
            if (st.getQuestItemsCount(SelMahumCookbookPage) < 5 && npc.getNpcId() == SelChef)
                st.doGiveOnKillItems(SelMahumCookbookPage, 1, 1, 5, 10);
