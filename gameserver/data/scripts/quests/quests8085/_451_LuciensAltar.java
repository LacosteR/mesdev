...
public class _451_LuciensAltar extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        if (event.equalsIgnoreCase("30537-03.htm")) {
            st.setCond(1);
            st.setState(STARTED);
            st.doGiveItems(REPLENISHED_BEAD, 5);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == DAICHIR) {
            if (cond == 0) {
                ...
                st.doRewardItems(ADENA_ID, 127690);
                st.doTakeItems(DISCHARGED_BEAD, -1);
                st.exitCurrentQuest(true);
    ...
    private void onAltarCheck(QuestState st) {
        st.doTakeItems(REPLENISHED_BEAD, 1);
        st.doGiveItems(DISCHARGED_BEAD, 1);
