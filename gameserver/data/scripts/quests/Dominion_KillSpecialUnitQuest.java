...
public abstract class Dominion_KillSpecialUnitQuest extends Quest implements ScriptFile {
    private final PcClass[] _classIds;

    public Dominion_KillSpecialUnitQuest() {
        super(PARTY_ALL);

        _classIds = getTargetPcClasss();
        DominionSiegeRunnerEvent runnerEvent = EventHolder.getInstance().getEvent(EventType.MAIN_EVENT, 1);
        for (PcClass c : _classIds)
    ...
    protected abstract PcClass[] getTargetPcClasss();
    ...
    public String onKill(Player killed, QuestState qs) {
        ...
        if (!ArrayUtils.contains(_classIds, killed.getPcClass()))
            return null;
