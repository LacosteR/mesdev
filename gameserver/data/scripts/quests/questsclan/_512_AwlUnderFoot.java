...
public class _512_AwlUnderFoot extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (st.getQuestItemsCount(FragmentOfTheDungeonLeaderMark) > 0) {
            st.doGiveItems(KnightsEpaulette, RATE_QUESTS_REWARD * st.getQuestItemsCount(FragmentOfTheDungeonLeaderMark));
            st.doTakeItems(FragmentOfTheDungeonLeaderMark, -1);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        for (Prison prison : _prisons.values())
            if (prison.getReflectionId() == npc.getPositionComponent().getReflectionId()) {
                switch (npc.getNpcId()) {
                        ...
                        if (party != null)
                            for (Player member : party.getPartyMembers()) {
                                ...
                                if (qs != null && qs.isStarted()) {
                                    qs.doGiveItems(FragmentOfTheDungeonLeaderMark, RewardMarksCount / party.getMemberCount());
                                    qs.playSound(SOUND_ITEMGET);
                                    qs.getPlayer().sendPacket(new SystemMessage(SystemMessage.THIS_DUNGEON_WILL_EXPIRE_IN_S1_MINUTES).addNumber(5));
                                }
                            }
                        else {
                            st.doGiveItems(FragmentOfTheDungeonLeaderMark, RewardMarksCount);
    ...
    private String enterPrison(Player player) {
        ...
        if (player.canEnterInstance(INSTANCE_ZONE_ID)) {
            ...
            r.setReturnLoc(player.getPositionComponent().getLoc());

            for (Player member : player.getParty().getPartyMembers()) {
                if (member != player)
                    newQuestState(member, STARTED);
                member.getPositionComponent().setReflection(r);
