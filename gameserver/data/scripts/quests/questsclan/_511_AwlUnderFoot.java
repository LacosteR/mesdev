...
public class _511_AwlUnderFoot extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (st.getQuestItemsCount(DungeonLeaderMark) > 0) {
            st.doGiveItems(KnightsEpaulette, RATE_QUESTS_REWARD * st.getQuestItemsCount(DungeonLeaderMark));
            st.doTakeItems(DungeonLeaderMark, -1);
                        ...
                        if (party != null)
                            for (Player member : party.getPartyMembers()) {
                                ...
                                if (qs != null && qs.isStarted()) {
                                    qs.doGiveItems(DungeonLeaderMark, RewardMarksCount / party.getMemberCount());
                                    qs.playSound(SOUND_ITEMGET);
                                    qs.getPlayer().sendPacket(new SystemMessage(SystemMessage.THIS_DUNGEON_WILL_EXPIRE_IN_S1_MINUTES).addNumber(5));
                                }
                            }
                        else {
                            st.doGiveItems(DungeonLeaderMark, RewardMarksCount);
