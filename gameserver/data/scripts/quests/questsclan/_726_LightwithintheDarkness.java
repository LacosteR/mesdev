...
public class _726_LightwithintheDarkness extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equals("dcw_q726_4.htm") && cond == 0) {
            ...
            st.doGiveItems(KnightsEpaulette, RATE_QUESTS_REWARD * 152);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 1 && npcId == KanadisGuide3 && checkAllDestroyed(KanadisGuide3, player.getPositionComponent().getReflectionId())) {
            if (player.isInParty())
            ...
            player.getPositionComponent().getReflection().startCollapseTimer(5 * 60 * 1000L);
        }
        return null;
    }
    ...
    private static boolean checkAllDestroyed(int mobId, int refId) {
        for (NpcInstance npc : GameObjectsStorage.getAllByNpcId(mobId, true))
            if (npc.getPositionComponent().getReflectionId() == refId)
                return false;
        return true;
    }
