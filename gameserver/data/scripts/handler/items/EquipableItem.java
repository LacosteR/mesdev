...
public class EquipableItem extends ScriptItemHandler {
    ...
    public boolean useItem(Playable playable, ItemInstance item, boolean ctrl) {
        ...
        if (player.isCastingNow()) {
            ...
            player.getAI().setIntention(CtrlIntention.AI_INTENTION_EQUIP, item);
            return false;
        }
        ...
        if ((bodyPart == ItemTemplate.SLOT_LR_HAND || bodyPart == ItemTemplate.SLOT_R_HAND) && player.isWeaponEquipBlocked()) {
            player.sendPacket(new SystemMessage(SystemMessage.S1_CANNOT_BE_USED_DUE_TO_UNSUITABLE_TERMS).addItemName(item.getItemId()));
            return false;
        }

        if (bodyPart == ItemTemplate.SLOT_LR_HAND || bodyPart == ItemTemplate.SLOT_L_HAND || bodyPart == ItemTemplate.SLOT_R_HAND) {
            if (player.isMounted() || (player.getActiveWeaponInstance() != null && player.getActiveWeaponInstance().isCombatFlag()) || player.isCursedWeaponEquipped() || player.getActiveWeaponFlagAttachment() != null || player.isClanAirShipDriver()) {
                player.sendPacket(new SystemMessage(SystemMessage.S1_CANNOT_BE_USED_DUE_TO_UNSUITABLE_TERMS).addItemName(item.getItemId()));
                return false;
            }
        }
        ...
        // Hero Selled items нельзя надевать, если вы не хиро.
        if (item.getTemplate().isHeroSelledItem() && !player.isHero()) {
            player.sendActionFailed();
            return false;
        }
