...
public class AlfredVonHellmann extends SiegeGuardFighter {
    ...
    public void onEvtAttacked(Creature attacker, int damage, Skill skill) {
        ...
        if (PositionUtils.getDistance(attacker, actor) > 300. && Rnd.chance(0.13))
            addUseSkillDesire(attacker, DRAIN_SKILL, 10000);

        Creature target = actor.getAggroList().getMostHated();
        if (target == attacker && Rnd.chance(0.3))
            addUseSkillDesire(attacker, DAMAGE_SKILL, 10000);
    }
}
