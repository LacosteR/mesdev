...
public abstract class MatchFighter extends Fighter {
    ...
    protected boolean thinkActive() {
        ...
        if (now - _checkAggroTimestamp > 1000L) {
    ...
    protected boolean checkAggression(Creature target) {
        ...
        if (target.isAlikeDead() || target.isDamageBlockedHp())
            return false;
    ...
    public void onEvtSpawn() {
        ...
        int x = (int) (actor.getPositionComponent().getX() + 800 * Math.cos(actor.headingToRadians(actor.getHeading() - 32768)));
        int y = (int) (actor.getPositionComponent().getY() + 800 * Math.sin(actor.headingToRadians(actor.getHeading() - 32768)));

        actor.setSpawnedLoc(new Location(x, y, actor.getPositionComponent().getZ()));
        addMoveToDesire(actor.getSpawnedLoc(), true, 5);
        doTask();
    }
