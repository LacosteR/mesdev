...
public abstract class SiegeGuard extends DefaultAI {
    ...
    protected boolean checkAggression(Creature target) {
        ...
        if (ai != null && ai.hate > 0) {
            if (!PositionUtils.isInRange(target, actor.getSpawnedLoc(), MAX_PURSUE_RANGE, true))
                return false;
        } else if (!PositionUtils.isInRange(target, actor.getSpawnedLoc(), 600, true))
            return false;
    ...
    protected boolean thinkActive() {
        ...
        if (!PositionUtils.isInRange(actor, sloc, 250, false)) {
            teleportHome();
            return true;
        }
