...
public class CoralGardenGolem extends DefaultAI {
    ...
    protected boolean thinkActive() {
        ...
        if (!actor.isMoving && !trapped) {
            ...
            if (closestItem != null)
                actor.moveToLocation(closestItem.getPositionComponent().getLoc(), 0, true);
        }
        return false;
    }
    ...
    protected void onEvtArrived() {
        ...
        if (!trapped && (actor.isInZone("[cry_cav_cor_gar_golem_trap_1]") || actor.isInZone("[cry_cav_cor_gar_golem_trap_2]"))) {
            ...
            if (!actor.getPositionComponent().getReflection().isDefault() && actor.getPositionComponent().getReflection().getInstancedZoneId() == 10)
                ((CrystalCaverns) actor.getPositionComponent().getReflection()).notifyGolemTrapped();
        }
    }
