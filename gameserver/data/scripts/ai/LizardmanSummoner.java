        ...
        if (!spawnedMobs && attacker.isPlayable()) {
            ...
            for (int i = 0; i < SPAWN_COUNT; i++) {
                try {
                    ...
                    int x = (int) (actor.getPositionComponent().getX() + 80 * Math.cos(actor.headingToRadians(actor.getHeading() - 32768 + radius)));
                    int y = (int) (actor.getPositionComponent().getY() + 80 * Math.sin(actor.headingToRadians(actor.getHeading() - 32768 + radius)));
                    sp.setLoc(actor.getPositionComponent().getLoc());
