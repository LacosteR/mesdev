...
public class FreyaStandNormal extends Fighter {
    ...
    protected void thinkAttack() {
        ...
        if (!actor.isCastingNow() && !actor.isMoving && _iceballReuseTimer < System.currentTimeMillis()) {
            if (topDamager != null && !topDamager.isDead() && PositionUtils.isInRange(topDamager, actor, 1000, true)) {
        ...
        if (!actor.isCastingNow() && !actor.isMoving && _deathsentenceReuseTimer < System.currentTimeMillis()) {
            if (randomHated != null && !randomHated.isDead() && PositionUtils.isInRange(randomHated, actor, 1000, true)) {
    ...
    protected boolean thinkActive() {
        ...
        Reflection ref = getActor().getPositionComponent().getReflection();
        if (!getActor().isDead() && _idleDelay > 0 && _idleDelay + 60000 < System.currentTimeMillis())
            if (!ref.isDefault()) {
                for (Player p : ref.getPlayers())
                    p.sendMessage(new CustomMessage("scripts.skills.freya.FreyaFailure", p));
                ref.collapse();
            }
