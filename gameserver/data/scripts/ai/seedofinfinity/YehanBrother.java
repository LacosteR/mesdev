...
public class YehanBrother extends Fighter {
    ...
    protected void thinkAttack() {
        ...
        if (!brother.isDead() && !PositionUtils.isInRange(actor, brother, 300, false))
            actor.altOnMagicUseTimer(getActor(), SkillTable.getInstance().getInfo(6371, 1));
        else
            removeInvul(actor);
        if (_spawnTimer + 40000 < System.currentTimeMillis()) {
            _spawnTimer = System.currentTimeMillis();
            NpcInstance mob = actor.getPositionComponent().getReflection().addSpawnWithoutRespawn(_minions[Rnd.get(_minions.length)], Location.findAroundPosition(actor, 300), 0);
