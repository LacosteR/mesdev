...
public class Kechi extends DefaultAI {
    ...
    final Skill Invincible; // Invincible against general attack and skills, buff/de-buff.
    ...
    protected void onEvtAttacked(Creature attacker, int damage, Skill skill) {
        ...
        if (actor.getPositionComponent().getReflection().getInstancedZoneId() == 10)
            ((CrystalCaverns) actor.getPositionComponent().getReflection()).notifyKechiAttacked();
    ...
    private void spawnMobs() {
        ...
        for (int[] run : guard_run) {
            NpcInstance guard = actor.getPositionComponent().getReflection().addSpawnWithoutRespawn(run[0], guard_spawn_loc, 0);
            Location runLoc = new Location(run[1], run[2], run[3]);

            guard.setRunning();
            DefaultAI ai = (DefaultAI) guard.getAI();

            ai.addMoveToDesire(runLoc, true, 5);
    ...
    protected void onEvtDead(Creature killer) {
        if (getActor().getPositionComponent().getReflection().getInstancedZoneId() == 10)
            ((CrystalCaverns) getActor().getPositionComponent().getReflection()).notifyKechiDead(getActor());
