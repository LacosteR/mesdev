...
public class Darnel extends DefaultAI {
    ...
    protected void onEvtAttacked(Creature attacker, int damage, Skill skill) {
        ...
        if (actor.getPositionComponent().getReflection().getInstancedZoneId() == 10)
            ((CrystalCaverns) actor.getPositionComponent().getReflection()).notifyDarnelAttacked();
    ...
    protected void onEvtDead(Creature killer) {
        if (getActor().getPositionComponent().getReflection().getInstancedZoneId() == 10)
            ((CrystalCaverns) getActor().getPositionComponent().getReflection()).notifyDarnelDead(getActor());
