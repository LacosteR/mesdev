...
public class Valakas extends DefaultAI {
    ...
    protected void onEvtAttacked(Creature attacker, int damage, Skill skill) {
        ...
        if (damage > 100) {
            if (PositionUtils.getDistance(actor, attacker) > 400)
    ...
    protected boolean createNewTask() {
        ...
        if (getAliveMinionsCount() < 100 && Rnd.chance(5)) {
            NpcInstance minion = Functions.spawn(Location.findPointToStay(actor.getPositionComponent().getLoc(), 400, 700, actor.getPositionComponent().getGeoIndex()), 29029);  // Valakas Minions
            minions.add(minion);
