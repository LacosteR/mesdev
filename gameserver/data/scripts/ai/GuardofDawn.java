...
public class GuardofDawn extends DefaultAI {
    ...
    protected boolean thinkActive() {
        ...
        if (!moveToEnd)
            addMoveToDesire(getEndPoint(), true, 5);
        else
            addMoveToDesire(getStartPoint(), true, 5);
        doTask();
    ...
    private boolean checkAroundPlayers(NpcInstance actor) {
        for (Playable target : World.getAroundPlayables(actor, _aggrorange, _aggrorange))
            if (target != null && target.isPlayer() && !target.isDamageBlockedHp() && GeoEngine.canSeeTarget(actor, target, false)) {
