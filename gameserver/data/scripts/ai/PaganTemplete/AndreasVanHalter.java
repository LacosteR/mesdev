...
public class AndreasVanHalter extends Fighter {
    ...
    protected void onEvtAttacked(Creature attacker, int damage, Skill skill) {
        ...
        if (actor.getPositionComponent().getZ() >= -10476)
            actor.teleToLocation(-16393, -53433, -10439);
            // Мы близко к дверям, телепортируемся подальше от них
        else if (PositionUtils.getDistance(actor, door1) <= 200)
            actor.teleToLocation(-15690, -54030, -10439);
        else if (PositionUtils.getDistance(actor, door2) <= 200)
            ...
            switch (_taskId) {
                case 1:
                    ...
                    for (Player player : World.getAroundPlayers(actor)) {
                        if (PositionUtils.getDistance(player, camera) <= _distance) {
