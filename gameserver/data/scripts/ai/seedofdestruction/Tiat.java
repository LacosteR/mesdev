...
public class Tiat extends Fighter {
    ...
    protected void onEvtAttacked(Creature attacker, int damage, Skill skill) {
        ...
        if (_notUsedTransform && actor.getCurrentHpPercents() < 50) {
            ...
            // Transform skills cast [custom: making Tiat invul while casting]
            ...
            ThreadPoolManager.getInstance().schedule(new RunnableImpl() {
                ...
                public void runImpl() throws Exception {
                    ...
                    getActor().stopDamageBlockedHp();
                    getActor().stopDebuffImmunity();
                }
    ...
    protected boolean thinkActive() {
        ...
        if (!_failed && _lastAttackTime != 0 && _lastAttackTime + COLLAPSE_BY_INACTIVITY_INTERVAL < System.currentTimeMillis()) {
            final Reflection r = actor.getPositionComponent().getReflection();
            _failed = true;
    ...
    private void spawnTraps() {
        ...
        for (int i = 0; i < TRAPS_COUNT; i++)
            actor.getPositionComponent().getReflection().addSpawnWithRespawn(TRAP_NPC_ID, TRAP_LOCS[i], 0, 180);
    }
}