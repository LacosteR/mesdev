...
public class ZakenNightly extends Fighter {
        ...
        switch (_stage) {
            case 0:
                if (actor_hp_precent < 90) {
                    r.addSpawnWithoutRespawn(pirates_zombie_captain_b, actor.getPositionComponent().getLoc(), 300);
                    _stage++;
                }
                break;
            case 1:
                if (actor_hp_precent < 80) {
                    r.addSpawnWithoutRespawn(doll_blader_b, actor.getPositionComponent().getLoc(), 300);
                    _stage++;
                }
                break;
            case 2:
                if (actor_hp_precent < 70) {
                    r.addSpawnWithoutRespawn(vale_master_b, actor.getPositionComponent().getLoc(), 300);
                    r.addSpawnWithoutRespawn(vale_master_b, actor.getPositionComponent().getLoc(), 300);
                    _stage++;
                }
                break;
            case 3:
                if (actor_hp_precent < 60) {
                    for (int i = 0; i < 5; i++)
                        r.addSpawnWithoutRespawn(pirates_zombie_b, actor.getPositionComponent().getLoc(), 300);
                    _stage++;
                }
                break;
            case 4:
    ...
    protected void onEvtDead(Creature killer) {
        ...
        actor.broadcastPacket(new PlaySound(PlaySound.Type.MUSIC, "BS02_D", 1, actor.getObjectId(), actor.getPositionComponent().getLoc()));
