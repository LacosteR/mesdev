...
public class Core extends Fighter {
    ...
    protected void onEvtDead(Creature killer) {
        ...
        actor.broadcastPacket(new PlaySound(PlaySound.Type.MUSIC, "BS02_D", 1, 0, actor.getPositionComponent().getLoc()));
        Functions.npcSay(actor, "A fatal error has occurred");
        Functions.npcSay(actor, "System is being shut down...");
        Functions.npcSay(actor, "......");

        try {
            NpcInstance cubic1 = NpcFactory.createNpc(TELEPORTATION_CUBIC_ID);
            cubic1.getPositionComponent().setReflection(actor.getPositionComponent().getReflection());
            cubic1.setCurrentHpMp(cubic1.getMaxHp(), cubic1.getMaxMp(), true);
            cubic1.spawnMe(CUBIC_1_POSITION);

            NpcInstance cubic2 = NpcFactory.createNpc(TELEPORTATION_CUBIC_ID);
            cubic2.getPositionComponent().setReflection(actor.getPositionComponent().getReflection());
