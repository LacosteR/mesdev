...
public class Patrollers extends Fighter {
    ...
    private void startMoveTask() {
        ...
        addMoveToDesire(Location.findPointToStay(_points[_lastPoint], 250, npc.getPositionComponent().getGeoIndex()), true, 5);
        if (npc instanceof MonsterInstance) {
            MonsterInstance _monster = (MonsterInstance)npc;
            if (_monster.getMinionList() != null && _monster.getMinionList() != null) {
                for (NpcInstance _npc : _monster.getMinionList()) {
                    if (npc.isAlikeDead()) {
                        continue;
                    }
                    _npc.setRunning();
                    ((Fighter)_npc.getAI()).addMoveToDesire(Location.findPointToStay(_points[_lastPoint], 250, _npc.getPositionComponent().getGeoIndex()), true, 5);
                }
            }
        }
        doTask();
