...
public class StakatoNest extends Fighter {
    ...
    protected void onEvtDead(Creature killer) {
        ...
        switch (actor.getNpcId()) {
            ...
            case SPIKED_STAKATO_BABY:
                //TODO: _leader = ((MinionInstance) actor).getLeader();
                _leader = (MonsterInstance)actor.getLeader();
    ...
    protected void onEvtSeeSpell(Skill skill, Creature caster) {
        ...
        actor.endDecayTask();
        try {
            NpcInstance mob = NpcFactory.createNpc(CANNIBALISTIC_STAKATO_CHIEF);
            mob.setSpawnedLoc(actor.getPositionComponent().getLoc());
            mob.getPositionComponent().setReflection(actor.getPositionComponent().getReflection());
    ...
    private NpcInstance getAliveMinion(NpcInstance npc) {
        if (npc.getMinionList() != null) {
            for (NpcInstance minion : npc.getMinionList()) {
                if(!minion.isAlikeDead())
                return minion;
            }
        }
        return null;
    ...
    private void spawnMonster(NpcInstance actor, Creature killer, int mobId) {
        try {
            NpcInstance npc = NpcFactory.createNpc(mobId);
