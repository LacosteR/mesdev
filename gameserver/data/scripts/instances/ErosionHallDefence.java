...
import quests.quests7680._697_DefendtheHallofErosion;
...
public class ErosionHallDefence extends Reflection {
    ...
    private class DeathListener implements OnDeathListener {
        ...
        public void onDeath(Creature self, Creature killer) {
            ...
            if (self.getNpcId() == AliveTumor) {
                ...
                final NpcInstance deadTumor = addSpawnWithoutRespawn(DeadTumor, self.getPositionComponent().getLoc(), 0);
                notifyTumorDeath();
                self.deleteMe();
                for (Player p : getPlayers())
                ...
                ThreadPoolManager.getInstance().schedule(new RunnableImpl() {
                    ...
                    public void runImpl() throws Exception {
                        deadTumor.deleteMe();
                        addSpawnWithoutRespawn(AliveTumor, deadTumor.getPositionComponent().getLoc(), 0);
                        handleTumorHp(0.25);
                        invokeDeathListener();
                        for (Player p : getPlayers())
                            p.sendPacket(new ExShowScreenMessage(NpcString.THE_TUMOR_INSIDE_S1_HAS_COMPLETELY_REVIVED_, 8000, ExShowScreenMessage.ScreenMessageAlign.MIDDLE_CENTER, false, 1, -1, false, "#" + NpcString.HALL_OF_EROSION.getId()));
    ...
    private void spawnCoffin(NpcInstance tumor) {
        addSpawnWithoutRespawn(RegenerationCoffin, new Location(tumor.getPositionComponent().getLoc().x, tumor.getPositionComponent().getLoc().y, tumor.getPositionComponent().getLoc().z, Location.getRandomHeading()), 250);
    }
