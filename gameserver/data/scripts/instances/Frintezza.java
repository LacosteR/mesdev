...
public class Frintezza extends Reflection {
            ...
            try {
                switch (_taskId) {
                    ...
                    case 18:
                        ...
                        Earthquake eq = new Earthquake(weakScarlet.getPositionComponent().getLoc(), 50, 6);
    ...
    private void showSocialActionMovie(NpcInstance target, int dist, int yaw, int pitch, int time, int duration, int socialAction) {
        if (target == null)
            return;
        for (Player pc : getPlayers())
            if (PositionUtils.getDistance(pc, target) <= 2550) {
                pc.enterMovieMode();
    ...
    private void block(NpcInstance npc, boolean flag) {
        ...
        if (flag) {
            npc.startDamageBlockedHp();
            npc.startDebuffImmunity();
        } else {
            npc.stopDamageBlockedHp();
            npc.stopDebuffImmunity();
        }
    }
            ...
            try {
                switch (_taskId) {
                    ...
                    case 7:
                        ...
                        loc.set(weakScarlet.getPositionComponent().getLoc());
    ...
    private void blockUnblockNpcs(boolean block, int[] npcArray) {
        for (NpcInstance n : getNpcs())
            if (ArrayUtils.contains(npcArray, n.getNpcId())) {
                if (block) {
                    n.block();
                    n.startDamageBlockedHp();
                    n.startDebuffImmunity();
                } else {
                    n.unblock();
                    n.stopDamageBlockedHp();
                    n.stopDebuffImmunity();
                }
            }
    }
