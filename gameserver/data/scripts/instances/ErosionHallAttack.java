...
public class ErosionHallAttack extends Reflection {
    ...
    private class DeathListener implements OnDeathListener {
        ...
        public void onDeath(Creature self, Creature killer) {
            ...
            if (self.getNpcId() == AliveTumor) {
                ...
                NpcInstance deadTumor = addSpawnWithoutRespawn(DeadTumor, self.getPositionComponent().getLoc(), 0);
                self.deleteMe();
    ...
    private class TumorRevival extends RunnableImpl {
        ...
        public void runImpl() throws Exception {
            if (conquestEnded)
                return;
            NpcInstance tumor = addSpawnWithoutRespawn(AliveTumor, _deadTumor.getPositionComponent().getLoc(), 0);
    ...
    private class RegenerationCoffinSpawn extends RunnableImpl {
        ...
        public void runImpl() throws Exception {
            if (conquestEnded)
                return;
            for (int i = 0; i < 4; i++)
                addSpawnWithoutRespawn(RegenerationCoffin, new Location(_deadTumor.getPositionComponent().getLoc().x, _deadTumor.getPositionComponent().getLoc().y, _deadTumor.getPositionComponent().getLoc().z, Location.getRandomHeading()), 250);
        }
    }
    ...
    private void notifyTumorRevival(NpcInstance tumor) {
        if (getAliveTumorCount() > 0 && cohemenes != null && !cohemenes.isDead()) {
            if(cohemenes.getMinionList() != null) {
                for(NpcInstance pvt : cohemenes.getMinionList()) {
                    pvt.deleteMe();
                }
            }
            cohemenes.deleteMe();
    ...
    private void cohemenesApperance() {
        ...
        //cohemenes.getMinionList().spawnMinions(); TODO сделать ПТСный АИ
        invokeDeathListener();
