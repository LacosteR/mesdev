    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (st.nextToken().equals("insertKey")) {
            ...
            for (Player partyMember : partyMembers)
                if (!PositionUtils.isInRange(this, partyMember, INTERACTION_DISTANCE * 2, false)) {
            ...
            if (Functions.getItemCount(player, KEY_ID) > 0) {
                ...
                player.getPositionComponent().getReflection().startCollapseTimer(COLLAPSE_TIME * 60 * 1000L);
                _activated = true;
                broadcastPacketToOthers(new SystemMessage(SystemMessage.THIS_DUNGEON_WILL_EXPIRE_IN_S1_MINUTES).addNumber(COLLAPSE_TIME));
                player.getPositionComponent().getReflection().setCoreLoc(player.getPositionComponent().getReflection().getReturnLoc());
                player.getPositionComponent().getReflection().setReturnLoc(new Location(16280, 283448, -9704));
                Functions.show("default/32343-1.htm", player, this);
                return;
            }
            // Нет ключа
