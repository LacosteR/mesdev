...
public class PathfinderInstance extends NpcInstance {
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (command.startsWith("ExitSoloKama")) {
            Reflection r = getPositionComponent().getReflection();
            if (r.getReturnLoc() != null)
                player.teleToLocation(r.getReturnLoc(), ReflectionManager.DEFAULT);
            else
                player.getPositionComponent().setReflection(ReflectionManager.DEFAULT);
            player.unsetVar("backCoords");
            r.startCollapseTimer(1000);
        } else if (command.startsWith("ListPossible")) {
            if (getNpcId() == 32484 && ConfigOthers.KAMALOKA_NIGHTMARES_PREMIUM_ONLY && !player.hasBonus()) {
            ...
            switch (getRewardRank()) {
                ...
                case 5:
                    ...
                    break;
                case 6:
                    if (getPositionComponent().getReflection().getInstancedZoneId() == INSTANCE_75LVL_ID) // G-Grade награда доступна только в соло камалоке 70-80 уровней.
                        htmlpath += "-G";
                    else
                        htmlpath += "-S";
                    break;
            ...
            if (!_rewarded) {
                int[][] rewards = getRewardList(getRewardRank(), getPositionComponent().getReflection().getInstancedZone());
                if (rewards != null)
