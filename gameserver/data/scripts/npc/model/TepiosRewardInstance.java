...
import quests.quests7680._694_BreakThroughTheHallOfSuffering;
import quests.quests7680._695_DefendtheHallofSuffering;
...
public class TepiosRewardInstance extends NpcInstance {
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (command.equalsIgnoreCase("getreward")) {
            ...
            int time = 0;
            if (getPositionComponent().getReflection().getInstancedZoneId() == 115)
                time = ((SufferingHallAttack) getPositionComponent().getReflection()).timeSpent;
            else if (getPositionComponent().getReflection().getInstancedZoneId() == 116)
                time = ((SufferingHallDefence) getPositionComponent().getReflection()).timeSpent;
            ...
            for (Player p : getPositionComponent().getReflection().getPlayers()) {
                ...
                if (qs != null && getPositionComponent().getReflection().getInstancedZoneId() == 115)
                    qs.exitCurrentQuest(true);
                if (qs2 != null && getPositionComponent().getReflection().getInstancedZoneId() == 116)
