...
public class SirraInstance extends NpcInstance {
    ...
    public String getHtmlPath(int npcId, int val, Player player) {
        ...
        if (ArrayUtils.contains(questInstances, getPositionComponent().getReflection().getInstancedZoneId()))
            htmlpath = "default/32762.htm";
        else if (ArrayUtils.contains(warInstances, getPositionComponent().getReflection().getInstancedZoneId())) {
            DoorInstance door = getPositionComponent().getReflection().getDoor(23140101);
            if (door.isOpen())
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (command.equalsIgnoreCase("teleport_in")) {
            for (NpcInstance n : getPositionComponent().getReflection().getNpcs())
