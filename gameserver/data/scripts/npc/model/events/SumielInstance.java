...
public class SumielInstance extends NpcInstance {
    ...
    public void onBypassFeedback(Player player, String command) {
        if (command.equals("teleport"))
            ...
            if (player.getInventory().getCountOf(15540) == 0)
                ...
                ItemFunctions.removeItem(player, 15540, 1, true);
                ItemFunctions.addItem(player, 15485, 1, true);
    ...
    private class TIMER_0 extends RunnableImpl {
        ...
        public void runImpl() throws Exception {
            for (NpcInstance npc : GameObjectsStorage.getAllNpcs()) {
                if (npc != null && npc.getNpcId() == 18913 && PositionUtils.getDistance(SumielInstance.this, npc) <= 1200)
    ...
    private class GAME_TIME_EXPIRED extends RunnableImpl {
        ...
        public void runImpl() throws Exception {
            ...
            ItemFunctions.removeItem(c_ai1, 15540, 1, true);
            c_ai0 = null;
            i_quest0 = 0;
            i_quest1 = 0;
        }
    ...
    public void setSCE_POT_ON(int i) {
        if (i == i_ai1 && i_ai0 == 1) {
            ...
            for (NpcInstance npc : GameObjectsStorage.getAllNpcs()) {
                if (npc != null && npc.getNpcId() == 18913 && PositionUtils.getDistance(this, npc) <= 1200)
                    ((FurnfaceInstance) npc).setSCE_GAME_END();
            }
