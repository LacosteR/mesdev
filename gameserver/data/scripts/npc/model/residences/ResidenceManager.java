...
import l2p.gameserver.config.ConfigResidence;
...
public abstract class ResidenceManager extends MerchantInstance {
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (actualCommand.equalsIgnoreCase("banish")) {
            ...
            if (val.equalsIgnoreCase("tele")) {
                ...
                html.replace("%all%", ConfigResidence.ALT_CH_ALL_BUFFS ? "<a action=\"bypass -h npc_%objectId%_support all\">Give all</a><br1><a action=\"bypass -h npc_%objectId%_support allW\">Give warrior</a><br1><a action=\"bypass -h npc_%objectId%_support allM\">Give mystic</a><br>" : "");
                sendHtmlMessage(player, html);
    ...
    private boolean useSkill(int id, int level, Player player) {
        ...
        if (skill == null) {
            player.sendMessage("Invalid skills " + id);
            return true;
        }
