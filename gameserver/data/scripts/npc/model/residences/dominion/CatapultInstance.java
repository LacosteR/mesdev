...
public class CatapultInstance extends SiegeToggleNpcInstance {
    ...
    public void onDeathImpl(Creature lastAttacker) {
        ...
        if (quests != null && quests.length > 0) {
            ...
            if (isRaid() && ConfigQuests.ALT_NO_LASTHIT) // Для альта на ластхит берем всех игроков вокруг
            {
                players = new ArrayList<Player>();
                for (Playable pl : aggroMap.keySet())
                    if (!pl.isDead() && (PositionUtils.isInRange(this, pl, ConfigOthers.PARTY_DISTRIBUTION_RANGE, true)
                            || PositionUtils.isInRange(killer, pl, ConfigOthers.PARTY_DISTRIBUTION_RANGE, true)))
