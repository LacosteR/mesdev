...
public class NewbieGuideInstance extends NpcInstance {
    ...
    public void showChatWindow(Player player, int val, Object... arg) {
        if (val == 0 && mainHelpers.contains(getNpcId()))
            if (player.getPcClass().getLevel() == 1) {
                if (player.getVar("NewGuidReward") == null) {
                    ...
                    boolean isMage = (player.getBaseRace() != Race.orc) && player.getPcClass().isMage();
                    if (isMage) {
    ...
    public String getQuestNpc(int quest, Player player) {
        ...
        if (val == 0) {
            _log.warn("WTF? L2NewbieGuideInstance " + getNpcId() + " not found next step " + quest + " for " + player.getPcClass());
            return null;
        }
        ...
        player.sendPacket(new RadarControl(2, 1, npc.getPositionComponent().getLoc()));// Убираем флажок на карте и стрелку на компасе
        player.sendPacket(new RadarControl(0, 2, npc.getPositionComponent().getLoc()));// Ставим флажок на карте и стрелку на компасе
