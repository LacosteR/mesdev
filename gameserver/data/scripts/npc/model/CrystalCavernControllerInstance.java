...
public class CrystalCavernControllerInstance extends NpcInstance {
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (command.equalsIgnoreCase("request_emerald")) {
            ((CrystalCaverns) getPositionComponent().getReflection()).notifyEmeraldRequest();
        } else if (command.equalsIgnoreCase("request_coral")) {
            ((CrystalCaverns) getPositionComponent().getReflection()).notifyCoralRequest();
            ...
            if (getPositionComponent().getReflection().getInstancedZoneId() == 10)
                player.teleToLocation(getPositionComponent().getReflection().getInstancedZone().getReturnCoords(), ReflectionManager.DEFAULT);
        } else
