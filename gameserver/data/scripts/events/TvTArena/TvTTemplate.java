...
import l2p.gameserver.config.ConfigEvents;
...
public abstract class TvTTemplate extends Functions {
    private static int REGISTRATION_ITEM_ID = ConfigEvents.TVT_ARENA_ITEM;
    private static int MAX_COUNT_ITEM = ConfigEvents.TVT_ARENA_COUNT;

    private static int MAX_LENGTH_TEAM = ConfigEvents.TVT_ARENA_LENGTH_TEAM;

    private static int MAX_TIME_TO_START = ConfigEvents.TVT_ARENA_TIME_TO_START;

    private static int MIN_LEVEL_TO_REG = ConfigEvents.TVT_ARENA_MIN_LEVEL;
    private static int MAX_LEVEL_TO_REG = ConfigEvents.TVT_ARENA_MAX_LEVEL;

    private static boolean ALLOW_BUFFS = ConfigEvents.TVT_ARENA_ALLOW_BUFF;
    // WTF ???????
    private static boolean ALLOW_CLAN_SKILL = ConfigEvents.TVT_ARENA_ALLOW_CLAN_SKILL;
    private static boolean ALLOW_HERO_SKILL = ConfigEvents.TVT_ARENA_ALLOW_HERO_SKILL;
    ...
    public void template_check1(Player player, NpcInstance manager, String[] var) {
        ...
        if (_price < 1 || _price > MAX_COUNT_ITEM) {
            show("Неправильная ставка", player);
            return;
        }
        if (_team1count < 1 || _team1count > MAX_LENGTH_TEAM || _team2count < 1 || _team2count > MAX_LENGTH_TEAM) {
            show("Неправильный размер команды", player);
            return;
        }
        if (_team1min < MIN_LEVEL_TO_REG || _team1min > MAX_LEVEL_TO_REG + 1 || _team2min < MIN_LEVEL_TO_REG || _team2min > MAX_LEVEL_TO_REG + 1 || _team1max < MIN_LEVEL_TO_REG || _team1max > MAX_LEVEL_TO_REG + 1 || _team2max < MIN_LEVEL_TO_REG || _team2max > MAX_LEVEL_TO_REG + 1 || _team1min > _team1max || _team2min > _team2max) {
        ...
        if (_timeToStart < 1 || _timeToStart > MAX_TIME_TO_START) {
            show("Неправильное время", player);
            return;
        }
        if (getItemCount(player, REGISTRATION_ITEM_ID) < _price) {
            player.sendPacket(SystemMsg.INCORRECT_ITEM_COUNT);
            return;
        }
        _creatorId = player.getStoredId();
        removeItem(player, REGISTRATION_ITEM_ID, _price);
        ...
        sayToAll(player.getName() + " создал бой " + _team1count + "х" + _team2count + ", " + _team1min + "-" + _team1max + "lv vs " + _team2min + "-" + _team2max + "lv, ставка " + _price + " " + ItemFunctions.getItemName(REGISTRATION_ITEM_ID) + ", начало через " + _timeToStart + " мин");
    ...
    public void template_announce() {
        ...
        if (_timeToStart > 1) {
            _timeToStart--;
            sayToAll(creator.getName() + " создал бой " + _team1count + "х" + _team2count + ", " + _team1min + "-" + _team1max + "lv vs " + _team2min + "-" + _team2max + "lv, ставка " + _price + " " + ItemFunctions.getItemName(REGISTRATION_ITEM_ID) + ", начало через " + _timeToStart + " мин");
            executeTask("events.TvTArena." + _className, "announce", new Object[0], 60000);
        } else if (_team2list.size() == _team2count && _team1list.size() == _team1count) {
    ...
    public void saveBackCoords() {
        for (Player player : getPlayers(_team1list))
            player.setVar("TvTArena_backCoords", player.getPositionComponent().getX() + " " + player.getPositionComponent().getY() + " " + player.getPositionComponent().getZ() + " " + player.getPositionComponent().getReflectionId(), -1);
        for (Player player : getPlayers(_team2list))
            player.setVar("TvTArena_backCoords", player.getPositionComponent().getX() + " " + player.getPositionComponent().getY() + " " + player.getPositionComponent().getZ() + " " + player.getPositionComponent().getReflectionId(), -1);
    }
    ...
    public void payItemToTeam(Integer team_id) {
        if (team_id == 1)
            for (Player player : getPlayers(_team1list))
                addItem(player, REGISTRATION_ITEM_ID, _price + _team2list.size() * _price / _team1list.size());
        else
            for (Player player : getPlayers(_team2list))
                addItem(player, REGISTRATION_ITEM_ID, _price + _team1list.size() * _price / _team2list.size());
    }

    public void returnItemToTeams() {
        for (Player player : getPlayers(_team1list))
            addItem(player, REGISTRATION_ITEM_ID, _price);
        for (Player player : getPlayers(_team2list))
            addItem(player, REGISTRATION_ITEM_ID, _price);
    }
        ...
        if (_status >= 2 && self.isPlayer() && self.getTeam() != TeamType.NONE && (_team1list.contains(self.getStoredId()) || _team2list.contains(self.getStoredId()))) {
            ...
            if (kplayer != null) {
                ...
                if (player.getTeam() == kplayer.getTeam() || !_team1list.contains(kplayer.getStoredId()) && !_team2list.contains(kplayer.getStoredId())) {
                    sayToAll("Нарушение правил, игрок " + kplayer.getName() + " оштрафован на " + _price + " " + ItemFunctions.getItemName(REGISTRATION_ITEM_ID));
                    removeItem(kplayer, REGISTRATION_ITEM_ID, _price);
                }
    ...
    public class ZoneListener implements OnZoneEnterLeaveListener {
        ...
        public void onZoneLeave(Zone zone, Creature cha) {
            ...
            if (_status >= 2 && player != null && player.getTeam() != TeamType.NONE && (_team1list.contains(player.getStoredId()) || _team2list.contains(player.getStoredId()))) {
                ...
                int x = (int) (cha.getPositionComponent().getX() + 50 * Math.sin(radian));
                int y = (int) (cha.getPositionComponent().getY() - 50 * Math.cos(radian));
                int z = cha.getPositionComponent().getZ();
