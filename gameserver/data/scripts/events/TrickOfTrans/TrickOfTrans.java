...
public class TrickOfTrans extends Functions implements ScriptFile, OnDeathListener, OnPlayerEnterListener {
    ...
    public void onDeath(final Creature cha, final Creature killer) {
        if (_active && SimpleCheckDrop(cha, killer) && Rnd.chance(ConfigEvents.EVENT_TRICK_OF_TRANS_CHANCE * killer.getPlayer().getRateItems() * ConfigRates.RATE_DROP_ITEMS * ((NpcInstance) cha).getTemplate().rateHp))
