...
public class Util extends Functions {
    public void Gatekeeper(String[] param) {
        ...
        if (player.getLastNpc() != null) {
            ...
            switch (npcId) {
                case 30483:
                    if (player.getLevel() >= ConfigOthers.CRUMA_GATEKEEPER_LVL) {
        ...
        if (player.getPositionComponent().getReflection().isDefault()) {
        ...
        Location pos = Location.findPointToStay(x, y, z, 50, 100, player.getPositionComponent().getGeoIndex());
    ...
    public void QuestGatekeeper(String[] param) {
        ...
        Location pos = Location.findPointToStay(x, y, z, 20, 70, player.getPositionComponent().getGeoIndex());
    ...
    public void ReflectionGatekeeper(String[] param) {
        ...
        player.getPositionComponent().setReflection(Integer.parseInt(param[4]));
    ...
    public void NoblessTeleport() {
        ...
        if (player == null)
            return;
        if (player.isNoble() || ConfigOthers.ALLOW_NOBLE_TP_TO_ALL)
    ...
    public void TakeNewbieWeaponCoupon() {
        ...
        if (player == null)
            return;
        if (player.getLevel() > 19 || player.getPcClass().getLevel() > 1) {
    ...
    public void TakeAdventurersArmorCoupon() {
        ...
        if (player == null)
            return;
        if (player.getLevel() > 39 || player.getPcClass().getLevel() > 2) {
            show("Your level is too high!", player);
            return;
        }
        if (player.getLevel() < 20 || player.getPcClass().getLevel() < 2) {
            show("Your level is too low!", player);
            return;
        }
    ...
    public void enter_dc() {
        ...
        player.setVar("DCBackCoords", player.getPositionComponent().getLoc().toXYZString(), -1);
